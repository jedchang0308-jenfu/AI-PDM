...
## AI-PDM continuous production release — DEV117_CONTINUOUS_RELEASE_V2

- `CONTINUOUS_NO_DWELL_V2` is governed by `.ai-doc/specs/SPEC-PDM-INDEPENDENT-PRODUCTION-DEPLOYMENT-001-app-owned-release-adapter.md` §§21–23 and its QA forward amendments, bound to Platform DEV-012 §§25–26 contract SHA-256 `c29974332ae0193bec330870b84a00766c6ae759b7d1522adc4faddc16490c3b`. The six-stage/manual-GO workflow is historical v1 evidence and must not be used as the v2 execution contract. The current owner implementation requires fresh S1B-20 runtime-bridge re-entry before S2; it is not deployed.
...
- Candidate mutation is template-only with zero traffic; activation and rollback are traffic-only and name an exact revision. `latest`, mutable tags, broad IAM, local ADC, synthetic evidence, and a workflow actor-entered GO are never release authority.
