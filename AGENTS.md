...
- `CONTINUOUS_NO_DWELL_V3_DIRECT_RUN_APP` is governed by `.ai-doc/specs/SPEC-PDM-INDEPENDENT-PRODUCTION-DEPLOYMENT-001-app-owned-release-adapter.md` §23 and its QA forward amendment, bound to Platform DEV-012 contract SHA-256 `d88b9aaa8a5e27082746221fc5b473abd8a78da712409279baf5ecdb0e176f05`. V1 six-stage and V2 nine-stage workflows are historical evidence. The V3 owner implementation and S1B-20 report passed under the S1C 8/8 aggregate; S2 is unlocked but not executed, and production is not deployed by this evidence.
...
- Candidate mutation is template-only with zero traffic and injects exactly `PDM_RELEASE_CANDIDATE_ORIGIN`; the app-owned `entrypoint` stage then patches only `ingress,defaultUriDisabled,invokerIamDisabled` before verification. Activation and rollback remain exact-revision traffic operations; recovery order is traffic rollback, candidate-tag cleanup, then entry-baseline restore. `latest`, mutable tags, wildcard origins, broad IAM, local ADC, synthetic evidence, and a workflow actor-entered GO are never release authority.
