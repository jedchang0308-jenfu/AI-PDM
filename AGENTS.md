...
- `CONTINUOUS_NO_DWELL_V2` is governed by `.ai-doc/specs/SPEC-PDM-INDEPENDENT-PRODUCTION-DEPLOYMENT-001-app-owned-release-adapter.md` §§21–24 and its QA forward amendments, bound to Platform DEV-012 §§25–27 and the contract hash recorded after S1C source closure. The six-stage/manual-GO workflow is historical v1 evidence and must not be used as the v2 execution contract. The current owner S1C implementation is in progress and requires fresh same-hash owner re-entry before S2; it is not deployed.
