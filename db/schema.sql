...
INSERT OR IGNORE INTO approval_platform_actions (
  action_code, domain_code, title, description, handler_key, risk_level, allow_batch, requires_impact_snapshot, metadata_json
)
VALUES
...
CREATE TRIGGER IF NOT EXISTS trg_approval_platform_impact_snapshots_no_update
BEFORE UPDATE ON approval_platform_impact_snapshots
BEGIN
  SELECT RAISE(ABORT, 'APPROVAL_PLATFORM_IMPACT_SNAPSHOT_IMMUTABLE');
END;

CREATE TRIGGER IF NOT EXISTS trg_approval_platform_impact_snapshots_no_delete
BEFORE DELETE ON approval_platform_impact_snapshots
BEGIN
  SELECT RAISE(ABORT, 'APPROVAL_PLATFORM_IMPACT_SNAPSHOT_IMMUTABLE');
END;

CREATE TRIGGER IF NOT EXISTS trg_approval_platform_decisions_no_update
BEFORE UPDATE ON approval_platform_decisions
BEGIN
  SELECT RAISE(ABORT, 'APPROVAL_PLATFORM_DECISION_APPEND_ONLY');
END;

CREATE TRIGGER IF NOT EXISTS trg_approval_platform_decisions_no_delete
BEFORE DELETE ON approval_platform_decisions
BEGIN
  SELECT RAISE(ABORT, 'APPROVAL_PLATFORM_DECISION_APPEND_ONLY');
END;

CREATE TRIGGER IF NOT EXISTS trg_approval_platform_events_no_update
BEFORE UPDATE ON approval_platform_events
BEGIN
  SELECT RAISE(ABORT, 'APPROVAL_PLATFORM_EVENT_APPEND_ONLY');
END;

CREATE TRIGGER IF NOT EXISTS trg_approval_platform_events_no_delete
...
WITH default_role_permissions(role_code, permission_kind, permission_code, allowed) AS (
  VALUES
    ...
    ('system_admin', 'action', 'numbering.attachments.manage', 1),
    ...
    ('system_admin', 'action', 'pdm.drawing_package.model_exception.confirm', 1),
    ('system_admin', 'action', 'pdm.manufacturing_baseline.release', 1),
    ...
    ('pdm_admin', 'action', 'numbering.attachments.manage', 1),
    ...
    ('pdm_admin', 'action', 'pdm.drawing_package.model_exception.confirm', 1),
    ('pdm_admin', 'action', 'pdm.manufacturing_baseline.release', 1),
    ...
    ('rd_manager', 'action', 'pdm.manufacturing_baseline.release', 1),
    ...
    ('manufacturing', 'page', 'numbering.search', 1),
    ('manufacturing', 'page', 'numbering.drawings.view', 1),
    ('manufacturing', 'page', 'numbering.reports', 1),
    ('procurement', 'page', 'numbering.search', 1),
    ('procurement', 'page', 'numbering.drawings.view', 1),
    ('procurement', 'page', 'numbering.reports', 1),
    ('external_specialist', 'page', 'numbering.search', 1),
    ('external_specialist', 'page', 'numbering.drawings.view', 1),
    ('external_specialist', 'action', 'pdm.comment.create', 1),
    ('external_specialist', 'action', 'pdm.advice.create', 1)
)
...
-- DEV-064 canonical Drawing aggregate. The existing candidate/formal tables
-- remain compatibility projections while all lifecycle identities converge on
-- these three tables.
CREATE TABLE IF NOT EXISTS pdm_local_data_migrations (
...
CREATE TABLE IF NOT EXISTS drawings (
  ...
  workspace_id TEXT,
  drawing_draft_id TEXT UNIQUE,
  candidate_reservation_id TEXT UNIQUE,
  formal_drawing_number_id TEXT UNIQUE,
  part_root_id TEXT,
...
CREATE TABLE IF NOT EXISTS drawing_revisions (
  ...
  approval_request_id TEXT,
  review_snapshot_hash TEXT,
  source_candidate_revision_id TEXT UNIQUE,
  source_revision_package_id TEXT UNIQUE,
  created_by TEXT,
  created_at TEXT NOT NULL DEFAULT (datetime('now')),
  updated_by TEXT,
  updated_at TEXT NOT NULL DEFAULT (datetime('now')),
  ...
  FOREIGN KEY (approval_request_id) REFERENCES approval_platform_requests(id) ON DELETE RESTRICT,
  FOREIGN KEY (source_candidate_revision_id) REFERENCES numbering_candidate_revision_drafts(id) ON DELETE RESTRICT,
  FOREIGN KEY (source_revision_package_id) REFERENCES drawing_revision_packages(id) ON DELETE RESTRICT,
  FOREIGN KEY (created_by) REFERENCES users(id) ON DELETE RESTRICT,
  FOREIGN KEY (updated_by) REFERENCES users(id) ON DELETE RESTRICT
);
...
CREATE TABLE IF NOT EXISTS drawing_revision_files (
  ...
  source_file_asset_id TEXT NOT NULL,
  source_candidate_file_id TEXT UNIQUE,
  source_package_file_id TEXT UNIQUE,
  role TEXT NOT NULL CHECK (role IN ('cad_3d', 'drawing_2d', 'intermediate', 'pdf', 'dwg_dxf', 'other')),
  role_source TEXT NOT NULL CHECK (role_source IN ('extension', 'user', 'migration', 'system')),
  display_name TEXT NOT NULL DEFAULT '',
  description TEXT NOT NULL DEFAULT '',
  ...
  FOREIGN KEY (source_file_asset_id) REFERENCES file_assets(id) ON DELETE RESTRICT,
  FOREIGN KEY (source_candidate_file_id) REFERENCES numbering_candidate_revision_files(id) ON DELETE RESTRICT,
  FOREIGN KEY (source_package_file_id) REFERENCES drawing_revision_package_files(id) ON DELETE RESTRICT,
  FOREIGN KEY (removed_by) REFERENCES users(id) ON DELETE RESTRICT,
  FOREIGN KEY (created_by) REFERENCES users(id) ON DELETE RESTRICT,
...
CREATE TRIGGER IF NOT EXISTS trg_drawings_terminal_state_guard
...
BEGIN
  SELECT RAISE(ABORT, 'DRAWING_TERMINAL_STATE_IMMUTABLE');
END;

CREATE TRIGGER IF NOT EXISTS trg_drawings_number_immutable_guard
...
BEGIN
  SELECT RAISE(ABORT, 'DRAWING_NUMBER_IMMUTABLE');
END;

CREATE TRIGGER IF NOT EXISTS trg_drawing_revisions_controlled_content_guard
BEFORE UPDATE OF revision, policy_snapshot_json, override_reason, drawing_id ON drawing_revisions
WHEN OLD.lifecycle_state IN ('rd_controlled', 'released', 'superseded')
  AND (
    ...
    OR NEW.drawing_id IS NOT OLD.drawing_id
  )
BEGIN
  SELECT RAISE(ABORT, 'DRAWING_REVISION_CONTROLLED_IMMUTABLE');
END;

CREATE TRIGGER IF NOT EXISTS trg_drawing_revisions_controlled_delete_guard
...
CREATE TRIGGER IF NOT EXISTS trg_drawing_revisions_state_transition_guard
BEFORE UPDATE OF lifecycle_state ON drawing_revisions
WHEN NEW.lifecycle_state <> OLD.lifecycle_state
  AND NOT (
    (OLD.lifecycle_state = 'preparing' AND NEW.lifecycle_state IN ('in_review', 'correction_required', 'rd_controlled', 'released', 'cancelled'))
    OR (OLD.lifecycle_state = 'in_review' AND NEW.lifecycle_state IN ('preparing', 'correction_required', 'rd_controlled', 'released', 'cancelled'))
    OR (OLD.lifecycle_state = 'correction_required' AND NEW.lifecycle_state IN ('preparing', 'in_review', 'rd_controlled', 'cancelled'))
    OR (OLD.lifecycle_state = 'rd_controlled' AND NEW.lifecycle_state IN ('released', 'superseded'))
    OR (OLD.lifecycle_state = 'released' AND NEW.lifecycle_state = 'superseded')
  )
BEGIN
  SELECT RAISE(ABORT, 'DRAWING_REVISION_STATE_TRANSITION_DENIED');
END;

CREATE TRIGGER IF NOT EXISTS trg_drawing_revision_files_controlled_insert_guard
BEFORE INSERT ON drawing_revision_files
WHEN EXISTS (
  SELECT 1 FROM drawing_revisions revision
  WHERE revision.id = NEW.drawing_revision_id
    AND revision.lifecycle_state IN ('rd_controlled', 'released', 'superseded')
)
BEGIN
  SELECT RAISE(ABORT, 'DRAWING_REVISION_FILES_CONTROLLED_IMMUTABLE');
END;

CREATE TRIGGER IF NOT EXISTS trg_drawing_revision_files_controlled_update_guard
...
WHEN EXISTS (
  SELECT 1 FROM drawing_revisions revision
  WHERE revision.id IN (OLD.drawing_revision_id, NEW.drawing_revision_id)
    AND revision.lifecycle_state IN ('rd_controlled', 'released', 'superseded')
)
BEGIN
  SELECT RAISE(ABORT, 'DRAWING_REVISION_FILES_CONTROLLED_IMMUTABLE');
...
CREATE TRIGGER IF NOT EXISTS trg_drawing_revision_files_controlled_delete_guard
BEFORE DELETE ON drawing_revision_files
WHEN EXISTS (
  SELECT 1 FROM drawing_revisions revision
  WHERE revision.id = OLD.drawing_revision_id
    AND revision.lifecycle_state IN ('rd_controlled', 'released', 'superseded')
)
BEGIN
  SELECT RAISE(ABORT, 'DRAWING_REVISION_FILES_CONTROLLED_IMMUTABLE');
...
CREATE TABLE IF NOT EXISTS drawing_recognition_sources (
  ...
  sort_order INTEGER NOT NULL DEFAULT 0,
  adapter_plan_json TEXT NOT NULL DEFAULT '[]',
  created_at TEXT NOT NULL DEFAULT (datetime('now')),
...
CREATE TABLE IF NOT EXISTS drawing_recognition_adapter_results (
  ...
  FOREIGN KEY (session_id) REFERENCES drawing_recognition_sessions(id) ON DELETE RESTRICT,
  FOREIGN KEY (source_id) REFERENCES drawing_recognition_sources(id) ON DELETE RESTRICT,
  FOREIGN KEY (company_id) REFERENCES companies(id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS drawing_recognition_observations (
  ...
  session_id TEXT NOT NULL,
  source_id TEXT NOT NULL,
  adapter_result_id TEXT NOT NULL,
  company_id TEXT NOT NULL,
  ...
  raw_payload_derivative_id TEXT,
  captured_at TEXT NOT NULL,
  FOREIGN KEY (session_id) REFERENCES drawing_recognition_sessions(id) ON DELETE RESTRICT,
  FOREIGN KEY (source_id) REFERENCES drawing_recognition_sources(id) ON DELETE RESTRICT,
  FOREIGN KEY (adapter_result_id) REFERENCES drawing_recognition_adapter_results(id) ON DELETE RESTRICT,
  FOREIGN KEY (company_id) REFERENCES companies(id) ON DELETE RESTRICT,
  FOREIGN KEY (raw_payload_derivative_id) REFERENCES file_derivatives(id) ON DELETE RESTRICT
);
...
CREATE TABLE IF NOT EXISTS pdm_part_attribute_values (
  ...
  unit_text TEXT,
  row_version INTEGER NOT NULL DEFAULT 1 CHECK (row_version >= 1),
  last_formalization_event_id TEXT NOT NULL,
  created_by TEXT NOT NULL,
  created_at TEXT NOT NULL DEFAULT (datetime('now')),
  updated_by TEXT NOT NULL,
  updated_at TEXT NOT NULL DEFAULT (datetime('now')),
  FOREIGN KEY (company_id) REFERENCES companies(id) ON DELETE RESTRICT,
  FOREIGN KEY (part_number_id) REFERENCES part_numbers(id) ON DELETE RESTRICT,
  FOREIGN KEY (attribute_definition_id) REFERENCES pdm_attribute_definitions(id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS drawing_recognition_formalization_links (
  ...
  FOREIGN KEY (event_id) REFERENCES drawing_recognition_formalization_events(id) ON DELETE RESTRICT,
  FOREIGN KEY (candidate_id) REFERENCES drawing_recognition_candidates(id) ON DELETE RESTRICT,
  FOREIGN KEY (company_id) REFERENCES companies(id) ON DELETE RESTRICT
);
...
FROM roles r
JOIN (
  SELECT 'numbering.recognition.run' AS permission_code
  UNION ALL SELECT 'numbering.recognition.review'
  UNION ALL SELECT 'numbering.recognition.formalize'
) p
WHERE r.role_code IN ('rd', 'rd_manager', 'pdm_admin', 'system_admin');

CREATE TRIGGER IF NOT EXISTS trg_drawing_recognition_sources_no_update BEFORE UPDATE ON drawing_recognition_sources BEGIN SELECT RAISE(ABORT, 'DRAWING_RECOGNITION_SOURCE_APPEND_ONLY'); END;
CREATE TRIGGER IF NOT EXISTS trg_drawing_recognition_sources_no_delete BEFORE DELETE ON drawing_recognition_sources BEGIN SELECT RAISE(ABORT, 'DRAWING_RECOGNITION_SOURCE_APPEND_ONLY'); END;
CREATE TRIGGER IF NOT EXISTS trg_drawing_recognition_adapter_results_no_update BEFORE UPDATE ON drawing_recognition_adapter_results BEGIN SELECT RAISE(ABORT, 'DRAWING_RECOGNITION_ADAPTER_RESULT_APPEND_ONLY'); END;
CREATE TRIGGER IF NOT EXISTS trg_drawing_recognition_adapter_results_no_delete BEFORE DELETE ON drawing_recognition_adapter_results BEGIN SELECT RAISE(ABORT, 'DRAWING_RECOGNITION_ADAPTER_RESULT_APPEND_ONLY'); END;
CREATE TRIGGER IF NOT EXISTS trg_drawing_recognition_observations_no_update BEFORE UPDATE ON drawing_recognition_observations BEGIN SELECT RAISE(ABORT, 'DRAWING_RECOGNITION_OBSERVATION_APPEND_ONLY'); END;
CREATE TRIGGER IF NOT EXISTS trg_drawing_recognition_observations_no_delete BEFORE DELETE ON drawing_recognition_observations BEGIN SELECT RAISE(ABORT, 'DRAWING_RECOGNITION_OBSERVATION_APPEND_ONLY'); END;
CREATE TRIGGER IF NOT EXISTS trg_drawing_recognition_candidate_observations_no_update BEFORE UPDATE ON drawing_recognition_candidate_observations BEGIN SELECT RAISE(ABORT, 'DRAWING_RECOGNITION_CANDIDATE_EVIDENCE_APPEND_ONLY'); END;
CREATE TRIGGER IF NOT EXISTS trg_drawing_recognition_candidate_observations_no_delete BEFORE DELETE ON drawing_recognition_candidate_observations BEGIN SELECT RAISE(ABORT, 'DRAWING_RECOGNITION_CANDIDATE_EVIDENCE_APPEND_ONLY'); END;
CREATE TRIGGER IF NOT EXISTS trg_drawing_recognition_decisions_no_update BEFORE UPDATE ON drawing_recognition_decisions BEGIN SELECT RAISE(ABORT, 'DRAWING_RECOGNITION_DECISION_APPEND_ONLY'); END;
CREATE TRIGGER IF NOT EXISTS trg_drawing_recognition_decisions_no_delete BEFORE DELETE ON drawing_recognition_decisions BEGIN SELECT RAISE(ABORT, 'DRAWING_RECOGNITION_DECISION_APPEND_ONLY'); END;
CREATE TRIGGER IF NOT EXISTS trg_drawing_recognition_events_no_update BEFORE UPDATE ON drawing_recognition_formalization_events BEGIN SELECT RAISE(ABORT, 'DRAWING_RECOGNITION_EVENT_APPEND_ONLY'); END;
CREATE TRIGGER IF NOT EXISTS trg_drawing_recognition_events_no_delete BEFORE DELETE ON drawing_recognition_formalization_events BEGIN SELECT RAISE(ABORT, 'DRAWING_RECOGNITION_EVENT_APPEND_ONLY'); END;
CREATE TRIGGER IF NOT EXISTS trg_drawing_recognition_links_no_update BEFORE UPDATE ON drawing_recognition_formalization_links BEGIN SELECT RAISE(ABORT, 'DRAWING_RECOGNITION_LINK_APPEND_ONLY'); END;
CREATE TRIGGER IF NOT EXISTS trg_drawing_recognition_links_no_delete BEFORE DELETE ON drawing_recognition_formalization_links BEGIN SELECT RAISE(ABORT, 'DRAWING_RECOGNITION_LINK_APPEND_ONLY'); END;
