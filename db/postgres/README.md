...
- Production migration version `047` is permanently reserved by `047_remove_bom_module.sql`. Production already applied this exact hard-retirement artifact; the compatibility manifest accepts only its verified immutable checksum.
- No later migration may recreate the retired schema, compatibility column, approval action, route, or runtime capability. DEV-096/099/104 artifacts are historical and excluded from the active package.
- Fresh databases use the generated current baseline `001_initial_schema.sql`, then the ordered forward migrations. Files `002_supabase_rls_plan.sql` and `011_gcs_pointer_numbering_continuity.sql` remain excluded from the Cloud SQL BFF slice; migrations `004` through `046` are forward-only history for databases created from older baselines.
- SolidWorks secure activation is migration `048_solidworks_credential_ui_activation.sql`.
- Standalone manufacturing-impact retirement is migration `049_retire_standalone_manufacturing_impact.sql`. Drawing-recognition owner enforcement is migration `050_drawing_recognition_part_owner_invariant.sql`.
- Canonical Part `structure_type` authority is migration `051_part_structure_type_authority.sql`. It preserves the active numbering/change-control classification independently and must not recreate any retired BOM table, `bom_usage_policy`, route, or runtime capability.
- DEV-106 retirement residue enforcement is migration `052_retired_workbench_residue_cleanup.sql`. It removes only an empty `relation_change_works` compatibility table and inert `bomUsagePolicy` keys from current Part work payloads, reinstalls the Relation-free company guard, and blocks both residues from future recovery writes. Recovery mappers must consume the ordered schema through 052 and must never replay migration 042 directly.
- DEV-005 application role catalog publication is migration `053_jenfu_role_catalog_publication.sql`. The active v3 catalog excludes every retired BOM permission and remains the AI-PDM request-time policy authority.
- DEV-008 last-known-good role capability display snapshot is migration `054_role_capability_display_snapshot.sql`; it is read-only fallback state and never authorizes a request.
