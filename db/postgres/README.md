...
- Migration `041` is reserved for DEV-088 and may be absent. DEV-087 canonical workbench authority is `042_status_data_rebuild.sql`; it is independent of 041 and must apply idempotently on a database whose latest migration is 040. If 042 has already applied before a later authorized 041 package exists, the singleton runner may apply 041 afterward without rewriting the 042 checksum or schema. Verification must cover fresh apply, re-run, provider parity, and forward-fix only; applied migration files are immutable.
