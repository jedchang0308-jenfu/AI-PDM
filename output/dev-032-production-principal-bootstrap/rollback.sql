-- DEV-032 production principal access rollback
...
WHERE platform_principal_id = 'iam:principal:prod-pdm-admin-001'
  AND pdm_user_id = 'prod-pdm-admin-001'
  AND mapping_source = 'shared_iam'
  AND external_subject = 'U57t2eIOzLdhAmNDUbFyOz3fdMm2';
...
WHERE id = 'auth-google-prod-pdm-admin-001'
  AND user_id = 'prod-pdm-admin-001'
  AND provider = 'google_oauth'
  AND provider_subject = 'U57t2eIOzLdhAmNDUbFyOz3fdMm2';
...
SET account_status = 'suspended',
    ...
    account_status_reason = 'DEV-032 production principal bootstrap rollback',
    updated_at = now()
WHERE id = 'prod-pdm-admin-001'
