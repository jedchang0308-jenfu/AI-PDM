...
export class AsyncSubmissionWriteRepository {
    ...
    async submissionRevisionExists(input) {
        const row = await this.client.queryOne(SELECT_ASYNC_SUBMISSION_REVISION_EXISTS_SQL, input);
        return Boolean(row);
    }
    async listSubmissionRevisionsByDrawing(input) {
        const rows = await this.client.query(SELECT_ASYNC_SUBMISSION_REVISIONS_BY_DRAWING_SQL, input);
        return rows.map((row) => ({
    ...
    async createSubmissionRecord(input) {
        ...
        const fileIdByName = new Map(fileEntries.map((file) => [file.originalFilename, { id: file.id, role: file.fileRole }]));
        ...
        const create = async (client) => {
            const item = await client.queryOne(UPSERT_ASYNC_SUBMISSION_ITEM_SQL, {
                id: itemId,
            ...
            for (const scope of partScopeEntries) {
                const scopeItem = await client.queryOne(UPSERT_ASYNC_SUBMISSION_ITEM_SQL, {
                    id: scope.itemId,
                    companyId: input.companyId,
                ...
                if (!scopeItem)
                    throw new Error(`Failed to create scoped item ${scope.partNumber}`);
    ...
    async materializeBomFromReferences(client, input) {
        const bomReferences = input.references
            .filter((reference) => reference.referenceType === "assembly_component" &&
            Boolean(reference.referencedPartNumber?.trim()))
            .sort(compareBomReferences);
        const header = await client.queryOne(UPSERT_ASYNC_SUBMISSION_BOM_HEADER_SQL, {
            id: this.idFactory(),
        ...
        if (!header)
            throw new Error("Failed to materialize BOM header");
...
function buildSubmissionSnapshotJson(base, generated) {
    return {
        ...base,
        submission: {
