-- DEV-046 staging initial principal bootstrap
...
BEGIN
  ...
  IF EXISTS (
    SELECT 1 FROM users
    WHERE (id = 'stg-pdm-admin-001' OR lower(email) = 'jedchang0308@jenfu.com.tw')
      AND (id IS DISTINCT FROM 'stg-pdm-admin-001' OR lower(email) IS DISTINCT FROM 'jedchang0308@jenfu.com.tw')
  ...
  IF EXISTS (
    ...
    WHERE (platform_principal_id = 'iam:principal:stg-pdm-admin-001'
        OR pdm_user_id = 'stg-pdm-admin-001'
        OR (mapping_source = 'shared_iam' AND external_subject = 'qxEv2napjvMEmiqIUqwhTCf6gjg2'))
      AND (platform_principal_id, pdm_user_id, mapping_source, external_subject)
        IS DISTINCT FROM ('iam:principal:stg-pdm-admin-001', 'stg-pdm-admin-001', 'shared_iam', 'qxEv2napjvMEmiqIUqwhTCf6gjg2')
  ) THEN RAISE EXCEPTION 'DEV046_PRINCIPAL_MAPPING_COLLISION'; END IF;

  IF EXISTS (
    SELECT 1 FROM auth_identities
    WHERE (id = 'auth-google-stg-pdm-admin-001'
        OR (provider = 'google_oauth' AND provider_subject = 'qxEv2napjvMEmiqIUqwhTCf6gjg2')
        OR (user_id = 'stg-pdm-admin-001' AND provider = 'google_oauth'))
      AND (id, user_id, provider, provider_subject)
        IS DISTINCT FROM ('auth-google-stg-pdm-admin-001', 'stg-pdm-admin-001', 'google_oauth', 'qxEv2napjvMEmiqIUqwhTCf6gjg2')
...
VALUES (
  'stg-pdm-admin-001', '[鉦富]張仕杰 Jed', 'jedchang0308@jenfu.com.tw', NULL,
...
VALUES ('stg-pdm-admin-001', 'company-jenfu', 1, now())
...
VALUES (
  'auth-google-stg-pdm-admin-001', 'stg-pdm-admin-001', 'google_oauth',
  'qxEv2napjvMEmiqIUqwhTCf6gjg2', 'jedchang0308@jenfu.com.tw', 'jedchang0308@jenfu.com.tw',
...
VALUES (
  'iam:principal:stg-pdm-admin-001', 'stg-pdm-admin-001',
  'shared_iam', 'active', 'qxEv2napjvMEmiqIUqwhTCf6gjg2', now(), now()
)
