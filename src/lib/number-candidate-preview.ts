...
export async function previewNewBundleNumbersAsync(
  client: AsyncDatabaseClient,
  companyId: string,
  purposeCode: NumberPreviewPurposeCode
) {
...
export async function previewAppendNumbersAsync(
  client: AsyncDatabaseClient,
  companyId: string,
  rootCode: string,
  purposeCode: NumberPreviewPurposeCode
  ...
  if (!row) {
    return { mode: "append" as const, reserved: false, root: rootCode, part: null, drawing: null, purposeCode, missingRoot: true };
  }
  return {
...
async function nextRootCode(client: AsyncDatabaseClient, companyId: string, ruleVersionId: string) {
  ...
  const reserved = await retainedCandidateCodes(client, companyId, "root", `${companyId}:root:${ruleVersionId}`);
...
async function nextPartNumber(client: AsyncDatabaseClient, companyId: string, rootCode: string, ruleVersionId: string) {
  ...
  const reserved = await retainedCandidateCodes(client, companyId, "part", `${companyId}:part:${rootCode}:${ruleVersionId}`);
...
async function nextDrawingNumber(
  client: AsyncDatabaseClient,
  companyId: string,
  rootCode: string,
  purposeCode: NumberPreviewPurposeCode,
  ...
  const reserved = await retainedCandidateCodes(client, companyId, "drawing", `${companyId}:drawing:${rootCode}:${purposeCode}:${ruleVersionId}`);
...
async function retainedCandidateCodes(client: AsyncDatabaseClient, companyId: string, itemType: NumberKind, sequenceScopeKey: string) {
