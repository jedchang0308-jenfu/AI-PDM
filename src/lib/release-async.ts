...
import { findReleasedFilenameConflictsAsync } from "@/lib/release-records-async";
import { updateFileGDriveStatusAsync } from "@/lib/submission-files-async";
...
export async function releaseSubmissionViaCloudFunctionAsync(submission: SubmissionDetail, approvedBy: string) {
  const files = submission.files.map((file) => ({
    fileRole: file.file_role,
    gdriveFileId: file.gdrive_file_id,
    originalFilename: file.original_filename
  }));
  const conflicts = await findReleasedFilenameConflictsAsync({
      ...
      for (const file of submission.files) {
        ...
        if (file.gdrive_file_id && file.gdrive_status === "uploaded") {
          try {
            ...
            await updateFileGDriveStatusAsync(file.id, "moved");
            movedFiles.push(file.original_filename);
          } catch (error) {
  ...
  const pendingFolderId = (await getSystemSettingAsync("gdrive_pending_folder_id")) || envPendingFolderId;
  const releasedFolderId = (await getSystemSettingAsync("gdrive_released_folder_id")) || envReleasedFolderId;
  const approvedAt = new Date().toISOString();
...
async function compensateMovedFiles(files: Array<{ fileId: string; previousParents: string[]; dbFileId: string; filename: string }>) {
  ...
  for (const file of files.reverse()) {
    try {
      ...
      await updateFileGDriveStatusAsync(file.dbFileId, "uploaded");
    } catch (error) {
