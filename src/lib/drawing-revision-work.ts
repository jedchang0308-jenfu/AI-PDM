...
import { uploadDrawingRevisionWorkFile, uploadDrawingRevisionWorkFilePrincipal } from "@/lib/drawing-revision-work-file";
...
export class DrawingRevisionWorkService {
  ...
  private async requirePrincipalCapability(verified: VerifiedPrincipalRequest,
    ...
    return { companyId: verified.profile.companyId, profileId: verified.profile.pdmUserId };
  }

  async read(workId: string, actor: DrawingRevisionActor) {
    return this.readForProfile(workId, { profileId: actor.id,
      companyId: actor.companyId, canEditNonOwned: actor.canEditNonOwned });
  }
  ...
  async readPrincipal(workId: string, verified: VerifiedPrincipalRequest) {
    ...
    const permissions = ["numbering.workspace.view", "numbering.workspace.update",
      "numbering.candidate.review.submit", "numbering.workspace.cancel"] as const;
    const decisions = await evaluatePrincipalWorkspacePermissionsInSnapshot(this.client,
      verified, permissions.map((permissionCode) => ({ permissionKind: "action", permissionCode })));
    if (decisions.length !== permissions.length || !decisions[0]?.allowed) {
      throw new CanonicalWorkbenchError("WORKBENCH_BAD_REQUEST", "無權限執行此操作", 403);
    }
    return this.readForProfile(workId, { profileId: verified.profile.pdmUserId,
  ...
  private async readForProfile(workId: string, actor: {
    profileId: string; companyId: string; canEditNonOwned: boolean;
    principalCapabilities?: { update: boolean; submit: boolean; cancel: boolean };
  }) {
    ...
    if (work.owner_user_id !== actor.profileId && !actor.canEditNonOwned) {
      throw new CanonicalWorkbenchError("WORKBENCH_BAD_REQUEST", "無權限執行此操作", 403);
    }
    ...
    const capabilities = actor.principalCapabilities;
    const permittedInteraction = capabilities ? {
      ...interactionState,
      canMutateContent: interactionState.canMutateContent && capabilities.update,
      canSubmit: interactionState.canSubmit && capabilities.submit,
      canCancel: interactionState.canCancel && capabilities.cancel,
      canApprove: false,
      canReturn: false
    } : interactionState;
    ...
    return { data: { entityType: "drawing" as const, entityId: work.drawing_id, workId: work.id, revisionId: work.revision_id, revision: work.target_label, rowVersion: Number(work.row_version), payload, identity: drawing, files, changeImpactRequired: changeImpactProjection.changeImpactRequired, relatedParts: changeImpactProjection.relatedParts, affectedParts: changeImpactProjection.affectedParts, readonly: !permittedInteraction.canMutateContent, interaction: permittedInteraction }, meta: { contractToken: await issueCanonicalWorkbenchContract(this.client, { companyId: actor.companyId, actorId: actor.profileId }), correlationId: crypto.randomUUID() } };
  }

  async targets(drawingId: string, sourceRowKey: string, actor: DrawingRevisionActor) {
    allow(actor.permissions.create);
    return this.targetsForProfile(drawingId, sourceRowKey,
      { companyId: actor.companyId, profileId: actor.id });
  }

  async targetsPrincipal(drawingId: string, sourceRowKey: string, verified: VerifiedPrincipalRequest) {
    const profile = await this.requirePrincipalCapability(verified, "numbering.workspace.create");
    return this.targetsForProfile(drawingId, sourceRowKey, profile);
  }

  private async targetsForProfile(drawingId: string, sourceRowKey: string, profile: {
    companyId: string; profileId: string;
  }) {
    const rowId = parseCanonicalRowKey(sourceRowKey); const repository = new DrawingRevisionWorkAsyncRepository(this.client); const source = await repository.readSourceState(this.client, profile.companyId, rowId);
    ...
    const candidates = await repository.listCandidates(this.client, source); const basisState = deriveDrawingRevisionBasis({ dataLayer: source.data_layer, baseProductionRevisionId: source.base_production_revision_id, currentProductionRevisionId: source.current_production_revision_id }); const contractToken = await issueCanonicalWorkbenchContract(this.client, { companyId: profile.companyId, actorId: profile.profileId });
  ...
  async create(drawingId: string, rawInput: unknown, actor: DrawingRevisionActor, context: CommandContext) {
    allow(actor.permissions.create); await this.verify(actor, context.contractToken);
    const plan = await this.planCreate(drawingId, rawInput,
      { companyId: actor.companyId, profileId: actor.id }, context);
    const repository = new DrawingRevisionWorkAsyncRepository(this.client);
    return runDev087IdempotentCommand(this.client, { companyId: actor.companyId, actorId: actor.id, command: "drawing.create", idempotencyKey: context.idempotencyKey, request: plan.request, effectKey: plan.effectKey, correlationId: correlation(context.correlationId) }, (tx) => repository.create(tx, { companyId: actor.companyId, sourceRowId: plan.sourceRowId, ownerUserId: actor.id, expectedRowVersion: context.expectedRowVersion, target: plan.target, selectionMode: plan.selectionMode, requestedMinor: plan.requestedMinor }));
  }

  async createPrincipal(drawingId: string, rawInput: unknown,
    ...
    const plan = await this.planCreate(drawingId, rawInput, profile, context);
    return runPrincipalDev087Command(this.client, verified, {
      command: "drawing.create", idempotencyKey: context.idempotencyKey,
      request: plan.request, effectKey: plan.effectKey,
      correlationId: correlation(context.correlationId)
    }, (tx) => new DrawingRevisionWorkAsyncRepository(tx).create(tx, {
      companyId: profile.companyId, sourceRowId: plan.sourceRowId,
      ownerUserId: profile.profileId, expectedRowVersion: context.expectedRowVersion,
      target: plan.target, selectionMode: plan.selectionMode,
      requestedMinor: plan.requestedMinor
    }));
  }

  private async planCreate(drawingId: string, rawInput: unknown, profile: {
    companyId: string; profileId: string;
  }, context: CommandContext) {
    const input = parseCreateSelection(rawInput); const sourceRowId = parseCanonicalRowKey(input.sourceRowKey); const repository = new DrawingRevisionWorkAsyncRepository(this.client); const source = await repository.readSourceState(this.client, profile.companyId, sourceRowId); if (!source || source.drawing_id !== drawingId) throw new CanonicalWorkbenchError("WORKBENCH_ROW_VERSION_CONFLICT", "重新讀取目前資料", 409);
    ...
    if (input.selectionMode === "recommended") {
      const candidate = verifyDrawingRevisionTargetToken(input.candidateToken, { companyId: profile.companyId, actorId: profile.profileId, drawingId, sourceRowId, sourceRowVersion: context.expectedRowVersion });
    ...
    return { sourceRowId, target, selectionMode: input.selectionMode, requestedMinor,
      request: { drawingId, sourceRowId, selectionMode: input.selectionMode,
        target: input.selectionMode === "recommended" ? target : null,
        requestedMinor, expectedRowVersion: context.expectedRowVersion },
      effectKey: input.selectionMode === "manual_minor"
        ? `drawing:${drawingId}:source:${sourceRowId}:v${context.expectedRowVersion}:manual:${requestedMinor}`
        : `drawing:${drawingId}:${target.major}.${target.minor}` };
  }
  ...
  async updatePrincipal(workId: string, value: unknown,
    verified: VerifiedPrincipalRequest, context: CommandContext) {
    const profile = await this.requirePrincipalCapability(verified, "numbering.workspace.update");
    await verifyCanonicalWorkbenchCommandContract(this.client,
      { companyId: profile.companyId, actorId: profile.profileId, token: context.contractToken });
    const payload = validateDrawingPayload(value);
    return runPrincipalDev087Command(this.client, verified, {
      command: "drawing.update", idempotencyKey: context.idempotencyKey,
      request: { workId, expectedRowVersion: context.expectedRowVersion, payload },
      effectKey: `drawing-work:${workId}:update`, correlationId: correlation(context.correlationId)
    }, async (tx) => {
      const repository = new DrawingRevisionWorkAsyncRepository(tx);
      const locked = await repository.readWork(tx, profile.companyId, workId, true);
      if (!locked) throw new CanonicalWorkbenchError("WORKBENCH_BAD_REQUEST", "圖面工作資料不存在", 404);
      if (locked.owner_user_id !== profile.profileId) {
        throw new CanonicalWorkbenchError("WORKBENCH_BAD_REQUEST", "無權限執行此操作", 403);
      }
      if (locked.predecessor_revision_id === null) {
        if (Object.prototype.hasOwnProperty.call(payload, "changeImpact")) {
          throw new CanonicalWorkbenchError("DRAWING_FFF_NOT_APPLICABLE", "首版工作不可寫入 FFF 判定", 422);
        }
        delete payload.changeImpact;
      } else {
        payload.changeImpact = await validateDrawingChangeImpactForWork(tx, {
          companyId: profile.companyId, drawingId: locked.drawing_id,
          revisionId: locked.revision_id,
          predecessorRevisionId: locked.predecessor_revision_id,
          impact: payload.changeImpact, mode: "draft"
        });
      }
      return repository.update(tx, { companyId: profile.companyId, workId,
        expectedRowVersion: context.expectedRowVersion, payload });
    });
  }
  ...
  async uploadFilePrincipal(workId: string,
    input: { file: unknown; displayName?: unknown; description?: unknown },
    token: string, context: CommandContext) {
    return uploadDrawingRevisionWorkFilePrincipal({
      client: this.client, token, workId, ...input, context
    });
  }
  ...
  async removeFile(workId: string, fileBindingId: string, actor: DrawingRevisionActor, context: CommandContext) {
    ...
    const result = await runDev087IdempotentCommand(this.client, {
      ...
      const outcome = await this.applyFileRemoval(tx, workId, fileBindingId,
        actor.companyId, actor.id, context.expectedRowVersion, actor.canEditNonOwned);
      storageKeyToRemove = outcome.storageKey;
  ...
  async removeFilePrincipal(workId: string, fileBindingId: string,
    token: string, context: CommandContext) {
    let storageKeyToRemove: string | null = null;
    const result = await withVerifiedJenfuPrincipalRequest(
      { ...principalRequestInput(token), database: this.client }, async (tx, verified) => {
        const service = new DrawingRevisionWorkService(tx);
        const profile = await service.requirePrincipalCapability(verified,
        ...
        return runPrincipalDev087Command(tx, verified, {
          command: "drawing.file.remove", idempotencyKey: context.idempotencyKey,
          request: { workId, fileBindingId, expectedRowVersion: context.expectedRowVersion },
          effectKey: `drawing-work:${workId}:file-remove:${fileBindingId}`,
          correlationId: correlation(context.correlationId)
        }, async (commandTx) => {
          const outcome = await service.applyFileRemoval(commandTx, workId,
            fileBindingId, profile.companyId, profile.profileId,
    ...
    if (storageKeyToRemove) {
      try { await createFileStorageService().deleteObject(storageKeyToRemove); }
      catch { /* DB tombstone remains authoritative; cleanup evidence records retry. */ }
    }
    return result;
  }

  private async applyFileRemoval(tx: AsyncDatabaseClient, workId: string,
    fileBindingId: string, companyId: string, actorId: string,
    expectedRowVersion: number, canEditNonOwned: boolean) {
    const repository = new DrawingRevisionWorkAsyncRepository(tx);
    const locked = await repository.readWork(tx, companyId, workId, true);
    if (!locked || Number(locked.row_version) !== expectedRowVersion || locked.handling !== "owner") {
      throw new CanonicalWorkbenchError("WORKBENCH_ROW_VERSION_CONFLICT", "重新讀取目前資料", 409);
    }
    if (locked.owner_user_id !== actorId && !canEditNonOwned) {
      throw new CanonicalWorkbenchError("WORKBENCH_BAD_REQUEST", "無權限執行此操作", 403);
    }
    await repository.assertWorkMutationBasis(tx, locked);
    const binding = await tx.queryOne<{
      file_binding_id: string; drawing_revision_id: string;
      source_file_asset_id: string; storage_key: string | null;
      ...
      { companyId, workId, fileBindingId });
    ...
    await tx.execute(`UPDATE drawing_revision_files SET removed_at = CURRENT_TIMESTAMP, removed_by = :actorId, updated_at = CURRENT_TIMESTAMP WHERE id = :fileBindingId AND company_id = :companyId AND removed_at IS NULL`, { fileBindingId, companyId, actorId });
    const ownsFileAsset = binding.linked_entity_type === "drawing_revision" && binding.linked_entity_id === locked.revision_id;
    if (ownsFileAsset) {
      await tx.execute(`UPDATE file_assets SET deleted_at = CURRENT_TIMESTAMP, deleted_by = :actorId, deleted_reason = 'drawing_revision_work_file_removed', updated_at = CURRENT_TIMESTAMP WHERE id = :fileAssetId AND linked_entity_type = 'drawing_revision' AND linked_entity_id = :revisionId AND deleted_at IS NULL`, { fileAssetId: binding.source_file_asset_id, revisionId: locked.revision_id, actorId });
    }
    await tx.execute(`UPDATE drawing_revision_works SET row_version = row_version + 1, updated_at = CURRENT_TIMESTAMP WHERE id = :workId AND company_id = :companyId AND row_version = :expectedRowVersion`, { workId, companyId, expectedRowVersion });
    await tx.execute(`UPDATE canonical_workbench_states SET row_version = row_version + 1, updated_at = CURRENT_TIMESTAMP WHERE company_id = :companyId AND work_id = :workId AND handling = 'owner'`, { companyId, workId });
    await repository.assertWorkFileSnapshot(tx, locked);
    return { result: { workId, fileBindingId, rowVersion: expectedRowVersion + 1, removed: true },
      storageKey: ownsFileAsset ? binding.storage_key : null };
  }
  ...
  async submit(workId: string, actor: DrawingRevisionActor, context: CommandContext) {
    ...
    return runDev087IdempotentCommand(this.client, { companyId: actor.companyId, actorId: actor.id, command: "drawing.submit", idempotencyKey: context.idempotencyKey, request: { workId, expectedRowVersion: context.expectedRowVersion }, effectKey: `drawing-work:${workId}:review`, correlationId: correlation(context.correlationId) },
      (tx) => this.applySubmission(tx, workId, actor.companyId, context.expectedRowVersion, false));
  }
  ...
  async submitPrincipal(workId: string, verified: VerifiedPrincipalRequest,
    ...
    await verifyCanonicalWorkbenchCommandContract(this.client,
      { companyId: profile.companyId, actorId: profile.profileId, token: context.contractToken });
    return runPrincipalDev087Command(this.client, verified, {
      command: "drawing.submit", idempotencyKey: context.idempotencyKey,
      request: { workId, expectedRowVersion: context.expectedRowVersion },
      effectKey: `drawing-work:${workId}:review`, correlationId: correlation(context.correlationId)
    }, (tx) => this.applySubmission(tx, workId, profile.companyId,
      context.expectedRowVersion, true, profile.profileId));
  }
  ...
  private async applySubmission(tx: AsyncDatabaseClient, workId: string,
    ...
    principalOwnerId?: string) {
      const repository = new DrawingRevisionWorkAsyncRepository(tx);
      const locked = await repository.readWork(tx, companyId, workId, true);
      if (!locked || Number(locked.row_version) !== expectedRowVersion) throw new CanonicalWorkbenchError("WORKBENCH_ROW_VERSION_CONFLICT", "重新讀取目前資料", 409);
      if (principalOnly && locked.owner_user_id !== principalOwnerId) {
        throw new CanonicalWorkbenchError("WORKBENCH_BAD_REQUEST", "無權限執行此操作", 403);
      }
      await repository.assertWorkMutationBasis(tx, locked);
      await assertDrawingRecognitionWriteReady(tx, { companyId, drawingId: locked.drawing_id, revisionId: locked.revision_id });
      ...
      if (locked.predecessor_revision_id === null) {
        ...
        submittedPayload.changeImpact = await validateDrawingChangeImpactForWork(tx, { companyId, drawingId: locked.drawing_id, revisionId: locked.revision_id, predecessorRevisionId: locked.predecessor_revision_id, impact: submittedPayload.changeImpact, mode: "submit" });
      }
      await tx.execute(`UPDATE drawing_revision_works SET proposed_payload = :payload, updated_at = CURRENT_TIMESTAMP WHERE id = :workId AND company_id = :companyId`, { workId, companyId, payload: JSON.stringify(submittedPayload) });
      ...
      const request = await reviews.create(tx, { companyId, requestKind: "drawing_revision", entityType: "drawing", canonicalEntityId: locked.drawing_id, workId, branchId: locked.branch_id, reviewerUserId, snapshotPayload: packagePayload, snapshotHash: "packageHash" in packagePayload ? packagePayload.packageHash : legacySnapshotHash });
      await tx.execute(`UPDATE drawing_revisions SET lifecycle_state = 'in_review', updated_at = CURRENT_TIMESTAMP WHERE id = :revisionId AND company_id = :companyId`, { companyId, revisionId: locked.revision_id });
  ...
  async cancelPrincipal(workId: string, verified: VerifiedPrincipalRequest,
    context: CommandContext) {
    const profile = await this.requirePrincipalCapability(verified, "numbering.workspace.cancel");
    await verifyCanonicalWorkbenchCommandContract(this.client,
      { companyId: profile.companyId, actorId: profile.profileId, token: context.contractToken });
    return runPrincipalDev087Command(this.client, verified, {
      command: "drawing.cancel", idempotencyKey: context.idempotencyKey,
      request: { workId, expectedRowVersion: context.expectedRowVersion },
      effectKey: `drawing-work:${workId}:cancel`, correlationId: correlation(context.correlationId)
    }, async (tx) => {
      const repository = new DrawingRevisionWorkAsyncRepository(tx);
      const locked = await repository.readWork(tx, profile.companyId, workId, true);
      if (!locked) throw new CanonicalWorkbenchError("WORKBENCH_BAD_REQUEST", "圖面工作資料不存在", 404);
      ...
      return repository.cancel(tx, { companyId: profile.companyId, workId,
        expectedRowVersion: context.expectedRowVersion });
    });
  }
  ...
  async decidePrincipal(requestId: string, decision: Dev087ReviewDecision,
    ...
    return runPrincipalDev087Command(this.client, verified, {
      ...
      if (parseReviewPackageSnapshot(locked.snapshotPayload).kind !== "v2") {
