...
export class DrawingRevisionWorkService {
  ...
  async decide(requestId: string, decision: Dev087ReviewDecision, actor: DrawingRevisionActor, context: CommandContext) {
    ...
    return runDev087IdempotentCommand(this.client, { companyId: actor.companyId, actorId: actor.id, command: "review.decision", idempotencyKey: context.idempotencyKey, request: commandRequest, effectKey: `review:${request.reviewCycleId}`, correlationId, terminalReview: true }, async (tx) => {
      ...
      if (!locked.workId) throw new CanonicalWorkbenchError("WORKBENCH_SNAPSHOT_DRIFT", "資料已改變，請退回修改後重新送審", 409); const repository = new DrawingRevisionWorkAsyncRepository(tx); const work = await repository.readWork(tx, actor.companyId, locked.workId, true); const rawPayload = work ? (typeof work.proposed_payload === "string" ? JSON.parse(work.proposed_payload) : work.proposed_payload) : null; const snapshots = work ? [{ payload: sanitizeDrawingRevisionWorkPayload(rawPayload), revisionId: work.revision_id, claimId: work.target_claim_id }, { payload: rawPayload, revisionId: work.revision_id, claimId: work.target_claim_id }] : []; if (!work || !snapshots.some((snapshot) => dev087RequestHash(snapshot) === locked.snapshotHash)) throw new CanonicalWorkbenchError("WORKBENCH_SNAPSHOT_DRIFT", "資料已改變，請退回修改後重新送審", 409);
