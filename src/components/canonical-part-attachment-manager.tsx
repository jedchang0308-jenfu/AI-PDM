...
export function CanonicalPartAttachmentManager({ partNumber, returnTo }: { partNumber: string; returnTo: string }) {
  ...
  const refreshAttachments = useCallback(async () => {
    const [activeResponse, deletedResponse] = await Promise.all([
      ...
      fetch(`${endpoint}?surface=deleted_data`, { cache: "no-store" })
    ]);
    const [activeBody, deletedBody] = await Promise.all([
      activeResponse.json().catch(() => null) as Promise<AttachmentResponse | null>,
      deletedResponse.json().catch(() => null) as Promise<DeletedAttachmentResponse | null>
    ]);
    if (activeResponse.status === 404 || deletedResponse.status === 404) throw new Error("PART_NUMBER_NOT_FOUND");
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (permissionBody?.actions?.["numbering.attachments.manage"] !== true) {
  ...
  return <PdmEditPageFrame
    ...
    onRetry={() => void load()}
  >
    <section className="pdm-edit-page-card part-attachment-upload-card" aria-label="附件上傳">
    ...
    <section className="pdm-edit-page-card" aria-labelledby="part-attachment-list-title">
      ...
      {attachments.length ? <ul className="part-attachment-list">{attachments.map((attachment) => <li key={attachment.id}>
        ...
        <div className="part-attachment-actions"><a className="secondary-button" href={attachmentDownloadHref(attachment)} download={attachment.fileName} aria-label={`下載 ${attachment.fileName}`} title={`下載 ${attachment.fileName}`}><Download size={15} aria-hidden="true" />下載</a><button className="danger-button" type="button" disabled={busy} onClick={() => void deleteAttachment(attachment)}><Trash2 size={15} aria-hidden="true" />刪除</button></div>
      </li>)}</ul> : <p className="canonical-empty">尚無附件</p>}
    </section>

    <details className="pdm-edit-page-card part-attachment-deleted">
