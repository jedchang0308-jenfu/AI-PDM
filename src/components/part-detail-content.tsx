...
import { PartWorkbench } from "@/components/part-workbench";
...
export function PartModule() {
  ...
  if (workbenchEnabled === null) return <section className="panel"><div className="empty">正在確認料號工作台狀態...</div></section>;
  if (!workbenchEnabled) return <LegacyPartsPage />;
  return <PartWorkbench renderFormalDetail={({ detail, busy, productionSliceEnforced, productionSliceUnopenedMessage, setBusy, onUpdated }) => (
    <PartDetailPanel
...
export function LegacyPartsPage() {
        ...
        <table className="pdm-identity-table pdm-identity-table-compact">
          ...
          <tbody>
            {parts.map((part) => (
              ...
              >
                <td data-label="料號">
                  <div className="pdm-identity-code"><SearchHighlight value={part.partNumber} query={query} /></div>
                </td>
                <td data-label="品名">
                  <div className="pdm-identity-name" title={part.partName}>
                    <SearchHighlight value={part.partName} query={query} />
                  </div>
                </td>
                <td data-label="圖號">
                  <div className="pdm-identity-code"><SearchHighlight value={part.primaryDrawingNumber ?? "未關聯圖號"} query={query} /></div>
                  {part.drawingCount > 1 ? <small className="pdm-identity-subline">共 {part.drawingCount} 張圖號</small> : null}
                  {part.variant ? <small className="pdm-identity-subline"><SearchHighlight value={variantLabel(part.variant)} query={query} /></small> : null}
                </td>
...
function Shared3dBaselinePanel({
  ...
  const loadShared3dState = useCallback(async () => {
    ...
    try {
      ...
      const resolverParams = new URLSearchParams({ ownerScope: "part_number", ownerCode: partNumber });
      const resolverResponse = await fetch(`/api/manufacturing-baselines/resolve?${resolverParams.toString()}`, { cache: "no-store" });
