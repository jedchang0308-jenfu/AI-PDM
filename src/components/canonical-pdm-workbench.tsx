...
function Drawer({ detail, loading, error, width, canManageAttachments, historyRevisionId, onHistoryRevisionChange, onStartResize, onClose, onAction, onManageAttachments, onPartPreviewCommitted, onBeforeCreate, onMatrixSaved, onMatrixDirtyChange, onOpenMatrixDrawing, onOpenMatrixPart }: {
  ...
  >
    {loading ? <p className="canonical-drawer-message" role="status">正在載入明細…</p> : error ? <p className="canonical-error" role="alert">{error}</p> : detail && presentation ? <div className="pdm-entity-drawer-body canonical-drawer-body">
      ...
      /> : null}
      {presentation.kind === "part" ? <section data-section="part-structure-readonly"><div className="canonical-drawer-section-heading"><h3>結構型態</h3></div><p className="part-structure-classification-summary">{presentation.bomContext.structureType === "assembly" ? "組合件" : presentation.bomContext.structureType === "single_part" ? "單件" : "未分類"}</p></section> : null}
      {presentation.kind === "part" ? <PartBomContext context={presentation.bomContext} partNumberId={detail.data.row.entityId} partNumber={detail.data.row.code} mode="readonly" /> : null}
