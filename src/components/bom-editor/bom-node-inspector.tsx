...
import { parseBomQuantity } from "@/lib/bom-unit-of-measure";
...
export function BomNodeInspector({
  ...
  onCommit: (patch: { groupName?: string; quantity?: number | string }) => void;
  ...
  if (!node) {
    return <aside className="bom-inspector"><div className="bom-inspector-empty">選取節點以查看詳細資料</div></aside>;
  }
  ...
  const commitQuantity = () => {
    ...
    try {
      const parsed = parseBomQuantity(quantity);
      next = parsed.canonical;
    } catch {
      setQuantityError("數量必須是大於 0、最多 6 位小數的數字");
      return;
    }
    setQuantityError("");
    if (String(next) !== String(node.quantity ?? "")) onCommit({ quantity: next });
  };
  return (
    <aside className="bom-inspector" aria-label="BOM 節點詳細資料">
      <header><h2>詳細資料</h2><span>{node.node_type === "group" ? "群組" : "料件"}</span></header>
      <div className="bom-inspector-body">
        {node.node_type === "group" ? (
          ...
          <>
            ...
            <label><span>數量{node.quantity_uom_code ? `（${node.quantity_uom_code}）` : ""}</span><input type="text" inputMode="decimal" value={quantity} aria-invalid={Boolean(quantityError)} disabled={!mutable} onChange={(event) => { setQuantityError(""); setQuantity(event.target.value); }} onBlur={commitQuantity} onKeyDown={(event) => event.key === "Enter" && event.currentTarget.blur()} /></label>
            {quantityError ? <p className="canonical-error" role="alert">{quantityError}</p> : null}
            {component && onCommitComponent ? (
              <SharedComponentEditor
                component={component}
                parents={applicableParents}
                mutable={mutable}
                allowParentMapping={allowParentMapping}
...
function SharedComponentEditor({ component, parents, mutable, allowParentMapping = true, onCommit }: {
  ...
  const toggleCandidate = (candidate: BomEditorItemCandidate) => {
    ...
    if (!allowParentMapping) {
      if (selected) return;
      onCommit({
        ...component,
        component_mode: "fixed",
        child_part_number_ids: [partNumberId],
        child_candidates: [{ part_number_id: partNumberId, part_number: candidate.part_number, part_name: candidate.part_name, part_root_id: candidate.part_root_id ?? component.child_part_root_id }],
        parent_selections: []
      });
      return;
    }
  ...
  return <section className="bom-component-editor" aria-label="適用料號對應">
    <h3>零件候選</h3>
    <p>{component.component_mode === "fixed" ? "固定零件" : "依適用料號對應"}</p>
    <div className="bom-component-candidates">
    ...
    {mutable ? <label><span>{allowParentMapping ? "加入候選（可複選）" : "替換非製造 BOM 料件"}</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜尋料號或品名" /></label> : null}
    {mutable && results.length ? <div className="bom-component-results">
      {results.map((result) => {
    ...
    {allowParentMapping && component.component_mode === "by_parent" ? <div className="bom-parent-mapping">
      <h3>適用料號對應</h3>
