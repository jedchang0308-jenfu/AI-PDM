...
type Observation = { id: string; rawText: string; confidenceBand: string; pageNumber: number | null; sheetName: string | null; configurationName: string | null };
type Candidate = {
...
const statusLabels: Record<string, string> = { queued: "等待辨識", extracting: "辨識中", review_ready: "待人工核對", extraction_partial: "部分完成，待核對", extraction_failed: "辨識失敗", ready_to_formalize: "可確認寫入", formalized: "已寫入 PDM", cancelled: "已由新版取代" };
const reviewLabels: Record<string, string> = { proposed: "待核對", conflict: "與正式值不同", accepted: "已接受", corrected: "已修正", mapped: "已歸類", ignored: "已忽略", deferred: "已延後", blocked: "需處理" };
...
function candidateReviewLabel(candidate: Candidate) {
  ...
  return reviewLabels[candidate.reviewState] ?? candidate.reviewState;
}
...
function impactBlockerReason(reason: string) {
  ...
  if (reason === "unresolved_conflict") return "與正式值不同；請確認接受、修正或排除。";
...
export function DrawingRecognitionReview({ sessionId, returnTo = null }: { sessionId: string; returnTo?: string | null }) {
  ...
  const load = useCallback(async () => {
    ...
    setSession(next);
    setDrafts(Object.fromEntries(next.candidates.map((candidate) => [candidate.id, candidateDraft(candidate)])));
  }, [sessionId]);
  ...
  async function saveDecisions(decisions: Array<Record<string, unknown>>, success: string) {
    ...
    const next = body.session as Session;
    setSession(next); setDrafts(Object.fromEntries(next.candidates.map((candidate) => [candidate.id, candidateDraft(candidate)])));
    setNotice(success);
  }
  ...
  return (
    <main className="drawing-recognition-page">
      <header className="drawing-recognition-header">
        ...
        <div className="drawing-recognition-header-state"><span>工作狀態</span><strong>{statusLabels[session.status] ?? session.status}</strong><small>{session.sources.map((source) => source.fileName).join("、")}</small></div>
      </header>
      ...
      {session.errorSummary ? <div className="drawing-recognition-alert is-error"><AlertTriangle size={16} />{session.errorSummary}</div> : null}
      ...
      <footer className="drawing-recognition-footer"><div><strong>{session.status === "formalized" ? "這批結果已正式寫入" : session.status === "extraction_failed" ? "辨識失敗，請更換來源檔或重新辨識" : ["queued", "extracting"].includes(session.status) ? "辨識處理中" : pendingCount > 0 ? `尚有 ${pendingCount} 筆需要核對` : "所有必要候選已核對"}</strong><span>確認寫入內容會列出每個欄位的寫入前後值，不會直接修改資料。</span></div><div><button type="button" className="secondary-button" disabled={busy || ["queued", "extracting"].includes(session.status)} onClick={() => void rerun()}><RefreshCcw size={15} />重新辨識</button><button ref={impactTriggerRef} type="button" className="primary-button" disabled={busy || pendingCount > 0 || session.status === "formalized" || ["queued", "extracting", "extraction_failed", "cancelled"].includes(session.status)} onClick={() => void previewImpact()}><ChevronRight size={15} />確認寫入內容</button></div></footer>
