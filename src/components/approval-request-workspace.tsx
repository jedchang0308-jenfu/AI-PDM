...
export function ApprovalRequestWorkspace({ requestId }: { requestId: string }) {
  ...
  const [noticeTone, setNoticeTone] = useState<ApprovalOutcomeFeedbackTone>("success");
  ...
  const outcome = request ? projectApprovalDecisionFeedback(request) : null;
  ...
  return (
    <main className="dev079-workspace" data-dev="DEV-079" data-workspace-kind="reviewer">
      <header className="dev079-workspace-header">
        <div className="dev079-workspace-heading"><button className="icon-button" type="button" onClick={() => router.push(returnTo)} aria-label="返回審核清單"><ArrowLeft size={18} /></button><div><span className="eyebrow">審核工作區</span><h1>{request.title || request.actionTitle}</h1><p>{request.actionTitle} · {outcome?.label}</p></div></div>
        <button className="secondary-button" type="button" disabled={busy === "reload"} onClick={() => void load()}><RefreshCcw size={15} />重新整理</button>
      </header>
      {notice ? <div className={`dev079-workspace-notice is-${noticeTone}`} role="status">{noticeTone === "success" ? <CheckCircle2 size={16} /> : <XCircle size={16} />}{notice}</div> : null}
      ...
      <div className="dev079-workspace-grid">
        <section className="dev079-workspace-editor approval-request-workspace-body" aria-label="審核證據與決策">
          ...
          {outcome?.canRetryApply ? <section className="dev079-card dev079-decision-card"><p>{request.applyError || outcome.message}</p><button className="secondary-button" type="button" disabled={Boolean(busy)} onClick={() => void retryApply()}><RefreshCcw size={15} />重試正式化</button></section> : null}
        </section>
