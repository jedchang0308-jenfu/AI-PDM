...
export async function POST(request: Request) {
  ...
  try {
    ...
    const job = await claimDrawingRecognitionJob({ workerId: requireSafeRecognitionId(body.workerId, "RECOGNITION_WORKER_ID_INVALID"), maxAttempts: Number(body.maxAttempts ?? 2) });
