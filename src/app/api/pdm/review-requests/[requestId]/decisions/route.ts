...
export async function POST(request: Request, { params }: { params: Promise<{ requestId: string }> }) {
  ...
  try {
    const { requestId } = await params; const body = await dev087Json(request); const decision = validateDev087ReviewDecision(body.decision); const client = getAsyncDatabaseClient(); const reviews = new PdmWorkReviewAsyncRepository(client); const item = await reviews.get(client, { companyId: access.actor.companyId, requestId });
    if (!item) {
