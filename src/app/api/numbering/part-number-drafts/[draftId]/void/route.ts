...
import { voidPartNumberDraft } from "@/lib/pdm-change-control";
...
export async function POST(request: Request, { params }: { params: Promise<{ draftId: string }> }) {
  ...
  try {
    ...
    const draft = await voidPartNumberDraft({ draftId, actor });
    return NextResponse.json({ draft, pdmCompany: companyResult.company });
  } catch (error) {
    return pdmChangeControlErrorResponse(error, "Failed to void part-number draft");
  }
}
