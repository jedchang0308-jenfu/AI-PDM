...
import { recyclePartNumberDraft } from "@/lib/pdm-change-control";
...
export async function POST(request: Request, { params }: { params: Promise<{ draftId: string }> }) {
  ...
  try {
    ...
    const draft = await recyclePartNumberDraft({ draftId, actor });
    return NextResponse.json({ draft, pdmCompany: companyResult.company });
  } catch (error) {
    return pdmChangeControlErrorResponse(error, "Failed to recycle part-number draft");
  }
}
