...
import { publishRoleCapabilityChange } from '@/lib/repositories/ai-pdm-role-capability-repository'
import { requirePdmRouteAuthorizationAsync } from '@/lib/auth-async'

export const runtime = 'nodejs'

function parseBody(body: Record<string, unknown>): RoleCapabilityMutationInput & { commandId: string } {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(await publishRoleCapabilityChange(parsed), { headers: { 'cache-control': 'no-store' } })
  } catch (error) {
    const code = error instanceof Error ? error.message : 'ROLE_CAPABILITY_PUBLISH_FAILED'
    return NextResponse.json({ error: code }, { status: code.includes('REVISION') || code.includes('CONFLICT') || code.includes('PROCESSING') ? 409 : code.includes('UNAVAILABLE') || code.includes('UNKNOWN') ? 503 : 400 })
  }
}
