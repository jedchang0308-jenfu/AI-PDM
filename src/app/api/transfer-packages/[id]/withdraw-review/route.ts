...
import { withdrawTransferPackageReview } from "@/lib/transfer-package-phase1d";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const access = await requireNumberStateCommandAccessAsync(request, "transfer.package.review.withdraw", body);
  if (access.response || !access.metadata) return access.response;
  const { id } = await params;
  try {
    const result = await withdrawTransferPackageReview({
    ...
    return transferPhase1dErrorResponse(error, "withdraw_review");
  }
}
