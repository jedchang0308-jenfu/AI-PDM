# DEV-069 low-cost Staging IaC

Status: DEV-046 Phase 2B staging resources were created on 2026-07-14 under
...
- Cloud SQL and Cloud Run use deletion protection. Staging Cloud SQL has private
  ...
  authentication. DEV-069 changes both environments to a prelaunch
  `db-f1-micro` / `ZONAL` posture.
- Staging Cloud SQL uses `activation_policy = NEVER` between release-validation
  windows. Override it to `ALWAYS` only for the bounded validation run, then
  restore `NEVER`.
- Cloud Run remains min 0 and is capped at max 2 per revision; the application
  pool is capped at 2.
- The external Application Load Balancer is disabled by default and removed
  from the live state. Firebase Hosting remains the canonical HTTPS entrypoint.
...
- Historical external HTTP/HTTPS Application Load Balancer on
  `136.68.130.84`; DEV-069 decommissions the complete chain and reserved IP.
