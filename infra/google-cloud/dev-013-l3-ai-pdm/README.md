...
The subsequent one-time Secret pinning revision accepts the observed legacy omission of the three fail-closed plain settings declared in `runtime.baselinePlainEnvironmentDefaults`. It materializes only `PDM_TRUST_GOOGLE_WORKSPACE_MFA=false`, `PDM_ALLOW_GOOGLE_WORKSPACE_AAL1_PRIVILEGED=false`, and `PDM_GOOGLE_WORKSPACE_DOMAINS=jenfu.com.tw` while replacing the three `latest` aliases with provider-read numeric versions. An already present value remains unchanged, `PDM_JENFU_SSO_HANDOFF_MODE` must be absent (the application default is `off`) or explicitly `off`, and any other missing preserved environment setting fails closed.

Cloud Run retires an untagged zero-traffic revision immediately when `minInstances=0`. The Secret pin hard join therefore binds the exact Revision API name and etag and requires `Ready=CONDITION_SUCCEEDED`, `ContainerReady=CONDITION_SUCCEEDED`, and `Active=CONDITION_FAILED / RETIRED`, while the Service readback must still show the previous revision at 100% traffic. A separate etag-bound activation plan is the only path that may assign traffic to the pinned revision.
