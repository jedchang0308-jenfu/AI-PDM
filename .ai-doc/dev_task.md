...
- ! DEV-032 [關卡] [Blocked / Release Gate In Progress] [P0] [Release Gate Required] ERP 平台 production release work package
  ...
  - 任務清單：
    ...
    - [x] `Gate A - Production Configuration & Plan Review`：production Firebase/Identity/Web App、env source、Secret Manager metadata、exact artifact及credentialled Terraform plan已完成；saved plan為53 create/5 update/0 delete/0 replace、USD 210，GCS file authority為0。Google provider的Console啟用與production MFA/2SV evidence仍保留為人類工作，不冒充已完成。
    - [ ] `Gate B - Production Resource Apply`：已取得整包核准並部分套用reviewed saved plan；Cloud SQL Regional HA、private VPC、Cloud Run service/job、IAM、Identity config、monitoring與regional logs已完成。apply因憑證過期、Budget幣別及KMS rotation錯誤非零停止；後兩者已修正，待re-auth後readback/import與corrective plan，不得直接重用已部分套用的舊plan。
  ...
  - 下一步：完成一次Google CLI re-auth後，先做Gate B state/cloud readback；若backend service已由失效前的operation完成則import，重新產出corrective plan並確認0 delete/0 replace及USD 240停止線，再apply。Gate B readback通過前不得bootstrap/migrate/seed。
