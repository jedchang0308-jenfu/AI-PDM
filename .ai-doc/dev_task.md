...
- ! DEV-032 [關卡] [Blocked / Release Gate In Progress] [P0] [Release Gate Required] ERP 平台 production release work package
  ...
  - 任務清單：
    ...
    - [ ] `Gate D - Immutable Deploy & Smoke`：exact application image index `sha256:b4fb8e9f...`已部署，Cloud Run執行的linux/amd64 child manifest `sha256:570dd9f0...`父子關係已驗證；production AAL1 pilot契約已對齊，Cloud Run v2 traffic-only rollback/restore與後續Terraform no-drift通過。rollback runner已固定`updateMask=traffic`、禁止會產生template drift的`gcloud run services update-traffic`路徑，並以current latest revision完成production `validateOnly` preflight。依使用者既有決策，短期production pilot採`https://jenfu-ai-pdm-prod.web.app`且不設定DNS；2026-07-16 Hosting gateway saved plan為1 create/2 update/0 delete/0 replace、USD 210，套用後Cloud Run revision `ai-pdm-prod-00005-hfs`維持相同immutable image digest，Terraform 0 drift。專用production Hosting config已發布；`/login`、`/api/auth/mode`、Firebase auth handler/init、未登入protected API 401、private/no-store headers及direct `run.app` session exchange 403均通過。初次popup發現OAuth `redirect_uri_mismatch`，已在production OAuth client補上`web.app` JavaScript origin與`/__/auth/handler` redirect URI並由管理介面回報儲存成功；重測已到達Google帳號選擇頁。Gate D仍待真人登入後的authenticated Level 4與功能smoke；Phase 3A file paths保持fail-closed。
  ...
  - 下一步：人類只需在`https://jenfu-ai-pdm-prod.web.app`以`jedchang0308@jenfu.com.tw`完成一次Google登入並回報結果。Codex再讀取verified Firebase UID，完成principal bootstrap、獨立restore/reconciliation、Level 3/4 authenticated smoke及production UI acceptance自動化。其餘1-3位canary不得由AI自行臆測；不要求設定DNS。
