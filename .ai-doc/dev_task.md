...
- `DEV-046` Phase 1A-1E、Phase 2A staging IaC、Phase 2B local application slices 與 Phase 2B staging authentication activation 均已完成。`HD-10-1 / 1A` 採單區 staging、Regional-HA production，保守估算 USD 210。staging 入口仍為 `https://jenfu-ai-pdm-stg-361825.web.app`，公開 DNS/TLS 依使用者決策延後；production 採 ALB/custom domain，並拆成 3A 領號／草稿與 3B GCS 檔案切換，ProJED 未修改。2026-07-16 `DEV-032 Gate A` 已完成：`jenfu-ai-pdm-prod` 已連結 Paid Billing、Firebase/Identity Platform/Web App、兩個通知 channel、兩個 deletion-protected session secrets、Artifact Registry、private regional Terraform state與exact-commit images均已建立；credentialled saved plan為53 create/5 update/0 delete/0 replace、USD 210且無GCS file-authority資源。Gate B 套用同一plan後，Cloud SQL Regional HA、private VPC、Cloud Run service/migration job、Identity config、IAM、monitoring、regional logs等主要資源已完成，但因gcloud token於長時間Compute polling期間失效，加上原Budget幣別與asymmetric KMS rotation設定錯誤而以非零結束；Budget已修正為TWD 9,600、KMS改manual key-version rotation，尚待一次Google re-auth後做state/cloud readback、import任何已完成但未入state的backend service並產出0 delete/0 replace corrective plan。未執行admin bootstrap、migration、seed、DNS或production Level 4 smoke。
...
- ! DEV-032 [關卡] [Blocked / Release Gate In Progress] [P0] [Release Gate Required] ERP 平台 production release work package
  ...
  - 任務清單：
    ...
    - [ ] `Gate C - Clean Seed & Continuity`：mutation前on-demand backup `1784136240742`、privileged admin bootstrap、18筆schema migration與立即idempotence rerun已完成；migration Job已還原dry-run posture。真實production Firebase user仍為0，必須先由`jedchang0308@jenfu.com.tw`完成一次Google登入取得verified `google.com` UID，之後才可執行principal seed、pre-canary reconciliation、獨立restore target及restore reconciliation。
    - [ ] `Gate D - Immutable Deploy & Smoke`：exact application image index `sha256:b4fb8e9f...`已部署，Cloud Run執行的linux/amd64 child manifest `sha256:570dd9f0...`父子關係已驗證；production AAL1 pilot契約已對齊，Cloud Run v2 traffic-only rollback/restore與後續Terraform no-drift通過。rollback runner已固定`updateMask=traffic`、禁止會產生template drift的`gcloud run services update-traffic`路徑，並以current latest revision完成production `validateOnly` preflight。依使用者既有決策，短期production pilot改採`https://jenfu-ai-pdm-prod.web.app`且不設定DNS；Hosting只做rewrite，Cloud Run/Cloud SQL權威不變，direct `run.app` session exchange維持fail-closed。Hosting gateway、Level 3 HTTP與Level 4 post-deploy smoke仍待執行；Phase 3A file paths保持fail-closed。
  ...
  - 下一步：Codex先以saved plan啟用production Firebase Hosting gateway並完成未登入smoke；之後人類只需在`https://jenfu-ai-pdm-prod.web.app`以`jedchang0308@jenfu.com.tw`完成一次Google登入並回報結果。Codex再讀取verified Firebase UID，完成principal bootstrap、獨立restore/reconciliation、Level 3/4 authenticated smoke及production UI acceptance自動化。其餘1-3位canary不得由AI自行臆測；不要求設定DNS。
