...
> **2026-09-22 DEV-118 local logout correction（current）**：Production browser檢查發現已登入側欄帳號入口仍是被動`/login`連結，未結束AI-PDM本地session，導致PDM C01／C02無法建立指定的fresh start。Current source已改為明確local logout：只呼叫既有`POST /api/auth/logout`，成功後進入`/login?reason=local-logout`；失敗時保留session並顯示可重試錯誤。Focused contract=`14/14 PASS`、authenticated real-browser=`33/33 PASS`、`typecheck:app=PASS`；browser run=`DEV118-browser-2026-09-21T22-24-37-833Z`，task-owned port、fixture、dist與`next-env.d.ts`均清理／還原。此修正尚未發布至Production；LOGIN full、C01／C02、Free、deny-path與global logout仍為`0/6 / NOT_RUN`，不得以local PASS升格為`LIVE_VERIFIED`。
...
- ◐ DEV-118 / Production Released / Firebase Google Provider Enabled / Workspace Target Partial Evidence / C01-C02 Browser Verification Pending / 118-A本地入口與118-B跨專案工程契約均已完成；118-C owner release已完成，剩餘工作只限Production互動驗收。
  ...
  118-B已登錄Platform `DEV-014 / 014-LOGIN`並完成Platform migrations 006／007、OrgMaster migrations 016／017、DWD與雙admission apply＋replay。118-C owner run `35592944590`已把exact source `68d019d93d267cf284ea8aba637f305558d9048e`發布至revision `ai-pdm-prod-face545d349c`，image `sha256:6b803aca8da983b909a05b2a4348f3c14ca47148197c44d71a91762ad81f2ec5`承接100% traffic；terminal receipt位於`DEV014-REL-20260921-AIPDM-R3`。2026-09-22 Firebase `google.com` provider由Admin API讀回`enabled=true`且client ID／secret均存在；Platform correction發布後，受控Workspace帳號由Platform normal entry建立session，Portal→AI-PDM未要求第二次登入，reload後`/api/auth/me`與`/api/admin/accounts`皆200，`accounts.lifecycle.manage`由`orgmaster_authority:6 / role-system-admin`允許。這是C03-like partial evidence；完整LOGIN六案與PDM C01／C02仍待受控Workspace／Cloud Identity Free principals完成。Authority 為
...
- ◐ DEV-118 [開發點] [Production Released；Firebase Google Provider Enabled；Workspace Target Partial Evidence；C01／C02 Browser Verification Pending] [P0] [RD／Owner Release Complete／正式互動驗證待完成] 平台登入入口對齊
