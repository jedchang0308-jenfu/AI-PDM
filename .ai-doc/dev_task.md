...
> **2026-09-15 DEV-012 R71 cancelled drawing correction（current）**：R71正式data import在單一transaction內因`drawings` reconciliation不一致安全ROLLBACK；legacy service已恢復，task-owned Job／temporary IAM皆已清除，candidate／entrypoint／traffic仍為0。Safe metadata顯示source=53、target=50，三筆缺件為合法cancelled歷史列；folded baseline 001的完整`UNIQUE(company_id,drawing_number)`與current lifecycle rule衝突。已新增forward-only migration 064移除obsolete constraint並保留既有「非cancelled才占號」partial unique index，V3 manifest、runner與cutover ledger denominator同步為15；DEV-010 N1C 26／26、continuous 32／32、data-cutover 17／17及DB boundary均PASS。Fresh R72須先完成clean source freeze、immutable migration image／APP_INFRA image rotation與064 provider receipt，再重跑151-table cutover；目前不得標Deployed或Release Ready。

> **2026-09-15 DEV-012 R66 production data cutover implementation（current）**：R63 的 `principal_not_active` 已確認是 legacy 正式資料尚未搬入 neutral `ai_pdm_core`，不是 Firebase token 或 TOTP 問題。本 repo 已完成 app-owned one-time `prepare → fence → export → import → reconcile → handoff` executor、151-table exact plan、13欄固定轉換、既有管理員 UID exact-one remap、generation-bound raw bundle、task-owned IAM／Job cleanup、owner prepare／verify hard gate，以及 `LIVE_VERIFIED` 後自動刪 raw bundle；後續 ordinary release 改讀已發布的 `NEUTRAL_AUTHORITY_LIVE` completion receipt，不再要求 legacy cutover。Local結果為data-cutover 15／15、continuous owner 32／32、cutover QC PASS；isolated PostgreSQL因本機磁碟低於資源安全線而 `NOT_RUN`，正式Billing／backup／PITR／DB／service／IAM／Job／traffic亦 `NOT_RUN`。Current source尚待commit／push與fresh source freeze，不能標為Release Ready或Deployed。
