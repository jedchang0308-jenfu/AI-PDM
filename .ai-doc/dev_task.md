...
- 狀態資料重建：`✓ DEV-087` 三工作臺狀態資料重建與人類語意極簡整頓。
  - 狀態：`Local RD Complete / Focused QA-QC PASS / Local Canonical-Only Preservation Migration Validated / Retirement Pending / Production Migration & Release Gated`。
  ...
  - UI-only完整生命週期QA：圖號27、料號20、圖料根號20，共67條真實UI journey；另有11個共同hard gates。AI只能透過rendered UI產生business mutation；API與DB只能唯讀取證。每個穩定checkpoint都必須UI／API／DB identity、數量、版次、handling、action與資料hash一致；完整PASS固定`67/67 + 11/11 + Blocked=0 + Not Run=0 + P0/P1=0`，focused runner或DEV-074舊58/58不得代替。
  ...
  - 本機保留 migration：使用者選擇 `A` 後，以 `--retain-unmapped-legacy` 對44筆ambiguous active bundle、9筆未核准part-only bundle與3筆legacy cancelled workspace共56筆資料標記 `retained_legacy_source`；不刪除、不補假 `0.1`、不拆成多個current work。主DB legacy graph hash與筆數不變，authority已切為`canonical_only`，canonical list API與頁面回應200；證據見`output/qa/dev-087-local-preserve-apply/manifest.json`及`.ai-doc/qa/qa-dev-087-status-data-rebuild-validation-plan-2026-08-21.md`。
  - 下一步：本機仍須完成獨立QA、UI-only 67/67+11/11、PostgreSQL rehearsal、retirement gate與release gate；production不得直接沿用local preservation apply。legacy source只保留於後台 quarantine，不進三工作臺；若未來要重新處理，必須另開明確 re-entry decision，不得由AI session自行猜測映射。
...
- ✓ DEV-087 [交付點] [Local RD Complete / Focused QA-QC PASS / Local Preservation Migration Validated / Retirement Pending] [P0] [Independent Full QA & Production Release Gated] 三工作臺狀態資料重建與人類語意極簡整頓
  ...
  - 下一步：本機schema、inventory/converter、canonical commands/read、UI/browser與retirement路徑已實作並通過focused QA/QC；local A preservation migration已使56筆legacy quarantine resolved且不刪source。正式環境仍須獨立QA、PostgreSQL rehearsal、same-window retirement、deploy/release授權，不得把local canonical-only結果當production readiness。
  ...
  - 證據：2026-08-21～22 使用者逐輪 Human Decision 已收斂；2026-08-22 完成14-table canonical schema、042/SQLite ensure、converter、三domain repository/commands/routes/UI、Drawing exact revision file/recognition、transient review與hard retirement。最新 `npm run qc:dev-087` aggregate 8/8 PASS：contract 28、repository 17、commands 39、migration 24、retirement 30、browser 46、typecheck與isolated build；另完成preservation sandbox與主DB apply：`unresolvedBeforeResolution=56`、`retainedLegacy=56`、`unresolved=0`、legacy graph hash unchanged、integrity／foreign-key／API smoke PASS。完整報告見`.ai-doc/qc/qc-dev-087-local-implementation-2026-08-22.md`、`.ai-doc/qc/qc-dev-087-ui-only-lifecycle-execution-2026-08-22.md`與`output/qa/dev-087-local-preserve-apply/manifest.json`；惡意token、CSRF、DoS、側通道與證據偽造攻防依使用者決策延後。
  - 獨立UI操作驗證：2026-08-22已補三個 rendered UI supplemental journey（Drawing／Part／Relation 建立或進版→進入工作頁→取消→回清單），並完成完整67-case runner。最新 full run `DEV087-ui-only-2026-08-22T07-00-51-231Z` 為`PASS=1／BLOCKED=66／FAIL=0`、C01～C11 `11/11 PASS`、infrastructure `5/5 PASS`、console／unexpected failure `0`；66 BLOCKED 均是缺少合法可重複 UI 前置或 Merged/history row，不得用 seed／SQL／business API 補造，亦不構成產品 PASS。先前 relation 取消後導向不存在路由的真實404已修正，最新 J03 PASS且目前無產品級FAIL；DEV-087仍未達67/67放行門檻。完整報告見`.ai-doc/qc/qc-dev-087-ui-only-lifecycle-execution-2026-08-22.md`。
  - 2026-08-22 QC journey 增補與根因修正：補入 `D21` 進版／作廢競合及 `D22` reviewer 同 request 決策競合的 rendered UI journey。首次 full run `DEV087-ui-only-2026-08-22T13-08-33-316Z` 證實 D22 的真實產品缺口：active review request 被刪除後，decisions route precheck 將第二 reviewer 回 404；已在 `pdm_work_review_terminal_receipts` 保留最小 `request_id + company_id + decided_at` receipt，並在 route precheck 與 service 統一回 `409 WORKBENCH_REVIEW_REQUEST_STALE`。新 focused run `DEV087-ui-only-2026-08-22T13-17-05-172Z` 與無 focus full run `DEV087-ui-only-2026-08-22T13-20-00-854Z` 均確認 D22 `200/409`、`J-D21/J-D22 PASS`、`FAIL=0`、`C01-C11=11/11 PASS`、consoleErrors=0；最新 full coverage 為 `40 PASS／27 BLOCKED／0 FAIL`。缺口 ledger 統一為 `GAP-PROD-01A`（workspace layout／Save interception，已關閉）、`GAP-PROD-01B`（preview terminal race，已關閉）、`GAP-PROD-02`（review terminal race，已關閉）；其餘27 BLOCKED分流為尚未取得合法 UI fixture／sequence、terminal/history UI 前置不存在及 DEV-088 attachment scope，尚無 open product FAIL。DEV-087仍是 `NOT PASS`，須補合法前置後達 `67/67 + Blocked=0 + NotRun=0` 才可放行。TypeScript `npm.cmd run typecheck:app` 與 runner `node --check` 通過，完整報告見`.ai-doc/qc/qc-dev-087-ui-only-lifecycle-execution-2026-08-22.md`。
  ...
  - Clean State Data Target（第一性原理）：
    - 一筆工作臺狀態只回答三件事：`哪個 canonical entity`、`哪一條 lane／revision`、`目前由哪個角色處理或是否受阻`。其餘資料是 domain evidence，不是另一套 current status。
    - 新 authority正式table名為 `canonical_workbench_states`。Drawing production唯一鍵為`company + drawing + production`，Drawing RD唯一鍵為`company + drawing + branch_id`，因此可有一列production與0..3個open branch latest；Part／Relation則維持`company + entity + data_layer`，最多一列formal與一列work。
  ...
  - Migration Direction：
    ...
    4. `Reconciliation`：對全量資料產出source/target count、identity hash、unmapped、duplicate、invalid branch/reference、duplicate target claim、production-without-approved evidence、work-without-owner與quarantine；ambiguous不得猜latest，全部repair／人工確認`source_unknown`／明確刪除後，unresolved必須為0。
  ...
  - Human Confirmed Drawer Contract（2026-08-22 amendment）：
    - 三工作臺使用同一個唯讀快速查閱骨架與固定順序：`主識別／品名／處理狀態` → `主要內容／預覽` → `直接關聯` → `受阻資訊（條件式）` → `歷史版次（僅圖號）` → `動作區`。不同階段只改變處理狀態、受阻資訊與可執行動作，不重排抽屜。
    - 抽屜不承載既有版本、料號或圖料根號的 mutation form；編輯與審核導向真實 canonical full-page workspace。「進版」是由目前圖號建立新版本的導覽入口，不是修改目前版本。
    - 圖號抽屜顯示圖號、品名、`量產版 {revision}`／`研發版 {revision}`、必要時的處理狀態、2D／3D 預覽、目前版受控檔案、直接關聯、受阻資訊與歷史版次。歷史版次固定呈現清單，每列只呈現版次與版本列；開啟後只能查看該版預覽／檔案，不得修改歷史版。
    - 料號抽屜顯示料號、品名、`正式資料`／`修改中`、必要時的處理狀態、料號基本資料、料號自身附件、直接關聯與受阻資訊；不得顯示料號版本或歷史版次。相關圖號可顯示自己的版次，但該版次明確屬於圖號。
    - 圖料根號抽屜顯示圖料根號、名稱、`正式關聯`／`調整中`、必要時的處理狀態、關聯結構摘要、直接關聯與受阻資訊；不得顯示根號版本、歷史版次或「共同檔案／圖料層直接管理的檔案」。
    - `直接關聯`是三抽屜唯一關係區名稱及共用 component：圖號列出直接關聯的圖料根號與料號；料號把既有「圖號關聯」與「所屬圖料根號」合併，列出直接關聯的圖號與圖料根號；圖料根號列出直接關聯的圖號與料號。不得另建同義關係區塊。
  ...
  - Human Confirmed Review／Formalization／Retention Contract：
    ...
    - legacy migration不得猜測來源；依2026-08-22 Human decision `A`，本機無法唯一映射的active／cancelled legacy source以`retained_legacy_source`保留在quarantine，不轉新minimal trace、不進UI、不驅動canonical current state，也不刪source graph。`--retain-unmapped-legacy`與discard flag互斥；若未明示保留，仍以unresolved quarantine阻擋切換。physical bytes只有零有效引用、approved-artifact guard與canonical-only gate全通過才永久刪除，不提供備份回復、使用者恢復或UI復原入口。
  ...
  - 清單與 lane contract：
    ...
    - `已取消／已作廢／已合併`等terminal record不進目前工作臺。新系統未核准取消work依SPEC刪除但minimal review trace保留；本機A preservation run的legacy cancelled source只保留於backend quarantine，不進canonical list。已核准Drawing history、Part／Relation approved snapshots仍保留。
  ...
  - Scope：
    ...
    - 收斂三工作臺唯讀抽屜的資訊順序、domain-specific projection、`直接關聯`、受阻資訊、圖號歷史版次與單一 action owner；圖號進版入口依 server applicability提供量產／研發兩個明確目標，idle RD另提供受控次要風險action `申請作廢`。
  ...
  - Acceptance Direction：
    ...
    4. 全量migration／converter reconciliation的unexpected unmapped、duplicate、invalid branch/reference、duplicate target claim、production-without-approved evidence、work-without-owner、hash mismatch與unresolved quarantine全為0；source／target counts與protected evidence hashes可重現。
    ...
    30. Part／Relation有未儲存變更時唯一primary是`儲存修改／儲存調整`，資料已儲存且完整時才切為`送交審核`；三domain review_owner都在同domain editor/layout fully-readonly，一般decision只有`核准／退回修改`；system／system_admin／blocked無編輯入口。
    31. review decision click才建立minimal trace；return沿用同work並新開下次cycle，approve凍結exact snapshot並自動正式化。Part附件不進snapshot/review；system retry不得重算latest。
    32. 新未核准cancel會刪除work/ref/predecessor/claim並允許revision重用，但保留已產生minimal trace；依Human decision A，legacy cancelled data連同old review保留於quarantine並標記`retained_legacy_source`，不進canonical、不轉minimal trace。physical bytes只有零有效引用、approved-artifact guard與另行核准的canonical-only destructive gate通過後才可永久刪除，且沒有備份回復功能。
...
- 2026-08-21（DEV-087初版RD Contract／歷史，已由後續決策取代）：新增`SPEC-PDM-STATUS-DATA-REBUILD-001`、配對ADR與`QA-087-001..066`，將Brief升級為可派工contract。初版方向僅作文件演進記錄；其中legacy cancelled清除方向已被2026-08-22 Human decision A 的quarantine保留策略取代，不代表目前狀態或QA PASS。
...
- 2026-08-22（QC journey 補齊後最新分流）：P05 fresh `DEV087-ui-only-2026-08-22T14-10-35-764Z` PASS，P04→P05 sequential `DEV087-ui-only-2026-08-22T14-13-10-742Z` PASS；儲存控制項重繪競態已由 UI runner 重解定位／有限重試修正。最新無 focus full `DEV087-ui-only-2026-08-22T14-13-47-545Z` 為`41 PASS／26 BLOCKED／0 FAIL`、gates `11/11`、console `0`。已證實產品缺口為`0`；D07/D09/D12/D14/D15/D16/D17/P04/P05/R13均由 fresh／sequential UI triad 證實為 runner／fixture sequence，不改產品邏輯；P11–P17屬DEV-088；D25–D27/P18–P20/R15–R20列為尚未證實的產品能力／契約缺口候選。完整DEV-087仍`NOT PASS`，放行門檻維持`67/67 + Blocked=0 + NotRun=0 + gates=11/11 + P0/P1=0`。詳見`.ai-doc/qc/qc-dev-087-ui-only-lifecycle-execution-2026-08-22.md` §25與`.ai-doc/qa/qa-dev-087-ui-only-lifecycle-operation-validation-plan-2026-08-22.md` §25。
