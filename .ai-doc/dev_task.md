...
- ✓ DEV-087 [交付點] [Local RD Complete / Focused QA-QC PASS / Local Preservation Migration Validated / Retirement Pending] [P0] [Independent Full QA & Production Release Gated] 三工作臺狀態資料重建與人類語意極簡整頓
  ...
  - 獨立UI操作驗證：2026-08-22新增67條三工作臺生命週期journey與11個UI／API／DB一致性hard gates；目前為`Focused UI Rerun Partial / Full UI-only Contract-blocked`。最新focused aggregate與system_admin／blocked fault profile皆通過，但尚未產生完整67條 journey evidence bundle；GAP-UI-01～03仍需產品／ADR固定或正式移出本DEV，GAP-UI-04已在focused scope關閉。API／DB只允許唯讀readback，任何一層不一致或缺合法UI前置均不得PASS；完整報告見`.ai-doc/qc/qc-dev-087-ui-only-lifecycle-execution-2026-08-22.md`。
  ...
  - Acceptance Direction：
    ...
    32. 新未核准cancel會刪除work/ref/predecessor/claim並允許revision重用，但保留已產生minimal trace；legacy canceled data連同old review全刪。physical bytes只在零有效引用、approved-artifact guard與canonical-only gate通過後永久刪除，且沒有備份回復功能。
