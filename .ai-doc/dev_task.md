...
Active objective: `DEV-PDM-CHANGE-CONTROL-001` Phase 3 drawing revision FFF flow is in local evidence capture; `DEV-SW-LICENSE-PDM-001` is committed locally; `DEV-SUPABASE-DB-001` production/cutover remains deferred.
...
|---|---|---|---|---|---|---|
| Evidence Captured / Phase 3 | `DEV-PDM-CHANGE-CONTROL-001` | Delivery point | None | Phase 1 local schema/domain service, Phase 2 part-number draft module, and Phase 3 drawing revision FFF flow implemented: FFF three-state assessment API/UI, confirmed-impact replacement draft creation, drawing part-number read/correction audit fields, self-made replacement part-number match gate, and original drawing-number preservation; no Phase 4-5 review/BOM release transaction or production/Supabase cutover implemented | PM chooses next slice: Phase 4 review flow, Postgres/Supabase migration mirror under existing runtime governance, or keep later phases deferred | ADR `.ai-doc/decisions/ADR-PDM-CHANGE-CONTROL-001-reserved-draft-number-policy.md`; SPEC `.ai-doc/specs/SPEC-PDM-CHANGE-CONTROL-001-revision-part-bom-flow.md`; implementation contract `.ai-doc/specs/SPEC-PDM-CHANGE-CONTROL-001-implementation-contract.md`; QA plan `.ai-doc/qa/qa-pdm-change-control-validation-plan-2026-06-24.md`; `db/schema.sql`; `src/lib/pdm-change-control-domain.ts`; `src/lib/pdm-change-control.ts`; `src/lib/pdm-change-control-api.ts`; `src/app/api/numbering/part-number-drafts/*`; `src/app/api/numbering/drawing-revisions/fff-assessments/route.ts`; `src/app/numbering/part-drafts/page.tsx`; `src/app/numbering/revisions/page.tsx`; `scripts/qc-pdm-change-control.mjs`; QC reports `.ai-doc/reports/qc/qc-pdm-change-control-phase-1-report-2026-06-24.md`, `.ai-doc/reports/qc/qc-pdm-change-control-phase-2-report-2026-06-24.md`, `.ai-doc/reports/qc/qc-pdm-change-control-phase-3-report-2026-06-24.md`; `npm.cmd run qc:pdm-change-control` 37/37; `npx.cmd tsc --noEmit --pretty false`; focused ESLint; localhost `/numbering/part-drafts` and `/numbering/revisions` HTTP 200 |
...
|---|---|---|
| PDM change-control Phase 1-3 | Passed / local schema, domain service, draft API/UI, and drawing revision FFF API/UI evidence captured; review/BOM release flows deferred | `npm.cmd run qc:pdm-change-control` |
