...
> **2026-09-26 DEV-121 principal 管理員撤銷登入（本機，未發布）**：補齊原 principal 帳號頁缺少的「撤銷登入」與 API v2 路徑；獨立驗 `accounts.session.revoke`、已驗 principal AAL2 管理員及 target exact principal↔profile。forward-only 068 owner command 在 SERIALIZABLE transaction 原子推進 lifecycle/session barrier、撤銷 principal registry 並寫 immutable operation receipt，不改帳號狀態或舊 `users` 欄位；UI 保存 operation ID／reason 供未知完成重播。聚焦 API／service 4／4、隔離 PostgreSQL schema／ACL 22／22（含拒絕自撤、能力否定、重播與版本不重增）、DEV-005 runtime QC、route classification、TypeScript、scoped ESLint、DB boundary PASS，測試 cluster／埠／暫存已清理。此切片尚未 PR／merge／Production migration／release／L4；全體 cutover、source attestation、recovery 與舊 UID／v1 正常 runtime 退役仍未完成。

> **2026-09-26 DEV-121 最新進度**：AI-PDM PR #74 已合併至受保護 main `f0ac74902b1053a33e19cbd6629b3d6c742c5c53`，修正後 immutable inventory image `sha256:affb1f4fa335737ad630a38a87925b2963e38830974c8f0a46c1f8f7cf76d33f` 經 Cloud Build／Artifact Registry readback。Production coverage R1 證明 4 個 active profile 起始均無 marker；Jed 的唯一已發布 exact Firebase pair 經 preview、source hash／row version 0 綁定後，受控註冊為 `legacy_compatible:1`。相同 operation replay receipt bytes 不變，coverage R2 證明只有這一筆 marker 改變，另 3 個 profile 與全部來源列未變；暫時 Job 已刪除。此切片沒有 role／ACL／session／authority 或 service／traffic 變更，不是 principal cutover 或 L4。migration-only R2 已套用 065／066／067，OrgMaster v4 consumer readback 已 PASS。PR #76 已合併至受保護 main `94ec0e8133389e1c012c115142f678c357a896bc`，交付未啟用的 profile-transfer 預備切片：舊本地 UID 與新 principal pair 分開核對，唯讀 v2 preview 讀回確認收據，owner 套用入口拒絕缺少或不符收據；隔離 PostgreSQL 21／21、preview／operator 7／7 與 PR 兩項 CI PASS。這不構成 Production apply 放行、回復或 L4 證據，也未更動 Production service／traffic。剩餘 profile 的舊 Firebase UID 未必等於新 managed Google 登入 pair，逐人確認前不得假造 alias 或切換權限。朱宇鴻與張祐豪待確認 Employee／個人主帳號及角色；蕭旋吟的共用 `info@` profile 暫不設定，待其個人帳號。證據及承接契約見 [DEV-121](specs/DEV-121-target-authorization-boundary.md)。

> **2026-09-26 DEV-121 R5 唯讀診斷與來源收斂（現行）**：protected main `7ccdd4995a7574e358d414cb471201e84e34734a` 的私有 receipt `gs://jenfu-platform-prod-aipdm-release/receipts/releases/DEV121-PRINCIPAL-INVENTORY/coverage-7ccdd499-r5.json#1790374337857692` 再確認四個 active profile 均無 cutover marker：一個管理者 Firebase pair 有 canonical／typed，另三個 Firebase pair 缺 OrgMaster 對應。兩筆本地 Google OAuth alias 雖標 active，正式 `firebase_bff` 的舊 OAuth start／callback 已回 404，不是當前可登入來源。私有標籤顯示一筆使用已裁定的共用信箱，仍須核對個人承接者；不得依姓名／email 自動綁定。臨時 AI-PDM Job 已刪除，immutable image／receipt 保留。owner inventory preview／register 的本機修正以明示 `firebase_bff` policy 只核對可登入的 Firebase pair，不要求歷史 Google alias 另造 principal；舊 session fence 獨立驗證。operator 6／6、隔離 PostgreSQL 21／21、typecheck、DB boundary PASS；本修正尚未 protected merge／發布，帳號轉換、candidate／traffic、L4 仍未執行。

> **2026-09-26 DEV-121 雙 owner 缺列 readback（現行）**：AI-PDM R4 與 OrgMaster read-only Job 以六個精確 provider pair hash 對照；僅既有 admin Firebase pair 在 canonical／typed／治理／reservation 可解析，其餘五組四個來源皆無列。OrgMaster 一筆未綁 provider pair 的 managed identity 不能被猜作其一。為辨識三個未映射 Firebase profile，私有 coverage 收據補顯示名稱與聯絡 email，僅供人工核對，不參與 identity mapping／authorization；正式切流仍暫停。證據與停止條件見 [DEV-121 雙 owner readback](specs/DEV-121-target-authorization-boundary.md)。

> **2026-09-25 DEV-121 正式命令去舊橋接（本機）**：PostgreSQL outbox 命令現在要求當次 request 的 v2 verified principal context，舊 target session／legacy mapping 在讀寫前拒絕；`pdm_user_id` 只定位業務 profile，不能重定 security principal。聚焦命令／snapshot 25／25、app typecheck PASS。這個 source 尚未發布；正式切流須先把有效 cohort 的權限與 principal materialize 完成並控制寫入窗口，避免尚在 v1 的使用者命令被中途拒絕。SQLite 只作隔離歷史 fixture，後續仍須移除 v1 登入／UID resolver／local ACL 正常 runtime 分支及完成 Production L4。

> **2026-09-25 DEV-121 principal grant v2 consumer（本機，未發布）**：正式 entitlement reader、AAL2、reviewer 候選與 cutover producer seal 已改成以 canonical `principal_id` 讀 OrgMaster 025 的單一 v2 grant view；provider alias 僅作登入／typed admission 驗證，grant 事實不含 issuer／subject。歷史 065 migration 不改，新增 forward-only 067 將帳戶管理 SQL 權限檢查從 v1 alias grant view 轉到 v2，owner profile 已納入第18筆。聚焦單元 29／29、隔離 PostgreSQL authority snapshot 2／2、schema／ACL／command 21／21、owner release 9／9、DB boundary 與 app typecheck PASS；PG 測例在套用 067 後刪除 v1 grant fixture 仍通過。OrgMaster producer 隔離回歸 17／17（含 AI-PDM runtime／migrator 精確讀取、Platform 拒讀及 unresolved pair 拒授權）。這些是 dirty worktree 本機證據；三專案 protected source、真實 provider readback、正式 owner release、全 cohort cutover、recovery、L4 與 v1 正常 runtime 退役仍未完成。下方逐 alias 對等與 065／066 尾筆描述為當時紀錄，不能作目前部署指令。

> **2026-09-25 DEV-121 舊映射 writer 退役與 preview 封裝（本機）**：刪除唯一直接從 `users` 產生 `pdm:*` principal／舊組織映射的本機 generator 及 npm 入口，避免繼續製造舊安全主體；歷史資料與原始 Git 證據保留。唯讀 cutover preview 改用獨立最小 operator Dockerfile／ESM package，沿用鎖定的 `pg` lock，不改既有 migration-runner image；完整執行時 import closure／runner 聚焦 4／4、TypeScript 模組載入 PASS。尚未 build immutable image、執行 Production Job、取得 provider source attestation 或切換任何 principal。全 cohort／回復驗收後繼續拆除舊 UID／v1／legacy mapping 正常 runtime 授權分支。

> **2026-09-25 DEV-121 cutover 唯讀 owner preview（本機）**：新增正式目標鎖定的 read-only owner Job runner，可由固定 bucket operation 在同一 RR 唯讀交易讀當前 local／Platform／OrgMaster facts，輸出有界 workspace shadow 與 source hashes 的 immutable receipt；同 operation 重跑先核對既有receipt，相符則重播、衝突拒絕。receipt 固定標明 `sourceBindingsAttested=false`、`applyAllowed=false`，不執行 principal 切換。補齊隔離 PostgreSQL 的完整 v4 catalog fixture 後，schema／ACL 21／21、runner 3／3、TypeScript、scoped ESLint PASS，測試 cluster／port／temp 已清理。2026-08-25 本機舊資料快照僅見10個有效 profile、0筆有效顯式指派／委派／scope rule，**不能代表目前 Production**。正式來源綁定、現況 preview readback、完整 resource／delegation behavior shadow、owner apply、全 cohort、v1/v2 recovery 與 Production L4 仍未完成；切換及回復驗收後須拆除舊 UID／role 正常 runtime 授權分支。

> **2026-09-25 DEV-121 cutover producer alias 一致性（本機）**：同一 `principal_id` 的多個已驗 provider alias，封存前須讀到相同 Platform state、OrgMaster authority 及同一完整有效 grant 集合；只在雜湊與比較時去除 grant 的 issuer／subject 欄，來源 receipt 仍保留各 alias 原始事實。任一 alias 少 grant、多 grant 或重複列即拒絕，不把 alias 差異合併成 principal 權限。聚焦 3 檔 9／9、TypeScript、scoped ESLint PASS；這是來源一致性修正，完整 resource／delegation behavior shadow、owner prepare／apply、real-provider 與 Production L4 仍未完成。

> **2026-09-25 DEV-121 審批證據檔案 principal 收斂（本機）**：v2 `approval_evidence` GET 已接 verified principal RR snapshot，核對 `approval.request.decide`、案件公司、request impact snapshot 中的 exact asset membership 與檔案綁定；因 `file_assets` 無公司欄，v1／v2 共用查檔 SQL 另要求候選圖面／圖面 package／revision／提交檔案的同公司關聯。舊審批資料僅作歷史業務讀取，不再作 UID 授權來源。外公司案件、未列於快照、無公司關聯的資產及無能力均不得讀取物件。附件／審核聚焦 5 檔 37／37、TypeScript、scoped ESLint、DEV-005 inventory 77／94／103、route classification 255／293、DB boundary PASS；隔離 PostgreSQL provider 9／9、實際附件 SQL 同公司／跨公司／孤兒／刪除／錯誤綁定 8／8 PASS，task-owned cluster 與埠 55439 已清理。整體 cohort、real-provider 與 Production L4 尚未完成；最後一個仍在使用的 v1 cohort 切換及回復驗收後，舊 UID／role route 分支須從正常 runtime 移除，不作長期相容層。

> **2026-09-25 DEV-121 附件 direct-read principal 收斂（本機）**：v2 已將候選圖面、圖面 revision／package、圖面附件與零件附件的 GET 連到 verified principal 快照，依路由契約分別驗 `numbering.drawings.view`／`numbering.search`、公司及 exact context／binding／asset；審核中的附件仍須走 v2 immutable `review_package`，不能夾帶 reviewRequestId 借 direct-read scope。審核包 SQL 另排除 removed binding 與 deleted asset。`approval_evidence` v2 尚未有 request-scoped 契約，明確拒絕而不落回 UID guard。聚焦附件 13／13、附件／審核閉環 31／31、TypeScript、scoped ESLint PASS；隔離 PostgreSQL provider 9／9、DEV-121 授權快照／authority race 2／2 PASS，埠 55439 及暫存 cluster 已清理。該 PG 驗證尚未涵蓋新附件 SQL；這不是全體 cohort cutover 或 Production L4 證據。歷史 v1 路徑在未切換 cohort 驗收前仍需保留，終態依 DEV-015 拆除。

> **2026-09-25 DEV-121 principal 審核及本人工作附件讀取（本機）**：v2 reviewer 的 `review_package` 檔案下載於同一 verified RR 唯讀快照檢查 AAL2、`approval.inbox.view`＋`approval.request.decide`、精確 reviewer、pending、v2 package 與 binding／asset hash；本人 `drawing_revision_work` 檔案另驗 `numbering.workspace.view`、目前 work owner、company 與 exact binding，不借舊 `users.role`。兩者都在授權交易結束後才進行原始檔／預覽 I/O。圖面來源另要求 `revision.drawing_id=contextId`；work join 也核對 company。未轉換的 v2 檔案 context 明確 503，不落回舊 UID 身分；原 v1 只供未切換 cohort。審核／附件聚焦 4 檔 20／20、TypeScript、scoped ESLint、DEV-005 inventory PASS。完整檔案 context、隔離 PostgreSQL、全體 cutover、real-provider 與 Production L4 未完成。

> **2026-09-25 DEV-121 principal 審核對象與差異比較接線（本機）**：v2 reviewer 可沿既有 `GET /api/pdm/review-requests/[requestId]/targets/[entityType]/[entityId]` 及 `/comparison` 讀取完整審核包對象，不再因這兩個讀取端點落回舊 UID／role 入口而中斷。兩者在同一 verified principal RR snapshot 依 reviewed route policy 核對 `approval.inbox.view`、AAL2、`approval.request.decide`、精確指定 reviewer／pending、v2 package 完整性與目標鍵；圖面同時讀現行基準，stale 時不顯示核准動作。DEV-005 清單／產物更新為 77 檔／94 methods／103 entries；聚焦 3 檔 14／14、TypeScript、scoped ESLint、route classification 255／293 PASS。舊路徑只供未切換 cohort；其他 API／SSR、隔離 PostgreSQL、全體切換與 Production L4 未完成，不能提前刪除仍在用的 v1 入口。

> **2026-09-25 DEV-121 圖面 principal 垂直切片合併回歸**：工作 GET／候選 GET／建立／更新／取消／提交、檔案上傳／移除及 `drawing_revision` 審核讀取／決策的 v2 本機路徑已接通；本輪聚焦 8 檔 55／55、DEV-087 commands 61 checks、DEV-100 contract 8／8／repository 12／12、DEV-005 inventory 75／92／101、DEV-121 全 API 255 檔／293 methods 分類 PASS、TypeScript 與 scoped ESLint PASS。這不是 cutover 或 Production PASS：`drawing_rd_void`、其他 API／SSR 舊入口、隔離 PostgreSQL 競態、real-provider conformance、全體 cohort、v1/v2 recovery 與 Production L4 仍依原任務完成；舊授權分支只保留到切換與回復驗收後拆除。

> **2026-09-25 DEV-121／DEV-087 圖面檔案 principal 移除（本機）**：v2 `DELETE /api/pdm/drawing-revision-works/[workId]/files/[fileBindingId]` 已接到 verified principal、workspace update、workbench contract、鎖定 work 的目前 owner／row version 與 exact current-revision file binding；DB tombstone 和 principal receipt 在同一 serializable transaction，物件儲存刪除僅於交易成功返回後執行。v1／v2 共用檔案資料轉移，v2 不讀舊角色；principal 圖面／檔案聚焦 26／26、TypeScript、既有 DEV-100 repository 12／12 PASS。DEV-005 inventory/map 現為 75 檔／92 methods／101 entries；隔離 PostgreSQL 競態、其他未轉換路由及 Production L4 仍待完成，不能因此切流或刪除整個 v1 runtime。

> **2026-09-25 DEV-121／DEV-087 圖面檔案 principal 上傳（本機）**：v2 `POST /api/pdm/drawing-revision-works/[workId]/files` 已接通；檔案暫存前在 readonly verified snapshot 檢查 principal workspace update、workbench contract 與目前 owner，寫入前再以 serializable snapshot 重驗相同權限／owner，鎖定 work 後再次核對 owner 與 row version，再用 principal receipt 提交檔案 metadata。物件儲存 I/O 不佔用資料庫交易；第二次驗證失敗會清除本次暫存物件。未切換 v1 流程維持原命令，未作永續 bridge。DEV-005 inventory/map 現為 74 檔／91 methods／100 entries；principal 檔案聚焦 3／3、route dispatch 加入後 10／10、既有 DEV-100 repository 12／12、TypeScript PASS。檔案移除、其他圖面命令、隔離 PostgreSQL 與 Production 驗證仍待完成。

> **2026-09-25 DEV-121／DEV-087 principal 圖面審核閉環（本機）**：v2 已允許讀取及決策 `drawing_revision` 審核案，要求 AAL2、精確指派 reviewer、principal capability、workbench contract、鎖定中的 pending request 與 v2 review package；核准前重驗辨識、圖面基準、提交流程快照，退回仍走同一業務轉移。`drawing_rd_void` 因申請命令未切到 principal，v2 仍拒絕。共用 route adapter 另改以實際 Request URL／method 比對清單及 readOnly 設定，避免宣稱路由與請求不一致。圖面審核聚焦 23／23、route adapter／圖面／零件聚焦 37／37、TypeScript PASS；檔案上傳／移除與其他未轉換路由仍未完成，Production 未發布。

> **2026-09-25 DEV-121／DEV-087 圖面 principal 提交（本機）**：`POST /api/pdm/drawing-revision-works/[workId]/submit` 已進入 v2 verified principal route；AAL2、workspace capability、workbench contract、目前 owner 及 row version 皆在受控交易內檢查，審核人改由 principal/selected-authority selector 選取，principal 路徑強制 v2 review package 並寫入 principal receipt。v1 與 v2 共用圖面業務提交與檔案／辨識檢查，但各自的授權來源及 reviewer selector 分開，切換後可移除舊 selector。DEV-005 map 現為 73 檔／90 methods／99 entries；聚焦 26／26、TypeScript、scoped ESLint PASS。圖面檔案、decision 及其他未轉換命令仍待接線；尚不可切流或宣稱 Production L4 完成。

> **2026-09-25 DEV-121／DEV-087 圖面工作 principal 垂直接線增量（本機）**：既有 v2 GET 工作詳情外，已接通候選版次 GET、建立 POST、工作更新 PATCH 與取消 POST；目前五個 method 均使用 reviewed route policy、verified principal snapshot 與 principal-keyed workspace capability。建立綁 server-read source／target token／版本與 principal receipt；更新及取消在同一交易鎖住目前 work，要求 profile 為 owner 並保留 If-Match、workbench contract 與 idempotency receipt。Platform DEV-005 來源清單與 AI-PDM route map 已同步至 72 檔／89 methods／98 entries；聚焦 23／23、TypeScript、scoped ESLint、route inventory 與分類 QC PASS。提交／檔案／決策、principal reviewer 與完整 cutover 仍未完成，Production 尚未發布；舊 v1 lane 僅供尚未切換 cohort 使用，切換與 recovery 驗收後應拆除。

> **2026-09-25 DEV-121／DEV-087 圖面工作 principal 讀取接線（本機）**：`GET /api/pdm/drawing-revision-works/[workId]` 已在 v2 session 下走 reviewed route policy、同一 verified 唯讀快照及 principal-keyed workspace capability；service 再核對目前 profile 對圖面工作的公司與 owner，互動按現行 update／submit／cancel grant 限制，不從舊 `users.role` 借權。Platform DEV-005 來源清單與 AI-PDM route map 的先前 7 筆漂移已校正並重生，目前 69 檔／85 methods／94 entries；聚焦 13／13、route classification PASS、TypeScript、scoped ESLint PASS。圖面 create／update／submit／file／decision 等仍未完成 v2 垂直流程，Production 未發布，不得以此單一路由宣告 cutover ready。

> **2026-09-25 DEV-121 舊命令 marker 缺口封口（本機）**：舊 canonical／DEV-087 PostgreSQL 命令現在只接受明示 `legacy_compatible` 的 cutover marker；`principal_active` 仍於讀寫前回 410，缺 marker 或未知狀態回明確 503，絕不把無列視為舊權限。聚焦 23／23、TypeScript、scoped ESLint PASS。此路徑只供尚未切換帳號過渡；完整 v2 業務命令接線及全體 cutover 後應刪除，不保留永久 bridge。

> **2026-09-25 DEV-121 正式命令的合成身分封口（本機）**：`executePdmCommandWithOutbox` 現於 PostgreSQL 讀取 cutover marker、mapping 或 receipt 之前，對所有命令要求已驗 actor，並拒絕只供 SQLite fixture 使用的 `pdmUserId=system` 哨兵；不再讓它跳過正式命令的 principal 驗證。聚焦 17／17 與 TypeScript PASS。這不代表舊命令已完成 principal 轉換；未切換 cohort 的既有 mapping 仍只讀，v2 業務路徑須逐條接上 principal 命令與正式切換後才能刪除相容 reader。

> **2026-09-25 DEV-121 舊 provider mutation API 退役（本機）**：移除產品無 caller、僅舊 QC 模擬使用的 Firebase 建帳、發密碼連結、停用、撤銷 token、刪除身分與舊 offboard helper；AI-PDM 的 Firebase adapter 現只驗證切換期 v1 token。舊邀請 repository 只保留歷史邀請讀取及本地撤銷，移除無 caller 的建立／重發／登入啟用 writer；已完成本地補償的撤銷重試不再重複推進 lifecycle version。共享 provider 身分生命週期歸 Platform，歷史 QC 010–012 記錄保留於 Git 歷史，本次現行 Phase 1B 為 12／12、Phase 2B 為 16／16、邀請聚焦 3／3、TypeScript 與變更檔 whitespace 檢查 PASS。v1 token 入口須待 principal cohort 切換與 recovery 完成後再退役；Production 仍 NOT_RUN。

> **2026-09-25 DEV-121 owner release 與舊 bridge 收斂（目前狀態）**：AI-PDM owner profile 已納入 065／066 為第 16／17 筆 forward-only migration，保留 DEV-010 的 15 筆歷史 baseline；同一 clean source capsule 可先完成 `prepare → build → migrate`，在未建立 candidate、未切 traffic 時讓 OrgMaster 讀回並發布 v4，再續行 AI-PDM candidate／traffic。owner release 40／40、adapter 12／12、v4 generator 2／2、command 32／32、typecheck、DB boundary PASS。舊 PostgreSQL command 不再自動建立 principal 或 organization mapping，principal_active 的舊命令在 receipt／業務寫入前拒絕；切換期未轉換帳號只能讀既有 mapping。下列「066 尚未加入 bundle」是先前階段紀錄，已由本筆取代；正式 migration receipt、OrgMaster provider readback、全體 principal cutover、Production L4 仍 NOT_RUN，不可切流或宣布完成。

> **2026-09-25 DEV-121 舊 provider lifecycle 所有權修正（本機）**：AI-PDM 歷史邀請撤銷先經資料庫 fence 停用本地 mapping／account，再撤銷本地邀請；不再停用、revoke token 或刪除 Platform 所有的共享 Firebase provider identity。已切 `principal_active` 的 profile 若遭 legacy writer fence 拒絕，不會發生外部身分副作用。聚焦 2／2、DEV-046 BFF QC 16／16、舊 Phase 2B 本地 preflight 19／19、typecheck PASS。其兩個 staging provider blocker 屬舊 DEV-046 證據，未升格為本輪 owner release gate；Production 與三系統切流仍 NOT_RUN。

> **2026-09-25 DEV-121／DEV-087 principal 零件送審基礎與 v4 目錄（本機、未可發布）**：`submitPrincipal` 在同一 verified SERIALIZABLE transaction 重驗 `numbering.candidate.review.submit`、本人 work 與工作台 contract，使用 principal reviewer selector、canonical command receipt，強制產生 v2 immutable package；不再從 `users.role` 選人。service 聚焦 5/5、typecheck PASS。新 v4 目錄 artifact 只依已套用 012／016 ACL 的明確四角色／四能力產生，保留 v3 不改，catalog hash `32f3593d7a0d2a5cad4875181a62b8f5c49a06c9cbba8835cd1b82e9b44ca08a`；generator 2/2 與重算 PASS。未出貨的 066 forward-only migration 在隔離 PostgreSQL 4/4 PASS（錯誤 baseline rollback、原子切換、重播零新增、竄改拒絕），DB boundary PASS，task-owned cluster／埠／暫存及容量租約已清理。066 尚未加入現行 owner release bundle；OrgMaster readback、HTTP route 與整條 v2 流程尚缺，故此切片不能部署或宣稱 Production grant 已生效。

> **2026-09-25 DEV-121／DEV-087 舊審核快照退出 principal 路徑（本機、未可發布）**：`GET /api/pdm/review-requests/[requestId]` 的 v2 lane 已用單一 verified snapshot 檢查 AAL2、`approval.inbox.view`、`approval.request.decide`、指定 reviewer 與 pending 狀態；詳情和決策只接受可驗完整性的 canonical v2 review package，舊 snapshot 回 409。舊待審單在切換前由舊 cohort 完成／清理，不向 principal lane 借用舊資料。聚焦 route／service 10/10、型別、ESLint、route classification 255 檔／293 方法 PASS。這是前面時間序列紀錄之後的進度；submit／principal reviewer 實際接線、圖面決策、outbox consumer、全站舊 runtime 退役與 Production L4 仍未完成，不得發布局部切片。

> **2026-09-25 DEV-121／DEV-087 principal 零件審核決策入口（本機、未可發布）**：`POST /api/pdm/review-requests/[requestId]/decisions` 的 v2 lane 使用既有 catalog `approval.request.decide`，於同一 SERIALIZABLE transaction 核對 verified principal session、AAL2、現行權限、工作台 contract、指定 reviewer 及 request state；`PartChangeWorkService` 在命令層再次驗權，再以 canonical receipt 執行核准／退回。缺 request 時只允許相同 actor／hash 的 receipt 重播；其他 review kind 的 v2 lane 暫回 503，避免偷走舊 route。route 聚焦 3/3、既有 DEV-087 commands 61/61、route classification 255 files／293 methods PASS，`typecheck:app` 與 scoped ESLint PASS，隔離 fixture 已清理。**GET review detail、drawing review decision、submit／reviewer selector、outbox consumer 與全路徑回復仍未接線；v1 lane 暫存，不得單獨部署此切片。**待完整垂直流程可用後移除舊 lane，不維持永久雙軌。

> **2026-09-25 DEV-121／DEV-087 principal command receipt 基礎（本機、未接 route）**：新增只接受同一已驗 principal admission 的 pinned PostgreSQL SERIALIZABLE 寫入 transaction 的 `runPrincipalDev087Command`；新 command receipt 直接寫 canonical `principal_id`、業務 profile actor 與受控 `actorBinding`，舊 `platform_principal_id`／`platform_organization_id` 保持 NULL，不建立 `pdm:*` 映射。重播核對相同 actor、principal、company、request hash、effect key與完整 envelope；執行失敗時業務效果和 receipt 同筆 rollback，completion 零列拒絕。隔離 PostgreSQL 21/21 PASS，包含成功、重播不再執行、跨 principal 拒絕及失敗回滾；task-owned cluster／port／temp 已清理。尚未接入 DEV-087 route、reviewer 指派或正式業務命令，不能據此宣稱 v2 審核已可用；下一切片把已驗 route／permission、命令及選人器接成同一垂直流程，然後刪除正常路徑的舊 idempotent command 與選人器。

> **2026-09-25 DEV-121 principal reviewer 選人基礎（本機、未接 runtime）**：新增只接受 pinned PostgreSQL RR／SERIALIZABLE transaction 的選人器，從本地 active `principal_accounts`＋`principal_active` cutover 與 OrgMaster active typed principal 取精確候選；同一 transaction 依 current role priority、selected OrgMaster authority 或 principal-keyed local ACL 判定 `approval.request.decide`，拒絕 alias／profile 歧義、不同 alias 權限漂移、顯式 deny 與無候選，保留非本人優先的既有決定性排序。未發布 authority 的個別候選不具資格，不阻擋另一位已證明合格者；契約錯誤與雙重 authority 仍拒絕。候選 eligibility 不是代替其日後審核時的 AAL2 登入。新 selector／舊 fail-closed selector 聚焦 8/8、`typecheck:app`、scoped ESLint PASS；隔離 PostgreSQL 20/20，含本地 principal ACL、已發布 OrgMaster authority、runtime 權限與停用員工，叢集／port／temp 已清理。尚未接入 DEV-087 principal route／同交易命令、未發布；不可讓舊 `runDev087IdempotentCommand` 因新 selector 存在就接受 v2 actor。依使用者最新方向，v2 垂直切片完成時直接移除正常命令的舊選人器及 UID／role fallback，不建立永久雙軌；歷史審核紀錄可留作不可授權的業務／稽核識別。

> **2026-09-25 DEV-121 命令內舊組織橋接拆除（本機、未可發布）**：移除無任何 runtime caller 的 `PlatformMappingAsyncRepository.linkSharedOrganization` 舊 shared-core 寫入器；v2 verified principal actor 的 `platformOrganizationId` 現為 `null`，拒絕 caller 塞入舊映射，共用命令也不再把 company ID 偽裝成該舊欄。任何帶 verified actor 的空 `principalId` 現直接拒絕，不再退回 `pdm:*`。v1 切換期的既有 organization readback 仍保留，直到全體 principal cutover／回復驗收才刪除。command context／receipt／snapshot 聚焦 26/26、`typecheck:app`、scoped ESLint PASS。此項只拆除 canonical command 內的假 bridge，不代表 DEV-087 路由、審核人或 Production 已切換。

> **2026-09-25 DEV-121／DEV-087 工作台 v2 接線盤點（仍 fail closed）**：`resolveDev087RouteActor` 現以舊 numbering page/action guard、`resolveNumberingCompanyContextAsync` 與 `users.role` 衍生操作權；相關 DEV-087 API method 尚未進受控 `jenfu-route-permission-map.v1.json`，而 `withPdmWorkbenchReadSnapshot` 會自行開 read-only transaction 並設定 isolation，不能直接巢狀放到已查過 principal admission 的 transaction 中。故不能只打開一個 POST 或讓 v2 cookie 回退舊 guard；下一切片須同時接上受控 route policy、單一 verified principal transaction、selected-authority reviewer、principal receipt／outbox與合法工作台 contract，再移除正常流程舊分支。現有 25 個未入 v3 catalog 的能力代碼維持已核定 deny／retire，不猜測 grant；此盤點不新增人工 gate，也不宣稱 v2 功能已上線。

> **2026-09-25 DEV-121 receipt／outbox 原子性（本機、未可發布）**：共用命令outbox若遇相同company／event type／idempotency key，不再默默略過事件卻把command完成；`INSERT ... RETURNING id`無新列即拋錯，交由同一transaction回滾。completion也須以相同actor／canonical principal／舊欄組合命中processing receipt並`RETURNING id`，零列即拒絕並回滾，避免業務與事件寫入但receipt仍未完成。已完成receipt以明確狀態區分「結果為null」與「不存在」，前者直接重播不再次執行；缺result的損壞envelope及新命令undefined結果拒絕。隔離SQLite實表驗重複event key及正確／錯誤principal completion；command／principal snapshot focused 23/23、型別與ESLint PASS。此修正不代表DEV-087新route policy、v2工作台契約或Production切流完成。

> **2026-09-25 DEV-121／DEV-087 舊命令重播邊界（本機、未可發布）**：DEV-087／canonical command 的既有terminal replay現在核對`actor_id`與目前操作者，且拒絕把principal receipt當成舊receipt；審核完成保留reviewer ID供相同操作者重播，無actor舊receipt fail closed。五個preflight replay caller均傳actor ID。focused 3/3、型別與ESLint、隔離SQLite DEV-087 commands 61/61 PASS，含真實receipt actor readback；task-owned暫存已清理。這是v1切換期防止跨操作者重播，不是principal v2命令完成，後者仍須同一已驗snapshot內重驗權限並寫canonical receipt。

> **2026-09-25 DEV-121 command provenance 拆除舊 mapping 依賴（本機、未可發布）**：未出貨的 065 在既有 command receipt／outbox 增加 canonical `principal_id`，以 `(company_id,actor_id,principal_id)` 外鍵驗證同一 workspace／profile／security subject；v2 command 只寫此欄，舊 `platform_principal_id` 與 `platform_organization_id` 為NULL，v2共用命令也不再建立舊 organization mapping。v1／歷史row保留舊欄但不能與canonical欄混用。SQLite 增加同名本機欄位及舊資料庫升級。隔離 PostgreSQL 18/18 PASS，含無舊 mapping 的新 principal 寫入、跨 profile／workspace與混用舊欄拒絕；focused command／snapshot 20/20（含重播actor及payload綁定）、`typecheck:app`、ESLint、DB boundary PASS，叢集／port／temp均清理。這解除共用命令儲存層的 legacy FK 阻礙，不代表 DEV-087 舊 idempotent route、全體 API／SSR、resource／delegation shadow 或 Production 切流已完成；保留 fail closed。

> **2026-09-25 DEV-121 reviewer 舊角色隔離（本機、未可發布）**：DEV-087 舊 `selectReviewer` 在 PostgreSQL 不再把 `principal_active` profile 的歷史 `users.role`／指派列納入候選；無其他候選時明確拒絕。隔離關聯式資料測例證明混合 cohort 只選舊候選、全體切換後舊 selector 拒絕；聚焦 2 檔 11/11 PASS。DEV-087 repository QC 先因舊文案預期「正式資料」與現行產品「主檔 · 已發布」不符而失敗，對齊現行契約後 38/38 PASS；commands QC 61/61 PASS。這只是切換期防旁路；v2 工作台入口仍拒絕，舊 idempotent command receipt 仍清空 `platform_principal_id`。必須把入口、同一 owner transaction 的 principal selected-authority 選人及 receipt/outbox actor binding 作為同一垂直切片後才能切流，不保留永久雙軌。

> **2026-09-25 DEV-121 舊命令 mapping 權責收斂（本機、未可發布）**：`executePdmCommandWithOutbox` 不再允許 legacy mapping 將已確定的 command principal 改寫；既有相容帳號的 mapping 僅用於 exact principal/profile pair 核對，`principal_active` 路徑仍只用已驗 principal。新增 SQLite 舊 mapping 不符即拒絕的回歸案例；command／snapshot／workspace-shadow 聚焦 6 檔 31/31、型別與 scoped ESLint PASS。舊 mapping 只能待全體切換及回復驗收後退役，不能因本項局部修正宣稱正式切流完成。

> **2026-09-25 DEV-121 workspace 行為 shadow（本機、未可發布）**：切換 preview 現在以同一source snapshot及遷移plan，比對legacy與principal候選在所有既有page/action policy、敏感能力及未列能力代表碼的workspace決策；輸出decision數、差異數、resource／delegation缺口與穩定shadow hash。active scoped assignment、role scope rule、delegation、未來assignedAt或決策差異不被稱為等價；其中尚需resource adapter者標示`requires_resource_adapter`。workspace shadow hash已納入三owner source seal，owner鎖後重讀若狀態非PASS，在任何materialization之前以`PRINCIPAL_CUTOVER_WORKSPACE_SHADOW_INCOMPLETE`拒絕。pure聚焦10/10、`typecheck:app`、migration 065隔離PostgreSQL 17/17 PASS，含scope rule真實來源拒絕及原子rollback/replay；task-owned叢集／port／temp清理。此證據只覆蓋workspace層，完整project/resource／delegation行為shadow、provider attestation、owner runner、API/SSR與Production L4仍未完成，不能切流。

> **2026-09-25 DEV-121 principal ACL 角色收斂（本機、未可發布）**：修正 workspace evaluator 原先把可授權角色硬限於 system_admin／pdm_admin／qa 的實作缺陷。現行 priority 內的啟用角色，只有在 principal-keyed 指派明確為 `workspace_all`、沒有 named scope 或額外 role scope rule 時，才可取得 workspace allow；一般角色還須具該 action/page 的明確 allow policy。既有 system_admin 對非敏感能力的預設 allow 例外維持，敏感能力仍須明確 policy。高權限角色仍須 AAL2，顯式 deny 仍依 priority 生效。此規則避免一般角色（例如 rd）在 cutover 後無故失權，且不以舊 UID／`users.role` 補權。先前「僅 admin／PDM admin／QA 可 allow」記載是當時實作限制，已由本修正取代。unit 聚焦 13/13、typecheck、scoped ESLint、隔離 PostgreSQL schema／ACL QC 17/17與authorization snapshot／race QC 2/2 PASS，兩個測試叢集與暫存資料均已清理。其餘 resource scope、owner cutover 與 Production L4 尚未完成。

> **2026-09-25 DEV-121 reviewer 指派旁路盤點（待實作）**：正常業務提交會呼叫 `PdmWorkReviewAsyncRepository.selectReviewer`，其 SQL 仍以 `users.role`、`user_role_assignments` 與 `user_company_memberships` 挑選審核人；`drawing-revision-lifecycle-adoption.collectReviewers` 的歷史採納工具也使用這些舊欄位。principal cohort 若沿此路徑運作，已驗 actor 雖為 principal，新的 reviewer 卻可能由過期舊權限決定。正常業務命令必須在同一 transaction 依 selected authority、principal-keyed 有效角色／scope、active principal account 與唯一 PDM profile 解析 reviewer，審核記錄只把 `pdm_user_id` 當 domain selector；不能沿用舊角色 SQL 或以其結果補權。歷史採納工具須明確限制於尚未 cutover 的資料，或改用相同新投影。此為既有 DEV-121 命令鏈的必修，不增加主任務或新的人工 gate；完成之前相關 v2 提交仍拒絕，不能切流。

> **2026-09-25 DEV-121 舊公司授權橋接拆除進度（本機、未可發布）**：`/api/auth/me`、numbering v2 guard 共用同一 verified principal snapshot 的公司解析；只接受唯一 `principal_accounts` 所屬 `company-jenfu` 與對應 company row，不再以 `user_company_memberships`、email 或歷史 `users.role` 決定 v2 公司。共用 `requireNumberingCompanyPermissionAsync` 將已驗 principal 與公司成對交給無額外專案／單筆資源條件的唯讀路由，目前接到搜尋、圖面解析、圖面送審 context／readiness、匯出工作與月報的清單／單筆讀取；v2 公司不符立即拒絕，不回退到舊會員查詢。報表寫入命令維持未遷移v2拒絕。v1 分支僅供尚未切換的相容期使用，必須隨全體 cutover 與回復驗收拆除，不能作終態。舊ERP QC已移除「命令須即時建立principal」的反向要求，改驗已驗actor原樣貫穿交易。聚焦8檔21/21、`typecheck:app`、ESLint、route classification（255 files／293 methods）及ERP foundation QC 26/26 PASS；其他 API／SSR、resource adapter、owner cutover 與 Production L4 仍未完成。

> **2026-09-25 DEV-121 principal guard 與approval角色來源（本機、未可發布）**：顯式v2 numbering guard以已驗principal snapshot及workspace evaluator判定；未審核的舊numbering guard收到v2 cookie拒絕，避免其下游舊company／role捷徑取得授權。已接線的`GET /api/numbering/search`另核對已解析資源公司與actor exact company，跨公司不查資料。project／action缺server resource proof或workspace不符拒絕，request-free舊helper遇v2 actor不得再回`users.role`。下游domain user標示非授權角色`Principal`，投稿資料僅同公司且本人可讀，缺company拒絕；approval batch記錄的角色只來自permission decision，忽略client body及歷史profile role，無選中角色拒絕。focused 5檔16/16及typecheck PASS；其餘API／SSR、resource adapter、behavior shadow、owner cutover與Production均未完成。

> **架構定案：已定案／RD Implementation In Progress**：[AIPDM/DEV-121 單一交接入口](specs/DEV-121-target-authorization-boundary.md#architecture-final)固定目前 owner 契約、來源／雜湊與失敗／恢復；與 JENFU/DEV-015 同名節對齊。沿用原子任務，依相依順序實作；下方「Documents Only」是當時架構文件階段的歷史標記，現已有未接入runtime的本機程式與測試。consumer conformance／recovery／Production L4 尚未完成，下方歷史紀錄不覆蓋此入口。
...
> **2026-09-24 DEV-121 RD 過渡切片（產品未完成）**：已驗人類actor的命令不再以`pdm_user_id`自動建立`pdm:*`或覆寫principal；既有profile只讀核對，receipt／outbox使用同一principal，重播核對actor binding。新增未接入runtime的principal-keyed epoch reader、principal account readback及無`pdm_user_id`安全claim的v2 token；opaque principal長度200／201／255均原樣通過，超限拒絕。四個聚焦檔15/15、numbering permission回歸合計10/10、entitlement repository 32/32、`typecheck:app` PASS。Platform-owned v2 SSO契約已以manifest `816cf07ac0b29d6f340289fc5e38ff765f40d323a62c6fd4d1b3f7402227cf6e`鎖定本repo副本；後續新增獨立v2純解析器，但現行callback仍只接受v1。未驗actor的舊caller、065資料轉換、principal-only runtime及Production L4仍待完成，Platform的AI-PDM v2 emitter不得開啟。v1／UID安全路徑必須在同一子任務退役，不另設永久相容軌。

> **2026-09-24 DEV-121 RD 後續切片（仍未可發布）**：新增v2 handoff純解析與target principal assurance政策計算，來源locked vectors及負向案例10/10 PASS；principal-keyed本地session registry的寫入、撤銷及epoch／版本／policy hash readback 10/10 PASS；principal request guard以同一`REPEATABLE READ READ ONLY`快照核對typed source、v3 epoch、local account／membership與session，僅回傳principal session及業務profile，不讓舊users.role／status進入新安全context；同交易callback供後續permission／resource decision使用，聚焦5/5 PASS，尚未接入正式callback或API guard。未出貨的065新增唯一principal↔profile link、principal-keyed ACL／session、單向cutover、不可變operation及舊users／assignment／delegation、UID session、membership、mapping與alias writer防線；隔離PostgreSQL 9/9含舊writer與cutover競態及RR舊snapshot拒絕，`check:db-boundary`及`typecheck:app` PASS，cluster／port／暫存資料均清理。065尚缺原子cutover/provision命令、inventory等價readback、共用policy writer收斂、principal-aware membership命令、principal runtime授權evaluator與release bundle綁定；不得將本地DDL PASS視為切流或Production驗收，也不得啟用AI-PDM v2 emitter。使用者已確認舊架構無保留必要：完成principal切換後，同子任務移除正常登入／API／command的UID與pdm:* fallback；舊歷史資料僅保留稽核與回復所需的不可授權記錄，不維持永久雙軌runtime。

> **2026-09-24 DEV-121 command 退役進度（仍未可發布）**：PostgreSQL 人類命令未帶verified actor立即拒絕，legacy_compatible mapping僅准既有link readback，不再由命令自建`pdm:*`；三個原本未傳actor的作廢 API 已沿共用入口傳遞verified metadata，入口缺actor回401、跨公司回403。schema2 actor的命令metadata不再混入`users.role`／legacyRole，只帶本次allow選中的role。focused ingress／route／command 14/14、`typecheck:app`、`check:db-boundary` PASS；本機隔離資料與暫存路徑已清理。`createFallbackCommandMetadata`仍服務未轉換的SQLite／歷史caller，正式PostgreSQL人類命令已fail closed；正式principal evaluator、callback、owner cutover及全路徑退役仍未完成，不得部署此未完整切片。

> **2026-09-24 DEV-121 principal evaluator 進度（尚未接正式API）**：新增workspace-only principal permission入口，在同一verified snapshot中以principal actor、可信company、單一decisionAt、現行priority與OrgMaster selected authority評估grant；高權限AAL1拒絕。`legacy_authority`現由principal-keyed ACL reader讀`principal_role_assignments`與既有role policy；僅明確`workspace_all`、無額外scope rule的admin／PDM admin／QA可產生workspace allow。既有system admin非敏感代碼預設allow與敏感代碼明列policy已抽成同一規則供舊／新讀取器使用；顯式deny優先，其他scope拒絕，絕不以`users.role`補allow。repository／service聚焦11/11、實際migration 065與runtime privilege隔離PostgreSQL 10/10、型別檢查PASS；一次性叢集／port／資料均清理。project/resource/delegation、callback與API guard接線、owner cutover及全route接線仍缺，故目前不能發行v2 session、啟用v2 emitter或切流；舊架構僅保留稽核／回復所需的不可授權歷史資料。

> **2026-09-25 DEV-121 v2 handoff session發行切片（尚未接正式callback）**：target owner service在同一短交易內核對typed principal、canonical epoch、active principal account、唯一profile membership及selected authority高風險角色，計算target assurance，簽發principal-only v2 token並寫session registry；未使用Firebase UID resolver或`users.role`，沿用source session expiry並處理local session barrier。v1/v2相關聚焦6檔34/34、`typecheck:app` PASS，測試資料已清理。現行callback／API仍只讀v1，不能單獨開啟v2 emitter；下一步是direct與handoff統一principal admission、API/SSR同snapshot授權、舊cookie拒絕、owner cutover與回復驗證。

> **2026-09-25 DEV-121 assurance再驗證切片（尚未接正式API）**：selected authority的高權限角色是否要求AAL2，現在由session發行與principal request guard共用同一principal-keyed讀取器；AAL1 session若在發行後被授予system_admin／pdm_admin／rd_manager，下一次request在同一snapshot拒絕，不沿用舊pilot或`users.role`。OrgMaster grants在同一交易時間核對identity／有效期／authority version，混版拒絕；local ACL按principal讀有效角色。聚焦guard／發行服務14/14、型別檢查PASS；正式callback、direct入口及各API／SSR仍待接線。

> **2026-09-25 DEV-121 舊runtime阻斷切片（尚未可發布）**：v1 SSO、Firebase直接登入與v1 cookie驗證新增暫時cutover讀取器，僅明列`legacy_compatible`且精確principal/profile配對可用；`principal_active`、未知／未綁定／錯配拒絕，DB不可用fail closed。這只防止切換期舊cookie旁路，終態仍須刪除UID resolver、v1 session發行／讀取及`pdm:*`正常runtime fallback；既有資料留不可授權的稽核／回復記錄。focused 9/9、舊auth runtime QC 15/15、SSO回歸12/12、型別檢查與DB boundary PASS。新讀取器依賴未出貨065及inventory，callback／API principal-only接線、owner cutover與recovery未完成，不得單獨部署。

> **2026-09-25 DEV-121 v2 SSO callback切片（尚未可發布）**：回呼現在嚴格解析Platform v2 proof並直接由principal owner service發行principal-only session，入口測例證明不呼叫UID resolver（1/1）；v1只服務明列`legacy_compatible`。v1/v2 parser及session service聚焦22/22、型別檢查PASS。正式API／SSR尚未讀v2 cookie，direct登入、logout、owner cutover與recovery仍未完成，Platform AI-PDM v2 emitter不得啟用。

> **2026-09-25 DEV-121 principal runtime接線增量（尚未可發布）**：v2 local logout先撤銷principal session registry再清cookie，撤銷失敗回503；`/api/numbering/permissions`的v2請求改讀同快照principal evaluator，批次投影page/action，partial-result拒絕且不走舊user role／auth。logout／registry 12/12、permission API／evaluator 4/4及partial-result 2/2、型別檢查PASS。其餘API／SSR、direct登入、resource scope、global logout與owner cutover仍缺，故不能啟用AI-PDM v2 emitter或部署。

> **2026-09-25 DEV-121 本人資訊v2接線（尚未可發布）**：`/api/auth/me`對v2 cookie在同一principal verified snapshot讀唯一profile／company，回傳顯示與聯絡資料、session，但不把歷史`users.role`當成新權限（role=null）；錯配拒絕。current-user／permissions／logout路由聚焦6/6、型別檢查PASS。UI舊role affordance、其他API／SSR、owner cutover與L4仍缺，不能把本人資訊200算成全站登入完成。

> **2026-09-25 DEV-121 舊入口退役相依盤點**：DEV-118的SSO-on UI已只顯示Platform入口，但隱藏的`/api/auth/firebase/session`與DEV-117 owner candidate-smoke仍依賴UID交換。終態要以現有normal-entry principal smoke取代並封閉SSO-on直接交換，不能只刪endpoint使owner release失去驗證；這屬原DEV-121／DEV-118範圍與原驗收分母，不另增gate。route classification仍為255 files／292 methods、25 deny disposition、pending 0，DB boundary與diff檢查PASS。

> **2026-09-25 DEV-121 既有身分建立入口與切換約束（本機、尚未可發布）**：未出貨065已修正inventory先記已核實principal、再建立local account的資料順序；`principal_active`仍由資料庫deferred exact pair FK拒絕缺帳戶或錯配。隔離PostgreSQL實體DDL／ACL 11/11 PASS，叢集／port／暫存資料已清理。正式登入模式的舊`POST /api/admin/account-invitations`現回409 `principal_enrollment_required`，不再從此入口建立Firebase UID／`pdm:*` mapping；查詢／撤銷歷史邀請仍保留。聚焦路由2/2、`typecheck:app`、DB boundary及diff檢查PASS。這尚未建立新的principal provisioning入口；管理UI、direct登入、其他API／SSR、owner cutover／recovery、candidate-smoke及Production L4仍待接線，不能單獨部署或啟用Platform AI-PDM v2 emitter。舊建立函式與資料須待新入口／cutover驗證後從正常runtime一併移除，不作長期雙軌。

> **2026-09-25 DEV-121 principal登入裝置管理（本機、尚未可發布）**：新session registry現在保存並核對不可變`assurance_level`；`GET /api/account/sessions`對v2 cookie在已驗principal的同一唯讀快照按principal讀hash化session selector，不查UID session表，其他principal或異常列fail closed。撤銷其他裝置的POST在同一已驗principal的`REPEATABLE READ READ WRITE`交易按`principal_id + session hash`更新；目前工作階段要求走登出，跨principal記錄不會被修改。安全頁可顯示principal登入裝置與AAL。聚焦4檔25/25、隔離PostgreSQL DDL／ACL 11/11、型別、route classification及DB boundary PASS，暫存cluster／port已清理。這仍只是局部API：direct登入、其餘API／SSR、owner cutover、候選smoke與Production L4未完成，尚未可發布；完整切流與舊runtime移除仍沿既定依賴順序。

> **2026-09-25 DEV-121 principal候選查詢（本機、尚未可發布）**：`GET /api/admin/accounts?view=principal-candidate&principalId=...`已在同一已驗principal唯讀snapshot內，以`accounts.invitation.manage`決定後才按exact principal讀OrgMaster typed contract；不做email／UID／PDM user fallback。合法多provider aliases分列，employee／type衝突、雙列同pair及contract漂移拒絕。一般帳號列表保留原`accounts.lifecycle.manage`規則；contextual-only路由仍缺discriminator即拒絕，manifest僅允許此一處混合規則。候選repo／route／policy聚焦20/20、route classification 255/292與型別檢查PASS。新provision mutation與管理UI尚未接入，因此候選200不代表可建立帳號或完成切流。
> **2026-09-25 DEV-121 principal-only 新帳號契約（本機、尚未可發布）**：已加入 create-only 輸入 parser／producer exact tuple 比對；拒絕 role、company、既有 PDM ID、Firebase UID 等舊身分或授權提示，聯絡 email 不作 identity join，帳戶預設 disabled。聚焦測試10/10、`typecheck:app`與diff檢查PASS。資料庫 owner function 的初版設計遭安全審查指出只驗 active actor、未在 SECURITY DEFINER 內驗 `accounts.invitation.manage`；未採用該函式或觸發器例外，現有 DB fence 保持原狀。DEV-121 契約已補明授予 runtime EXECUTE 前的 DB 內權限證明要求。POST、原子建立／重播、管理UI及Production均仍 NOT_RUN。使用者已要求 principal 切換後不保留不必要舊架構：UID／`pdm:*` 只能作歷史不可授權資料，相關正常 runtime helper／入口須於恢復驗證後實際退役。

> **2026-09-25 DEV-121 principal 建帳實作更新（本機、尚未可發布）**：在未出貨065完成 owner SECURITY DEFINER 的同交易有效session、OrgMaster typed／authority、active catalog與`accounts.invitation.manage`重驗；新建 profile 的workspace歸屬由`principal_accounts.company_id`及`users(id,company_id)`複合FK固定，不寫舊membership、UID或預設grant。新增不可變operation receipt、同ID重播與異輸入衝突，isolated PostgreSQL 13/13 PASS且暫存叢集／port已清理；既有`POST /api/admin/accounts`接入SERIALIZABLE verified principal service，回傳再次核對operation／principal，聚焦24/24與型別檢查PASS。`/api/auth/me`的v2路徑改由principal account的公司歸屬與company主檔取顯示資料，無舊membership的新帳號不再被誤拒；聚焦7/7與型別檢查PASS。Platform DEV-005 inventory已追加GET candidate與POST，AI-PDM manifest重新產生為62 files／77 methods／86 entries；source-hash、contract、runtime-boundary及route classification（255 files／293 methods）均PASS。舊資料仍只作追溯／有限回復，不能授權新帳號。管理UI、principal-aware帳戶列表／lifecycle、其他API／SSR、owner cutover、recovery及Production L4尚未完成；不得啟用Platform AI-PDM v2 emitter或單獨發布此切片。

> **2026-09-25 DEV-121 principal 管理讀取與頁面切片（本機、尚未可發布）**：`GET /api/admin/accounts`與`GET /api/admin/accounts/[userId]`在v2 session下以同一verified snapshot、`accounts.lifecycle.manage`及可信company讀`principal_accounts`狀態，不從`users.role`或舊membership推斷；新帳號即使沒有舊membership，列表／明細仍可讀。`/settings/accounts`依已驗session版本只顯示principal帳號面板與create-only表單，不帶入舊密碼／UID邀請／local role控制；操作ID在重試時保留。隔離PostgreSQL 14/14（含無舊membership readback與runtime ACL）PASS，暫存cluster／port／資料已清理；相關route／repository聚焦18/18、型別、route classification 255/293與DEV-005 runtime boundary 86 entries PASS。principal-aware lifecycle／role management、其他API／SSR、正式browser與cutover/recovery仍未完成，此切片不可單獨發布。

> **2026-09-25 DEV-121 principal 帳戶生命週期切片（本機、尚未可發布）**：未出貨065新增受限 owner `update_principal_account_lifecycle_v1`，對已驗actor在同一SERIALIZABLE交易重查有效AAL2 session、當前`accounts.lifecycle.manage`、exact target cutover與轉態；原子更新principal account/barrier、撤銷目標session、寫不可變receipt，不回寫舊users安全欄位。既有lifecycle POST於v2 session走同源／manifest／principal service；帳號頁加入暫停、恢復、離職、復職入口。隔離PostgreSQL 15/15與聚焦測試21/21、`typecheck:app`、DB boundary、route classification 255/293及DEV-005 runtime boundary 86 entries PASS；task-owned cluster／port／資料已清理。此切片仍需管理UI未知結果後重試／正式browser、其他帳戶mutation與全部API／SSR、cutover/recovery，不能單獨發布或啟用Platform AI-PDM v2 emitter。用戶要求舊架構能拆就拆，故principal切流完成後舊security writer／UID session路徑須實際移出正常runtime，不以永久雙軌收尾。

> **2026-09-25 DEV-121 lifecycle 重試與契約一致性補正（本機）**：principal 管理頁把已送出但未確認的 lifecycle 操作 ID／原因保留於同一瀏覽器分頁，重開對話框沿原操作 replay，結果核對前不發起另一筆異動；owner receipt 的 operation／target／reason／時間與正整數版本由 service 再驗。DEV-005 runtime-boundary QC 現明確檢查 lifecycle v2 的同源、route policy、principal token 與 owner command 並以移除命令的 mutant 驗證；86 entries PASS。Platform DEV-015／ADR-005 及本 DEV 的 AI-PDM `principal_active` v1 驗收矛盾已更正為拒絕舊 proof、v2 正常重登。聚焦 lifecycle 5/5、typecheck與 ESLint PASS；正式 browser／其餘受保護 API、cutover、recovery、Production L4 仍未完成，未執行 Production mutation。

> **2026-09-25 DEV-121 舊邀請建帳入口收斂（本機）**：`POST /api/admin/account-invitations` 在 v2 cookie 下先完整驗證 current principal session，再回 `409 principal_enrollment_required`，不經舊 UID／role guard 或建帳 writer；無效 v2 cookie 回 401，未登入仍不能探知原管理路徑狀態。既有 v1／local相容支線尚在明列切換期，需在 cohort 切流與回復驗證後退役。路由聚焦3/3、型別檢查 PASS；DEV-005 runtime QC 已增加 principal verification 的 mutant，正式 browser 與 Production 均未執行。

> **2026-09-25 DEV-121 建帳未知結果重試（本機）**：principal 管理頁在送出前將操作ID、已選已發布principal reference與正規化表單暫存於該瀏覽器分頁；若回應遺失，重新整理會恢復原輸入並只允許同操作重試，取得可核對receipt後移除暫存，才可開始另一個帳號。server每次仍重查producer typed source與actor權限，暫存不是身分或授權來源。型別／ESLint PASS；正式browser、cutover與Production均未執行。

> **2026-09-25 DEV-121 principal 人類命令同交易守門（本機）**：追查`executePdmCommandWithOutbox`發現v2 actor stamp可從已完成的前一授權交易流入另開的業務寫入交易，無法保證session／epoch／permission與mutation共用snapshot。為避免以舊actor放行，現行未出貨source對所有v2 principal人類命令先回`PLATFORM_PRINCIPAL_COMMAND_CONTEXT_REQUIRED`，不取profile mapping、不寫receipt/outbox；已列legacy_compatible的舊session路徑暫維持。命令／入口聚焦3檔14/14及型別檢查PASS。下一個RD切片須把原始verified proof、principal evaluator、resource predicate及命令寫入放入同一RR或更強的transaction-bound client，再移除此暫時拒絕；在此之前principal業務命令不可用，AI-PDM v2 emitter與Production發布仍封鎖。

> **2026-09-25 DEV-121 舊 writer 移除與命令快照補強（本機、未可發布）**：已移除無產品 caller 的Firebase UID新建函式及`pdm:*`自動建立／UID link writer；歷史邀請撤銷與`legacy_compatible`既有mapping唯讀路徑仍限切換視窗。CI及deploy的DEV-046 QC以writer已不存在的防回歸檢查取代死路徑建帳案例，另接v2 enrollment／command snapshot測試。命令service現可在同一RR／SERIALIZABLE寫入交易重驗v2 session、epoch、profile、route workspace permission、actor與cutover後執行command／outbox；route policy綁實際HTTP method、path及同一session cookie，舊actor混入principal request、以GET權限執行mutation及RC隔離均拒絕。現有業務route尚未接入新上下文及可信resource guard，v2命令仍fail closed。`typecheck:app`、DEV-046 QC 16/16、principal enrollment／command Vitest 11/11、既有帳號邀請browser QC 36/36、ESLint與diff檢查PASS；任務啟動的Next／Playwright／port／資料已清理。歷史DEV-046 preflight另有兩項既存source檢查不通過，與此writer移除無關。不得以局部測例宣告全站v2／Production可用。

> **2026-09-25 DEV-121 精確來源讀回切片（本機、未接切換）**：新增 owner prepare 可用的 `jenfu-principal-inventory-repository.ts`，以單一 PostgreSQL RR／SERIALIZABLE snapshot 將既有 Firebase shared-IAM mapping／已驗 Google OAuth subject、唯一 PDM profile與OrgMaster 已發布 typed principal 精確核對；兩種來源分別固定 Firebase 專案 issuer 與 Google OAuth 正規化 issuer，email／姓名／員編不作 join。單來源與兩來源完整集合皆可讀回；兩來源須同principal／employee／account type，缺一筆或混人拒絕。錯誤 company、principal、來源版本／時間、跨 issuer、重複 typed row、RC 快照或停用／未驗 provider 身分也拒絕，後者須列 unresolved；suspended profile 的狀態、system-role開關、lifecycle版本及session barrier照原值返回，不能升為active。`typecheck:app`與隔離 PG principal ACL QC 16/16 PASS，叢集／port／暫存資料已清理；此函式尚未接 owner register／prepare／apply，未提供 ACL 等價性或切換許可，Production 未變更。

> **2026-09-25 DEV-121 owner inventory 註冊核心（本機、未接owner executable）**：新增 `jenfu-principal-inventory-registration.ts`，以 migrator 身分在新RR交易內重驗完整來源集合及版本化 identity-only hash，才插入／CAS刷新 `legacy_compatible` marker；相同來源重播不寫入、來源漂移／錯row version拒絕，已`principal_active`永不重置。隔離PG principal ACL QC 擴至17/17 PASS，涵蓋首次、重播、來源改變、版本衝突及active拒絕；typecheck PASS，暫存叢集／port已清理。此hash不是涵蓋ACL／委派／policy／session的最終cutover source hash；尚缺受控manifest／owner CLI、未解析principal處置、完整prepare／apply與等價性驗證，不能啟用v2 emitter或Production切流。

> **2026-09-25 DEV-121 inventory owner executable（本機、未執行雲端）**：已將上述註冊核心接入 `scripts/dev121-production-principal-inventory-runner.mjs`。精確 target／image revision、GCS object generation＋SHA-256／CRC32C、migrator DB readback、固定operation／receipt prefixes與唯讀preview／受控register分離；來源manifest不接受email或未列欄位，DB transaction維持同一RR讀點，preview同snapshot讀目前marker／CAS row version並拒絕active或錯綁，register在新交易重查，receipt排除重播旗標與執行時間以保證unknown outcome後bytes相同。`npm run qc:dev-121:inventory-runner` 4/4、隔離PG principal ACL 17/17、TS loader import、`typecheck:app`、DB boundary PASS；task-owned PostgreSQL cluster／port／temp均清理。此CLI只建立切換期 inventory，不含完整ACL source hash／apply，也未建image／Job、寫Production或開始退役；後續須將全部有效human切為principal_active並實際移除v1／UID授權路徑，DEV-015不得以相容期作終態。

> **2026-09-25 DEV-121 ACL 候選搬移（本機、preview-only）**：未出貨065的principal delegation增加`created_at`保留原建立時間。新增純deterministic planner，將明列legacy base role及既有assignment／delegation規劃為principal-keyed列，保留原scope、sponsor、時間、actor追溯，不建立新role或principal。外部引用只接受同snapshot核實已`principal_active`的account/profile pair；未知active sponsor／委派拒絕，無法重建canonical actor的已撤銷歷史仍留原表，並先拒絕不合owner schema的時間順序。聚焦Vitest 6/6、065隔離PostgreSQL 17/17、`typecheck:app`、ESLint與DB boundary PASS，task-owned PG cluster／port／temp已清理。候選列`planHash`不是完整來源雜湊或權限等價證據；尚未接完整owner prepare／apply、全路徑v2授權或Production，不能用此planner切換或啟用v2 emitter。切換後仍須實際移除正常runtime舊UID／`pdm:*`路徑。

> **2026-09-25 DEV-121 ACL 唯讀來源預覽（本機、未接apply）**：新增`previewPrincipalAclMigration`，在同一唯讀RR owner snapshot重驗完整provider／typed來源集合、inventory marker source hash、legacy profile、唯一active priority與全體候選assignment／delegation；外部sponsor／委派只有已`principal_active`且account/profile pair相符才轉為principal參照，未切換時預覽拒絕。隔離PostgreSQL QC已覆蓋「外部未切換→拒絕；切換後→保留sponsor與委派」；不變更Production。預覽尚未形成完整跨owner來源hash及舊新權限等價，故不能作cutover apply或啟用v2 emitter；正式切流後仍須拆除正常runtime舊UID／`pdm:*`授權。

> **2026-09-25 DEV-121 principal-keyed entitlement runtime（本機）**：AI-PDM正式 reader 現以已驗 `principal_id` 查全部 active provider aliases 與 effective grants；當前登入 alias 只證身分，各 alias grant facts 必須完全一致，缺列、未知 alias、重複 role/scope、authority version 漂移一律拒絕，禁止聯集或退回UID ACL。Reviewer selection 改為每個 principal 評估一次。聚焦 repository／snapshot／session／reviewer `7 files / 55 tests PASS`、typecheck、scoped ESLint PASS；未執行Production readback／L4。使用者已明示舊架構無須保留；全 cohort 切換及回復觀察完成後刪除舊 runtime bridge／authority branch，歷史資料僅供稽核，不把臨時相容層當完成架構。

> **2026-09-25 DEV-121 本地來源漂移雜湊（本機、非完整cutover hash）**：同一唯讀RR預覽另讀所有本地role permission／scope rules、唯一active priority版本、cohort membership與UID session安全欄，與inventory、profile、marker、assignment／delegation和外部已active pair生成版本化`localSourceHash`；不讀密碼或token原文。聚焦hash／planner Vitest 9/9、隔離PostgreSQL 17/17（含同來源重播相同hash、policy deny變更hash不同）、typecheck PASS，臨時cluster／port／資料已清理。此hash當時尚未包含跨owner來源；下列`producerSourceHash`現已另行補上，但兩者都不能替代正式prepare source hash、shadow等價或apply；切換後舊UID／`pdm:*`正常runtime仍須移除。

> **2026-09-25 DEV-121 跨owner來源漂移雜湊（本機、非完整cutover hash）**：同一唯讀RR預覽以單一SQL statement讀取Platform v3純讀函式及OrgMaster已發布typed pair、authority／effective grants contract，按principal及已驗provider pair生成`producerSourceHash`；缺列、雙authority、pair／mapping／authority版本錯配及legacy authority混入grants均拒絕，同principal雙alias須具相同Platform state與authority。隔離PostgreSQL 17/17含單statement計數、Platform epoch、OrgMaster authority及effective grant漂移，以及legacy authority混入grant；hash／planner Vitest 9/9、typecheck及ESLint PASS，task-owned cluster／port／資料清理。尚缺source revision／manifest綁定、**apply鎖後再次**單SQL producer capture、舊新權限shadow等價及atomic cutover，因此兩個preview hash都不能作切換許可，也未修改Production。

> **2026-09-25 DEV-121 owner apply 來源鎖切片（本機、未接 mutation）**：新增固定19張本地來源／目的table的`SHARE ROW EXCLUSIVE`鎖順序與operation→sorted subject advisory locks；同operation的成功receipt需kind／input／cohort hash均相同才在cohort／table鎖前回傳，衝突拒絕。切換專用inventory reader只在`READ COMMITTED`且同connection的19把鎖全部由`pg_locks`讀回後允許來源重讀，普通preview仍只接受RR／SERIALIZABLE。共用`readPrincipalAclMigrationSource`在隔離PostgreSQL中證明RR preview與RC已鎖source的local／producer／plan hash相同。盤點發現active AI-PDM catalog原未綁來源，現將完整發布角色定義／版本納入local hash，且三張catalog owner table加入鎖；catalog定義改版時local hash必變。隔離PG 17/17、鎖順序／衝突Vitest 4/4、local hash Vitest 3/3、typecheck／ESLint PASS，臨時cluster／port／資料已清理。這尚未形成完整prepare hash、權限shadow等價、commit receipt或實際apply；**不得**拿此API單獨切換帳號或啟用v2 emitter。終態仍須移除舊UID／`pdm:*`正常授權路徑，而不是永久保留`legacy_compatible`分支。

> **2026-09-25 DEV-121 owner source seal（本機純函式）**：鎖後與preview現在都輸出同一組候選account／marker CAS版本，隔離PG證明逐欄一致。新增版本化`sourceHash`封存三owner exact source revisions、各自contract manifest hash、local／producer來源與plan hash，另生cohort／operation input hash；不同revision、manifest、policy、grant、plan或operation ID的漂移會改變對應雜湊，重複principal與`pdm:*`拒絕。聚焦source seal／locks／local hash Vitest 10/10、型別與ESLint PASS。這僅計算已供給facts的封存值：provider revision／manifest readback、權限shadow等價、owner atomic apply與receipt、Production證據仍須完成，不能憑單一hash授權切流。

> **2026-09-25 DEV-121 owner source gate（本機、零切換寫入）**：prepare envelope現保留重讀所需的完整來源輸入與三owner綁定；新`requireCurrentPrincipalCutoverSource`在migrator READ COMMITTED交易中先查同operation成功receipt，再取固定來源鎖，重讀inventory、local ACL／catalog及Platform／OrgMaster producer，重算cohort／source／input hash，與prepared值不符即拒絕。隔離PostgreSQL驗無漂移可續行、catalog版本／定義變更在寫入前拒絕，17/17 PASS，typecheck／ESLint PASS，cluster／port／temp已清理。此gate尚未驗provider對source revision／manifest的attestation，亦無shadow等價或atomic materialization；未變更Production，不能以此宣布principal切換完成。

> **2026-09-25 DEV-121 有效ACL關係圖檢查（本機、仍未可切換）**：preview與鎖後重讀現各自對候選搬移列執行`assertPrincipalAclGraphPreserved`，檢查所有有效base／explicit role、scope、sponsor、時間、delegation及最低assurance既未增加也未遺漏；`graphHash`已納入完整source seal，漂移會阻止後續apply。聚焦圖檢查與seal Vitest 6/6、隔離PG 17/17、typecheck PASS，臨時叢集已清理。這是結構性證據，**不是**舊新permission evaluator、resource predicate及明列安全修正的完整behavior shadow；後者、owner atomic write與Production均未完成，不能據此啟用v2 emitter或退役仍在服務未切換帳號的暫時路徑。

> **2026-09-25 DEV-121 owner原子寫入核心（本機、未接正式執行器）**：新增只接受同connection鎖後verified source object的migrator materializer；以同一READ COMMITTED transaction寫唯一principal account、base／explicit assignment、delegation、UID session撤銷、單向marker CAS及append-only operation receipt。來源gate固定UTC，消除預覽／鎖後因連線時區不同而產生的偽漂移；verified object不可變、複製物拒絕。隔離PG synthetic subject已實際COMMIT並驗deferred FK、帳戶／marker／舊session及receipt replay，另測CAS衝突與整筆ROLLBACK後無殘留；17/17 PASS、typecheck／ESLint PASS、臨時cluster／port／temp清理。此primitive尚未取得三owner revision／manifest provider attestation、完整behavior shadow與正式owner CLI綁定，**不得單獨呼叫到Production或啟用v2 emitter**。舊架構正常runtime路徑須待全體受控cutover／回復觀察後移除。

> **2026-09-25 DEV-121 全profile覆蓋讀回（本機、未執行雲端）**：同一owner runner新增`coverage`唯讀模式，從AI-PDM每個本地profile列出來源計數、marker狀態、缺列／停用／多義／未解析原因及active尚未principal化分母；principal_active帳號以`principal_accounts`狀態為準，即使無舊Firebase／OAuth映射亦不誤列provider missing。preview也在同一RR快照返回目前marker CAS row version，避免人工猜0；註冊仍在新交易重驗。隔離PG 17/17與runner 5/5 PASS、任務專用cluster／port／temp已清理；此盤點不能證明最後v1 session到期或切流完成，Production尚未執行。
...
> **2026-09-16 DEV-117 production incident recovery（current）**：R78 data cutover與artifact provenance仍有效；正式圖號／料號workbench因service template缺`PDM_BUILD_COMMIT`，且cutover完整保留舊正式authority commit `bb30682c...`，造成runtime handshake 503。已完成on-demand backup `1789532631908=SUCCESSFUL`；Cloud SQL live export證明singleton row_version=8後，以serializable CAS只把expected_commit綁到R78並readback row_version=9，未重匯或修改業務資料。Current recovery採R78 exact image＋零流量candidate＋signed contract-token authority／50圖號／59料號readback；`production-activation`為獨立人工gate，traffic GO待另一個明確決策。
...
> **2026-09-11 DEV-117 R38 pre-auth closure（historical v1 event）**：R35 exact production migration replay已由provider execution `ai-pdm-prod-migration-runner-7gjhv`完成，結果為`14 replayed`；candidate／entrypoint／traffic皆未執行。Owner改以Cloud Run v2 `conditions[type=Completed]`判定execution終止，並把own migration Job resource-scoped viewer納入IaC complete-set。DEV-117舊V1 adapter契約與current V3 direct-run workflow測試已拆分：legacy 12／12、V3 28／28、abort 6／6、release-adapter QC、DB boundary、typecheck與isolated production build均PASS。因該次測試修正形成新source，R37 source lock／foundation及其下游authority全部作廢；R37未執行AI-PDM app-infra apply。Operator當時取得新授權後只接受fresh R38，不使用`pdm.jenfu.com.tw`、Firebase Hosting、shared LB或TOTP。此段保留歷史事實；現行DEV-013 v2只重建受source變更影響的machine evidence，不因SHA本身重新取得人類授權。
...
> **2026-09-05 DEV-010 R1Q snapshot producer**：Platform `c194081`已完成reviewed mapping＋四provider captures到source／candidate snapshots的typed producer，raw rows不落盤；focused=`7/7 PASS`、含assembler／adapter=`35/35 PASS`。Provider-native capture executors與actual captures仍未完成，current `BLOCKED 9`且R1-05／R02皆`NOT_RUN`。

> **2026-09-05 DEV-010 R1R provider capture executor**：Platform `fa2aaf6／11c79b9`已完成legacy AI-PDM DB、OrgMaster frozen inventory、neutral DB及neutral object storage四來源default-deny executor source；DB為repeatable-read read-only＋always rollback，OrgMaster綁source bytes／R1-01 revision，GCS只describe candidate DB exact pointer，raw rows不落盤。Unit=`7/7 PASS`、combined=`42/42 PASS`；current `BLOCKED 9`，reviewed mapping／projection catalog、actual captures、R1-05與R02仍`NOT_RUN`。本切片不啟用GCS writer、不搬檔、不新增常駐成本或解鎖Level 4。

> **2026-09-05 DEV-010 R1S／R1T／R1U operational safety executors**：Platform已完成R1-08 describe-only rollback-readiness source、R1-06三類source write-fence authority與post-fence zero-delta finalizer。R1-06以Platform無connected legacy runtime、OrgMaster frozen Git／local-json、AI-PDM dedicated fence controller同transaction撤銷migration membership與`pdm_runtime` DML／sequence ACL，並以DB-resident exact restore manifest形成可恢復authority；R1U再強制使用post-cutoff獨立四來源read-only capture、由capture重建的雙snapshot SHA hard join、同migration cursor與12組exact zero，禁止為符合證據schema虛構candidate write。Platform R1T／R1U各=`6/6 PASS`，相鄰R1-05／06／08／adapter鏈=`61/61 PASS`，AI-PDM controller source=`2/2 PASS`。這些只關閉source gap；controller provider apply、actual captures／finalizer與R1-06 receipt仍`NOT_RUN`，也不替代R02的`company-smoke`真實COMMIT＋reload。R1-08 actual plan／provider readback亦仍`NOT_RUN`。

> **2026-09-05 DEV-010 R1V／R1W observation and retirement readiness**：Platform R1V已完成R1-09 least-privilege read-only observation executor，R1W再修正live migration前要求consumer與rollback dependency歸零的循環矛盾。R1-10現只接受R1-01 exact consumer manifest與promotion後action、R1-08 rollback retained、R1-09至少30分鐘且P0／P1=0及三owner signoff；`RETAIN_LEGACY`與提前retire／delete固定FAIL。R1-09=`10/10 PASS`、R1-10=`7/7 PASS`及focused QC；current在credential／plan／provider／DB前`BLOCKED 9`，actual R1-09／10與R02均`NOT_RUN`。Actual legacy retirement另受post-promotion gate，不增加固定成本或調高USD 100 budget。

> **2026-09-05 DEV-010 R1X guarded three-version credential rotation**：Platform commit `20f020a`已完成R1-12 executor source，將模糊的「old version revoked」拆成`rollbackProtectedVersion`、`currentVersion`與相異的`supersededVersion`。前兩版必須維持`ENABLED`；只有盤點全部Cloud Run revisions與jobs、證明superseded跨所有reference為0後，才可用exact單一permission custom role執行五個Secret Manager `disable`。預定義`roles/secretmanager.secretVersionManager`因同時含add／enable／destroy而禁止使用；progress ledger若有未知結果即阻擋重跑。Unit=`8/8 PASS`、release=`19/19 PASS`、IaC=`8/8 PASS`；actual R1-12仍`BLOCKED / NOT_RUN`且mutation=0。此gate不替代R02的`company-smoke` Production COMMIT＋reload，也不允許寫入`company-jenfu`。

> **2026-09-05 DEV-010 R1Y isolated runtime-pointer rollback drill**：Platform commit `21f5f3a`已完成R1-13 executor source。只在同project／region／tier／PG17 private clone建立task-owned schema／role，三元件各執行previous→candidate→previous的catalog pointer、single-writer membership與無schema名稱runtime query，共9次readback；candidate只寫3筆clone sentinel，復原後data／auth before=after、unknown window=0、source unchanged且task DB objects／clone residue=0。Unit=`10/10 PASS`、release=`19/19 PASS`、focused QC=`PASS`；actual R1-13仍`BLOCKED / NOT_RUN`且mutation=0。此gate只證明rollback mechanics，不增加R02分子，也不能替代`company-smoke` Production COMMIT＋reload。
...
- ◐ DEV-118 / Production Released / Workspace Google + Global Logout Current Evidence / Full Four-cell Browser Matrix Pending / 118-A本地入口與118-B跨專案工程契約均已完成；118-C owner release已完成，剩餘工作只限Production互動驗收。
  2026-09-17續行audit更正舊A03驗證缺口：已補10秒逾時、必要欄位驗證、abort／late-response保護；2026-09-21 exact snapshot browser 30/30、typecheck、affected lint 0 error／2既有warning、isolated build PASS，詳見118-A QC receipt。舊12/12只作歷史。
  118-B已完成Platform migrations 006／007、OrgMaster migrations 016／017、DWD與雙admission；current 118-C run `35669545522`把main `6e21bb4c2f39d4b4777320d202a2037bde609b00`發布至`ai-pdm-prod-f5ee2af2d7ec`，image `sha256:fee7d3e6f653e29332a77a87ca53fa897b76aed215dcdee54f16fd585fde10bd`承接100% traffic，十個owner stages與15筆migration replay均PASS。Workspace Google-first與`JFS0005`工號起手皆完成Platform session、AI-PDM handoff／reload、accounts與permission；global logout後AI-PDM `/api/auth/me`、accounts與permissions均401。AI-PDM local logout另以POST=200、三個protected APIs=401及Platform session免Google重驗重新handoff=200證明target-only session撤銷。`PDM-W-G`及`PDM-W-E`具current evidence；Free-only／無Gmail fixture及negative／rate／race cells仍缺，完整LOGIN六案與PDM C01／C02四格尚未完成。Authority 為
...
- 現行已完成本地foundation：`✓ DEV-116` Production Level 4 smoke tenant 隔離與可稽核證據關卡。
  - 狀態：`Local RD Implemented / QA-QC 31 of 31 PASS / 116-R R01 Started / Production Read-only Preflight Complete / DEV-010 R1E Source Closed and Production Binding Gated / R02-R03 NOT_RUN`。
  ...
  - 可接受技術債：Platform `TD-010-ZONAL-01`只延後shared database HA，不改DEV-116隔離或Level 4。若restore超出核准RTO／RPO、發生zone-attributable P0／P1 outage或Business Owner不再接受單區停機，REGIONAL決策與新target會使舊R01／R02／R04 evidence失效；必須重驗，不能把測試寫回`company-jenfu`。
  - 下一步：Platform ZONAL machine contract、R02雙producer、010-R1E verifier source／local QC、三repo R1-04A local packaging、A／F／B guarded producer、R1T source-fence、R1U zero-delta finalizer、R1V read-only observation、R1W retirement-readiness、R1X guarded credential-rotation、R1Y isolated rollback-drill、R1-14／15與owner waiver requalification均已完成。Exact neutral project container已`ACTIVE`，但Billing link quota使`billingEnabled=false`；先完成quota核准與Billing link provider readback。解除後由R1-04A建立provider-attested artifacts／正式scan receipt；只有neutral service不存在時才執行F，且holding effective IAM須無Cloud SQL／五個runtime-secret access、503-only、no env／secret／Cloud SQL、internal、default URL disabled、zero direct invoker binding、min=0及gen2 512 MiB。Release reviewer再產出只含exact numeric secret version reference的runtime manifest，由B以exact digest建立zero-canonical-traffic candidate，再執行R1-11兩輪數值容量。回REL-116後依序完成`063→062`、dedicated smoke principal與exact verifier membership；R1-06須執行source fence並以post-cutoff capture證明final delta=0，R02則在同candidate執行browser observation及provider readback，R1-09再以least-privilege verifier確認requests／connections／alerts與smoke quarantine。R1-08 reviewed rollback plan成立後才可執行R1-12及clone-only R1-13，兩案都不能取代R02。15案全PASS後才可依3A進live migration；R1-07／R02兩邊一致PASS才成立AI-PDM Production Level 4，promotion另需獨立GO。
...
- 本機已完成、發布併入 `DEV-032`：`✓ DEV-106` production退役殘留 forward-only cleanup。
  - 狀態：`Local RD + Pre-release QA/QC Complete / 13 of 13 Local Gates PASS / Production Release Owned by DEV-032`。
...
- 已關閉的產品候選：`× DEV-041`、`× DEV-015`。
  ...
  - `DEV-015` 因未選定Phase 2+切片且後續canonical Drawing／審核工作臺已承接主要需求，現行任務以「未選擇、不實作」關閉；未來新需求不得沿用舊大包直接派工。
...
- ◐ DEV-121 [開發中] [B principal-first Architecture Finalized／RD Implementation Ready；產品未完成] [P0] [Production L4 NOT_RUN] 目標端授權邊界重構
  ...
  - 計入交付：否；此為既有登入與權限交付的後續開發點，文件子任務不改算既有證據。
...
- ✓ DEV-119 [開發點] [013-S4-L3-AIPDM-ENV READY_FOR_NONPROD_APPLY] [P0] [Shared staging only] DEV-013 owner-native staging release adapter
  ...
  - 邊界：Terraform只擁有state prefix、artifact repository、evidence bucket與exact IAM，完整8-address plan須驗證exact after fields；existing service不納Terraform。broker及target origin只接受provider readback；`PDM_SESSION_CURRENT_SECRET`、`PDM_SESSION_PREVIOUS_SECRET`、`PDM_WORKBENCH_CONTRACT_SECRET`分別綁三個既有Secret，alias只可進入migration planner，產生revision與receipt後一律numeric且不讀payload。保留entry、network、session、Firebase、DB、proxy、startup與capacity state；rollback只回同source／同digest的DEV-013 off revision，不得回pre-DEV-013 image。
  ...
  - Current：manifest v2／profile／baseline traffic＋deletion-protection plan／Secret pinning／target bootstrap v2／owner receipt v2／provider hard joins、Infra A byte-bound source freeze與complete-plan gate的focused tests為12／12 PASS；Terraform fmt／backend-disabled validate PASS。Fresh owner-native provider readback確認deterministic origin、runtime identity、ready revision=`ai-pdm-stg-00004-pvm`、`latest=100%`與deletion protection=false，已產生只含`traffic,deletionProtection`且template／labels／siblings=0的reviewable plan；尚未執行。Platform current source-bound exact read-only preflight另確認service labels仍不符DEV-013、三組Secret仍為`latest`，且`dev013-ai-pdm-staging` repository與evidence bucket不存在，因此bootstrap receipt fail closed。exact clean-HEAD source freeze於commit後產生並由handoff回報。Cloud Run revision／traffic mutation、Terraform apply、managed DB migration、L3 browser、production與legacy staging mutation=`NOT_RUN`。
...
- ◐ DEV-118 [開發點] [Production Released；Workspace Google + Global Logout Current Evidence；C01／C02 Four-cell Verification Pending] [P0] [RD／Owner Release Complete／正式互動驗證待完成] 平台登入入口對齊
  - 摘要：Google 是身分驗證方式，工號是同一核准身分的登入別名；SSO 啟用後由平台集中登入，
    PDM 只提供平台主要入口。Workspace 與 Cloud Identity Free 都走 Google，不以 Gmail 信箱分類 provider；
  ...
  - 下一步：不重做已完成的owner release、migration或admission；在production canonical以受控Workspace／Cloud Identity Free principals完成八個identity cells與PDM C01／C02四格、必要拒絕路徑、reload及global logout。
    118-A 無未完成本地項目；PDM C01／C02 保留 Workspace／Free × Google／工號四個整合 cells，與 Platform LOGIN-06 共用合格 evidence。
  - 阻塞／恢復條件：source、artifact、owner finalize、conformance receipt、admission與Firebase Google provider均已完成。唯一尚未取得的是受控帳號的互動browser evidence；production C01／C02及拒絕路徑未PASS前不得宣稱功能LIVE_VERIFIED，若security或核心登入失敗須依既有workflow回previous known-good revision。
  - 證據：[DEV-118 唯一契約](specs/SPEC-PDM-PRODUCTION-GOOGLE-SIGN-IN-READINESS-001-provider-capability-and-entrypoint-gate.md)、[118-A QC receipt](qc/qc-dev-118-local-implementation-2026-09-17.md)、[118-B Platform 唯讀交接查核](qc/qc-dev-118-platform-handoff-readonly-2026-09-17.md)。
    ...
    118-A 歷史 contract 10/10、browser 12/12、DEV-013 handoff 3/3、DEV-046 alias 21/21、typecheck、isolated build PASS；Browser evidence：`output/qa/dev-118-login-entry/DEV118-browser-2026-09-17T00-48-58-748Z/manifest.json`。Platform R2／LOGIN-R1均Architecture Finalized，QA分別0／20與0／6。2026-09-17依「升級到架構確定」同步API／state／recovery／runner契約與ADR／QA；118-B/C仍未驗收，本輪未執行產品測試。
    同日RD技術主管複審修正Platform重試／清理、取消競態、unknown readback與啟用驗證循環；PDM §7固定`PDM-W-G／PDM-W-E／PDM-F-G／PDM-F-E`，與LOGIN-06引用同一份target evidence。維持LOGIN-R1；文件檢查不計入產品PASS。
    目標續行另補A03真實缺陷：10秒timeout／abort、缺少SSO狀態不fallback、重試晚回隔離；修正browser viewport設定。最新`DEV118-browser-2026-09-21T03-13-12-373Z`綁定HEAD `4de5cdc1f6452ae153d91ddb04bde2c255b8377d`與dirty fingerprint `7defab02835b80a41ccf3e65902400a6f7f5e4533bd2ae18fa760cad13488415`，30/30、typecheck、affected lint 0 error／2既有warning、isolated build PASS，temporary runtime／port／data／Next dist與隔離worktree均清理。同日20:00的來源靜態交接審查曾記錄active工號、orphan rollback、generation／TTL、重試command與final admission五項P1；這些後續已由Platform R1～R5 source closure及local gate關閉。118-C owner release、provider啟用與normal-entry account chooser已實際完成；受控帳號矩陣仍待互動驗證。
...
- ✓ DEV-116 [開發點] [Local RD Implemented / Tech Lead Approved after Corrections / 116-A-B-C Complete / QA-QC 31 of 31 PASS / R02 Receipt 5 of 5 + Browser 7 of 7 + Provider Producer 5 of 5 PASS / local-foundation] [P0] [DEV-010 R1E Source Blocker Closed / Production Binding and 116-R Activation Gated] Production Level 4 smoke tenant 隔離與可稽核證據關卡
  ...
  - 成本與容量guard：`config/platform/cost-budget.template.json`的`USD 30／月`只保留為DEV-010之前的歷史app規劃基線；neutral三系統shared database current authority為Platform DEV-010的`USD 65～80／月`正常估計與`USD 100` alerts-only budget。邏輯company本身不新增固定SKU；低頻smoke的DB rows、log與request增量`USD 0～1／月`只是planning target。R04在release前依當次SKU做單run upper-bound與fixed-floor=0 gate；首次啟用後的10次run或30日實際差額改由`OBS-116-01`判定，超標則暫停後續例行write smoke，不循環阻擋第一次驗證。
  - Current Phase closure：`116-A Company authority`、`116-B Query/API isolation`與`116-C Level 4 evidence foundation`已依序完成，Current claim=`local-foundation`。R02 machine receipt verifier與Platform projection為`5/5 PASS`；authenticated browser executor contract為`7/7 PASS`，Platform provider observation producer為`5/5 PASS`。DEV-010 R1-05 snapshot producer、四來源capture executor及R1-08 rollback-readiness executor source各為`7/7 PASS`；R1-06 source-fence與post-fence zero-delta finalizer各為`6/6 PASS`，R1-09 read-only observation executor為`10/10 PASS`，R1-10 retirement-readiness finalizer與R1-14／15各為`7/7 PASS`，focused QC皆PASS。R1U把final reconciliation固定為read-only exact zero；R1V由provider alert ledger推導P0／P1、驗證full-window connection samples，並把R1-07 exact一筆`company-smoke` pending outbox固定為consumer-disabled quarantine evidence；R1W則固定consumer完整分類、promotion後action與rollback retained，禁止提前retire／delete。Owner evidence waiver已完成requalification；exact neutral project已`ACTIVE`但Billing未啟用，current full preflight=`BLOCKED 2`，另一項capacity只可於candidate建立後執行。Reviewed mapping／projection catalog／rollback／write-fence／observation／retirement plans、provider-native captures／readback與actual finalization仍未執行。Preflight BLOCKED時不讀憑證、不啟動browser，任一單邊evidence不能finalize。Future `116-R Production activation`仍為`Release Gate Required / NOT_RUN`；R1-04A／F／B provider execution與reviewed runtime manifest仍未完成。Actual provider readback、R1-05／06／08／09／10／14／15、R02與全系統audit收斂未被本地PASS偷渡完成。
  ...
  - QA contract：Current固定`QA-116-001..031`，machine registry=`.ai-doc/qa/dev-116-current-case-registry.json`；六個producer＋aggregate各有唯一case owner。Final run涵蓋fresh/history雙provider migration、identity tamper、audit／sequence／read-model隔離、normal UI commit＋reload、response loss、cleanup failure、rollback、6/6 mutants、source drift、redaction、primary invariant與task-owned cleanup，結果`31/31 PASS`。R02 receipt contract `5/5 PASS`涵蓋exact雙case ID、candidate、smoke actor、commit/reload、Jenfu invariant、side-effect與tamper/redaction；browser executor contract另為`7/7 PASS`，固定READY preflight、UI-only mutation、environment-only credential與provider join。Future release `QA-116-R01..R04`仍`NOT_RUN`且不得混入local分子，只有R02 actual execution能形成Production Level 4 claim。
  - Spec Impact Preflight：`Compatible amendment`。沿用`SPEC/ADR-PDM-PRODUCTION-SLICE-001`既有smoke company決策；新增focused SPEC與QA把implementation/evidence gap正式化。010-R1E的PostgreSQL verifier role／evidence views只補最小權限provider evidence，不改production identity、topology或write path，故`No New ADR`；若改成第二套stack、Jenfu routine write、core-table blanket access或借用runtime／migrator才須重開審查。
  - Stop／release boundary：P0隔離證據未完成前，所有future production write smoke維持blocked；可使用staging write E2E、production authenticated read-only，以及另行核准的transaction rollback/fingerprint。`jenfu_r1_verifier` group、owner-owned views、exact login membership、positive／negative ACL與browser-before／provider-after read-only證據任一未完成，不得actual R02，也不得借用runtime／migrator／owner／superuser。任何remote schema／data／IAM／deploy／release、付費資源新增、正式號碼刪除／回收或既有正式資料修復，均須獨立高風險／release gate。
  ...
  - Authority／evidence plan：`.ai-doc/specs/SPEC-PDM-PRODUCTION-SMOKE-TENANT-001-level4-isolation-and-evidence.md`、`.ai-doc/qa/qa-dev-116-production-smoke-tenant-isolation-validation-plan-2026-09-04.md`；相容上位authority為production-slice SPEC／ADR／QA、`DEV-032/040/044/069`與Platform `DEV-010`。Current 31/31、R02 receipt 5/5、browser executor 7/7、Platform provider producer 5/5及pipeline 25/25已PASS；R01唯讀precheck已完成但整體BLOCKED，R04 policy precheck PASS但未綁final release commit，actual provider observation、R02／R03仍`NOT_RUN`，因此沒有Production Level 4或production release evidence。
  - Actual boundary摘要：新增`audit-scope.ts`、`production-smoke-runtime.ts`、PostgreSQL `063`、一份無credential policy config、shared DB boundary／evidence utility、六producer、aggregate、orchestrator、R02 receipt verifier，以及authenticated browser／provider observation join與finalizer；修改14個Current async/schema/UI/package檔，另更新package scripts與production pipeline static QC。Browser executor無deploy／migrate／promote／cleanup能力，憑證只走兩個環境變數且不進evidence。Sync repositories、generic audit facade/writers、preview/attachment/submission audit、production workflow、generic production smoke、固定`dev:local`、primary DB與remote target均未改；`next-env.d.ts`只被Next暫時重寫後byte-for-byte復原為既有SHA-256。
  - 下一步：010-R1E role／views／binding／read-only executor source、fresh N1A／N2／focused QC、R1T write-fence、R1U zero-delta finalizer、R1V read-only observation、R1W retirement-readiness、R1-14／15與owner waiver requalification均已完成。Current source lock=`FROZEN`；exact neutral project已`ACTIVE`但Billing未啟用，因此先完成Billing project-link quota核准與link readback。解除後建立provider-attested artifacts／zero-traffic candidate並執行R1-11兩輪容量；取得production明確授權後才依序套用Cloud SQL `063→062`、principal／membership，再完成15案provider execution。R1-06必須依`write fence → post-cutoff read-only captures → canonical snapshots → exact-zero finalizer`順序執行；R02必須是exact production candidate authenticated SMOKE commit＋reload並與Platform R1-07共用exact receipt；R1-09再以same candidate與least-privilege verifier驗證30～120分鐘健康、alerts與quarantine；R1-10只產生readiness，不停用舊consumer。不能重用本地31/31、R1-06 read-only evidence或generic smoke；deploy、promotion與actual legacy retirement各受獨立gate。
...
- × DEV-047 [開發點] [Accepted Technical Debt / Phase A0 Tooling Preserved / Closed Until Trigger] [P1] Legacy public schema bounded migration
  ...
  - 任務清單：
    ...
    - [x] Current disposition：Phase A0證據保存；Phase A～D不列現行backlog，接受legacy public schema技術債，避免在五人系統首發前進行高風險schema搬移。
    - Re-entry trigger：只有出現可量測的權限隔離缺口、命名衝突、維護事故、合規要求或明確平台擴張，才以新 DEV 重開 authoritative inventory、contract、rehearsal 與 release。
  - 執行範圍：Phase A0本機唯讀工具、focused QC與文件已完成；Phase A～D明確不納入本次或預設後續開發。接受技術債不代表schema move已完成，也不形成DEV-032 blocker。
  ...
  - 結案：`Closed as accepted technical debt`。恢復時以當時runtime/catalog重新立基，不直接沿用2026-07-13 inventory作執行authority。
  - 計入交付：否（平台治理證據，不是使用者功能）
...
- ✓ DEV-115 [開發點] [Local RD/QA-QC Complete / DEV-079 42 of 42 + DEV-080 12 of 12 PASS] [P1] [Local QA Infrastructure Only] DEV-079／080 current QA gate 收斂
...
- ✓ DEV-079 [交付點] [Local RD/QA-QC Complete / Current Matrix 42 of 42 PASS / Production Release Gated] [P1] 圖號唯讀抽屜與全頁編輯工作區分流
  ...
  - 成熟度：`Local RD/QA-QC Complete / Current Matrix 42 of 42 PASS / Production Release Gated`。079-A～D產品切片、accepted-state invariant、跨adapter resolver、受控reconciliation、GET zero-write、SQLite／PostgreSQL與三viewport均由DEV-115 current aggregate封口；successor-replaced案例均有精確case evidence。
...
- × DEV-080 [交付點／已整併] [Local QA-QC Complete 12 of 12 / Merged into DEV-087 and DEV-112] [P1] 全系統第一層狀態可見性與例外分層
  ...
  - 完成：DEV-115已完成12案current／successor／retired disposition、現行residual smoke與退役route/API 404/410零寫入，aggregate 12/12 PASS。依既定契約併入DEV-087／112，不再形成獨立產品或release owner。
...
- × DEV-041 [交付點] [Current Scope Closed at Phase 3A-0 / Future Capsule Only] [P1] 技轉包工作台、Pack-and-Go Intake 與整數 Baseline
  ...
  - 任務清單：
    ...
    - [x] Current disposition：Phase 3A-1～3C不納入第一版或現行backlog，轉為future product capsule，不以未實作項目降低目前完成率。
  - 執行範圍：Phase 3A-0 本機產品、SQLite schema、provider-neutral PostgreSQL migration artifact、API、共用工作台與 QC 已完成；現行DEV於此邊界結案。
  - 範圍外／future capsule：ZIP parser、分類、mapping/BOM/baseline、SolidWorks integration與後續審核；若五人實際流程提出明確Pack-and-Go需求，另以新DEV或明確重開決策封口範圍。
...
- × DEV-015 [開發點] [Closed Without Selection / Historical Handoff Only] [P1] 圖面送審工作台第 2+ 階段交接包
  ...
  - 結案判定：未選定任何Phase 2+切片，且後續DEV-079／087／101已承接canonical owner、狀態與審核主要需求；本項保留為歷史需求清單，不再是active/deferred backlog。
  ...
  - 恢復條件：只有出現上述successor未涵蓋、可描述且可驗收的新使用者問題時，建立新DEV；不得直接以本歷史大包恢復派工。
  - 計入交付：否
...
- × DEV-033 [開發點] [Future Capsule / Not Current Backlog / Includes DEV-037] [P2] GCS 檔案權威、保留、成本與 continuity package
  ...
  - 恢復條件：實際使用量或營運需求證明現有file fail-closed邊界不足，且使用者明確開啟file workflow後，另立可驗收DEV；先做inventory/cost/retention決策，再進staging adapter與release gate。
  - 計入交付：否
...
| × 關閉／未選擇 | `DEV-015` | `DEV-PDM-DRAWING-SUBMISSION-WORKBENCH-002-P2P` | 歷史交接 | successor已承接主要需求；新問題另立DEV，不直接恢復舊大包 |
...
| × 技術債接受 | `DEV-047` | `DEV-PDM-ERP-BOUNDED-SCHEMA-MIGRATION-001` | 平台治理 | Phase A0證據保存；僅在量測到隔離／合規／維護觸發時重開新DEV |
...
| × future capsule | `DEV-033` | `DEV-STORAGE-COST-001` | 非現行backlog | 保存GCS authority/cost/retention/continuity決策脈絡；已吸收DEV-037，需求出現時另立DEV |
...
使用者已授權本輪依Gate A-E順序執行所有Codex可完成工作；任何delete/replace、USD 240以上估算、target/provenance不一致、migration/restore/rollback/reconciliation/Level 3或4失敗仍須立即停止。Google re-auth、provider/MFA、DNS與必要的authenticated Level 4集中在人類／具登入能力的工作包；不再建立named-user Wave 0工作包。
...
- 2026-09-04（DEV-116-R R01 execution；歷史快照）：使用者要求執行後建立`REL-116-20260904`並進入production release lane。完成Google re-auth與Cloud Run／Cloud SQL唯讀盤點；Job execution `ai-pdm-prod-migration-runner-mcmxs`只作`BEGIN READ ONLY`後rollback，證實Production仍為`public` ledger 53／highest 056，DEV-010 neutral schemas、roles、principal／entitlement contracts與062全不存在。Release adapter改為`prepare→candidate→promote`、artifact build-once、manifest／DEV-010／principal／cost四證據fail-closed、revision三類side-effect disabled readback；legacy package硬排除DEV-010-owned 062，現為54支（既有53＋063）。Fresh adapter aggregate固定`31/31 PASS`、P0/P1=0、`claimLevel=local-foundation`、`productionLevel4Claimed=false`。其後三repo已重新freeze並由clean worktree取得DEV-010 N2 `48/48 PASS`；Windows canonical catalog CRLF差異以`.gitattributes eol=lf`根治，未放寬byte驗證。當時R01仍`BLOCKED`於neutral project identity、exact REGIONAL dedicated tier／cost及尚未實作／執行的DEV-010 R1 15案；此REGIONAL方向其後由下一筆ZONAL成本決策取代。R02/R03 NOT_RUN，production DB writes、deploy、traffic change、principal provisioning均為0。報告=`.ai-doc/reports/pm/pm-dev-116-r01-production-release-preflight-2026-09-04.md`。
- 2026-09-04（DEV-116／DEV-010 cross-gate與ZONAL決策收斂；歷史文件切片）：依使用者後續成本決策，以`db-custom-1-3840 / ZONAL_DEDICATED / USD 100`取代R01歷史REGIONAL方向，不回退backup／PITR／private IAM／schema-role隔離，也不宣稱HA。為避免低成本選項被誤用成放寬production smoke，`QA-116-R02`與Platform `QA-010-R1-07`的AI-PDM子流程固定共用同release／source lock／candidate／neutral target的`company-smoke`receipt；Jenfu write、receipt漂移、rollback-only或cleanup均不得PASS。Platform另固定R1-11兩輪production-like numeric gate。此切片當時仍缺machine更新；後續capacity v2／preflight v3／release-adapter v2與focused QC已關閉該缺口。Current剩餘阻塞為neutral identity、exact RTO／RPO、owners、ZONAL receipt、R1-11與15案provider executor；本切片未修改Cloud或Production狀態。
- 2026-09-04（DEV-116 R02 receipt contract implementation）：新增`dev116-r02-receipt` verifier／CLI／unit，固定`platformCaseId=QA-010-R1-07`、`pdmCaseId=QA-116-R02`、source lock、candidate image/revision、zero-traffic neutral target、hash actor、`company-smoke / SMOKE / production_smoke`、三件以上committed objects、API＋DB commit＋browser reload、Jenfu before=after／zeroLeak=0、side-effects disabled、redaction與self-hash；同時產生Platform exact projection。Unit `5/5 PASS`；CLI拒絕execute／deploy／migrate／promote。此結果只關閉opaque receipt驗章缺口，R02 actual execution仍`NOT_RUN`，production mutation為0。
- 2026-09-05（DEV-116 R02 authenticated browser executor foundation）：新增preflight／candidate-context／browser-observation／provider-observation四層self-hash contract、正常登入與UI create＋reload runner，以及browser／provider exact join finalizer。Executor必須同時具備`READY_FOR_R1_REHEARSAL`、neutral target、0% traffic、SMOKE Engineer、side effects disabled與exact write acknowledgement；credentials只走環境變數。Browser只建立一個root＋part＋drawing，不直接呼叫mutation API、不查DB、不deploy／migrate／promote／cleanup，輸出固定為`BROWSER_PASS_PROVIDER_OBSERVATION_REQUIRED`。Browser unit `7/7`、receipt `5/5`、production pipeline static QC `25/25 PASS`；current preflight仍9 blockers，故actual browser／provider／R02 production execution均`NOT_RUN`、production mutation=0。
- 2026-09-05（DEV-116 R02 Platform provider producer follow-up）：Platform新增self-hashed R1-07 provider readback contract、READY preflight／source lock／browser SHA／candidate／neutral ZONAL target／SMOKE actor／DB commit／Jenfu invariant／side-effect與30分鐘觀測窗fail-closed join，並動態使用AI-PDM R02 schema authority輸出`provider-observation.v1`。Unit `5/5 PASS`且已納入Platform release focused QC；CLI沒有query／gcloud／psql／credential／deploy／migrate／promote能力。這只關閉readback-to-observation producer缺口；provider-native readback與actual R02仍`NOT_RUN`，production mutation=0。
- 2026-09-05（DEV-010 bootstrap correction cross-repo requalification）：Platform production bootstrap的database ACL已由hard-coded `jenfu_dev010`改為identifier-safe `current_database()`，exact target guard仍在DDL前fail closed。Fresh N1A=`../../Jenfu-Management-system/output/dev-010/n1a/DEV010-N1A-20260904T164207461Z-6512/`，11/11 unit＋30/30 PostgreSQL QC PASS；三repo以detached exact-HEAD clean worktrees重跑N2，aggregate=`../../Jenfu-Management-system/output/dev-010/n2/aggregate/AGGREGATE-20260904T164946034Z-9908/aggregate-report.json`，48/48 PASS，report SHA-256=`548f54f43ee2896c0ac448789dd6905fefd72da9e241d937f603f75a246d5a1b`，canonical兩輪一致，55430與task resources均清理，`productionWrites=false`。此結果關閉bootstrap source gap，但不把local requalification升格為Production Level 4；post-commit source lock由Platform generated evidence依最新三repo HEAD重建，9項blocker與R02／15案`NOT_RUN`不變。
- 2026-09-04（DEV-010 legacy／neutral成本邊界）：AI-PDM repo內`jenfu-ai-pdm-prod / ai-pdm-prod-postgres / db-f1-micro / USD 300`仍是DEV-032 legacy單系統production現況與回復設定，不是DEV-010 current authority。不得先把legacy Terraform原地升成`db-custom-1-3840`或把其budget改成USD 100冒充neutral consolidation；這會造成待退休target擴容與過渡期重複成本。Current ZONAL／USD 100只由Platform capacity v2／preflight v3／release-adapter v2控制，neutral 15案、live migration、獨立GO與observation完成後才可退休legacy。若兩target短暫並存，必須把重疊時段與incremental cost納入R1-02 receipt。
- 2026-09-05（DEV-010 R1E local closure）：三repo完成verifier NOLOGIN group、八個owner-owned security-barrier evidence views、exact IAM DB login binding與provider-native `BEGIN READ ONLY` executor source；AI-PDM commit=`2ad790137`。Fresh N1A=`11＋30 PASS`、R1E combined unit=`14/14 PASS`、focused QC=`4/4 PASS`、detached exact-HEAD N2=`48/48 PASS`，negative base-table／DML／sequence／function／role escalation與cleanup均PASS，`productionWrites=false`。原P0 source blocker已關閉；production仍須`063→062`、正式membership、source lock、既有9項preflight與15案，R02／R03及traffic promotion維持`NOT_RUN`。
- 2026-09-05（DEV-116／DEV-010 R1-04 candidate producer contract）：RD技術主管審查指出新neutral Cloud Run service的第一個revision不能直接假設為0% traffic，且OrgMaster僅有server bundle不等於production runtime完整。文件因此將producer拆成`R1-04A` exact frozen source＋lockfile的immutable OCI build、必要時`R1-04F`無DB／無business route／無canonical入口的scale-to-zero holding revision，以及`R1-04B` digest-only neutral candidate revision；補上SBOM／provenance／secret scan、runtime manifest、AI-PDM legacy-neutral分離與R1-04↔R02 exact candidate join。此階段已由後續local packaging實作取代`NOT_STARTED`；R01／R02仍`BLOCKED / NOT_RUN`。
- 2026-09-05（DEV-116／DEV-010 R1-04A local packaging）：Platform `f5caab3`、OrgMaster `e9bf565`、AI-PDM `49e1760`三個detached exact commits完成local linux/amd64 OCI build；nonroot、runtime assets、OrgMaster no-`data/`、三份SPDX SBOM與bounded app secret scan PASS。AI-PDM image digest=`sha256:4399639605a9c506e5acd36b126d068cc3d30f1b31572309e8823f4681984d75`，primary SQLite before／after file／schema／identity hash及FK檢查一致。OrgMaster第一次lock drift被pinned npm 11.13.0 fail-closed後以前向commit修正。Current=`LOCAL_PACKAGE_BUILD_PASS / RELEASE_ARTIFACT_GATE_BLOCKED`；provider provenance／Artifact Registry／R1-04F-B、R01／R02 actual仍`NOT_RUN`，GCP／production／traffic mutation=0，USD 100與ZONAL風險契約不變。
- 2026-09-05（DEV-116／DEV-010 R1-04A provider producer source）：Platform `6c54439`新增preflight→foundation derivation、exact source-lock與two-key gate，READY後只允許clean locked clone、preexisting registry／bucket、digest-pinned builders、verified provenance、SBOM、secret scan及0 HIGH／CRITICAL；post-commit source lock=`FROZEN`，exact hash以generated receipt為權威。Unit `6／6`、focused QC PASS；current 8項foundation blocker使工具在provider auth前停止，故planned 3 uploads＋3 builds＋3 pushes均未發生。AI-PDM R01／R02仍`BLOCKED / NOT_RUN`，不因producer source完成而解鎖。
- 2026-09-05（DEV-116／DEV-010 R1-04F provider producer source）：Platform `38506b5／8889e47／287da0f`新增獨立no-role holding identity、503-only digest image、missing-service-only deploy、existing-service preserve-only baseline、gen2 512 MiB及zero direct invoker hardening；effective IAM逐項驗Cloud SQL connect／login及五個runtime secret access DENIED，新service再驗零env／volume／secret／Cloud SQL、internal、default URL disabled、private IAM與min=0。F必須先有A manifest；三service全新時resource-effect上限9，第一revision自動100%另列但canonical traffic change=0。Unit `8／8`、IaC合併`15／15 PASS`；current在gcloud前BLOCKED、實際mutation／cost=0，R1-04B source已完成但provider execution與R02未解鎖。
- 2026-09-05（DEV-116／DEV-010 R1-04B candidate producer source）：Platform commit `bc79bc662799bf5d28e8fe83bf6160918935ded2`新增exact A digest＋F receipt＋reviewed runtime manifest two-key gate，部署命令只建立三個`min=0／max=1`、startup Ready、0% traffic neutral revisions；service security／resolved traffic須前後一致，且沒有tag／authorized domain／migration／business write／public invoker／service-level ingress、scaling或promotion能力。IaC把三個混合runtime blobs修正為Platform／OrgMaster pepper及AI-PDM current／previous／workbench共五個獨立secret version streams，manifest拒絕`latest`。B 7＋F 8＋IaC 7=`22／22 PASS`、Terraform validate PASS；current A／F／runtime evidence缺件且foundation blocked，CLI在gcloud前停止，actual mutation／cost=0，R01／R02仍`BLOCKED / NOT_RUN`。
- 2026-09-05（DEV-116／DEV-010 R1T source write-fence dependency）：Platform完成R1-06 source executor，依R1-01分類把Platform固定為0 legacy runtime、OrgMaster固定為frozen Git／local-json、AI-PDM由不掛Cloud Run的dedicated fence IAM／NOLOGIN controller在PostgreSQL authority同transaction撤銷migration IAM membership與`pdm_runtime` table DML／sequence writes；撤銷前exact grant／membership manifest self-hash並存入DB ledger，restore只能逐物件及exact membership回復，本地report遺失仍可ledger-only recovery。AI-PDM新增controller role／IAM／Cloud SQL user source與`2/2 PASS` test；Platform Unit=`6/6`＋focused QC。Current preflight在provider／credential前BLOCKED，actual Production ACL／data／Cloud／traffic mutation=0。這只保護cutover source，不替代QA-116-R02；`company-smoke`仍須在neutral candidate真實COMMIT＋reload，且controller apply、post-cutoff final delta=0與actual R1-06仍`NOT_RUN`。固定月成本增量0，USD 100 budget不變。
- 2026-09-05（DEV-116／DEV-010 R1U post-fence zero-delta finalizer dependency）：Platform commit `adc0269`完成R1-06 finalizer source，把舊`candidateDatabaseWrites=true`證據矛盾更正為`POST_FENCE_ZERO_DELTA_RECONCILIATION / candidateDatabaseWrites=false`。Finalizer只接受freeze cutoff後、≤300秒skew、同source lock／candidate／cursor的四份read-only provider capture，於記憶體重建source／candidate canonical snapshots並與外部snapshot SHA逐一hard join；12組row／PK／content／FK／audit／file／migration ledger／domain／mapping及unclassified差異均須精確為0，restore receipt存在或maintenance window逾時即FAIL。Platform Unit／QC=`6/6 PASS`，相鄰R1-05／06／08／adapter鏈=`61/61 PASS`；current preflight仍`BLOCKED 9`，actual fence／capture／finalization／R1-06與R02均`NOT_RUN`。這不新增常駐SKU、固定月成本增量0，也不取代`company-smoke`在neutral candidate的真實COMMIT＋reload。
- 2026-09-05（DEV-116／DEV-010 R1V read-only observation dependency）：Platform完成R1-09 executor source，只有full preflight與R1-04／07／08 same-candidate PASS後，才冒用least-privilege `dev010-r1-verifier`讀Logging requests、Monitoring connections／alerts及DB contract views。Connection samples須unique且完整覆蓋window；P0／P1由provider alert ledger推導。R1-07 exact一筆`company-smoke` pending outbox固定為consumer-disabled quarantine evidence，其他eligible backlog須為0，不得啟用consumer、發通知或刪證據清零。Platform Unit=`10/10 PASS`、focused QC PASS；current在credential前`BLOCKED`，actual R1-09／R02均`NOT_RUN`，production／cloud／traffic mutation=0，固定月成本增量0、ZONAL／USD 100不變。
- 2026-09-05（DEV-116／DEV-010 R1W retirement-readiness dependency）：Platform前向修正R1-10生命週期，移除「live migration前legacy consumer與rollback dependency歸零」的不可達條件；改以R1-01 exact consumer manifest、逐consumer promotion後action、R1-08 rollback retained、R1-09至少30分鐘且P0／P1=0及三owner signoff。`RETAIN_LEGACY`、漏consumer、action錯配、提前retire／delete或移除rollback皆FAIL；actual legacy retirement另需post-promotion gate。Platform Unit=`7/7 PASS`、focused QC、release `19/19`、preflight `16/16 PASS`；current在plan／provider／DB前`BLOCKED`，actual R1-10／R02均`NOT_RUN`，mutation與固定月成本增量均為0。
