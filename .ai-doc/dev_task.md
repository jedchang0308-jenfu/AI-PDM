...
- `DEV-046` Phase 1A-1E、Phase 2A staging IaC、Phase 2B local application slices 與 Phase 2B staging authentication activation 均已完成。`HD-10-1 / 1A` 採單區 staging、Regional-HA production，production IaC 保守估算 USD 210；staging 與 production 均依使用者決策採 Firebase Hosting 預設網址，DNS 延後，ProJED 未修改。2026-07-16 `DEV-032` Gate A-D 已完成：`jenfu-ai-pdm-prod` Paid Billing、Firebase/Identity Platform、Cloud SQL Regional HA、private VPC、Cloud Run、IAM、monitoring、regional logs、TWD 9,600 budget、Admin principal、18 migrations、pre-canary reconciliation、HD-8-4 separate-target restore reconciliation、production slice smoke 與 authenticated Level 4 UI 領號驗收均通過；production entrypoint 為 `https://jenfu-ai-pdm-prod.web.app`，無 GCS file authority，Terraform no-drift。provider API readback 曾回傳 OAuth `clientSecret` 欄位於命令輸出，未落盤但命中 stop condition；使用者完成 A2 secret rotation 後，已用 sanitized readback 關閉。`DEV-032 Gate E` machine automation 已通過 15/15 並產出 human work package。後續只剩 `DEV-032 Gate E / A9` 人類 closure：3-5 位具名 Wave 0 使用者、named-user UI acceptance、non-allowlist human fail-closed 與產品負責人 go/no-go，不得恢復已取消的固定五個工作日 field gate。
...
- 唯一 P0 launch-moving 任務：`DEV-032` ERP 平台 production release work package。
  - 當前子關卡：`DEV-032 A9 / Gate E` Wave 0 go/no-go；A0-A8 已通過，包含 A2 secret rotation closure、hotfix deploy、production smoke、authenticated Level 4 UI 領號驗收與 Gate E machine automation 15/15。
  - 後續順序：完成 `output/dev-032-gate-e-automation/human-work-package.md`：提供 3-5 位具名 Wave 0 使用者、named-user UI acceptance、non-allowlist human fail-closed、零 open P0/P1 與產品負責人 go/no-go -> 才能宣稱 Wave 0 放行完成。
...
- ✓ DEV-048 [開發點] [本機整合完成] [P1] [Phase 1E P0 QC Passed / Local Only] 圖料號、草稿、狀態與技術移轉入口整合
  ...
  - 任務清單：
    ...
    - [x] Human Decision Gate `HD-048-01..03`：使用者於2026-07-13以`1C / 2C / 3C`關閉。
      - `HD-048-01 / 1C`：Phase 1B立即移除`料號草稿 / 領號申請 / 上傳送審 / 製造交接`四個可見側欄項目；舊URL只保留redirect/guidance、query與`returnTo`相容，不保留第二套mutation。
...
- ! DEV-032 [關卡] [Wave 0 Go/No-Go Pending] [P0] [Release Gate Required] ERP 平台 production release work package
  ...
  - 任務清單：
    ...
    - [x] `Gate D - Immutable Deploy & Smoke`：base application image index `sha256:b4fb8e9f...`曾部署至 revision `ai-pdm-prod-00006-lx5` 並完成 production slice Level 3；後續 production UI 領號發現 Cloud SQL Postgres `jsonb ~~ unknown`，已以 clean release worktree build hotfix commit `1936e93dfcd81be9ee308b1cf3d0249a282fb0be`、Cloud Build `297a7aa4-6257-4834-b1ae-fd8d964ad054`、image digest `sha256:5a58aa18d88610abfde5e1813e92c7230e223a4fa89e3a8e8f7ade8682c48fef`，並切 100% traffic 至 `ai-pdm-prod-00010-quc`。其後 Level 4 UI 發現料號明細的送審、3D/MA、附件與成本等 future workflows 仍可操作，已以 clean release worktree build hotfix commit `3ab5cffaaeb7a6d0a1b3e56165a390bb9bf5e728`、Cloud Build `7abc8bd3-b2fe-47cb-81f1-6bfe05915412`、image digest `sha256:742fbcab536c0cbccb3794907fec3795f345181124e54164cbd69966e0e5c42d` 修正；no-traffic tagged smoke 12/12、traffic switch 後 canonical post-traffic smoke 16/16、authenticated Level 4 UI 15/15 通過，Cloud Run 現為 `ai-pdm-prod-00012-kaf` 100% traffic。authenticated UI 確認既有正式草稿 `A0001` / `A0001-P01` / `A0001-M01` 與系列代號 `DEV032-HF` 仍持久顯示，未開放功能 visible-but-disabled 且 API fail-closed。短期 production pilot 繼續採 `https://jenfu-ai-pdm-prod.web.app`，不設定 DNS；GCS/CAD/BOM/file workflow 未開放。
    - [ ] `Gate E - Named-User Canary`：machine automation 15/15 已通過並確認 production UI readback、匿名 protected reads、未開放 write API fail-closed、direct run.app session exchange denied 與 human work package；尚需只開放核准的3-5位Google Workspace使用者，完成 named-user UI acceptance、non-allowlist human fail-closed、零open P0/P1與產品負責人 go/no-go。固定五日field gate已取消，不得自動擴大allowlist。
  - 下一步：完成 `output/dev-032-gate-e-automation/human-work-package.md`：提供 3-5 位具名 Wave 0 使用者、named-user UI acceptance、non-allowlist human fail-closed 與產品負責人 go/no-go；其餘 canary 不得由 AI 自行臆測，不要求設定 DNS，不恢復固定五個工作日觀察 gate。
  - 阻塞 / 恢復條件：每個子關卡需獨立 explicit approval；artifact provenance、target identity、cost、privacy、clean seed/source archive/non-reuse、connection budget、rollback、restore/reconciliation、smoke tenant任一缺失即停止。A2 secret exposure 已由使用者完成 secret rotation 並以 sanitized readback 關閉；若未來 provider readback 再出現 secret material，立即停止並重新旋轉。
  - 證據：`.ai-doc/reports/pm/pm-dev-032-production-gate-package-2026-07-15.md`、`.ai-doc/reports/pm/pm-dev-032-gate-a-b-execution-2026-07-16.md`、`.ai-doc/reports/pm/pm-dev-032-production-hosting-activation-2026-07-16.md`、`.ai-doc/reports/pm/pm-dev-032-production-principal-restore-reconciliation-2026-07-16.md`、`.ai-doc/reports/pm/pm-dev-032-human-handoff-2026-07-16.md`、`output/dev-032-production-terraform-plan/corrective/post-apply-readback.json`、`output/dev-032-production-hosting-plan/summary.json`、`output/dev-032-production-slice-activation/plan-review.json`、`output/dev-032-production-slice-activation/level3-smoke.json`、`output/dev-032-production-slice-activation/hotfix-1936e93d-tag-smoke.json`、`output/dev-032-production-slice-activation/hotfix-1936e93d-post-traffic-smoke.json`、`output/dev-032-production-slice-activation/hotfix-1936e93d-level4-ui.json`、`output/dev-032-production-slice-activation/hotfix-3ab5cffa-tag-smoke.json`、`output/dev-032-production-slice-activation/hotfix-3ab5cffa-post-traffic-smoke.json`、`output/dev-032-production-slice-activation/hotfix-3ab5cffa-level4-ui.json`、`output/dev-032-live-migration/`、`output/dev-032-production-auth-activation/summary.json`、`output/dev-032-production-auth-activation/provider-readback-sanitized.json`、`output/dev-032-production-auth-activation/secret-exposure-review.json`、`output/dev-032-production-live-readback/report.json`、`output/dev-032-production-activation-readiness/report.json`、`output/dev-032-production-principal-bootstrap/`、`output/dev-032-production-reconciliation/`、`output/dev-032-aal1-pilot-plan/post-apply-readback.json`、`output/dev-032-aal1-pilot-plan/runtime-manifest-provenance-hotfix-1936e93d.json`、`output/dev-032-aal1-pilot-plan/runtime-manifest-provenance-hotfix-3ab5cffa.json`、`output/dev-032-rollback-drill/v2-api-closure.json`、`output/dev-032-rollback-drill/traffic-rollback-validate-1784143002444.json`。
  - 計入交付：否
...
| ! Wave 0 pending | `DEV-032` | `DEV-CLOUDSQL-DB-001-PROD-GATE` | 關卡 | 唯一production入口；A0-A8已通過，剩3-5具名Wave 0與產品負責人go/no-go |
