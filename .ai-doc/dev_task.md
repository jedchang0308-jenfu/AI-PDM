...
- `DEV-PDM-NUMBERING-002` Phase 1-3 local implementation is implemented / verification passed after the user's 2026-07-07 RD authorization. New records now use compact v2 `00001 / 00001-P01 / 00001-M01 / 00001-R01`; `00001` remains a reusable design-object root, not a project/order/equipment root; normal creation uses `M/R`; v1 `MA/OT` and old `D-/P-` rows remain readable through semantic manufacturing/reference helpers; import/export examples, dry-run migration reporting, downstream manufacturing checks and focused QC are updated. Production deployment, production migration/cutover, applying v1-to-v2 data rewrite, direct DB repair/deletion, project/order/equipment numbering and extra visible category codes remain not authorized.
...
| Implemented / Verification passed | `DEV-PDM-NUMBERING-002` | Delivery point / numbering core | `DEV-PDM-NUMBERING-001`; `SPEC-PDM-NUMBERING-001`; `DEV-PDM-DRAWING-PART-WORKBENCH-001` | Phase 1-3 local implementation is complete: compact v2 rule/default creation, five-digit roots, `{root}-P01`, `{root}-M01`, `{root}-R01`, semantic `MA/M` manufacturing and `OT/R` reference compatibility, import/export sample updates, migration dry-run reporting, downstream submission/shared-3D/baseline compatibility, UI wording and focused QC updates. | Monitor APP validation feedback. Phase 4 production cutover, applying v1-to-v2 data rewrite, direct data repair/deletion, project/order/equipment numbering and extra visible category codes require explicit approval and release gate. Build was not run because the local-dev guard refused to clean `.next` while an existing 3000 server was active. | `.ai-doc/specs/SPEC-PDM-NUMBERING-002-compact-root-drawing-part-numbering.md`; `.ai-doc/decisions/ADR-PDM-NUMBERING-002-compact-root-drawing-part-identity.md`; `.ai-doc/qa/qa-pdm-numbering-v2-compact-identity-validation-plan-2026-07-07.md`; `.ai-doc/qc/qc-pdm-numbering-v2-compact-identity-report-2026-07-07.md`; `src/lib/numbering-identity.ts`; numbering repositories/API/UI/QC; `db/postgres/004_numbering_v2_compact_identity.sql`; `supabase/migrations/20260707000000_numbering_v2_compact_identity.sql`; evidence: `tsc`, lint, v2 compact QC 13/13, migration dry-run QC, numbering core 241/241, API regression 27/27, data consistency 16/16, concurrency 32/32, draft lifecycle 29/29, request UI 66/66, search UI 28/28, impact UI 24/24, DVT UI 24/24, master attachments 101/101, master workbench 224/224, Supabase runtime migrations 25/25. |
...
Authorized phase: Phase 1-3 local implementation and compatibility work were authorized by the user's 2026-07-07 RD execution request and are complete. Phase 4 production cutover, applying v1-to-v2 rewrite, direct data repair/deletion and production migration remain not authorized.
...
- `npm.cmd run build`: blocked by the intentional local-dev guard because PID 30948 was already listening on `http://127.0.0.1:3000`; no bypass was used.
...
|---|---|---|
| Applying existing-data v1 to v2 rewrite | Same Spec Phase 4 / Not Authorized | Dry-run evidence exists; applying changes to real data still needs approval, backup and release/data gate |
| Downstream compatibility | Same Spec Phase 3 / Completed locally | Semantic compatibility implemented and verified; production evidence belongs to Phase 4 if deployed |
...
- Monitor APP validation feedback. Do not run production cutover, apply v1-to-v2 rewrite, direct data repair/deletion, project/order/equipment numbering or extra visible category-code work without explicit approval and the required release/data gate.
...
- 2026-07-07: Implemented and verified `DEV-PDM-NUMBERING-002` Phase 1-3 locally after user RD authorization. New normal numbering now creates compact v2 roots/parts/drawings (`00001`, `00001-P01`, `00001-M01`, `00001-R01`), normal drawing purpose creation is `M/R`, v1 `MA/OT` remains readable through semantic helpers, approval-rule defaults are available for v2, downstream manufacturing checks use `MA/M` semantics, import examples and migration dry-run reporting were updated, Supabase runtime migration 004 was added and registered, and master identity tables now keep mobile card stacking for the updated status/context column. Verification passed: `npx.cmd tsc --noEmit --pretty false`, `npm.cmd run lint -- --quiet`, `npm.cmd run qc:pdm-numbering-v2-compact-identity` 13/13, `npm.cmd run qc:pdm-numbering-v2-migration-dry-run`, `npm.cmd run qc:pdm-numbering-core` 241/241, API regression 27/27, data consistency 16/16, concurrency reuse 32/32, draft lifecycle 29/29, request UI 66/66, search UI 28/28, impact UI 24/24, DVT UI 24/24, `qc:master-attachments` 101/101, `qc:pdm-master-workbench-layout` 224/224 and `qc:supabase-runtime-migrations` 25/25. `npm.cmd run build` was blocked by the intentional local-dev guard because an existing Next server owned `http://127.0.0.1:3000` with PID 30948; no bypass, production deploy, production migration, v1-to-v2 data rewrite, direct DB repair/deletion, project/order/equipment numbering or extra visible category code work was performed.
