...
> **2026-09-16 DEV-117 production incident recovery（current）**：R78 data cutover與artifact provenance仍有效；正式圖號／料號workbench因service template缺`PDM_BUILD_COMMIT`而以authority mismatch回503。已完成on-demand backup `1789532631908=SUCCESSFUL`，current recovery採R78 exact image＋零流量candidate＋signed contract-token authority readback，固定不重匯、不改DB。`production-activation`為獨立人工gate；目前只核准推進至candidate驗證完成，traffic GO待另一個明確決策。
