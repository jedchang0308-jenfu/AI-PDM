...
> **2026-09-26 DEV-121 最新進度**：AI-PDM PR #74 已合併至受保護 main `f0ac74902b1053a33e19cbd6629b3d6c742c5c53`，修正後 immutable inventory image `sha256:affb1f4fa335737ad630a38a87925b2963e38830974c8f0a46c1f8f7cf76d33f` 經 Cloud Build／Artifact Registry readback。Production coverage R1 證明 4 個 active profile 起始均無 marker；Jed 的唯一已發布 exact Firebase pair 經 preview、source hash／row version 0 綁定後，受控註冊為 `legacy_compatible:1`。相同 operation replay receipt bytes 不變，coverage R2 證明只有這一筆 marker 改變，另 3 個 profile 與全部來源列未變；暫時 Job 已刪除。此切片沒有 role／ACL／session／authority 或 service／traffic 變更，不是 principal cutover 或 L4。migration-only R2 已套用 065／066／067，OrgMaster v4 consumer readback 已 PASS。PR #76 為尚未合併的 profile-transfer draft：本機切片已分開驗舊本地 UID 與新 principal pair，唯讀 v2 preview 會讀回確認收據，隔離 PostgreSQL 21／21、preview runner 5／5、operator package 2／2 PASS；preview 尚未形成 Production apply 放行與回復證據。剩餘 profile 的舊 Firebase UID 未必等於新 managed Google 登入 pair，逐人確認前不得假造 alias 或切換權限。朱宇鴻與張祐豪待確認 Employee／個人主帳號及角色；蕭旋吟的共用 `info@` profile 暫不設定，待其個人帳號。證據及承接契約見 [DEV-121](specs/DEV-121-target-authorization-boundary.md)。
