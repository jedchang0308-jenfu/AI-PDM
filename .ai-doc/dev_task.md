...
- ◇ DEV-032 [關卡] [Authenticated Verification In Progress] [P0] [Release Gate Required] ERP 平台 production release work package
  ...
  - 2026-08-28本分支pre-deploy checkpoint：release source boundary維持0 included dirty source／0 unknown-risk，判定依`.ai-doc/reports/pm/pm-dev-032-source-boundary-classification-2026-07-15.md`與`output/dev-032-release-source/manifest.json`；production migration package已增為49筆ordered migrations（001至051），其中050維持退役standalone manufacturing-impact功能，未復原退役功能或其驗證。相同exact HEAD的本機Linux/amd64 app與migration-runner映像已完成OCI revision檢查；app登入頁、auth mode與OCR public資產煙霧均PASS，migration runner只以dry-run確認`connectionAttempted=false`。production target preflight仍因gcloud token需互動式重新驗證而fail closed，未執行push、production migration、deploy、traffic或正式資料操作。
