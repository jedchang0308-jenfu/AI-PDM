...
Active objective: execute `DEV-SW-LICENSE-PDM-001` backend company-scope slice after user approval.

`DEV-SUPABASE-DB-001` staging GATE-B remains passed and production/cutover remains deferred. Current approved implementation work is limited to the SW license / PDM company separation backend slice.
...
| In Progress | `DEV-SW-LICENSE-PDM-001` | Delivery point | None | Backend company-scope plus Web/Add-in company selection, direct read-path company permissions, core numbering company scope, company-aware CAD metadata adapter profile resolver, and numbering workflow/detail/task company scope implemented: company schema, membership auth payload, upload/Add-in payload, submission/search/metadata/preflight company context, detail/download/release package/BOM/workbench/action read checks, core numbering create/search/detail/update/obsolete/drawing/part-list company context, approval/import/export/monthly-audit/DVT/impact/part-detail/task/notification company context, company-scoped item/submission/numbering uniqueness, workflow/task/notification `company_id`, company-specific Document Manager/equivalent extractor env profile selection, redacted adapter profile response, and QC scripts | Commit boundary remains open because the worktree contains broad pre-existing unrelated changes and the index already has Supabase GATE-B staged files; do not commit until PM confirms index cleanup or grouping | SPEC-SW-LICENSE-PDM-001 / ADR-SW-LICENSE-PDM-001 / PM plan / Git boundary handoff / `qc:sw-license-pdm-company-scope` / `qc:sw-license-pdm-numbering-company-scope` / `qc:sw-license-pdm-metadata-adapter-profile` / `qc:sw-license-pdm-git-boundary` / `qc:sw-addin-company-selection` / `qc:native-cad-extractor-contract` |
...
| [ ] | `DEV-SW-LICENSE-PDM-001` | SW license / PDM company operational shared strategy | Backend company-scope, Web/Add-in company selection, direct detail/download/release package/BOM/workbench/action read checks, core numbering create/search/detail/update/obsolete/drawing/part-list company scope, numbering approval/import/export/monthly-audit/DVT/impact/part-detail/task/notification company scope, and company-aware metadata adapter profile resolver implemented and verified; commit boundary remains open due broad dirty worktree plus existing unrelated Supabase staged files |
