...
- ! DEV-032 [關卡] [Blocked / Release Gate In Progress] [P0] [Release Gate Required] ERP 平台 production release work package
  ...
  - 任務清單：
    ...
    - [ ] `Gate D - Immutable Deploy & Smoke`：exact application image index `sha256:b4fb8e9f...`已部署，Cloud Run執行的linux/amd64 child manifest `sha256:570dd9f0...`父子關係已驗證；production AAL1 pilot契約已對齊，Cloud Run v2 traffic-only rollback/restore與後續Terraform no-drift通過。rollback runner已固定`updateMask=traffic`、禁止會產生template drift的`gcloud run services update-traffic`路徑，並以current latest revision完成production `validateOnly` preflight。Cloudflare DNS、managed TLS ACTIVE、Level 3 HTTP與Level 4 post-deploy smoke仍待人類DNS/首次登入後執行；Phase 3A file paths保持fail-closed。
  ...
  - 下一步：人類只需在Cloudflare新增DNS-only `A pdm -> 136.69.102.146`，等待`https://pdm.jenfu.com.tw/login`可正常顯示後，以`jedchang0308@jenfu.com.tw`完成一次Google登入並回報結果；Codex再讀取verified Firebase UID，完成principal bootstrap、獨立restore/reconciliation、Level 3/4 smoke及production UI acceptance自動化。其餘1-3位canary不得由AI自行臆測。
