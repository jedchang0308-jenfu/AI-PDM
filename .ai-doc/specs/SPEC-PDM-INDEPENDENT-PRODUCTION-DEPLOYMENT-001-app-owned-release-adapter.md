...
2026-09-10 cross-OS source identity correction：AI-PDM `sourceSha256`只接受clean official revision的未壓縮`git archive --format=tar --prefix=source/` bytes SHA；owner build先重算驗章，再gzip上傳Cloud Build source object，壓縮物件GCS SHA獨立記錄。禁止以跨OS `tar.gz` bytes相等作source identity；任一tar drift或gunzip不等在Cloud Build前FAIL。
