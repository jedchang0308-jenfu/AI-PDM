...
> **2026-09-18 DEV-013 Production L4 authorization schema amendment（current）**：跨應用L4 human authorization只綁target environment／resources、allowed／forbidden actions、risk scope與expiry；`jenfu.dev013.l4-execution-authorization.v2`及`sequence-root.v2`不得包含source revision。AI-PDM owner source SHA、artifact digest、migration manifest、candidate與deployed revision仍由source freeze、owner release capsule與receipts精確綁定。來源改變會使受影響的build、test、migration、candidate與provenance evidence失效並重跑，但不得僅因SHA改變要求新人類授權。正式migration、live-data、IAM、traffic activation、rollback、canonical smoke、scope／target改變及破壞性操作的既有gate全部保留；歷史v1 receipts不改寫且不能作v2 root。Local QC見[`qc-dev-117-dev013-production-authorization-policy-2026-09-18.md`](../qc/qc-dev-117-dev013-production-authorization-policy-2026-09-18.md)。
...
> **2026-09-11 R38 pre-auth execution authority（historical v1 event）**：R35 exact production migration execution `ai-pdm-prod-migration-runner-7gjhv`完成`14 replayed`；此forward-only provider事實不因owner後續readback缺口而回寫為NOT_RUN。當時以Cloud Run v2 `conditions[type=Completed]`驗terminal，並要求own exact migration Job resource-scoped viewer。DEV-117 V1 historical adapter與V3 direct-run測試已拆分，legacy 12／12、V3 28／28、abort 6／6及release adapter QC均PASS。R37因source drift作廢且沒有AI-PDM app apply；當時operator取得新授權後只接受fresh R38。這是歷史事實，不是現行SHA-bound human authorization規則；現行政策依上方2026-09-18 amendment。Candidate／entrypoint／traffic當時仍NOT_RUN，current serving不使用custom domain、Hosting、shared LB或TOTP。
...
DEV-121 principal-first 發布新增同一 owner 的 `deploy-ai-pdm-principal-migrations-production.yml`：它只接受 `releaseCapsuleRef`，共用 `production-release-ai-pdm-prod` concurrency，依序重用既有 executor 的 `prepare／build／migrate`，成功後結束；不提供 actor stage override，也不執行 candidate、service 或 traffic mutation。既有完整 owner workflow 以同一未過期 capsule 重播前置 receipts 後續行。兩個 workflow 的 WIF 限定在同一 numeric repository／owner、main、production 與 `workflow_dispatch`；新增 exact workflow ref 僅能由 `APP_INFRA_AIPDM_PRINCIPAL_MIGRATION_WIF` 的完整 Terraform plan 執行，且唯一 update 為既有 provider 的 `attribute_condition`，before／after 除該欄外完全相同，apply 後 readback 必須精確相符。migration-only 成功不是服務已切流或整體 release 已完成的證據。
...
Historical R38 execution已留下R35 production migration replay PASS；當時operator取得新授權後，由fresh R38重建remote source freeze、Billing／budget／quota／capacity、Identity／entry baseline、notification／numeric Secret、foundation及AI-PDM APP_INFRA_A/B provider receipts，再以idempotent migration readback續接candidate、entrypoint、DEV-116 R02、traffic與canonical smoke。此處只記錄歷史執行；現行v2在source/profile drift時重建受影響的machine evidence，不因source SHA本身要求新的人類授權。任一UNKNOWN、`remainingHumanAction>0`、human scope／target／risk／expiry改變或需要改上述架構契約仍停止。
...
2026-09-10 shared-foundation handoff correction：shared foundation維持Platform單一apply authority，但Platform native readiness必將相同canonical foundation receipt鏡像到AI-PDM own release bucket，供owner intent與prepare讀取。Receipt原樣保留`ownerApplicationId=shared-foundation`與Platform `sourceRevision`；`foundation`是唯一豁免AI-PDM owner／source equality的prerequisite，AI-PDM source lock、infra及runtime不得豁免，且app-infra必hard-join AI-PDM owner／source與同一foundation manifest。缺mirror、bytes/provenance drift或豁免擴張均在provider write前FAIL。
...
唯一可執行入口是app-owned provider CLI；`run`固定依序執行 `prepare → fence → export → import → reconcile → handoff`，失敗時先清task resources，再依保存的etag與baseline恢復legacy access並發布terminal abort。Export只由task-owned Job以repeatable-read／read-only將151張table直接寫入own release bucket immutable generation；Import只由neutral task-owned Job以serializable transaction、advisory lock、FK＋受控trigger dependency topological order及self-FK convergence寫入 `ai_pdm_core`。任何bundle缺少、增加或漂移固定trigger dependency set均在DB write前`DATA_CUTOVER_IMPORT_PLAN_MISMATCH`。Raw row不得落本機、stdout、receipt、Git或non-production。
