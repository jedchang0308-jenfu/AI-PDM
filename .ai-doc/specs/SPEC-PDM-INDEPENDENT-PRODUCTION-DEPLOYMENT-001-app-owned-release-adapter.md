...
2026-09-10 shared-foundation handoff correction：shared foundation維持Platform單一apply authority，但Platform native readiness必將相同canonical foundation receipt鏡像到AI-PDM own release bucket，供owner intent與prepare讀取。Receipt保留Platform `sourceRevision`；`foundation`是唯一豁免AI-PDM source equality的prerequisite，AI-PDM source lock、infra及runtime不得豁免，且app-infra必hard-join AI-PDM source與同一foundation manifest。缺mirror、bytes drift或豁免擴張均在provider write前FAIL。
