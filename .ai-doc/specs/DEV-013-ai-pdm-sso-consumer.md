...
- Status: `013-S4-L3-AIPDM-ENV complete / production owner profile guard off / L4 production-scope authorization gated`
...
The ordinary production owner profile `config/release/dev117-ai-pdm-independent-production-v3.json` now defaults `PDM_JENFU_SSO_HANDOFF_MODE=off`, permits only the controlled values `off／on`, and fixes the Platform broker origin to `https://jenfu-platform-prod-9536592944.asia-east1.run.app`. The owner runtime can add these fields safely to the prior production environment and rejects an unknown transition, missing legacy value, fixed-value drift, or alternate origin before a release capsule can proceed. The `dev013-transition-authority` prerequisite stage reads the active Cloud Run revision and its current controlled environment, then reads and hash-verifies either a live `jenfu.dev013.l4-execution-authorization.v2 / HUMAN_PRODUCTION_SCOPE` root or a released owner terminal. The human root binds the exact production target, allowed and forbidden actions, risk scope and expiry, and deliberately carries no source revision set. The derived owner authorization／readiness receipts bind the exact owner source revision, release ID, controlled environment, prior revision value and verified predecessor. A source change invalidates those machine-verifiable owner receipts and requires the affected build, test, migration, candidate and provenance gates to run again; it does not by itself require renewed human authorization. Fresh DEV-117 owner tests, abort-controller tests, database-boundary check, application typecheck, and isolated Next.js build are required after this change.
