...
5. Part attachments are a deliberate DEV-084 exception: they remain independently and immediately mutable, are excluded from the Part approved snapshot and active-review lock, and do not roll back with Part work. The reviewer sees the current live attachment list with the adjacent review-only note `附件獨立維護，不屬於本次資料核准`. This note does not appear in a workbench list, normal drawer or general filter.
