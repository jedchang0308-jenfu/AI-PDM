...
將未來「一張圖對應多個料號，且不同料號成本不同」的產品資料模型整理成可開發規格。此規格延伸既有圖號與料號自動化模組，讓系統同時具備圖號工作台與料號工作台，兩者透過相同主根號自動關聯，但各自保留不同責任邊界。
...
| 模組期望 | 系統同時有圖號及料號工作台，彼此依相同主根號自動關聯。 |
...
當圖號與料號具有相同 `part_root_id` 時，系統自動建立或提示建立關聯，讓使用者在圖號工作台與料號工作台都能看到彼此。
...
| 角色 | 能力 |
|---|---|
| RD / 工程師 | 可建立圖號、料號、變體說明；可查看成本狀態但不可查看成本金額或核准成本。 |
| 採購 | 可建立與修改成本草稿、送出成本變更；可查看成本金額。 |
| 主管 | 可審核成本 profile 與標準成本指定；可查看成本金額。 |
| PDM 管理員 | 可維護資料字典、修正關聯、處理例外與 audit；可查看成本金額。 |
| 製造 / 品保 | 可查看料號、圖號、標準成本狀態與生效版本；不可查看成本金額。 |
...
- [x] 料號流水號不把材質/顏色編碼進號碼，只保留在屬性欄位。
- [x] 標準成本第一版預設基準數量採 `1 pcs`，其他數量差異由成本級距處理。
- [x] 成本金額限制為 `Admin`、`R&D Manager`、`Procurement` 與 ACL `system_admin`、`pdm_admin`、`rd_manager`、`procurement` 可見；一般 RD、製造、品保與供應商只看狀態。
- [x] 材質與顏色第一版先用自由欄位，保留 nullable `material_code` / `color_code` 供後續建立字典。
