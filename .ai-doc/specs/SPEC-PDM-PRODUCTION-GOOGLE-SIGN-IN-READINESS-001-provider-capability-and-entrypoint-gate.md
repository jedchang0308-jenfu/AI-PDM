...
Platform machine plan `config/dev-014/login-production-fixture-plan.json`固定四個disposable identities、八個identity cells、四個PDM target cells及七類deny fixture，預設`executionAuthorized=false`。`jenfu.dev014.login-production-completion.v1` finalizer另要求六個LOGIN cases、rate／race／logout、exact release／admission與完整cleanup；只有其self-hashed receipt通過，才把DEV-118從Browser Verification Pending改為Production Complete。
