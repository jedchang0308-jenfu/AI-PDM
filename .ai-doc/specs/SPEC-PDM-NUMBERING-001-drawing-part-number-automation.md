...
> 2026-08-11 需求決策：既有「圖號總表匯入」功能已從現行系統撤除。以下歷史條款、`import_batches` / `import_staging_rows` 資料模型與 import-batches API 均不再是現行產品契約；總表僅保留匯出能力。

> 2026-08-24 DEV-093 item-kind consolidation：本 Draft 文件的舊料件分類僅保留作歷史追溯；現行產品只允許`依圖製作件`／`外購標準件`，`共用件`改為獨立勾選，委外件與自訂不再是新資料分類。依圖製作件包含廠內與委外依圖加工。
...
| override | 例外放行或例外規則，例如 DVT 依圖製作件無 MA 圖仍轉有效。 |
...
### 5.3.1 2026-06-24 修訂：預留草稿號例外

上述不可重用規則適用於受控圖號與受控料號。`DEV-PDM-CHANGE-CONTROL-001` 新增「預留草稿號」例外，用於尚未進入 PDM 受控關聯的新料號草稿。
...
任一前提不成立時，該號碼即視為受控料號，仍適用不可重用規則。

此修訂由 `ADR-PDM-CHANGE-CONTROL-001` 管制。實作時應將可回收草稿號放在獨立 `part_number_drafts` 或等效草稿模型中，不得讓正式 `part_numbers` 同時承擔可回收與受控兩種語意。
...
本節原規劃的 staging 匯入、檢查報告與管理員確認流程已於 2026-08-11 需求變更中撤除，不得再實作或恢復相關 UI、API、權限或資料表。
