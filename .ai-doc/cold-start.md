...
- `DEV-118`為目前P0待排開發點，狀態=`RD Contract Ready / Production Provider Change and Release Gated`。
  Production canonical host已在Firebase authorized domains，但Google provider authorization start回
  `OPERATION_NOT_ALLOWED: The identity provider configuration is not found.`；current UI capability與release smoke
  都未覆蓋此失效。先讀`.ai-doc/specs/SPEC-PDM-PRODUCTION-GOOGLE-SIGN-IN-READINESS-001-provider-capability-and-entrypoint-gate.md`；
  下一步由shared identity／Platform owner確認provider mutation與回復責任，再升級為`RD Implementation Ready`。
- `DEV-117`已由R78完成正式release：owner terminal=`RELEASED`、revision=`ai-pdm-prod-29a4a765563c`、
  canonical traffic=100%、remainingHumanAction=0。其app-owned release lane是DEV-118的release authority，
  不需重開DEV-117歷史S1／S2工作。
- 目前沒有可直接正式部署的任務；production、Cloud SQL/GCS cutover、provider pointer、migration、release、rollback 或 production smoke 必須走 release gate。AI_PDM candidate／traffic／receipt由DEV-117擁有，DEV-116只擁有`company-smoke` Level 4 evidence，Platform DEV-010只供應shared foundation evidence。
