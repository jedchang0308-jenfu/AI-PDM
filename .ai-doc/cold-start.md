...
- `DEV-087`目前是`RD Implementation Ready (RD Supervisor Reviewed) / Human Confirmed / RD Not Started`；本次只完成文件，尚未授權local產品/schema實作或任何資料遷移。Cloud SQL migration固定`042`（041屬DEV-084），SQLite使用`ensureDev087CanonicalWorkbenchSchema`；exact schema/DTO/API/module/query budget/converter與`QA-087-001..120`已封口。同一圖號最多3個open RD branches；Drawing／Part／Relation採專用work tables。DEV-087 review使用transient request adapter，不寫既有`approval_platform_decisions`；永久只留審核cycle/entity/time。延續same-company non-owner edit scope；latest approved idle RD可受審核作廢並關閉branch。與過去矛盾時以DEV-087新決策為主，安全可拆的舊current-state/filter/projection/command/fallback必須同DEV移除，舊QA只作activation前歷史證據。physical bytes僅在零引用、approved-artifact與canonical-only gate後永久刪除且無restore；DB/schema/binding rollback仍保留。transition只允許`legacy_only → shadow_compare（隔離）→ cutover_window → canonical_only`，production不得dual authority／dual write或fallback。
