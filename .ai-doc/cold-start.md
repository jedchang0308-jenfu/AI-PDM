...
> **2026-09-22 DEV-118 zero-paid-seat handoff（現行）**：Platform DEV-014 已固定 v2 plan；不新增付費席次、不改 `employee-shijie`／`jedchang0308@jenfu.com.tw`，只建立兩個 Cloud Identity Free-only／無 Gmail disposable principals 及其 marked fixture state。Workspace pending 兩格改由 current Workspace principal 的 Google／工號直接證據輸出 equivalence receipts，Free pending／active、PDM Free 四格中的兩格、deny／rate／race／logout與cleanup仍須直接執行。只以 `jenfu.dev014.login-production-completion.v2` PASS 將 DEV-118 結案。
...
- `DEV-118 / 118-A` 本地實作與focused QA/QC完成；118-B跨專案工程契約、migration、DWD與admission完成；118-C Production owner release與Firebase Google provider啟用完成；Workspace Platform→AI-PDM免二次登入、reload與管理權限已取得partial evidence。現行狀態=`Production Released / Provider Enabled / Workspace Partial Browser Evidence / Full Browser Validation Pending`。
  ...
  [Platform 014-LOGIN](../../Jenfu-Platform/ai-doc/specs/DEV-014-managed-identity-production-activation.md#dev014-login) 與 [OrgMaster DEV-049](../../OrgMaster/ai-doc/specs/DEV-049-existing-google-primary-account-link.md) 是來源契約。§18.5已定案G1～G4；固定staging owner mutation、三份owner receipt與runtime readback已完成至`OWNER_READY_FOR_L3_BROWSER`，可在輸入未漂移時重用。缺token-free fixture manifest只使選配staging runner維持BLOCKED，不再阻塞開發或release entry。重用bridge／Firebase exchange／SSO，不回pre-auth hint；R2基線QA 0／20、LOGIN full 0／6與PDM C01／C02四格改由Protected Release在production canonical完成。
...
- `DEV-117` R78 已完成；DEV-118 不重開 R78、不修改 smoke 憑證與 release 契約。
  118-C 依既有DEV-117 app-owned workflow執行Protected Release；不新增DEV-118專屬staging、重複build、額外Release Capsule或第二次批准。發布後production canonical須完成C01／C02與拒絕路徑，AI-PDM不取得shared identity write authority。
- `DEV-010／DEV-116` current production topology採neutral `db-custom-1-3840 / ZONAL_DEDICATED`，alerts-only budget為`USD 100／TWD 3,200`，正常月費估計`USD 65～80`。Backup、PITR、deletion protection、private IAM、schema／role與`company-smoke`隔離不變；只有REGIONAL standby／automatic cross-zone failover延後，故不得宣稱HA。AI-PDM legacy `db-f1-micro／USD 300`是待退役單app target的歷史／回復設定，不是DEV-010 current authority；不得原地改造或拿來通過R1。Platform R1T已完成DB-authority source write-fence source與6/6 QC，但actual fence／final delta仍NOT_RUN；它不替代Production Level 4，後者仍須同candidate真實COMMIT＋reload、Jenfu不變、DEV-010十五案全PASS與獨立traffic GO。
