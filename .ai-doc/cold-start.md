...
- `DEV-010／DEV-116` current production topology採neutral `db-custom-1-3840 / ZONAL_DEDICATED`，alerts-only budget為`USD 100／TWD 3,200`，正常月費估計`USD 65～80`。Backup、PITR、deletion protection、private IAM、schema／role與`company-smoke`隔離不變；只有REGIONAL standby／automatic cross-zone failover延後，故不得宣稱HA。AI-PDM legacy `db-f1-micro／USD 300`是待退役單app target的歷史／回復設定，不是DEV-010 current authority；不得原地改造或拿來通過R1。Production Level 4仍須同candidate真實COMMIT＋reload、Jenfu不變、DEV-010十五案全PASS與獨立traffic GO。
