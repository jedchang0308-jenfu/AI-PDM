...
2026-09-05 owner evidence waiver為`Intentional replacement`：使用者明確豁免DEV-010 budget／restore／incident／maintenance四份獨立owner證據。Platform已改以`CONSOLIDATED_USER_WAIVER` self-hashed receipt保存單一accountable authority與四項責任範圍，並拒絕混入虛構owner email；RTO=`240分鐘`、RPO=`15分鐘`、maintenance=`60分鐘`固定不豁免。Fresh focused tests為operations `8／8`、R1-02 `8／8`、R1-15 `7／7`、release `19／19`、foundation `4／4 PASS`；current DEV-010 preflight由9項降為2項，只剩neutral target identity與R1-11 capacity。由於provider憑證目前要求重新登入，target仍未驗章，AI-PDM R02／R03仍`NOT_RUN`。本變更不改`company-smoke`真實COMMIT＋reload、`company-jenfu` before=after、zero leak、side effects disabled或獨立traffic GO。
