...
後續證據不得再以自由格式段落結案。Platform受版控fixture plan v2採零新增付費席次：`employee-shijie`／`jedchang0308@jenfu.com.tw` 只讀重用 `PDM-W-G`／`PDM-W-E` 及其 Workspace direct evidence；只新增 `dev014-fp-google@jenfu.com.tw`、`dev014-fp-number@jenfu.com.tw` 兩個 Cloud Identity Free-only／無 Gmail principals。`W-P-G-EQV`／`W-P-E-EQV` 由same-principal direct evidence產生等價性 proof；Free pending／active與`PDM-F-G`／`PDM-F-E`直接執行。共用`jenfu.dev014.login-production-completion.v2` finalizer要求PDM四格、LOGIN六案、identity equivalence、deny／rate／race／logout、exact current release／admission與cleanup全部PASS。此checkpoint保留為Workspace current partial evidence，最終只引用通過驗章的token-free aggregate receipt。
