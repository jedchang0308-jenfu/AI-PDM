...
| DEV-067 PDM 統一實體明細投影、審核全景與送審鎖定 | `.ai-doc/dev_task.md` (`DEV-067`); implementation authority `.ai-doc/specs/SPEC-PDM-ENTITY-DETAIL-DRAWER-001-unified-object-detail-contract.md` (`DEV-067 RD Implementation Contract`); architecture `.ai-doc/decisions/ADR-PDM-UNIFIED-ENTITY-DETAIL-PROJECTIONS-001-composer-and-policy.md`; QA `.ai-doc/qa/qa-dev-067-unified-pdm-entity-detail-validation-plan-2026-08-12.md`; data authority `.ai-doc/specs/SPEC-PDM-UNIFIED-DRAWING-AGGREGATE-001-single-data-layer.md`; relation/core/review/lock amendments `.ai-doc/specs/SPEC-PDM-DRAWING-PART-RELATION-VIEW-001-root-drawing-part-relation-list.md`, `.ai-doc/specs/SPEC-PDM-WORKBENCH-CORE-001-shared-read-and-controller-contract.md`, `.ai-doc/specs/SPEC-PDM-APPROVAL-PLATFORM-001-system-approval-platform.md`, `.ai-doc/specs/SPEC-PDM-NUMBER-STATE-FLOW-001-unified-numbering-draft-and-transfer-functional-spec.md`. Status: `RD Implementation Ready / Human Confirmed / RD Not Started / Local Implementation Eligible / Production Release Gated`。三工作台使用同一composer與domain-owned projections；Drawing/Part一般surface依任務刪減、Relation為full aggregate、assigned reviewer僅在exact request/company scope內取得full aggregate與review context。Exact envelope/API/read snapshot/review receipt/owner resolver/transaction lock/preview/return/query budget/Phase 1A～1D與`UDD-001..050`均已固定；本機可派工，不授權schema、production/staging、merge/deploy/release。 |
| DEV-069 AI-PDM 預上線 GCP 降本與低成本 Staging | `.ai-doc/dev_task.md` (`DEV-069`); authority `.ai-doc/specs/SPEC-PDM-GCP-COST-OPTIMIZATION-001-prelaunch-runtime-topology.md`; decision `.ai-doc/decisions/ADR-PDM-GCP-COST-OPTIMIZATION-001-prelaunch-firebase-hosting-zonal-micro.md`; QA `.ai-doc/qa/qa-dev-069-gcp-cost-optimization-validation-plan-2026-08-12.md`; QC/live evidence `.ai-doc/qc/qc-dev-069-gcp-cost-optimization-report-2026-08-12.md`; machine contract `config/platform/dev-069-gcp-cost-optimization.json`。Status: `Live Implementation Complete / Billing Measurement Pending 24-72h`。Production=`db-f1-micro/ZONAL/RUNNABLE`，Staging=`db-f1-micro/ZONAL/STOPPED`；兩套 ALB 與 Restore 已移除，private network、backup/PITR、deletion protection、Firebase Hosting 與 migration runner 保留；targeted Terraform final plans 均 exit 0。 |
...
| DEV-070 privacy notice runtime retirement decision and validation | `.ai-doc/decisions/ADR-PDM-PRIVACY-NOTICE-002-retire-runtime-acknowledgement.md`; `.ai-doc/qa/qa-dev-070-privacy-notice-runtime-removal-validation-plan-2026-08-14.md`; `.ai-doc/qc/qc-dev-070-privacy-notice-runtime-removal-2026-08-14.md`. Status: `Production Deployed / Authenticated QC Pending`. |
...
- Employee privacy notice runtime: superseded on 2026-08-14 by `DEV-070` and `ADR-PDM-PRIVACY-NOTICE-002`. `/privacy*`, `/api/privacy*`, acknowledgement/session/protected-API gates, Admin evidence and machine-readable runtime contract are removed. The Pilot v1.0 SPEC and existing database schema/migrations remain historical evidence only; no formal database rows are deleted or rewritten.
...
- `DEV-PDM-UNIFIED-ENTITY-DETAIL-REVIEW-001` / `DEV-067`: `☐ RD Implementation Ready / Human Confirmed / RD not started`。現況只有drawer shell/content skeleton部分共用；Drawing candidate/formal preview與sections、Part candidate/formal body、Relation root/target composition仍分叉，`/approvals`也仍保留snapshot-derived body。下一交付依Phase 1A～1D以`UnifiedPdmEntityDetailDrawer + ProjectionComposer`收斂三域，domain保留projection/data/command authority；一般Drawing/Part依任務刪減，Relation與assigned active review使用server-authorized full aggregate。Read `.ai-doc/dev_task.md` (`DEV-067`)、`.ai-doc/specs/SPEC-PDM-ENTITY-DETAIL-DRAWER-001-unified-object-detail-contract.md`、`.ai-doc/decisions/ADR-PDM-UNIFIED-ENTITY-DETAIL-PROJECTIONS-001-composer-and-policy.md`、`.ai-doc/qa/qa-dev-067-unified-pdm-entity-detail-validation-plan-2026-08-12.md`及相關data/review/core amendments。本機implementation可直接開始；schema、production/staging、merge/PR/deploy/release仍gated。
...
- `DEV-PDM-ENTITY-DETAIL-DRAWER-001`: Phase 1C shared shell/content skeleton is implemented locally. `DEV-059` proves focused current-route modal recovery, but `DEV-067` reopens the broader single-visible-module claim because Drawing/Part/Relation state and target compositions remain behaviorally separate. The human-confirmed next contract is one `UnifiedPdmEntityDetailDrawer` composer with domain-owned projections and server visibility policy; implementation has not started. Parent release remains gated; read `.ai-doc/specs/SPEC-PDM-ENTITY-DETAIL-DRAWER-001-unified-object-detail-contract.md`, `.ai-doc/decisions/ADR-PDM-UNIFIED-ENTITY-DETAIL-PROJECTIONS-001-composer-and-policy.md` and `.ai-doc/qa/DEV-059-real-operation-evidence-2026-08-09.md`. Production deploy, merge, PR, rollback and release artifacts remain unauthorized.
...
Status: Phase 1A shared shell/core sections implemented locally; `DEV-067` cross-domain projection amendment is `RD Implementation Ready / Human Confirmed / RD not started / Local implementation eligible`; release not authorized. The three task entry pages remain separate, but the target architecture is one `UnifiedPdmEntityDetailDrawer` composer with domain-owned projections. Object identity, data truth and permission never vary by source; visible projection depth intentionally varies by server policy: Drawing/Part owner surfaces are task-focused reductions, Relation is the full aggregate, and assigned active review is full only inside exact request/company scope.
...
- DEV-067 composer/projection/server-policy implementation is `RD Implementation Ready / Local implementation eligible`; execute the same DEV Phase 1A～1D and QA `UDD-001..050`, with release still unauthorized.
