...
> **2026-09-23 DEV-121 目標端授權邊界（RD implementation in progress）**：native `AIPDM/DEV-121#target-authorization` 承接主責 `JENFU/DEV-015`，producer 為 `ORGMASTER/DEV-057#identity-grants`。[DEV-121 contract](specs/DEV-121-target-authorization-boundary.md)定案 handoff 版號語意、短暫唯讀 repeatable-read、共用 transaction decision time、verified actor、session company workspace mapping、唯一 active role priority、fail-closed mode 與 typed route discriminator。AI-PDM focused `7 files / 42 tests`、typecheck、DB boundary、DEV-005 route／contract／runtime QC與D121-PG-01／02隔離race PASS；全 API 255 files／292 methods分類完成且直接role gate=0。已建立 `config/access-control/jenfu-route-policy-dispositions.v1.json`，25筆字面代碼全部唯一並採 `deny_or_retire / deny / 403`；route-policy QC PASS，保留 OrgMaster authority fail-closed，沒有新增猜測式 grant。既有 DEV-005 catalog source discovery 已修正為優先 canonical `Jenfu-Platform`，`check:jms-dev-005-role-catalog`與contract QC均PASS。Platform typecheck PASS；OrgMaster producer migration 021及task-owned PostgreSQL D57-01～06亦PASS。Local normal-entry browser、v1/v2 recovery與Production L4仍待完成；Production尚未執行。不修改DEV-118 checkpoint或C01／C02分母。
...
| DEV-095 BOM 模組硬刪除 | `.ai-doc/dev_task.md` (`DEV-095`)；ADR `.ai-doc/decisions/ADR-PDM-BOM-RETIREMENT-001-hard-delete-and-rebuild-boundary.md`；QA `.ai-doc/qa/qa-dev-095-bom-retirement-validation-plan-2026-08-24.md`；QC `.ai-doc/qc/qc-dev-095-bom-retirement-2026-08-24.md`；SQLite runner `scripts/migrate-dev-095-bom-retirement.mjs`。Status: `Closed / Local Hard Retirement Complete / Focused QC PASS / Typecheck PASS / Isolated Build PASS / Production Retirement Out of Scope`。現行產品不提供BOM、Where-used、結構分類或舊BOM審核契約；原正式 Cloud SQL deletion／047 migration 方案保留為歷史追溯，不是本 DEV current scope。 |
