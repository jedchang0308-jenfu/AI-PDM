...
> **2026-09-07 DEV-117 independent deployment authority（現行）**：使用者已確認AI_PDM與Jenfu-Platform分開部署。AI_PDM current app-owned release authority為[DEV-117 SPEC](specs/SPEC-PDM-INDEPENDENT-PRODUCTION-DEPLOYMENT-001-app-owned-release-adapter.md)與[QA-DEV-117](qa/qa-dev-117-ai-pdm-independent-production-deployment-validation-plan-2026-09-07.md)：target固定`jenfu-platform-prod / asia-east1 / ai-pdm-prod`、runtime=`aipdm-prod-runtime@jenfu-platform-prod.iam.gserviceaccount.com`、database=`jenfu-platform-prod-pg / jenfu_prod`、canonical=`https://pdm.jenfu.com.tw`。現行legacy workflow只屬`jenfu-ai-pdm-prod`，不得替代neutral lane。DEV-117=`117-S1 Local Implementation Complete / QA-QC 12 of 12 PASS / Provider NOT_RUN / Production Release Gated`；profile、純validator、preflight CLI、六階段AI-only workflow與receipt aggregate已完成，legacy pipeline 25/25、DB boundary、typecheck、isolated build／primary invariant／cleanup均PASS。Current dirty branch preflight正確為`INVALIDATED / BLOCKED`。下一步先收斂至clean `origin/main`並建立fresh `REL-117-*` source lock；production仍blocked於neutral billing、DEV-010 provider foundation／migration／capacity、DEV-116 R02、canonical evidence及獨立Product Owner GO。
