...
> **2026-09-05 Platform DEV-010 physical-topology successor（現行）**：AI-PDM `DEV-032`第一版production release維持已完成，但三系統共用資料庫的physical project／instance／database、schema placement與cross-app role契約改由Platform [DEV-010 direct spec](../../Jenfu-Management-system/ai-doc/specs/DEV-010-three-system-database-consolidation-contract.md) 接管。Current production target=`db-custom-1-3840 / ZONAL_DEDICATED / USD 100`；backup、PITR、deletion protection、private IAM與schema-role隔離不變，HA延後且不得宣稱automatic cross-zone failover。RD技術主管複審判定此成本決策未走回頭路；其後010-R1E已完成PostgreSQL verifier NOLOGIN group、三app owner-owned evidence views、exact IAM login binding與read-only executor source，fresh N1A=`11＋30 PASS`、R1E=`14＋4 PASS`、detached exact-HEAD N2=`48/48 PASS`，原P0 source blocker已關閉。R1-04 read-only verifier、三repo A immutable artifact producer及必要時F fail-closed first-revision producer source已完成；A=`6/6 PASS`，F=`8/8`、IaC合併=`15/15 PASS`，B digest-only neutral revision仍未實作。Production R1仍有既有9項human／target／capacity blocker，正式migration／binding、A／F provider execution、B、15案provider execution、deploy、DNS及authority／traffic switch只能由獨立release gate啟動；AI-PDM write smoke另要求Platform R1-04、R1-07與DEV-116 R01／R02引用同一AI-PDM source／digest／revision及exact company-smoke receipt。
