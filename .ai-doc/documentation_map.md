...
8. Current production warning: `DEV-032` is the only active production/release entry. It retains `DEV-030/031` as
   Gate B/C sources and now also owns the production remainder of Platform `DEV-005/009`, `DEV-053`, `DEV-069`
   and `DEV-109`; ownership consolidation does not waive any source acceptance or approval gate.

Current unfinished-owner index（2026-09-03）：`DEV-032`是唯一release owner；`DEV-115`是唯一可派工的
本機QA gate收斂開發點；`DEV-079`等待current retained matrix，`DEV-080`等待successor disposition與
current residual smoke後併入`DEV-087／112`；`DEV-041`、`DEV-015`、`DEV-047`與`DEV-033`為deferred。`DEV-037`已吸收進
`DEV-033`，不再獨立派工。Platform `DEV-005/009`、`DEV-053`、`DEV-109`的本機開發task已關閉，
production／effectiveness餘項只由`DEV-032`承接。
...
| Need | Read |
|---|---|
| DEV-115 DEV-079／080 current QA gate收斂 | `.ai-doc/dev_task.md` (`DEV-115 / DEV-PDM-QA-GATE-CONVERGENCE-001`)；DEV-079 current-matrix amendment=`.ai-doc/qa/qa-dev-079-drawing-readonly-drawer-fullpage-workspace-validation-plan-2026-08-19.md` §13；DEV-080 successor-disposition amendment=`.ai-doc/qa/qa-dev-080-system-status-visibility-validation-plan-2026-08-19.md` §16。Status: `RD Implementation Ready / Local QA Infrastructure Only / P1 / Medium`。DEV-115只修正case registry、fixture lifecycle、current browser oracle與隔離aggregate，不修改產品程式、API、schema、permission或正式資料；DEV-079保留current能力的獨立matrix，DEV-080不恢復retired `bomCreate`／XMind／舊status-help契約，current residual smoke通過後以`× 併入 DEV-087／112`關閉。 |
| DEV-114／CAPA-001 核准結果、正式資料語意與生命週期證據一致性 | `.ai-doc/dev_task.md` (`DEV-114 / DEV-PDM-APPROVAL-OUTCOME-TRUTH-001`)；唯一CAPA Register=`.ai-doc/capa-register.md#capa-001`；authority=`.ai-doc/specs/SPEC-PDM-APPROVAL-OUTCOME-TRUTH-CAPA-001-domain-outcome-and-lifecycle-evidence.md` §9～§14；QA=`.ai-doc/qa/qa-dev-114-approval-outcome-truth-validation-plan-2026-09-02.md`；QC=`.ai-doc/qc/qc-dev-114-approval-outcome-truth-2026-09-02.md`；evidence=`output/qa/capa-001-approval-outcome/`。Status: `Local Effectiveness Verified / Production Activation Gated / P1 / High`。共享`status/applyStatus` outcome projector與native apply postcondition已落地，兩個approval UI對`apply_failed`顯示「核准已保存，正式化未完成」並保留重試；Part `part_formal + Draft`顯示`主檔 · 草稿（未發行）`、`available`顯示`資料可見`。Focused QC 22/22、隔離fault 7/7、A0001 Part／approval inbox／generic workspace authenticated browser PASS；primary inventory唯讀、FK=0、exact UUID維持Draft與`blocked_pending_release_authority`。無schema／migration／production data修復；正式activation／deploy／release仍走DEV-032 gate。 |
| DEV-113 料號工作台唯讀抽屜、單一維護入口與113-E直接編輯 | `.ai-doc/dev_task.md` (`DEV-113 / DEV-PDM-PART-WORKBENCH-SINGLE-EDIT-ENTRY-001`)；authoritative contract=`.ai-doc/specs/SPEC-PDM-ENTITY-DETAIL-DRAWER-001-unified-object-detail-contract.md`的DEV-113-E amendment；relation compatibility=`.ai-doc/specs/SPEC-PDM-INLINE-RELATION-MATRIX-001-direct-formal-edit.md`的DEV-113-E immediate activation amendment；fixed QA=`.ai-doc/qa/qa-dev-113-part-workbench-single-edit-entry-validation-plan-2026-09-01.md`；current aggregate=`output/qa/dev-113/aggregate/report.json`（回復前一輪完整基線；後續重跑曾於G02 isolated build被範圍外既有TS2345 blocker擋住）；current browser evidence=`output/qa/dev-113/browser-real/DEV113-2026-09-02T06-05-22-456Z/`（compact-minimum與`編輯`文案，B01～B12=`12/12 PASS`）；可重現UI=`output/design/part-workbench-single-edit-entry-ui-v1.html`；interaction spec=`output/design/part-workbench-single-edit-entry-ui-v1-spec.md`。Status: `DEV-113-E Local RD Implemented / RD Tech Lead Corrections Closed / P0-P1 Planning Gap 0 / Human Confirmed / Full QA-QC Passed 28/28 prior baseline retained / Compact-minimum browser smoke 12/12 / Production Release Gated / P1 / Medium`。Part drawer、唯一workspace、root/source scope、Part immediate relation、Drawing explicit與same-key recovery均已落地；附件lazy-load request-generation guard已補足並行載入競態；維護預覽／結構型態／附件／關聯矩陣共用一致section shell、heading、最小內部間距與responsive，結構型態唯一動作顯示`編輯`，並保留主要按鈕觸控尺寸與keyboard/focus語意；共用frame loading不再顯示空白錯誤框，真正error／權限／衝突訊息仍保留；附件嵌入頁移除重複card shell；BOM單一零件empty state明示不適用並可安全回到maintenance調整結構型態，assembly-only建立條件維持。Current implementation以DEV-113既有boundary為基線，本次只增加Part workspace呈現層與B08 oracle相容增量，未新增API／schema／permission／writer；fixed denominator=`C01～C08+R01～R04+B01～B12+G01～G04=28`，最新B01～B12逐案manifest missing／duplicate=0、artifacts存在、`productionWrites=false`；正式PostgreSQL provider、production migration、deploy、activation與release仍受獨立gate管制。 |
| DEV-110 智慧辨識共用值、料號例外與下游工作銜接 | `.ai-doc/dev_task.md` (`DEV-110 / DEV-PDM-RECOGNITION-COMMON-VALUE-EXCEPTIONS-001`)；authoritative SPEC=`.ai-doc/specs/SPEC-PDM-RECOGNITION-COMMON-VALUE-EXCEPTIONS-001-upstream-part-work-handoff.md` §20～§26；Accepted／Repository Assessed ADR=`.ai-doc/decisions/ADR-PDM-DRAWING-RECOGNITION-PART-WORK-HANDOFF-001-common-projection-and-atomic-draft-transfer.md`；fixed 60 QA=`.ai-doc/qa/qa-dev-110-recognition-common-value-part-work-handoff-validation-plan-2026-08-31.md` §7～§10；local receipt=`.ai-doc/qc/qc-dev-110-local-implementation-2026-08-31.md`；aggregate=`output/qa/dev-110/DEV110-aggregate-2026-08-31T13-51-48-003Z/aggregate.json`；視覺基線=`output/design/drawing-recognition-common-value-with-sources-ui-v2.png`；panel=`output/design/drawing-recognition-common-value-with-sources-panel-v2.png`；靜態稿=`output/design/drawing-recognition-common-value-with-sources-ui-v2.html`；legacy baseline=`DEV-107`；downstream per-Part work／review authority=`DEV-108`。Status: `RD Implemented Locally / Human Confirmed / Full QC Passed 60/60 / Production Release Gated / P1 / Medium`。圖面一般區域／明確共用列是overall common，料號／資料對照表是exact per-Part override；Current Phase只接受linked adapter可驗證的exact owner ID／完整canonical anchor，或同欄位observation以token boundary唯一命中formal eligible集合中的完整canonical料號；舊suffix／unanchored owner、persisted `resolved`標記、`P03`縮寫／模糊／多重命中fail closed，resolver不得擴張targets。禁止unique mode／majority fan-out，人工common／override可無candidate。formal-only scope最多100 Parts，single handoff使用existing Part work boundary、stable idempotency與all-or-nothing transaction；aggregate已完成C01..C08、R01..R16、P01..P08、B01..B16、I01..I08與G01..G04。event用`schemaVersion=2`＋`destination=part_work`；manual-only／no-op可無link，zero-delta可0 links。schema／migration／dependency／new permission／root writer／combined review／structured table parser=0；production release仍受gate管制。 |
| DEV-109 BOM 單一 domain、建立入口與候選料號 | `.ai-doc/dev_task.md` (`DEV-109 / DEV-PDM-BOM-CANONICAL-CREATE-PAGE-001`)；current contract=`.ai-doc/specs/SPEC-PDM-BOM-CREATE-PAGE-001-canonical-entry-and-candidates.md` §29～§34；current ADR=`.ai-doc/decisions/ADR-PDM-BOM-DOMAIN-002-unified-composition-and-deferred-execution-policy.md`；current QA=`.ai-doc/qa/qa-dev-109-unified-bom-domain-validation-plan-2026-09-01.md` §12；CAPA receipt=`.ai-doc/qc/qc-dev-109-capa-2026-09-01.md`；production read-only receipt=`output/qa/dev-109-capa-production/2026-09-01T0315Z/p01-readonly.json`；historical QC=`.ai-doc/qc/qc-dev-109-local-completion-2026-08-31.md`。Status: `Local Development Closed / CAPA Local Corrected / CAPA-P01 Read-only Observed / Production Effectiveness Owned by DEV-032 / P1 / High / Lane 3`。CAPA-L01～L08與歷史54/54 regression baseline已通過；正式readback仍顯示migration `053/054`缺少、A0044-P01為`Draft/single_part`且active primary exact `.SLDASM` target=`0`。禁止seed；P02僅NO_OP eligible，P03/P04待clean immutable artifact與activation。這表示local開發task可關閉，但CAPA本身不能宣稱production有效；live apply、readback、canonical smoke與排程effectiveness audit統一由DEV-032執行。 |
| DEV-108 料號工作台 Excel 轉置資料總表 | `.ai-doc/dev_task.md` (`DEV-108 / DEV-PDM-PART-NUMBER-EXCEL-MATRIX-WORKSPACE-001`)；authoritative contract=`.ai-doc/specs/SPEC-PDM-ENTITY-DETAIL-DRAWER-001-unified-object-detail-contract.md`的`2026-08-31 DEV-108 Amendment`＋`RD Implementation Readiness Upgrade（RD技術主管修正版）`；核准視覺稿`output/design/part-number-workspace-excel-matrix-ui-v1.png`；提示摘要`output/design/part-number-workspace-excel-matrix-ui-v1-prompt.md`；附件=`DEV-061`；exact Part分類=`DEV-099`；review snapshot=`DEV-101`。Status: `RD Implemented Locally / Focused QA Passed / PostgreSQL Provider Gate Not Run / Production Release Gated / P1 / Medium`。正式Part workspace改為一張Excel式轉置總表：橫軸required exact-root current Parts、縱軸欄位，現行可維護格直接呈現輸入控制項，不需先點擊顯示值進入編輯；2+ canonical normalized values只以淡黃色整列提示，附件count不比較；材質、熱處理、表面處理等已確認屬性各自獨立成列，不再彙總成「其他已確認屬性」。唯一新增API是bounded read-only matrix GET，same-snapshot domain projection最多三個statements、100 Parts上限、initial preview bytes=0。UI沿用同一per-Part writer；首個delta由既有create optional `initialPayload`原子保存，same logical command固定idempotency key直到receipt／readback收斂，15分鐘token到期無損refresh；same-Part序列、跨Part最多3、global submit自然序concurrency=1，partial retry只處理未terminal remaining。附件active list對合法reader開放，deleted-data由server重驗manage；PDF／image使用既有canonical file read按需preview，其他download-only。material/color固定code＋label直接編輯清舊code，不保留no-root stub。exact add／modify／no-touch files、12.5–17.5 person-days、62項SQLite／PostgreSQL／browser registry、fixture／fault、dirty boundary及cleanup已封口；schema／migration／new permission／dependency／new write route／combined review package=0。Intentional-replace DEV-083 Part owner單筆呈現／顯式save，DEV-101 immutable reviewer保持不變。`qc:dev-108` aggregate已完成可用gate：C01..C12、SQLite R01..R14、real Chromium B01..B22 PASS，PostgreSQL R01..R14因未設定`PDM_POSTGRES_URL`明確NOT_RUN；`typecheck:app` PASS，task-owned runtime／port／fixture cleanup與primary invariants通過。證據入口=`output/qa/dev-108/`；本機實作已完成，正式schema／資料／release未變更。 |
| DEV-107 智慧辨識內嵌一鍵寫入與送審前再編輯 | `.ai-doc/dev_task.md` (`DEV-107 / DEV-PDM-RECOGNITION-INLINE-FORMALIZATION-001`)；主SPEC `.ai-doc/specs/SPEC-PDM-DRAWING-RECOGNITION-001-candidate-review-and-formalization.md` §34；ADR `.ai-doc/decisions/ADR-PDM-DRAWING-RECOGNITION-AMENDMENT-LINEAGE-001-origin-overlay-and-synchronized-commit.md`；placement amendment `.ai-doc/specs/SPEC-PDM-ENTITY-DETAIL-DRAWER-001-unified-object-detail-contract.md`；umbrella amendment `.ai-doc/specs/SPEC-PDM-UNIFIED-DRAWING-WORKBENCH-001-single-page-lifecycle-workbench.md`；主QA `.ai-doc/qa/qa-dev-068-drawing-recognition-validation-plan-2026-08-12.md` §10；QC receipt `.ai-doc/qc/qc-dev-107-local-completion-2026-08-31.md`；DEV-079歷史case disposition `.ai-doc/qa/qa-dev-079-drawing-readonly-drawer-fullpage-workspace-validation-plan-2026-08-19.md` §12。Status: `RD Implemented Locally / Human Confirmed / Local QA-QC Complete 38/38 PASS / Production Release Gated`。`智慧辨識`panel是唯一可見mutation owner；一次click只送single server commit，由同一transaction保存draft、重驗exact source／target並同步，positive顯示`已寫入 PDM`，accepted zero-delta建立`appliedCount=0`event後顯示`PDM 已是最新`。amendment採053 `session_purpose`＋`evidence_origin_session_id` candidate overlay，不複製raw source／adapter／observation；partial unique index固定one-open，global lock order固定Drawing→source→lineage→sorted target。submit與review package只接受current fingerprint latest formalized leaf；raw-only／recognition缺席仍非gate。舊route只作server-safe redirect。最新aggregate evidence=`output/qa/dev-107/DEV107-aggregate-2026-08-31T06-54-27-649Z/manifest.json`，QA-107-001..038 exact 38/38 PASS；SQLite／PostgreSQL／browser均在task-owned runtime完成，primary/source invariants、FK與cleanup gate通過。正式migration／activation／deploy／release仍未執行。 |
| DEV-106 銷售組合包 BOM（已取代） | `.ai-doc/dev_task.md` (`DEV-106 / DEV-PDM-SALES-KIT-BOM-001`)；historical SPEC `.ai-doc/specs/SPEC-PDM-SALES-KIT-BOM-001-commercial-bundle-structure.md` §27；superseded ADR `.ai-doc/decisions/ADR-PDM-BOM-PURPOSE-001-manufacturing-and-sales-kit-separation.md`；historical QA `.ai-doc/qa/qa-dev-106-sales-kit-bom-validation-plan-2026-08-31.md`。Status: `Superseded Before Production Release / Historical Local Implementation and QA-QC 30/30 Retained / Not a Current Release Candidate`。`manufacturing | sales_kit`、integer-only kit、purpose-specific UI／consumer與052 migration保留為歷史程式／證據，不得部署或發行為target design。Current authority改為DEV-109、`ADR-PDM-BOM-DOMAIN-002`與`SPEC-PDM-BOM-CREATE-PAGE-001` §29～§34。 |
| Jenfu Platform DEV-004共同IAM／AI-PDM app-local session相容 | [DEV-004 direct spec](../../Jenfu-Management-system/ai-doc/specs/DEV-004-common-iam-app-local-session-implementation.md)；AI-PDM active authority=`.ai-doc/specs/SPEC-PDM-ACCESS-CONTROL-001-user-identity-permission-architecture.md` 2026-08-31 amendment；S2 QC=`.ai-doc/qc/qc-dev-004-s2-ai-pdm-zero-disruption-2026-08-31.md`；S5 evidence=`../../Jenfu-Management-system/output/dev-004/DEV004-S5-20260831T054317661Z-75bb1fcc/`。Status：`DEV-004 004-S0～S5 Local PASS / AI-PDM Adapter Local Complete / Live G2 Gated`。default-off／cookie-only／single-mode adapter保留Firebase UID／`pdm_user_id`；S5 fresh gate再驗證focused 14／14、DEV-046 16／16、login alias 21／21、typecheck／lint／isolated build及1440／1024／390 browser evidence均PASS。Production Firebase／Cloud SQL／deploy／release未授權。 |
| Jenfu Platform DEV-005 Position驅動Application Role指派 | [DEV-005 direct spec](../../Jenfu-Management-system/ai-doc/specs/DEV-005-position-derived-application-role-assignment.md)；[AI-PDM角色能力UI契約](../../Jenfu-Management-system/ai-doc/specs/DEV-005-ai-pdm-role-capability-ui.md)；[authorization route inventory](../../Jenfu-Management-system/ai-doc/specs/DEV-005-ai-pdm-authorization-route-inventory.md)；AI-PDM active authority=`.ai-doc/specs/SPEC-PDM-ACCESS-CONTROL-001-user-identity-permission-architecture.md`；OrgMaster authority=`../../OrgMaster/ai-doc/specs/DEV-040-jenfu-platform-entitlement-user-integration.md` §21。Status：`005-S0～S4B Local-Isolated PASS / S5 Local Targeted PASS / Local Development Closed / Production Activation Owned by DEV-032 + Platform Release Owner`。正常UI為`/settings/workflow`單一「角色能力」與Role→adopted Position→Employee；target catalog=`ai-pdm.role-catalog.2026-09-02.v2`九角色。EntitlementRepository request enforcement、route bypass retirement、change-feed／cross-repo consumer與targeted gates均已完成；未執行remote migration、authority switch、正式資料、deploy或release。 |
| Jenfu Platform DEV-009 System Administrator特權Principal治理 | [DEV-009 direct spec](../../Jenfu-Management-system/ai-doc/specs/DEV-009-system-admin-privileged-principal-governance.md)；[AI-PDM角色能力UI契約](../../Jenfu-Management-system/ai-doc/specs/DEV-005-ai-pdm-role-capability-ui.md)；AI-PDM authority=`.ai-doc/specs/SPEC-PDM-ACCESS-CONTROL-001-user-identity-permission-architecture.md` 2026-09-02 amendment；OrgMaster authority=`../../OrgMaster/ai-doc/specs/DEV-040-jenfu-platform-entitlement-user-integration.md` §23。Status：`009-S0～S4 Local-Isolated Complete / Targeted QA-QC PASS / Local Development Closed / 009-R1 Owned by DEV-032 + Platform Release Owner`。AI-PDM一般角色保留workspace v2；system_admin顯示workspace v3的OrgMaster管理說明、redacted holder與authority資料時間，stale／unavailable唯讀，runtime只讀Tier-0且不依賴OrgMaster HTTP。Canonical aggregate=`21db33b350f8908402e405c870eb2cf269cee85c57b56ef762b66f36d91b04a3`；S3 frozen aggregate=`output/qa/dev-009/aggregate/DEV009-S3-AGGREGATE-2026-09-02T18-19-00-268Z/report.json`，S4 frozen report=`../../Jenfu-Management-system/output/qa/dev-009/cross-repo/DEV009-S4-2026-09-02T18-21-31-904Z/report.json`；production／bootstrap／deploy／release仍未執行。 |
| DEV-104 BOM 工作台 V2 受控產品結構與精簡編輯重構 | `.ai-doc/dev_task.md` (`DEV-104 / DEV-PDM-BOM-WORKBENCH-V2-001`)；current RD implementation authority=`.ai-doc/specs/SPEC-BOM-WORKBENCH-001-bom-workbench.md` §18；DEV-071 preserved data／interaction boundary=`.ai-doc/specs/SPEC-BOM-VISUAL-EDITOR-001-xmind-style-bom-editor.md`；domain authority=`.ai-doc/specs/SPEC-PDM-ASSEMBLY-BOM-REBUILD-001-contextual-shared-structure.md`。Status: `Local RD Implemented / Fixed QA 48/48 PASS / Aggregate QA-QC Complete / Production Release Gated`。Outliner為desktop唯一可寫editor；Map只讀瀏覽／選取／定位，Floating Topic為收合advanced Draft staging；tablet／phone只讀／審核／匯出。清單／明細路由已分離，舊ReactFlow／XMind production graph與樣式已移除；不建V3 route或可寫fallback。Exact file／command／recovery／retirement／fixture與固定48-case denominator見§18.13～18.21；最終aggregate=`output/qa/dev-104/DEV104-2026-08-30T16-28-09-638Z/manifest.json`，QA-104-001..048 exact case set 48/48 PASS，三runner evidence／primary invariant／cleanup均有效。Quality Gates：DEV-071 API 16/16、flag-off 10/10、巢狀 DEV-096 88/88（disposable PostgreSQL 7/7、browser 26/26）、lifecycle UI 29/29、typecheck、affected lint、isolated build 全部 PASS。新增schema／migration／public endpoint／permission／lifecycle／feature flag=0，只有既有flag-off PATCH全面fail closed；DEV-071 Floating／CAS／server gate與DEV-096 domain／review／release authority保留。正式activation、deploy與release仍受既有gate管制。 |
| DEV-103 工作臺關聯矩陣編輯入口與冗餘提示收斂 | `.ai-doc/dev_task.md` (`DEV-103`)；`src/components/canonical-pdm-workbench.tsx`；`scripts/qc-dev-090-contract.mjs`。Status: `Local RD Implemented / Focused Contract QC 25/25 / Typecheck + Isolated Build + Browser PASS / Production Release Gated`。只改 presentation/action placement：移除關聯矩陣常駐 helper，將既有「編輯關聯」置於關聯矩陣標題列的「＋建立編號」左側；矩陣 editing、儲存／取消、API、schema、資料與權限不變。截圖所示「有新版本可用」獨立 modal 不存在於現行 source，沒有建立替代元件。 |
| DEV-102 圖號報表獨立頁面拆除 | `.ai-doc/dev_task.md` (`DEV-102`)；`scripts/qc-pdm-numbering-report-retirement.mjs`。Status: `Local RD Implemented / Focused Retirement QC 13/13 / Typecheck + Isolated Build PASS / Browser PASS / Production Release Gated`。`/numbering/reports` route、sidebar/dashboard/handoff 現行入口與 report status-scope registry 已拆除；不保留舊網址轉址。月報／匯出 API、資料表與 `numbering.reports` API 權限保留，歷史 report-center UI spec/QC 不再視為現行 UI 契約。 |
| DEV-101 審核工作臺共用圖號／料號完整工作區 | `.ai-doc/dev_task.md` (`DEV-101`)；SPEC `.ai-doc/specs/SPEC-PDM-APPROVAL-CANONICAL-REVIEW-WORKSPACE-001-snapshot-package-and-shared-renderers.md`；ADR `.ai-doc/decisions/ADR-PDM-UNIFIED-ENTITY-DETAIL-PROJECTIONS-001-composer-and-policy.md` DEV-101 amendment；QA `.ai-doc/qa/qa-dev-101-approval-canonical-review-workspace-validation-plan-2026-08-26.md`；CAPA `.ai-doc/qc/qc-dev-101-approval-inbox-discoverability-capa-2026-08-27.md`與`.ai-doc/qc/qc-dev-079-dev-101-recognition-owner-review-parity-capa-2026-08-27.md`；RD receipt `.ai-doc/qc/qc-dev-101-rd-implementation-completion-receipt-2026-08-27.md`；independent closure `.ai-doc/qc/qc-dev-101-independent-qa-qc-closure-2026-08-27.md`；current manifest `output/qa/dev-101-independent-aggregate/DEV101-INDEPENDENT-AGGREGATE-2026-08-27T15-19-16-555Z/manifest.json`。Status: `Local RD Implemented / Independent QA-QC Complete / Fixed QA 48 of 48 PASS / Production Release Gated`。actor-scoped inbox、normal submit→list→page→return、v1／v2分離、exact full recognition projection／inner hash、shared owner-review projector／panel、owner fail-closed與latest-session isolation已完成；current-source四runner aggregate為48/48。existing v1不backfill／改hash；reviewer只讀immutable package。Primary/staging/production與actual runtime activation仍受release gate。 |
| DEV-087 三工作臺狀態資料重建、漏接功能回復、Drawing最多三個研發分支與人類語意極簡整頓 | `.ai-doc/dev_task.md` (`DEV-087`)；SPEC `.ai-doc/specs/SPEC-PDM-STATUS-DATA-REBUILD-001-canonical-workbench-state-and-branching.md` §15／§15.3／§15.9／§15.15；ADR `.ai-doc/decisions/ADR-PDM-STATUS-DATA-REBUILD-001-single-current-state-authority.md` §11～§12；主QA `.ai-doc/qa/qa-dev-087-status-data-rebuild-validation-plan-2026-08-21.md` §25～§28；UI-only子契約 `.ai-doc/qa/qa-dev-087-ui-only-lifecycle-operation-validation-plan-2026-08-22.md` §0.1A／§6／§27～§30；current QC `.ai-doc/qc/qc-dev-087-post-fff-source-revalidation-2026-08-31.md`；historical QC `.ai-doc/qc/qc-dev-087-trusted-solo-local-completion-2026-08-28.md`、`.ai-doc/qc/qc-dev-087-new-architecture-zero-loss-local-2026-08-23.md`；historical anti-cheat child `DEV-097`；CAPA `DEV-092`、`DEV-094`、`DEV-100`。Status: `Local RD/QA-QC Complete / Human Confirmed / 94 of 94 Product Cases + 3 of 3 Quality Gates PASS (2026-08-31 post-FFF source revalidation) / Production Release Gated`。current evidence=`output/qa/dev-087-aggregate/DEV087-aggregate-2026-08-30T15-29-47-476Z/manifest.json`，source fingerprint=`7c54dc2aac2852b313400c003cecf1d2b7304718b4bc5847829a72ab5924c12f`；21/21 commands、94/94 cases、3/3 gates、`completionCandidate=true`。Provider 6/6＋12 mutation receipts、security zero-write 8/8、UI family-viewport 32/32皆PASS，DEV-100 18/18與isolated typecheck/build PASS；2026-08-30T14-15-03-301Z因BOM既有工作檔案在執行中變更而fail-closed，保留為歷史邊界，不替代本次重驗證。server只依`predecessor_revision_id`判定FFF：首版只顯示中性`relatedParts`且zero FFF，進版才顯示判定範圍、分離`affectedParts`並要求三軸人工明確判定。actual AT維持選配；DEV-097為`Skipped / Historical Supporting`且不阻擋completion。schema／migration／production backfill=`none`；production migration、deploy、release未執行，`part_root`搜尋結果自己的明細與動作仍為本期明確排除。 |
| DEV-097 DEV-087 QA完成分母與反假PASS封口 | `.ai-doc/dev_task.md` (`DEV-097`)；DEV-087 SPEC historical §15.12～§15.14；主QA historical §27H；UI-only QA historical §29H。Status: `Skipped / Historical Supporting / Superseded by Trusted-Solo QA Decision`。既有registry/schema、case-ID migration、oracle、mutants、integrity runner、raw G4、receipt template與fail-closed aggregate均保留，不刪除產品或測試artifact；但QA-087-219..228、Independent QC／AT receipt與anti-cheat結果不再計入DEV-087 current denominator或release re-entry。若未來要恢復多人稽核，必須由使用者另行明確決策。 |
| DEV-098 圖面版次與研發分支統一生命週期 | `.ai-doc/dev_task.md` (`DEV-098`)；SPEC `.ai-doc/specs/SPEC-PDM-DRAWING-REVISION-BRANCH-LIFECYCLE-001-unified-revision-and-branch-flow.md` §15；ADR `.ai-doc/decisions/ADR-PDM-DRAWING-REVISION-BRANCH-LIFECYCLE-001-bounded-manual-minor-and-stale-freeze.md`；QA `.ai-doc/qa/qa-dev-098-drawing-revision-branch-lifecycle-validation-plan-2026-08-25.md`；QC `.ai-doc/qc/qc-dev-098-local-completion-2026-08-28.md`；關聯authority為`DEV-050`、`DEV-087`、`DEV-053`與`DEV-079`。Status: `Local RD/QA-QC Complete / Fixed QA 31 of 31 PASS / Production Release Gated`。使用者確認`1C-bounded／2A／3A`；contract、repository、normal-path browser、disposable PostgreSQL、affected DEV-087 regression與engineering gates已由current aggregate全部PASS，evidence=`output/qa/dev-098/DEV098-aggregate-2026-08-28T07-21-30-116Z/manifest.json`，`completionCandidate=true`。單人可信任RD→QA→QC不要求獨立人員或防作弊收據；schema／migration=`none`，未執行production migration、deploy、traffic或release。 |
| DEV-092 遷移圖號工作檔案快照完整性修復 | `.ai-doc/dev_task.md` (`DEV-092`)；authority amendment在DEV-087 SPEC §0.3／§3.1.1／§8／§10.2／§12；QA在DEV-087主QA §23（`QA-087-179..186`）；QC record在`.ai-doc/qc/qc-dev-087-new-architecture-zero-loss-local-2026-08-23.md`。Status: `RD Implemented / QA-087-179..186 PASS / Browser PASS / Disposable PostgreSQL PASS / Production Data Repair & Release Gated`。A0006-M01 work-file snapshot、converter、idempotent forward repair、PostgreSQL composite receipt、runtime invariant與recognition exact-context gate均已完成並由DEV-094 fresh aggregate重驗；禁止UI fallback、改寫舊recognition context或未授權production mutation。ADR判定`No New ADR`。 |
| DEV-100 遷移圖號工作檔案合法替換後快照一致性 CAPA | `.ai-doc/dev_task.md` (`DEV-100`)；DEV-087 SPEC §0.5／§15.7.2；DEV-087主QA §28（`QA-100-001..018`）；QC `.ai-doc/qc/qc-dev-100-drawing-work-file-replacement-capa-2026-08-26.md`。Status: `Local RD Implemented / CAPA Effective / Local QA-QC Complete / Local Fix Verified / A0044 Data Repair Human-Gated / Production Release Gated`。active/tombstone classifier、post-write gate、409 stale-state freeze與same-role exact filename提示已完成；fresh aggregate `output/qa/dev-100/DEV100-2026-08-26T11-50-35-191Z/manifest.json`為18/18、13/13 commands，SQLite／disposable PostgreSQL、headed browser、雙mutant與build均PASS。舊DEV-087 parent曾因DEV-097 anti-cheat分母未完成而fail closed，該父判定已降為Historical；DEV-100產品／資料CAPA仍是DEV-087新94＋3契約的必要回歸依賴。保留單一Drawing入口與一個primary 3D，不實作assembly parser／自動BOM；A0044 apply前仍須人類選A0043或A0044，目前apply count 0。ADR=`No New ADR`。 |
| DEV-111 料號核准後 canonical state 與關聯矩陣導航完整性 CAPA | `.ai-doc/dev_task.md` (`DEV-111`)；主SPEC `.ai-doc/specs/SPEC-PDM-STATUS-DATA-REBUILD-001-canonical-workbench-state-and-branching.md` 的 `DEV-111 CAPA amendment`；QA `.ai-doc/qa/qa-dev-111-canonical-state-transition-capa-validation-plan-2026-09-01.md`；QC `.ai-doc/qc/qc-dev-111-canonical-state-transition-capa-2026-09-01.md`；primary apply=`output/qa/dev-111-primary-repair/PRIMARY-APPLY-2026-09-01T07-29-00/manifest.json`；production release evidence=`output/qa/dev-111-production-release/`。Status: `CAPA Closed / Local + Primary SQLite + Production Code/Schema Effectiveness Verified / Production Released`。地端唯一repairable A0044-P01已backup/fingerprint/dry-run/apply/replay；正式Cloud SQL pre/post inventory的Part/Drawing gap、non-navigable link與repair plan皆為0，故live data repair=NO-OP。PR #28 merge commit=`221b25c1`、revision=`ai-pdm-prod-gh-221b25c1-33457759159`、candidate/canonical smoke=14/14、production-bound rollback rehearsal formal=1/work=0且持久指紋不變。正式`/approvals`仍由production slice關閉；authenticated approval E2E保留為未來slice activation gate，不得標示PASS。ADR=`No New ADR`。 |
| DEV-112 圖號／料號工作台三種顯示模式 | `.ai-doc/dev_task.md` (`DEV-112 / DEV-PDM-WORKBENCH-THREE-VIEW-MODES-001`)；preview SPEC `.ai-doc/specs/SPEC-PDM-WORKBENCH-PREVIEW-GALLERY-001-drawing-part-3d-preview-mode.md` §0T／§0S；topbar SPEC `.ai-doc/specs/SPEC-UX-PDM-WORKBENCH-TOPBAR-001-unified-toolbar-muscle-memory.md` §0E／§0D；QA `.ai-doc/qa/qa-dev-065-workbench-preview-gallery-validation-plan-2026-08-11.md` §0T／§0S（`TVM-001..024`）及DEV-066 QA §0；parent=`DEV-065`，related=`DEV-066/087/105`。Status: `Local RD Implemented / RD Tech Lead Corrections Closed / Human Confirmed / QA-QC Complete 24/24 / P1 / Medium / Production Release Gated`。固定 `/numbering/drawings` 與 `/parts` 的結果顯示為 `文字清單／3D 清單／預覽圖`三種互斥模式；頁首只保留名稱與建立動作，篩選只管理資料範圍，顯示方式位於結果區上緣。3D清單沿用表格並在編號儲存格呈現84×63／窄版約62×47縮圖，不新增欄或3D開關；URL=`layout=list|list_3d|preview`，只有URL key missing可讀stored preference，invalid raw必回list。文字清單不下載圖片或poll；Current Phase不做viewport lazy，image-bearing mode首次media request受current cursor page ready rows（limit<=100）約束；gallery／inline共用wrapper處理non-image／decode failure。`qc:dev-112:contract` C01..C16=16/16、task-owned Chromium TVM-001..024=24/24、P0/P1=0，Drawing／Part三模式×四viewport截圖24張；security no-cookie=401、跨公司／tampered=404、受保護bytes與raw authority leak=0。Aggregate evidence=`output/qa/dev-112-three-view-modes/DEV112-aggregate-2026-09-01T04-22-28-720Z/aggregate-manifest.json`，contract/browser candidate fingerprint match、DEV-065 contract、`typecheck:app`與`build:isolated`均PASS，primary logical invariants與task-owned cleanup通過。Spec Impact=`Intentional replacement + compatible reuse`；schema／API route／permission／data mutation=`none`。未連線或寫入production，production release仍受獨立gate。 |
| DEV-094 SQLite 圖料主檔遷移完整性與 runtime 隔離 CAPA | `.ai-doc/dev_task.md` (`DEV-094`)；SPEC `.ai-doc/specs/SPEC-PDM-SQLITE-MIGRATION-INTEGRITY-CAPA-001-root-recovery-and-runtime-isolation.md`；QA `.ai-doc/qa/qa-dev-094-sqlite-migration-integrity-capa-validation-plan-2026-08-24.md`；QC `.ai-doc/qc/qc-dev-094-sqlite-migration-integrity-capa-2026-08-24.md`。Status: `Local RD Implemented / QA-QC PASS / CAPA Effective / P0 Closed / Production Release Gated`。主SQLite已exact recovery為roots/parts=3/3、FK=0、staging=0且第二次NO_OP；initializer跨process lock、atomic migration、isolated build data root、detail local anomaly與browser pre-seed source gate均完成。fresh DEV-087 aggregate 16/16 PASS。ADR=`No new ADR required`。 |
| DEV-095 舊組合件建立工作流與資料 authority 退役 | `.ai-doc/dev_task.md` (`DEV-095`)；SPEC `.ai-doc/specs/SPEC-PDM-ASSEMBLY-LEGACY-WORKFLOW-RETIREMENT-001-canonical-workbench-boundary.md`；ADR `.ai-doc/decisions/ADR-PDM-ASSEMBLY-MASTER-ENTRY-001-canonical-workbenches-only.md`；QA `.ai-doc/qa/qa-dev-095-assembly-legacy-workflow-retirement-validation-plan-2026-08-24.md`；QC `.ai-doc/qc/qc-dev-095-assembly-retirement-2026-08-24.md`。Status: `RD Implementation Complete / Human Confirmed / Local QA-QC PASS / Production Migration & Release Gated`。Intentional-replace DEV-060 的 `/bom/new`、組合件證據、CAD/XLS來源與from-assembly authority；保留Drawing／Part canonical workbench、`.SLDASM`通用檔案能力、generic manual BOM edit/review/release及DEV-041技轉包。checkpoint、isolated migration、manual BOM regression、三viewport UI、122-page build與primary logical invariant均PASS；未套主要／正式migration、未deploy/release。 |
| DEV-099 結構型態延後分類與 BOM readiness 解耦 | `.ai-doc/dev_task.md` (`DEV-099`)；SPEC `.ai-doc/specs/SPEC-PDM-DEFERRED-STRUCTURE-CLASSIFICATION-001-numbering-and-bom-readiness.md`；ADR `.ai-doc/decisions/ADR-PDM-PART-STRUCTURE-CLASSIFICATION-001-deferred-exact-part-authority.md`；QA `.ai-doc/qa/qa-dev-099-deferred-structure-classification-validation-plan-2026-08-26.md`。Status: `RD Implementation Complete / Human Confirmed / Full Aggregate QA 48/48 / Production Release Gated`。Intentional-replace DEV-093／096的new-root required structure、first-Part inheritance、unclassified append block、purchased assembly 422及Part change-only限制；建號省略時明確寫unclassified，exact Part drawer才分類並可複選同root變體。只有assembly顯示BOM區，manufactured＋primary M才有製造BOM action；purchased assembly可分類但本期無BOM action。schema／migration／backfill=0；同一 aggregate 已完成四viewport headed browser、SQLite／PostgreSQL provider parity、DEV-093／096 regression與primary invariant/cleanup gate，證據為`output/qa/dev-099/DEV099-2026-08-26T09-03-03-967Z/manifest.json`。正式migration、activation、deploy與release仍受gate。 |
| DEV-096 組立件情境式共用 BOM 重建 | `.ai-doc/dev_task.md` (`DEV-096`)；SPEC `.ai-doc/specs/SPEC-PDM-ASSEMBLY-BOM-REBUILD-001-contextual-shared-structure.md`；入口ADR `.ai-doc/decisions/ADR-PDM-ASSEMBLY-MASTER-ENTRY-001-canonical-workbenches-only.md`；共用結構ADR `.ai-doc/decisions/ADR-PDM-BOM-STRUCTURE-SHARING-001-variant-part-applicability.md`；QA `.ai-doc/qa/qa-dev-096-assembly-shared-bom-rd-contract-validation-plan-2026-08-24.md`；QC `.ai-doc/qc/qc-dev-096-assembly-shared-bom-local-execution-2026-08-25.md`。Status: `RD Implementation Complete / Human Confirmed / Local QA-QC Complete / Production Migration & Release Gated`。不新增入口；Current Phase以Part Number structure type與Part drawer唯一action承接manufactured assembly＋primary M，支援多Parent適用性、stable logical line、exact Parent-to-Child mapping及schema-v2 immutable review/release evidence。096-A～E已完成；fresh aggregate 88/88、42個fault checkpoint、SQLite與PostgreSQL實際mutation、四viewport browser、typecheck、affected ESLint及123/123 isolated build均PASS，primary invariant未變。`.SLDASM` parser、purchased assembly及Released Parent移除／detach／fork仍為Future Phase；正式migration、flag activation、deploy與release未授權。分類時點／purchased assembly／unclassified append將由未實作的DEV-099有意取代，shared BOM authority與歷史88-case不變。 |
| DEV-086 三工作台量產／研發最新版雙列投影 | `.ai-doc/dev_task.md` (`DEV-086`, CAPA Corrective Implementation Verified / Local QA-QC PASS)；`.ai-doc/specs/SPEC-PDM-WORKBENCH-PRODUCTION-RD-LANES-001-dual-latest-projection.md`；`.ai-doc/decisions/ADR-PDM-WORKBENCH-PRODUCTION-RD-LANES-001-dual-lane-authority.md`；`.ai-doc/qa/qa-dev-086-production-rd-lanes-validation-plan-2026-08-20.md`；CAPA `.ai-doc/qc/qc-dev-086-dual-lane-completion-capa-2026-08-21.md`。Status: `Historical Runtime Baseline / Superseded by DEV-087 and DEV-090 / Production Release Gated`。目前runtime baseline與2026-08-21 evidence保留作歷史追溯；Drawing雙列／單一RD aggregate由DEV-087多open-branch latest取代，Relation formal/work兩列由DEV-090 inline matrix取代，Part仍由DEV-087 formal/work契約負責。不得把歷史evidence誤報為現行runtime。 |
| DEV-088 替代料號附件人工沿用與安靜選擇 | `.ai-doc/dev_task.md` (`DEV-088`)；authority SPEC `.ai-doc/specs/SPEC-PDM-PART-ATTACHMENT-REUSE-002-replacement-selection-snapshot.md`、ADR `.ai-doc/decisions/ADR-PDM-PART-ATTACHMENT-REUSE-002-file-asset-snapshot.md`、QA `.ai-doc/qa/qa-dev-088-replacement-part-attachment-selection-2026-08-22.md`、QC `.ai-doc/qc/qc-dev-088-local-implementation-2026-08-22.md`；歷史DEV-084三文件僅供追溯。Status: `Local RD Implemented / Focused QA-QC PASS / P1 / Production Migration & Release Gated`。來源formal Part active direct附件預設全選，可取消／新增，Drawing類檔案排除；一次性target `file_assets` snapshot共用immutable storage bytes，不移動source、不後續同步。最小兩表schema與041、source stale、dedupe、idempotency、atomic promotion、batch insert及安靜UI已完成；`qc:dev-088` 7/7 PASS，cleanup補強後HTTP 15/15、browser 37/37。原DEV-084五表、權限改寫與whole-part lease永久不作現行authority；Cloud SQL 041 apply、deploy、release與physical GC仍受gate管控。 |
| DEV-089 Canonical 圖料工作臺抽屜關聯矩陣 | `.ai-doc/dev_task.md` (`DEV-089`)；authority沿用`.ai-doc/specs/SPEC-PDM-STATUS-DATA-REBUILD-001-canonical-workbench-state-and-branching.md` §6.3、§9.1、§12；focused evidence由`qc:dev-087:repository`、`qc:dev-087:contract`、`qc:dev-087:browser`產生。Status: `Historical / Superseded by DEV-090 / Production Release Gated`。Relation list/drawer owner、formal/work projection與Relation current work語意只作歷史基線；matrix軸、cell語意、root/company validation與single projection證據由DEV-090保留。 |
| DEV-090 圖料工作台退役與抽屜關聯矩陣直接編輯 | `.ai-doc/dev_task.md` (`DEV-090`)；SPEC `.ai-doc/specs/SPEC-PDM-INLINE-RELATION-MATRIX-001-direct-formal-edit.md`；ADR `.ai-doc/decisions/ADR-PDM-RELATION-EDITING-001-direct-formal-authority.md`；QA `.ai-doc/qa/qa-dev-090-inline-relation-matrix-validation-plan-2026-08-23.md`。Status: `RD Implementation Complete / Human Confirmed / Local RD Implemented / Local QA-QC Complete / Production Release Gated`。Drawing／Part drawer以同一root-level matrix取代直接關聯並可原位編輯；一個`儲存`可原子提交最多2,500格，以strong ETag＋idempotency＋transaction立即更新`drawing_part_links`，不autosave、不建立Relation work、不送審。`RelationFormalAuthorityRepository`（含SQLite sync adapter）是唯一formal writer，全部編號／正式化／替代料號／主圖恢復／direct edit flow共用root-first lock；舊Relation route/service/repository已移除。SQLite current projection cleanup、PostgreSQL 043 fail-closed migration與provider-aware rehearsal runner已完成；正式PostgreSQL parity、zero-loss reconciliation與release仍須另行授權。 |
| DEV-093 Canonical 統一建立編號 | `.ai-doc/dev_task.md` (`DEV-093`)；SPEC `.ai-doc/specs/SPEC-PDM-CANONICAL-NUMBER-CREATION-001-unified-contextual-create.md`；QA `.ai-doc/qa/qa-dev-093-canonical-number-creation-validation-plan-2026-08-24.md`。Status: `Local RD Implemented / Corrective QA-QC Passed / Production Release Gated`。本次只精簡existing-root：UI只保留root、料號／圖號／兩者、必要M／R與追加原因、單行「將建立」及一個primary；不顯示五項料件profile或「沿用設定」列，canonical request也不傳。server在transaction內繼承root Part的`itemKind/structureType/isUniversal/seriesCode/customSpecification`，unclassified雙層fail closed；Part提交另以root-scoped lock依正式Part與recovery reservation重算最低可用號，`numbering_sequences`不再是唯一權威，SQL／constraint錯誤不進UI；new-root完整命名流程維持不變。QA-093-001..111、兩輪fresh Chromium stale-counter fault injection、desktop／320px、DB/API/UI reconciliation、legacy caller=0與isolated build均PASS；證據`output/qa/dev-093/DEV093-2026-08-25T01-32-17-561Z/`。不新增schema，不恢復draft workspace、candidate reservation、legacy publication、fallback或dual-write；正式migration／deploy／release均未執行。required structure／first-Part inheritance／unclassified block將由未實作的DEV-099有意取代；歷史111-case不變。 |
| DEV-085 三工作台 Excel 式複選篩選器 | `.ai-doc/dev_task.md` (`DEV-085`, historical／superseded)`；`.ai-doc/specs/SPEC-PDM-WORKBENCH-MULTISELECT-FILTER-001-excel-style-filter-contract.md`；`.ai-doc/qa/qa-dev-085-workbench-multiselect-filter-validation-plan-2026-08-20.md`；parent `DEV-066`；current consumers `DEV-087`、`DEV-090`。Status: `Historical / Superseded by DEV-087 and DEV-090 / Shared Mechanics Retained / No Separate Release Target`。原三工作台 Excel 式複選的 explicit `all / none / some`、repeated URL、cursor reset、RWD／keyboard mechanics已保留並由 current canonical workbench 承接；舊 domain adapter、狀態 query、版本列與 Relation semantic 已退役，不得復活 legacy source。selection／query／contract／browser evidence保留於 `output/qa/dev-085-workbench-multiselect-filter/` 作歷史追溯；舊分母與 independent QC不再是 current completion gate。無新增 schema、permission、mutation或lifecycle；production release仍受 current DEV-087／DEV-090 gate控制。 |
| DEV-079 2026-08-20 density/layout amendment | `.ai-doc/dev_task.md`、`.ai-doc/specs/SPEC-PDM-ENTITY-DETAIL-DRAWER-001-unified-object-detail-contract.md`、`.ai-doc/specs/SPEC-PDM-UNIFIED-DRAWING-WORKBENCH-001-single-page-lifecycle-workbench.md`、`.ai-doc/qa/qa-dev-079-drawing-readonly-drawer-fullpage-workspace-validation-plan-2026-08-19.md`。Owner workspace 移除預覽計數／重複圖號／預覽 footer 檔名／受控檔案輔助 metadata 與多餘留白，2D／3D tab 顯示檔名；`npm.cmd run qc:dev-079:layout-browser` 三 viewport PASS。Evidence：`output/qa/dev-079-layout/20260820020110-browser/`。不改 domain／permission／lifecycle／submit authority，DEV-079 仍為 `Independent QC Pending / Production Release Gated`。 |
| DEV-079 2026-08-20 recognition density amendment（歷史切片） | `.ai-doc/dev_task.md`、`.ai-doc/specs/SPEC-PDM-ENTITY-DETAIL-DRAWER-001-unified-object-detail-contract.md`、`.ai-doc/specs/SPEC-PDM-UNIFIED-DRAWING-WORKBENCH-001-single-page-lifecycle-workbench.md`、`.ai-doc/qa/qa-dev-079-drawing-readonly-drawer-fullpage-workspace-validation-plan-2026-08-19.md`。移除 badge、輔助工具標籤、重複說明／統計／分類標題及候選輔助文字；該輪逐欄操作已由下一項 silent auto-recognition amendment 取代。Evidence：`output/qa/dev-079-recognition-layout/20260820013849-browser/`。 |
| DEV-079 2026-08-20 silent auto-recognition amendment | `.ai-doc/dev_task.md`、`.ai-doc/specs/SPEC-PDM-DRAWING-RECOGNITION-001-candidate-review-and-formalization.md`、`.ai-doc/specs/SPEC-PDM-ENTITY-DETAIL-DRAWER-001-unified-object-detail-contract.md`、`.ai-doc/specs/SPEC-PDM-UNIFIED-DRAWING-WORKBENCH-001-single-page-lifecycle-workbench.md`、`.ai-doc/qa/qa-dev-079-drawing-readonly-drawer-fullpage-workspace-validation-plan-2026-08-19.md`。取消正常流程`開始辨識`；candidate upload成功後server自動ensure去重，進頁自動補建舊檔缺漏並輪詢。候選欄位直接定位、無座標顯示來源提示、逐欄按鈕改為單一批次儲存及已修改訊號。Focused contract、layout／recognition browser、typecheck、lint、DEV-068 contract與三viewport browser PASS。Evidence：`output/qa/dev-079-layout/20260820161642-browser/`、`output/qa/dev-079-recognition-layout/20260820161949-browser/`。DEV-079仍為`Independent QC Pending / Production Release Gated`。 |
| DEV-079 2026-08-27 unresolved part-owner correction | `.ai-doc/dev_task.md`、`.ai-doc/specs/SPEC-PDM-DRAWING-RECOGNITION-001-candidate-review-and-formalization.md` §32、`.ai-doc/qa/qa-dev-079-drawing-readonly-drawer-fullpage-workspace-validation-plan-2026-08-19.md` QA-079-29。A0044-M01 non-terminal unique-owner self-heal、SQLite／PostgreSQL雙併發與主要環境readback均保留為historical supporting evidence；report=`output/qa/dev-079-owner-resolution/DEV079-OWNER-2026-08-27T08-40-52-011Z/report.json`。A0002已證明accepted legacy不在self-heal範圍，且editor→review immutable parity未被覆蓋，因此原`Production Recurrence Gate Wired`宣告撤回。Status：`Supporting Evidence Only / Superseded by Owner-Review Parity CAPA / Production Release Gated`。 |
| DEV-079／DEV-101 2026-08-27 智慧辨識料號歸屬與審核快照一致性 CAPA | CAPA `.ai-doc/qc/qc-dev-079-dev-101-recognition-owner-review-parity-capa-2026-08-27.md`；tasks `.ai-doc/dev_task.md` (`DEV-079`、`DEV-101`)；recognition SPEC `.ai-doc/specs/SPEC-PDM-DRAWING-RECOGNITION-001-candidate-review-and-formalization.md` §32；review SPEC `.ai-doc/specs/SPEC-PDM-APPROVAL-CANONICAL-REVIEW-WORKSPACE-001-snapshot-package-and-shared-renderers.md`；closure `.ai-doc/qc/qc-dev-101-independent-qa-qc-closure-2026-08-27.md`。Status：`Corrective Actions Implemented Locally / Local Effectiveness Verified / Primary SQLite Reconciled / Production Effectiveness Pending / Release Gated`。DEV-079單一resolver、accepted-state command＋SQLite／PostgreSQL DB invariant、受控reconciliation與GET zero-write已通過隔離驗證；DEV-101 immutable projection與固定48案為48/48 PASS。Primary SQLite依授權套用guard與21筆exactly-one repair，零差異重跑為0且最終40筆全valid；A0002既有v1 request不backfill、不改hash。Staging／production schema／data apply、writer activation、deploy／traffic／release仍需人類與release gate。 |
| Current task, blockers, next executable work | `.ai-doc/dev_task.md` |
| PDM 核心物料身份治理：何時升 Drawing/BOM Rev、何時換 Part Number | `.ai-doc/decisions/ADR-PDM-MATERIAL-IDENTITY-REVISION-001-part-number-vs-controlled-definition-revision.md`; limited amendment `.ai-doc/decisions/ADR-PDM-BOM-STRUCTURE-SHARING-001-variant-part-applicability.md`; `.ai-doc/specs/SPEC-PDM-CHANGE-CONTROL-001-revision-part-bom-flow.md`; `.ai-doc/specs/SPEC-BOM-WORKBENCH-001-bom-workbench.md`。Part Number 是無版次的物料身份；Drawing 與 BOM 各自獨立版控。同身份只升受影響定義 Rev；建立新 Part Number時必須具有明確BOM適用綁定與確定投影。結構相同的變體可共用一份BOM，結構不同時須建立或fork獨立BOM；Released history不得靜默改寫。未來跨模組 DEV 必須遵守。 |
...
| 料號／製造成本模組退役 | `.ai-doc/decisions/ADR-PDM-PART-COST-RETIREMENT-001.md`; retired historical spec `.ai-doc/specs/SPEC-PDM-PART-COST-001-root-linked-drawing-part-cost.md`; local migration `db/postgres/032_remove_part_cost.sql`; QC `npm.cmd run qc:pdm-part-cost-retirement`。現行產品不再提供料號成本 UI/API/repository/schema；檔案儲存、雲端預算與 AI API cost governance 不在退役範圍。正式 Cloud SQL apply 仍受 production migration gate 管制。 |
| DEV-064 圖號單一資料層與共用明細入口 | `.ai-doc/dev_task.md` (`DEV-064`); authority `.ai-doc/specs/SPEC-PDM-UNIFIED-DRAWING-AGGREGATE-001-single-data-layer.md`; decision `.ai-doc/decisions/ADR-PDM-UNIFIED-DRAWING-AGGREGATE-001-canonical-drawing-and-revision.md`; validation `.ai-doc/qa/qa-dev-064-unified-drawing-aggregate-validation-plan-2026-08-11.md`; QC `.ai-doc/qc/qc-dev-064-unified-drawing-aggregate-report-2026-08-11.md`與DEV-052 QC report的2026-08-15 focused amendment。Status: `Local RD Implemented / Focused Verification Passed / Existing Independent QC Baseline Retained / Production Migration & Release Gated`。此 DEV intentional-replace DEV-052/053 的 candidate→formal 雙物件權威：canonical Drawing/Revision/File 保持 stable identity，workspace 與既有 candidate/formal tables 僅作流程容器／相容投影；workbench只由`drawings`產生stable row，待處理與研發可用共用圖號明細入口；server/DB共同驗證狀態與受控不可變性。2026-08-15 user-visible authority由DEV-064 spec與`projectNumberLifecycleUserView`共同約束：所有preformal／nonterminal舊保留號只顯示「首版準備」，來源state／approval／recovery仍為DEV-052 server/admin evidence；正式／發布／terminal保持真實狀態。focused evidence為DEV-064 8/8、DEV-052 data protection 6/6、UI 17/17與current unified drawer Chromium 7/7；production另受reservation-ID級零遺漏gate約束，unmapped/duplicate/renumbered全為0，cutover freeze期間source-hash-changed為0；未執行production migration/deploy/release。 |
| DEV-105 SolidWorks 3D 預覽首次載入同步、silent gap 與 worker capability CAPA | `.ai-doc/dev_task.md` (`DEV-105`); authority `.ai-doc/specs/SPEC-PDM-SW-NATIVE-PREVIEW-WORKER-001-windows-solidworks-preview-derivatives.md` §0B／§0A；gallery amendment `.ai-doc/specs/SPEC-PDM-WORKBENCH-PREVIEW-GALLERY-001-drawing-part-3d-preview-mode.md` §0R；QA `.ai-doc/qa/qa-dev-105-3d-preview-recovery-validation-plan-2026-08-30.md` §7；CAPA `.ai-doc/qc/qc-dev-105-3d-preview-capa-2026-08-30.md` §7；related `DEV-056/065/087`。Status: `RD Implemented / QA-QC Complete / Effectiveness Reclosed / QA-105-019..030 12 of 12 PASS / Primary Backfill Human-Gated / Production Release Gated`。2026-08-31已修正first-load list convergence、no-source sourceType、binding＋job-intent transaction、pending-only visible polling、strict map/state與卡片內精簡loader/reduced-motion；aggregate=`output/qa/dev-105-3d-preview/DEV105-aggregate-2026-08-31T02-21-16-600Z/aggregate-manifest.json`，指定回歸與primary/cleanup invariant均保留。無新schema/ADR、SSE/WebSocket、primary apply或production deploy/release。 |
| DEV-065 圖號／料號工作台預覽圖模式（新架構恢復） | `.ai-doc/dev_task.md` (`DEV-065`); authority `.ai-doc/specs/SPEC-PDM-WORKBENCH-PREVIEW-GALLERY-001-drawing-part-3d-preview-mode.md`（Phase 1 Drawing §0、Phase 2 Part implementation/result §0.16，best-available amendment §0.16.19）; ADR `.ai-doc/decisions/ADR-PDM-PART-PREVIEW-AUTHORITY-001-part-setting-and-shared-projection.md`; QA `.ai-doc/qa/qa-dev-065-workbench-preview-gallery-validation-plan-2026-08-11.md`（Phase 1 result §0.6、Phase 2 execution §0.7）; QC `.ai-doc/qc/qc-dev-065-part-preview-local-execution-2026-08-24.md`; parent `.ai-doc/specs/SPEC-PDM-STATUS-DATA-REBUILD-001-canonical-workbench-state-and-branching.md`與file authority `.ai-doc/specs/SPEC-PDM-FILE-OWNERSHIP-001-contextual-drawing-part-files-and-3d-reuse.md`。Status: `Phase 1 Drawing + Phase 2 Part Local RD Implemented / SQLite + Browser QA Passed / PostgreSQL Shadow Blocked / Capability Default Off / Production Release Gated`。Part預覽代表Part本身：auto只取direct unique primary manufacturing Drawing，production ready優先，否則取latest open active RD ready；custom使用persistent `part_preview_settings`＋reserved Part-owned `part_preview_image`，custom invalid不silent fallback。Schema 046/DB guards、direct `sharp@0.35.3`／`server-only@0.0.1`、idempotent service/storage、active-delete 409、neutral `previewByRowKey`、same-snapshot bounded query與shared resolver/media/panel/gallery均已實作，沒有新增branch preference或第二套元件。Local evidence為contract 28/28、SQLite 30/30、Chromium 112 checks；A0005-P01四viewport顯示`研發預覽 · A0005-M01 · 0.1`。DEV-087/088 focused regressions與isolated build 126/126先前PASS；PostgreSQL runner因無safe shadow為BLOCKED，不得宣稱full provider QA、activation或release。Relation已由DEV-090退役；production migration/deploy/release另走gate。 |
| DEV-066 三工作台頂部欄一致化與肌肉記憶 | `.ai-doc/dev_task.md` (`DEV-066`); historical authority `.ai-doc/specs/SPEC-UX-PDM-WORKBENCH-TOPBAR-001-unified-toolbar-muscle-memory.md`; supersession guard `scripts/qc-dev-066-workbench-topbar.mjs`; successors `DEV-087`／`DEV-090`／`DEV-112`。Status: `Historical / Superseded by DEV-087/DEV-090/DEV-112 / Shared Mechanics Retained / No Separate Release Target`。舊三工作台 source 已退役；現行 canonical filter／pagination／history由DEV-087承接、Relation matrix由DEV-090承接、Drawing／Part三模式由DEV-112承接。歷史TB與screenshots僅作追溯，不得當作現行分母；successor contract／browser／aggregate才是current evidence。 |
| DEV-067 PDM 統一實體明細投影、審核全景與送審鎖定 | `.ai-doc/dev_task.md` (`DEV-067`); implementation authority `.ai-doc/specs/SPEC-PDM-ENTITY-DETAIL-DRAWER-001-unified-object-detail-contract.md` (`DEV-067 RD Implementation Contract`); architecture `.ai-doc/decisions/ADR-PDM-UNIFIED-ENTITY-DETAIL-PROJECTIONS-001-composer-and-policy.md`; QA `.ai-doc/qa/qa-dev-067-unified-pdm-entity-detail-validation-plan-2026-08-12.md`; data authority `.ai-doc/specs/SPEC-PDM-UNIFIED-DRAWING-AGGREGATE-001-single-data-layer.md`; relation/core/review/lock amendments `.ai-doc/specs/SPEC-PDM-DRAWING-PART-RELATION-VIEW-001-root-drawing-part-relation-list.md`, `.ai-doc/specs/SPEC-PDM-WORKBENCH-CORE-001-shared-read-and-controller-contract.md`, `.ai-doc/specs/SPEC-PDM-APPROVAL-PLATFORM-001-system-approval-platform.md`, `.ai-doc/specs/SPEC-PDM-NUMBER-STATE-FLOW-001-unified-numbering-draft-and-transfer-functional-spec.md`. Status: `Local RD Implemented / Focused Contract + Query + PostgreSQL Concurrency + Authenticated Browser Matrix Passed / Production Release Gated`。三工作台使用同一composer與domain-owned projections；Drawing/Part一般surface依任務刪減，Relation full aggregate／Relation owner與review clauses已由DEV-090標為歷史取代；assigned reviewer仍僅在exact request/company scope內取得Drawing／Part scope。Feature flag default off；query budget、canonical lock order、isolated build、static contract/preflight與三入口／審核 owner route browser matrix已通過；browser evidence為18/18 cases、4 viewport、flag on/off、focus/keyboard/returnTo與console/network/5xx sweep。schema、production、deploy與release仍受 gate 管制。 |
| DEV-072 PDM 四工作台明細動作可發現性與鎖定提示 | `.ai-doc/dev_task.md` (`DEV-072`); authority `.ai-doc/specs/SPEC-PDM-ENTITY-DETAIL-DRAWER-001-unified-object-detail-contract.md`（2026-08-14 amendment）; scoped next-step exception `.ai-doc/specs/SPEC-PDM-NEXT-STEP-UX-001-actionable-state-guidance.md`; Drawing lifecycle compatibility `.ai-doc/specs/SPEC-PDM-UNIFIED-DRAWING-WORKBENCH-001-single-page-lifecycle-workbench.md`; QA `.ai-doc/qa/qa-dev-072-pdm-action-discoverability-ai-real-operation-validation-plan-2026-08-14.md`; QC `.ai-doc/qc/qc-dev-072-pdm-action-discoverability-2026-08-14.md`. Status: `Local RD/QA/QC Complete / Human Confirmed / Production Release Gated`。Drawing／Part／Relation／Approval owner detail共用`pdm-entity-detail.v2` server action truth；applicable action固定可見，locked reason由hover／focus／touch取得，inapplicable與terminal mutation省略。Final AI Chromium run `DEV072-20260814T050039Z-113d57e2`為21/21、13 screenshots、0 unexpected error；403/409旁路資料不變，五種正向mutation exactly once，aggregate與isolated build通過；manifest含實際HEAD與scoped dirty/content SHA-256。 |
| DEV-073 狀態、責任與審核工作項一致性 CAPA | `.ai-doc/dev_task.md` (`DEV-073`); authority `.ai-doc/specs/SPEC-PDM-STATUS-ACTIONABILITY-CAPA-001-state-workitem-consistency.md`; QA `.ai-doc/qa/qa-dev-073-status-actionability-capa-validation-plan-2026-08-14.md`; QC `.ai-doc/qc/qc-dev-073-status-actionability-capa-2026-08-14.md`; evidence `output/qa/dev-073-status-actionability/`; related authorities `SPEC-PDM-STATUS-UX-004`、`SPEC-PDM-ENTITY-DETAIL-DRAWER-001`、`SPEC-PDM-UNIFIED-DRAWING-AGGREGATE-001`、`SPEC-PDM-APPROVAL-PLATFORM-001`. Status: `Local RD/QA/QC Complete / Human Confirmed / Production Release Gated`。A0005-M01與0.2／0.3／0.5已safe repair為effective `rd_controlled`，physical package／review evidence未變；三viewport顯示研發可用與建立新版次／歷史／返回，active inbox正確排除歷史；orphan review fail closed為負責人待確認並提供PDM管理者恢復提示。正式／staging資料、schema、permission與decision authority不在本輪。 |
| DEV-074 料號／圖號全生命週期 AI UI 真實操作驗證 | `.ai-doc/dev_task.md` (`DEV-074`); QA authority `.ai-doc/qa/qa-dev-074-pdm-complete-lifecycle-ui-real-operation-validation-plan-2026-08-15.md`; QC `.ai-doc/qc/qc-dev-074-pdm-complete-lifecycle-ui-real-operation-report-2026-08-15.md`; evidence `output/qa/dev-074-pdm-complete-lifecycle-ui/`; parent authorities `DEV-052/053/060/061/062/064/067/068/070/071/072/073`。Status: `Executed / QC Passed 58/58 / Historical Pre-DEV-087 Baseline / Local Isolated Only`。2026-08-15～16以真實rendered UI執行7個家族、58條in-scope journey，Blocked=0、open P0/P1=0；`B09`、`D15`、`E02`、`F08`與工程內容差異為當期Out of Scope。此結果只證明DEV-087前舊模型，不得替代DEV-087新增的67條UI-only lifecycle與11個triad gates。 |
| DEV-076 還原資料候選關聯自動投影與移轉對帳 | `.ai-doc/dev_task.md` (`DEV-076`); authority amendment `.ai-doc/specs/SPEC-PDM-DRAWING-PART-RELATION-VIEW-001-root-drawing-part-relation-list.md` § DEV-076; validation amendment `.ai-doc/qa/qa-dev-062-unified-part-relation-workbench-validation-plan-2026-08-10.md` § DEV-076；QC `.ai-doc/qc/qc-dev-076-candidate-relation-reconciliation-2026-08-17.md`；evidence `output/qa/dev-076-candidate-relation-reconciliation/DEV076-20260817-182941-staging-readonly/`；parents `DEV-062/064`。Status: `RD/QA/QC Complete / Authenticated Staging Read-only Passed / Production Release Gated`。Commit `89e8023a...` 已部署 staging revision `ai-pdm-stg-dev07689e802`；A0002/A0003/A0004 tree、matrix、API 與 zero-write hash對帳PASS，使用者不需手動轉換。Exact-commit local Chromium覆蓋1440/1024/390，live staging覆蓋authenticated data/UI/API/console；未執行production write/deploy/release。 |
| DEV-077 正式編號草稿作廢與 production lifecycle 收斂 | `.ai-doc/dev_task.md` (`DEV-077`, `RD Implementation Contract`); amended decision `.ai-doc/decisions/ADR-PDM-LIFECYCLE-ACTIONS-001-ui-vocabulary-and-backend-lifecycle.md`; amended authorities `.ai-doc/specs/SPEC-PDM-PRODUCTION-SLICE-001-official-numbering-draft-launch.md`, `.ai-doc/specs/SPEC-PDM-LIFECYCLE-ACTIONS-001-delete-restore-obsolete.md`, `.ai-doc/specs/SPEC-PDM-NUMBERING-004-contextual-numbering-lifecycle-entrypoints.md`; QA `.ai-doc/qa/qa-dev-077-official-numbering-obsolete-production-lifecycle-validation-plan-2026-08-18.md`; browser evidence `output/qa/dev-077-browser/dev-077-desktop.png`、`output/qa/dev-077-browser/dev-077-tablet.png`、`output/qa/dev-077-browser/dev-077-mobile.png`; parents `DEV-040`、`DEV-PDM-NUMBERING-004`、`DEV-PDM-LIFECYCLE-ACTIONS-001`。Status: `RD Implementation Ready / Human Confirmed / RD Implemented / Local-Staging Implementation Complete / Production Release Gated`。`HD-077-01..03` 已採用；server policy、A/B/C env gate、SQLite／PostgreSQL transaction contract、Idempotency-Key、snapshot、stable errors、no-migration判定已收斂；focused contract 12/12、gate 5/5、isolated API 14/14、PostgreSQL concurrency 17/17、authenticated browser 27/27、既有 API 23/23、numbering concurrency 32/32、approval platform 123/123、production-slice 34/34、typecheck／isolated build已完成。Production deployment／traffic／write smoke／rollback仍由 QA／DEV-032 release gate 管控。 |
| DEV-078 固定責任稱謂與六狀態 UI 投影 | `.ai-doc/dev_task.md` (`DEV-078`, Phase 2 RD Implementation Contract); amended authority `.ai-doc/specs/SPEC-PDM-STATUS-UX-004-human-status-projection.md` §18～§19; compatible CAPA amendment `.ai-doc/specs/SPEC-PDM-STATUS-ACTIONABILITY-CAPA-001-state-workitem-consistency.md` §7～§9; amended ADR `.ai-doc/decisions/ADR-PDM-STATUS-UX-004-task-driven-human-status-projection.md`; QA `.ai-doc/qa/qa-dev-078-responsibility-status-vocabulary-validation-plan-2026-08-18.md` §11; parents `DEV-055`、`DEV-073`; lifecycle dependencies `DEV-052`、`DEV-053`。Status: `Phase 1 + Phase 2 Local RD Implemented / Human Confirmed / Full Aggregate QC Passed / Production Release Gated`。Phase 2 visible vocabulary固定為六項；inventory為17 required source＋12 required tests＋`package.json`，共30 direct files，另有4 validation-only source、9 consumer files／13 badge points／5 filter points。shared presentation projector、query／history scope與aggregate prevention gates均已實作並通過；DEV-078 projection 42/42、contract 53/53、browser、parent regressions、typecheck與isolated build均PASS，evidence見SPEC §19.7與QA §11.3.1；不改schema、permission或write flow。 |
| DEV-079 圖號唯讀抽屜與全頁編輯工作區分流 | `.ai-doc/dev_task.md` (`DEV-079`, CAPA-corrected); primary UI／implementation authority `.ai-doc/specs/SPEC-PDM-ENTITY-DETAIL-DRAWER-001-unified-object-detail-contract.md`；recognition authority `.ai-doc/specs/SPEC-PDM-DRAWING-RECOGNITION-001-candidate-review-and-formalization.md` §32；cross-DEV CAPA `.ai-doc/qc/qc-dev-079-dev-101-recognition-owner-review-parity-capa-2026-08-27.md`；QA `.ai-doc/qa/qa-dev-079-drawing-readonly-drawer-fullpage-workspace-validation-plan-2026-08-19.md`。Status: `Local RD Complete / Owner-Review Parity CAPA Local Correction Verified / Focused Provider + Browser QA-QC PASS / QA-079-01～42 Independent Full-Matrix Pending / Verification Only`。Canonical routes、Drawing drawer zero-write與visual-first workspace保留；all-adapter resolver、accepted-state invariant、controlled reconciliation及GET zero-write已在isolated SQLite／PostgreSQL PASS，並與DEV-101 immutable package/hash銜接；primary SQLite guard＋21筆repair已完成。DEV-101 48/48只關閉immutable review parity，不替代QA-079-01～42完整actor／failure matrix；production餘項由DEV-032管理。 |
| DEV-080 全系統第一層狀態可見性與例外分層 | `.ai-doc/dev_task.md` (`DEV-080`, Local Implementation Complete); authority `.ai-doc/specs/SPEC-PDM-STATUS-UX-005-first-layer-status-visibility-policy.md`; QA `.ai-doc/qa/qa-dev-080-system-status-visibility-validation-plan-2026-08-19.md`; parents `DEV-055`、`DEV-078`; related `DEV-049`、`DEV-060`、`DEV-062`、`DEV-068`、`DEV-071`、`DEV-079`。Status: `Local RD Complete / DEV-080 Focused QC Passed / Existing Cross-DEV Baseline Findings Recorded / Independent Aggregate Pending / Verification Only`。每個item第一層維持一個primary＋最多一個最高嚴重度exception；focused gates為projection 15/15、contract 26/26、browser 240/240、DEV-071 browser 56/56、typecheck與124-page isolated build PASS。`qc:dev-060-bom-create` fixture、`qc:dev-068:browser` fixture與`qc:ux-attribute-hierarchy` retired API三項仍須由各owner修正後重跑完整`qc:dev-080`；不得刪assertion或改名為DEV-080缺陷。production餘項由DEV-032管理。 |
| DEV-081 主管與系統管理員跨負責人編輯 | `.ai-doc/dev_task.md` (`DEV-081`); authority `.ai-doc/specs/SPEC-PDM-SUPERVISOR-EDIT-SCOPE-001-manager-admin-nonowner-edit.md`; QA `.ai-doc/qa/qa-dev-081-supervisor-nonowner-edit-scope-validation-plan-2026-08-19.md`; parents `DEV-052`、`DEV-060`、`DEV-062`、`DEV-067`、`DEV-079`; related `DEV-068`。Status: `Local RD Implemented / Human Confirmed / Focused QA Passed / Disposable Mutation QC Passed / Production Release Gated`。`R&D Manager`、`Admin`與`Engineer`可跨負責人編輯同公司Drawing／Part／Relation／BOM，並依action permission取消、撤回未決案、審核與發行；保留company、lifecycle／已決案鎖定、owner identity與audit actor，migration `040`補齊`rd_manager`發行權。Focused contract、typecheck、lint、isolated build、affected regressions、三角色 browser／API mutation matrix與Part三 viewport已PASS；最新 evidence=`output/qa/dev-081-mutation/DEV081-MUT-20260902T174802Z-b73b92b3/manifest.json`，disposable runtime cleanup、browser errors與failed responses均通過。正式環境 migration／deploy／release及未納入runner的完整 cancel／withdraw／review／release／OCR仍受獨立 gate。 |
| DEV-082 PDF OCR 跨來源版次整合與證據定位 | `.ai-doc/dev_task.md` (`DEV-082`, child of `DEV-068`); Current Phase authority `.ai-doc/specs/SPEC-PDM-DRAWING-RECOGNITION-001-candidate-review-and-formalization.md` §0.13; QA `.ai-doc/qa/qa-dev-068-drawing-recognition-validation-plan-2026-08-12.md` §9; QC `.ai-doc/qc/qc-dev-082-browser-pdf-ocr-2026-08-20.md`; related `DEV-035`、`DEV-079`。Status: `RD Implemented / Local QA-QC Complete / OCR-082-001..044 PASS / Production Release Gated`。§0.13以完整文字優先、自適應倍率及同一已載入PDF page proxy高解析crop取代固定3×二次放大預覽canvas；082-I 已實作 target/crop、direct high-res render、single yellow ring、LRU/recovery diagnostics，082-J runner 已補 039..044 斷言。isolated A0002 successor fixture 的 Chromium 1440／1024／390 已證明 `OCR-082-039..044` 全部 PASS、完整 `不鏽鋼SUS304`、coverage `1`、backing scale `2.5`、單一黃色鏡框與無外框螢光筆；最新 contract、repository、synthetic OCR、cross-DEV regression與gate共證44/44 PASS。不新增schema、OCR主機、API key、server OCR compute或第三方流量。 |
| DEV-083 料號／圖料唯讀抽屜與完整 URL 編輯工作區 | `.ai-doc/dev_task.md` (`DEV-083`, RD Implemented); authority `.ai-doc/specs/SPEC-PDM-ENTITY-DETAIL-DRAWER-001-unified-object-detail-contract.md` (`2026-08-20 DEV-083 RD Implementation Contract`); amended ADR `.ai-doc/decisions/ADR-PDM-UNIFIED-ENTITY-DETAIL-PROJECTIONS-001-composer-and-policy.md`; compatible core `.ai-doc/specs/SPEC-PDM-WORKBENCH-CORE-001-shared-read-and-controller-contract.md` §0B; QA `.ai-doc/qa/qa-dev-083-part-relation-readonly-drawer-fullpage-workspace-validation-plan-2026-08-20.md`; QC `.ai-doc/qc/qc-dev-072-pdm-action-discoverability-2026-08-14.md`; parents `DEV-062/067/079`; related `DEV-070/072/081`。Status: `RD Implemented / Focused Contract+API+Authenticated Browser PASS / Disposable Mutation PASS / Typecheck + Affected Lint + Isolated Build PASS / Latest completed aggregate 29/30 PASS with one accepted-superseded parent baseline / DEV-067 parent browser 18/18 PASS / DEV-072 accepted-superseded QC disposition recorded / QA-083-01～24 PASS / QA-083-24 Closed / Production Release Gated`。canonical routes、page frame、candidate／Part／Relation／review owner與drawer zero-write已落實；`qc:dev-083:contract`、`:api`、最新22-check authenticated browser與`qc:dev-083:mutation`已PASS。最新mutation=`output/qa/dev-083-mutation/DEV083-MUT-20260820T115907Z-a9063105/manifest.json`（31/31 result rows PASS，cleanup=removed），以disposable SQLite＋Chromium證實candidate lifecycle/recovery、Part variant、Part／Drawing／Relation Engineer owner/non-owner與Manager／Admin同公司正向、Manufacturing fail-closed、cross-company denial、Relation五種操作與reviewer `needs_info`／reject／approve、scope denial、snapshot drift與retry formalization的exactly-once/readback/audit；QA-083-11/12/13/17/18/19已閉合。另修正cancel payload邊界、Part editor server actionBar gate、審核 retry JSON content type與Part／Drawing same-company resource guard。證據：browser=`output/qa/dev-083-part-relation-fullpage-workspaces/DEV083-20260820T115715Z-6b9c5ec8/manifest.json`；completed aggregate=`output/qa/dev-083-aggregate/DEV083-aggregate-20260820T115712Z-15206e0d/manifest.json`（30 child／29 PASS／1 DEV-072 parent baseline FAIL，`accepted-superseded`）；DEV-072 bounded runner=`output/qa/dev-072-pdm-action-discoverability/DEV072-20260820T120228Z-4a4dff7c/run-manifest.json`保留cleanup與obsolete marker觀測；DEV-067 parent latest=`output/playwright/dev-067-unified-entity-detail/DEV067-20260820T120043Z-e58ce7cb/manifest.json`（18/18、browserErrors=0、failedResponses=0）。Part／Relation candidate、formal、legacy、unified與approval drawer全部zero-write；DEV-072舊action runner保留FK／cleanup與obsolete marker failure，現行DEV-079 contract 22/22、layout 3/3、recognition layout 3/3作replacement evidence，不誤報舊runner PASS；candidate唯一`/numbering/workspaces/[workspaceId]`，formal Part=`/parts/[partId]/workspace`，formal Relation=`/numbering/relations/[rootId]/workspace`，exact reviewer共用`/approvals/[requestId]`。只共用page mechanics與candidate editor，domain editor分離；Drawing不重構。無schema／permission／lifecycle／new write API／new flag；`qc-next-app-runner` readiness probe每次2秒可取消、DEV-072 legacy marker wait限縮5秒但保留舊expected；禁止GenericWorkbench、跨domain command runner、平行candidate URL與drawer/page雙寫。QA-083-24已由QC disposition關閉。 |
| DEV-068 圖面／CAD 全項辨識與人工確認入庫 | `.ai-doc/dev_task.md` (`DEV-068`, parent); child `DEV-082`; authority `.ai-doc/specs/SPEC-PDM-DRAWING-RECOGNITION-001-candidate-review-and-formalization.md`; QA `.ai-doc/qa/qa-dev-068-drawing-recognition-validation-plan-2026-08-12.md`; historical QC `.ai-doc/qc/qc-dev-082-browser-pdf-ocr-2026-08-20.md`與既有 DEV-068 baseline。Status: `Local RD/QA/QC Complete / OCR-082-001..044 PASS / Production Release Gated`。既有candidate、人工核對、impact、atomic formalization、A0005及PDF OCR抽取基線保留；DEV-035、DEV-068、DEV-079 affected regression、typecheck、affected lint、isolated build與DEV-082 gate均通過。production representative accuracy、正式檔案存取、migration、deploy與release仍受獨立 gate 管制。 |
| DEV-035 SolidWorks 原生屬性讀取與辨識診斷可視化 | `.ai-doc/dev_task.md` (`DEV-035 / DEV-CAD-001`); implementation authority `.ai-doc/specs/SPEC-PDM-SOLIDWORKS-METADATA-READER-001-native-property-extraction.md`（§0 Reopen Amendment與§0.4 Draft Population Amendment為現行authority）; QA `.ai-doc/qa/qa-dev-035-solidworks-native-metadata-reader-validation-plan-2026-08-19.md`; QC `.ai-doc/qc/qc-dev-035-a0002-real-repeatability-2026-08-28.md`; fixture expectation `.ai-doc/qa/fixtures/dev-035-a0002-property-expectations.md`; current evidence `output/qa/dev-035-a0002-repeatability/DEV035-A0002-REPEATABILITY-2026-08-28T06-49-41-924Z/manifest.json`; parent `.ai-doc/specs/SPEC-PDM-DRAWING-RECOGNITION-001-candidate-review-and-formalization.md`; worker/credential authorities `ADR-PDM-SW-NATIVE-PREVIEW-WORKER-001`、`SPEC-PDM-GCP-SECRET-MANAGER-001`; relations `DEV-056`、`DEV-058`、`DEV-079`。Status: `Local RD/QA-QC Complete / Current Real A0002 Closure 21/21 PASS / Production Release Gated`。Canonical workspace正常「重新辨識」連續兩次以真實Document Manager reader讀取相同495749-byte／SHA-256來源，均為14 observations、八欄exact且兩份現行projection hash一致；DPAPI、real probe、exact-version heartbeat、completion gate、browser、FK、primary invariant與cleanup皆PASS。未儲存狀態、Add-in、CAD回寫、OCR、2D preview與production release仍排除。 |
| DEV-069 AI-PDM 預上線 GCP 降本與低成本 Staging | `.ai-doc/dev_task.md` (`DEV-069`, merged source); implementation authority `.ai-doc/specs/SPEC-PDM-GCP-COST-OPTIMIZATION-001-prelaunch-runtime-topology.md`; decision `.ai-doc/decisions/ADR-PDM-GCP-COST-OPTIMIZATION-001-prelaunch-firebase-hosting-zonal-micro.md`。Status: `Local RD/QC Closed / Live Execution Merged into DEV-032 / Blocked: Google OAuth + ADC Refresh`。本地topology與targeted QC保留；credentialled plan、Production／Staging apply、Restore delete、DB restart、smoke與帳務readback只由DEV-032派工，DEV-069不再是第二個active release owner。 |
| DEV-070 審核清單共用 PDM 工作台骨架與精確返回 | `.ai-doc/dev_task.md` (`DEV-070`); implementation authority `.ai-doc/specs/SPEC-PDM-APPROVAL-PLATFORM-001-system-approval-platform.md` (`DEV-070 RD Implementation Contract`); shared mechanics `.ai-doc/specs/SPEC-PDM-WORKBENCH-CORE-001-shared-read-and-controller-contract.md`; spatial/UI amendment `.ai-doc/specs/SPEC-UX-PDM-WORKBENCH-TOPBAR-001-unified-toolbar-muscle-memory.md`; QA `.ai-doc/qa/qa-pdm-approval-platform-validation-plan-2026-07-08.md` (`APW-001..028`); related detail authority `.ai-doc/specs/SPEC-PDM-ENTITY-DETAIL-DRAWER-001-unified-object-detail-contract.md` (`DEV-067`)。Status: `Local RD Implemented / Focused Contract + Query + Browser QC Passed / Full APW Matrix Pending / Production Release Gated`。已完成 shared PDM workbench mechanics、六來源 scoped/keyset list、signed bidirectional cursor、exact summary、owner/fallback/unavailable navigation、company predicate、sidebar consolidation與 focused contract/query/navigation/core/regression/typecheck/build/browser evidence；PostgreSQL runtime parity與完整 `APW-001..028` 仍受 gate 管制。無schema/migration、新dependency或env；未stage/commit/merge/PR/deploy/release。 |
| DEV-063 編號、圖號、料號與圖料根號使用者詞彙統一 | `.ai-doc/dev_task.md` (`DEV-063`); current SPEC amendment `.ai-doc/specs/SPEC-PDM-STATUS-UX-003-state-axis-vocabulary-and-header-help.md`; related authority `.ai-doc/specs/SPEC-PDM-UNIFIED-DRAWING-WORKBENCH-001-single-page-lifecycle-workbench.md`, `.ai-doc/specs/SPEC-PDM-NUMBER-STATE-FLOW-001-unified-numbering-draft-and-transfer-functional-spec.md`; ADR `.ai-doc/decisions/ADR-PDM-NUMBERING-IDENTITY-VOCABULARY-001-stored-label-rewrite.md`; QA `.ai-doc/qa/qa-dev-063-numbering-vocabulary-rewrite-validation-plan-2026-08-11.md`. Status: `RD Implementation Ready / Human Confirmed / RD Not Started / Production Release Gated`。使用者已確認 `1C / 2C / 3B / 4A / 5C`；本 phase 不要求稽核證據保存，5C 直接納入人類可讀文字改寫與不可變資料顯示投影，raw audit／snapshot 不覆寫，不改 machine code、ID、hash、enum、權限、API shape 或 authority。2026-08-12 追加：user-facing `主根號` 改為 `圖料根號`，`同主根號料號` 改為 `同根料號`；底層 `reserved`／`candidate`／`official`、舊 query 與資料 authority 保留。 |
...
| Task-driven human status projection, six-state UI vocabulary, viewer actionability and R&D/production availability scope | `.ai-doc/specs/SPEC-PDM-STATUS-UX-004-human-status-projection.md` (`DEV-078` Phase 2 authority §19); `.ai-doc/decisions/ADR-PDM-STATUS-UX-004-task-driven-human-status-projection.md`; `.ai-doc/qa/qa-pdm-human-status-projection-validation-plan-2026-08-06.md`; `.ai-doc/qa/qa-dev-078-responsibility-status-vocabulary-validation-plan-2026-08-18.md` §11; `.ai-doc/qc/qc-dev-055-human-status-projection-2026-08-06.md`; `.ai-doc/dev_task.md` (`DEV-055`, `DEV-078`). DEV-055 status: `Local RD Implemented / QA-QC Passed / Production Release Gated`；DEV-078 Phase 1＋Phase 2：`Local RD Implemented / Human Confirmed / Full Aggregate QC Passed / Production Release Gated`。現行第一層可見語彙與exact file inventory以§19為authority；Phase 2 browser／aggregate evidence見SPEC §19.7與QA §11.3.1。 |
| System-wide first-layer status visibility and exception hierarchy | `.ai-doc/specs/SPEC-PDM-STATUS-UX-005-first-layer-status-visibility-policy.md`; `.ai-doc/qa/qa-dev-080-system-status-visibility-validation-plan-2026-08-19.md`; `.ai-doc/decisions/ADR-PDM-STATUS-UX-004-task-driven-human-status-projection.md` (`DEV-080 amendment`); `.ai-doc/dev_task.md` (`DEV-080`)。此authority只治理presentation visibility，不合併13條資料軸、不改六狀態primary、actionability、permission或lifecycle。`缺製造圖`等阻擋留第一層，`關聯完整`等正常完成訊號預設降層；hover必須同時有focus/click/touch/Escape替代。 |
| Phase 1 native SolidWorks preview auto-orchestration follow-up | `.ai-doc/specs/SPEC-PDM-SW-NATIVE-PREVIEW-WORKER-001-windows-solidworks-preview-derivatives.md`（2026-08-19 Phase 1E authority）; `.ai-doc/decisions/ADR-PDM-SW-NATIVE-PREVIEW-WORKER-001-windows-worker-derivative-boundary.md`; `.ai-doc/qa/qa-pdm-sw-native-preview-worker-validation-plan-2026-07-06.md`（Phase 1E matrix）; `.ai-doc/dev_task.md` (`DEV-056`)。Status: `RD Implementation Complete / Local E2E Verified / P0 / Local Only / Production Release Gated`。舊A0002 `drawing_pdf/queued`／2D `not_configured`僅保留為reopen baseline；現行A0002已由dedicated 2D worker以`native_thumbnail_png` claim並產生current-hash `image/png`，heartbeat `solidworks_2d_preview_png` ready，三viewport browser gate通過，且PNG以image renderer置中填滿預覽舞台。Evidence：`output/qa/dev-056-2d-preview/20260819132108/`、`output/qa/dev-056-2d-preview/20260819135345-browser/`。Phase 2 PDF、interactive 3D、broader `.SLDASM`與production rollout仍排除。 |
...
| DEV-032 Gate A-E canonical production execution sequence | `.ai-doc/dev_task.md` (`DEV-032`); current blocker=`Gate C2 exact 54-migration native separate-target rehearsal`；then D=immutable deploy/authenticated smoke, E=named-user canary；consolidated source sub-gates retain Platform DEV-005/009、DEV-053、DEV-069、DEV-109 exact acceptance |
...
- Local product-slice implementation and the production machine-verifiable baseline are complete, but production release closure is not. Production authority is Firebase Hosting → Cloud Run → Cloud SQL PostgreSQL; GCS is the file authority when enabled, and Supabase is retired/fail-closed. The live revision is `ai-pdm-prod-gh-f70c8982-29636150040`, source `f70c89821b717e6e98e3a6ef855af47e4b4a69dc`, image `sha256:6963bb079a12e3ba973d4b07e0945cd2ee34178de9326f9e5735e3e133a94b91`, with GitHub candidate/canonical smoke 13/13 and 2026-08-11 read-only GCP readback passing. `DEV-032 Gate D` still requires authenticated Level 4 evidence that exactly matches this release; Gate E still requires 3-5 named Wave 0 users and product-owner go/no-go. The only launch-moving boundary remains `DEV-032 Gate A-E`; `DEV-030/031` are its database/data-continuity sub-gates, while DEV-046 Phase 3B+ remains future scope.
- 2026-08-11 DEV-032 production evidence reconciliation: the read-only target preflight and live readback pass against `jenfu-ai-pdm-prod`; Cloud Run, Cloud SQL Regional HA, backup/PITR/deletion protection, private separate restore target, principal, migration/reconciliation and production-slice checks are green. GitHub Actions run `29636150040` and artifact `8427222934` prove exact source `f70c89821b717e6e98e3a6ef855af47e4b4a69dc`, revision `ai-pdm-prod-gh-f70c8982-29636150040`, image `sha256:6963bb079a12e3ba973d4b07e0945cd2ee34178de9326f9e5735e3e133a94b91`, candidate/canonical smoke 13/13 and 100% traffic promotion; OCI labels match the source. Readiness machine gates A0-A7 pass. The prior authenticated Level 4 evidence belongs to historical source `1936e93d` and is intentionally rejected for the current release, leaving A8 plus Wave 0 A9 pending human. Evidence lives in `config/platform/production-activation-evidence.json`, `output/dev-032-production-live-readback/report.json`, `output/dev-032-production-slice-activation/github-f70c8982-release-evidence.json` and `output/dev-032-production-activation-readiness/report.json`.
- 2026-08-26 DEV-032 current PDM candidate boundary: historical Gate C and separate-target restore cover 18 migrations only. The current proposal contains 47 ordered migrations with manifest SHA-256 `27c4d4e6cb85a95b8b63d0fceb9b1b505177fd1880b6f61dc1097e93651ee81f`; masked formal-data transport and disposable PostgreSQL formal-ledger rehearsal pass. Authenticated read-only Cloud SQL discovery now proves the production source is private/RUNNABLE with automated backup, PITR and deletion protection enabled; successful automated backup `1787680800000` completed at `2026-08-25T20:35:02.233Z`, within 48 hours. The historical restore target `ai-pdm-prod-restore-20260716a` now returns 404 and no longer exists, so `output/dev-032-cloudsql-backup-readiness/report.json` remains fail-closed only for a new separate restore target and exact-manifest native backup restore/apply/rerun/reconciliation evidence. The 18-version history must not satisfy this candidate. No production mutation, deploy or activation was performed.
- 2026-08-11 branch predeploy verification: `qc:full` passed 42/42 steps and `qc:industrialization` passed 41/41 steps, including isolated dev and standalone production API/UI/file-integrity execution; target preflight passed 16/16, activation readiness passed 18/18, and the dependency audit reported zero vulnerabilities. Strict production readiness remains intentionally blocked only by current-release authenticated Level 4 and named Wave 0/product-owner evidence; these machine results do not authorize deployment.
...
- `DEV-PDM-UNIFIED-ENTITY-DETAIL-REVIEW-001` / `DEV-067`: `Local RD Implemented / Focused Contract + Query + PostgreSQL Concurrency + Authenticated Browser Matrix Passed / Production Release Gated`。已落地 `UnifiedPdmEntityDetailDrawer`、Drawing/Part/Relation/Review projections、server policy/read facade、request-scoped receipt、review media scope、active-review mutation guards、ownerHref與safe returnTo；`/approvals` enabled path只提供 inbox並導向 owner route。Evidence包含SQLite查詢預算、disposable PostgreSQL row-lock interleavings、isolated build 125/125、affected lint/diff check，以及真實 Chromium 4 viewport × 3 owner entrypoint、review owner route、shared preview、a11y/overflow、focus restore、keyboard、close/Escape/returnTo與flag on/off。schema、production/staging、merge/PR/deploy/release仍gated；詳見 `.ai-doc/dev_task.md` (`DEV-067`)、`.ai-doc/specs/SPEC-PDM-ENTITY-DETAIL-DRAWER-001-unified-object-detail-contract.md`、`.ai-doc/decisions/ADR-PDM-UNIFIED-ENTITY-DETAIL-PROJECTIONS-001-composer-and-policy.md`、`.ai-doc/qa/qa-dev-067-unified-pdm-entity-detail-validation-plan-2026-08-12.md`及 browser manifest。

- `DEV-PDM-DETAIL-ACTION-DISCOVERABILITY-001` / `DEV-072`: `Local RD/QA/QC Complete / Human Confirmed / Production Release Gated`。DEV-067單一composer已升級為`pdm-entity-detail.v2` server action truth，固定capability/action resolver、stable IDs/group/order、nullable唯一primary、typed execution、focusable low-tone locked control、hover/focus/touch reason與fixed action slot均已落地；不適用、跨domain、terminal mutation在payload/DOM省略。Final AI Chromium run 21/21，403/409 direct bypass資料不變，submit/withdraw/needs-info/reject/approve各exactly once，四viewport、cleanup、DEV-067 regressions、typecheck及isolated build均PASS。無schema/migration、新permission/API/flag/dependency；production release仍gated。
...
- `DEV-PDM-ENTITY-DETAIL-DRAWER-001`: Historical Phase 1C shared shell/content record. It is superseded for current cross-domain composition by `DEV-067`; current status and evidence are maintained in the DEV-067 row above. Parent release remains gated; production deploy, merge, PR, rollback and release artifacts remain unauthorized.
...
- `DEV-PDM-BOM-MODULE-ENTRY-001` / `DEV-060`: `Local RD/QA/QC Passed / Commit Pending / Production Release Gated`；三路徑建立入口、組合件來源步驟、canonical Part Number owner、獨立 BOM Rev與review/release/export/read整合已完成。
  User confirmed Scheme B `1A`, material identity rule `2`, and source scope `3B` on 2026-08-10. `/bom/new` 首屏分流已偵測組合件、全新空白 BOM、已有 BOM 草稿；進行中 Draft 不再列為新建候選，空白 BOM 可直接建立，XLS 保留次要入口，組合件仍可進入 CAD/XLS/空白來源步驟。Additive schema/migration、canonical create APIs、permission、idempotency/readback、`draftId` handoff、review/release/export/read整合與isolated QA/QC均已完成；focused QC 50/50、migration baseline 21/21、TypeScript與affected lint通過。未執行live migration、production mutation、production allowlist change或release。
- `DEV-PDM-BOM-VISUAL-EDITOR-002` / `DEV-071`: `Local RD Implemented / AI Real-browser QC Passed / Production Release Gated`。52px／10-slot toolbar、右側 inspector、右下 Map/Outliner＋zoom、XMind 對應 shortcuts、canonical inline picker、Insert Parent/Floating/Group menu、node hover＋、context menu、三區 drop、leaf direct-delete＋Undo、branch confirmation、More lifecycle actions、semantic history，以及可保存／重開的「未納入 BOM」Floating Topic 已實作。最新 `npm run qc:dev-071-browser` 為 56/56、17 screenshots、console error 0、unexpected HTTP 0；獨立 flag-off browser 為 10/10，證明 FF-002 legacy surface、FF-003 blocked handoff、FF-004 409 fail-closed 與 floating graph unchanged。Contract 18/18、API 16/16、migration 21/21、PostgreSQL shadow 27/27 仍保留為 supporting evidence；完整規格與操作界線見 `.ai-doc/qa/qa-dev-071-ai-full-operation-validation-plan-2026-08-13.md`。正式 BOM 仍是嚴格樹；flag 預設關閉，未執行 production rollout。
...
Status: Historical parent record; Relation owner/work/review clauses are superseded by `DEV-090`. Phase 1A shared shell/core sections implemented locally; `DEV-067` cross-domain projection amendment is `Local RD Implemented / Local QA-QC Passed`; `DEV-083` is `RD Implemented / Focused Contract+API+Authenticated Browser PASS / Disposable Mutation PASS / Typecheck + Affected Lint + Isolated Build PASS / Latest completed aggregate 29/30 PASS with one accepted-superseded parent baseline / DEV-067 parent browser 18/18 PASS / DEV-072 accepted-superseded QC disposition recorded / QA-083-01～24 PASS / QA-083-24 Closed`; production release not authorized. The three task entry pages remain separate only for Drawing／Part tasks; current Relation owner surface is the Drawing／Part drawer matrix. Legacy Relation evidence and paths below are historical and must not be used as current route or mutation contract. Latest evidence includes the 22-check browser `output/qa/dev-083-part-relation-fullpage-workspaces/DEV083-20260820T115715Z-6b9c5ec8/manifest.json`, disposable mutation `output/qa/dev-083-mutation/DEV083-MUT-20260820T115907Z-a9063105/manifest.json`（31/31 result rows passed、cleanup=removed）, DEV-067 parent browser `output/playwright/dev-067-unified-entity-detail/DEV067-20260820T120043Z-e58ce7cb/manifest.json`（18/18、browserErrors=0、failedResponses=0）, and latest completed aggregate `output/qa/dev-083-aggregate/DEV083-aggregate-20260820T115712Z-15206e0d/manifest.json`（30 child／29 PASS／1 DEV-072 parent baseline FAIL）. DEV-072 bounded runner `output/qa/dev-072-pdm-action-discoverability/DEV072-20260820T120228Z-4a4dff7c/run-manifest.json`保留cleanup與obsolete marker observation；`qc-next-app-runner` readiness probe每次2秒可取消、legacy marker wait限縮5秒但保留舊expected。QA-083-11/12/13/17/18/19 are evidenced, with QA-083-19 now closed by Engineer owner/non-owner and Manager／Admin three-domain authority readback. DEV-079 contract 22/22、layout 3/3與recognition layout 3/3 are the current replacement evidence, not a rewritten PASS for the legacy runner. `.ai-doc/qc/qc-dev-072-pdm-action-discoverability-2026-08-14.md` records the accepted-superseded disposition and QA-083-24 closure. DEV-079 contract and `typecheck:app` have been rerun PASS. No expected or product data was changed.
...
9. DEV-067 architecture: `.ai-doc/decisions/ADR-PDM-UNIFIED-ENTITY-DETAIL-PROJECTIONS-001-composer-and-policy.md`
10. DEV-067 executable QA: `.ai-doc/qa/qa-dev-067-unified-pdm-entity-detail-validation-plan-2026-08-12.md`
11. DEV-083 RD Implementation Contract: `.ai-doc/specs/SPEC-PDM-ENTITY-DETAIL-DRAWER-001-unified-object-detail-contract.md` § `2026-08-20 DEV-083 RD Implementation Contract`
12. DEV-083 ADR amendment: `.ai-doc/decisions/ADR-PDM-UNIFIED-ENTITY-DETAIL-PROJECTIONS-001-composer-and-policy.md` § `2026-08-20 DEV-083 Amendment`
13. DEV-083 compatible core contract: `.ai-doc/specs/SPEC-PDM-WORKBENCH-CORE-001-shared-read-and-controller-contract.md` §0B
14. DEV-083 QA: `.ai-doc/qa/qa-dev-083-part-relation-readonly-drawer-fullpage-workspace-validation-plan-2026-08-20.md`
...
- Source context can change focus/highlight and server-authorized projection depth, but never object identity, underlying truth or permission. Omitted/summary projections are an explicit task policy, not another object version.
- Drawing owner does not expose Part detail; Part owner does not receive Drawing files/revision detail; Relation exposes the full aggregate; assigned active review exposes the exact reviewed aggregate.
- DEV-083: Part／Relation drawers follow the same browse-only task mode as Drawing; editing moves to canonical full-page URLs while the three list entry pages remain separate.
- DEV-083: one candidate aggregate has one `/numbering/workspaces/[workspaceId]`; formal Part and Relation use separate stable-ID owner routes. Share only the page mechanics and candidate editor body; keep Part attributes, Relation tree/matrix and Drawing preview/OCR domain-owned.
- DEV-083: do not refactor the current Drawing workspace in this phase and do not introduce `GenericWorkbench<T>`, schema-driven editors or a cross-domain action runner.
- DEV-083: Part／Relation approval drawer is also zero-write; `/approvals/[requestId]` becomes the existing-authority, three-domain exact reviewer workspace so the shared drawer does not retain a command runner.
- DEV-083: exact return preserves allowlisted list query/filter/sort/layout/history/cursor/page/detail/selection/focus; route identity or intent never expands permission.
...
- DEV-067 composer/projection/server-policy implementation is `Local RD Implemented / Local QA-QC Passed`; browser manifest and focused DB/build evidence are complete, with production deploy, merge, PR and release still unauthorized.
- DEV-083 documentation and local implementation are complete within the exact 28-source／19-script boundary, dirty-hunk protection, function/type delta, baseline and named runners. Focused contract/API/browser, disposable mutation, typecheck, lint and isolated build are evidenced; QA-083-11/12/13/17/18/19 are closed by mutation evidence including Engineer owner/non-owner and Manager／Admin three-domain authority readback; DEV-067 parent browser is now 18/18 PASS, while DEV-072 remains an explicitly retained `accepted-superseded` legacy runner disposition awaiting independent-QC acceptance and QA-083-24 closure. Commit／merge／PR／deploy／release remain unauthorized.
...
- RD requires a global reviewer permission bypass, cross-domain data owner or snapshot-backed visible object detail.
- RD cannot preserve Drawing preview/attachment/readiness or Part attributes/permitted documents inside their domain-owned projections.
- API returns `full` projection payloads to contexts that should receive `summary/none`, even if the UI hides them.
- Required Relation/review full projections cannot be hydrated in one bounded consistency boundary, or multi-target review has no canonical aggregate/decision scope.
- The implementation would merge the three entry pages instead of unifying object detail.
- DEV-083 would produce multiple candidate URLs, duplicate drawer/full-page command paths, retain any approval drawer decision, add domain conditionals／command href inside the shared page frame, require client-side cursor/data joins, create a universal editor/action runner, or require the current Drawing workspace to be refactored before Part／Relation can ship.
- Any step needs production/Supabase live changes, direct data repair/deletion, merge, PR, rollback or release artifacts.

### DEV-PDM-APPROVAL-PLATFORM-001

Status: Phase 1A-1B local implementation complete; Phase 1C-A reviewer entrypoint consolidation complete; Phase 1C-B legacy redirect implemented and locally verified; Phase 1C-C drawing object pending-review projection implemented and locally verified; Phase 1C-D / `DEV-067` is `Local RD Implemented / Local QA-QC Passed`; transitional adapters, friendly-route delegation and guarded migration dry-run/apply tooling present; release/live migration not authorized. DEV-090 is the current Relation amendment: new Relation matrix edits do not enter `/approvals`, and only Drawing／Part owner drawers provide matrix read/edit. Relation review traces／snapshots remain historical evidence only. It captures the user's 2026-07-08 decision that launch timing is not urgent, stability is preferred, and full-system approval platformization should be completed before launch. The selected architecture is shared approval core plus domain-specific handlers, not fragmented approval islands and not a monolithic all-domain apply module. ADR 002 selected additive v2 platform tables; local implementation added platform schema, APIs, `/approvals`, legacy adapters, friendly decision routes through the platform facade, focused QC, guarded migration reporting/self-test, build and browser evidence. Phase 1C-A now provides one `審核工作台` primary reviewer sidebar entry, a reviewer-role/company-scoped pending badge and workbench filters; Phase 1C-B now redirects legacy reviewer routes into equivalent workbench filter states after adding drawing revision impact review parity; Phase 1C-C now shows compact pending-review cues on affected drawing numbers and attachment revisions. The human-confirmed Phase 1C-D contract intentionally replaces approval-only visible details for Drawing/Part; `/approvals` remains the inbox, while assigned reviewers receive full projections only inside exact active request/company scope plus one context action bar and safe return path. Exact implementation and QA are in DEV-067 master SPEC/QA plan.
...
11. DEV-067 entity-detail UI authority: `.ai-doc/specs/SPEC-PDM-ENTITY-DETAIL-DRAWER-001-unified-object-detail-contract.md`; architecture: `.ai-doc/decisions/ADR-PDM-UNIFIED-ENTITY-DETAIL-PROJECTIONS-001-composer-and-policy.md`; canonical Drawing data authority: `.ai-doc/specs/SPEC-PDM-UNIFIED-DRAWING-AGGREGATE-001-single-data-layer.md`
12. DEV-067 executable QA: `.ai-doc/qa/qa-dev-067-unified-pdm-entity-detail-validation-plan-2026-08-12.md`

Current local implementation delivery:

- `DEV-067 / Phase 1C-D`（Drawing／Part current）：Drawing/Part converge on one `UnifiedPdmEntityDetailDrawer` composer; domain projections remain owner-controlled and assigned active review is a server-scoped full aggregate over the same locked owner data. Relation full-page owner/review is historical and superseded by DEV-090 inline matrix.
- Review context and decision descriptors enter the same projection order and single action bar for Drawing／Part; Relation direct save does not create an approval request or decision.
- Safe same-origin `returnTo` preserves the originating filters and selected request. DEV-090 is the current Relation authority; local implementation is complete, while production/schema/release remains unauthorized.
...
### DEV-PDM-BOM-VISUAL-EDITOR-002

Status: `DEV-071 Local RD Implemented / AI Real-browser QC Passed / Human Confirmed / Production Release Gated`.

Read:

1. `.ai-doc/dev_task.md` entry `DEV-071`
2. `.ai-doc/specs/SPEC-BOM-VISUAL-EDITOR-001-xmind-style-bom-editor.md` sections 7-8, especially section 8 RD Implementation Contract
3. `.ai-doc/qa/qa-dev-071-ai-full-operation-validation-plan-2026-08-13.md` for the current AI-operated, every-feature QC gate
4. `.ai-doc/qa/qa-dev-071-xmind-bom-editor-validation-plan-2026-08-13.md` for historical focused baseline only
5. `.ai-doc/qc/qc-dev-071-xmind-bom-editor-2026-08-13.md` for latest AI real-browser QC evidence and local release gate
6. `.ai-doc/specs/SPEC-BOM-WORKBENCH-001-bom-workbench.md` for current Draft, review, release and export authority
7. `.ai-doc/specs/SPEC-PDM-DETAIL-DRAWER-001-system-detail-drawer-standard.md` only when changing drawer behavior

Human-confirmed decision:

- Floating Topic is required during editing. It belongs to a visibly separate `未納入 BOM` Draft staging area, may survive save/reopen, and becomes canonical only after the user places it into the formal tree.
- Unresolved Floating Topics block review, release and formal export in both UI and server paths. They must never be silently deleted, excluded or assigned a guessed parent.

Execution boundary:

- Local Phase 1A～1D is complete: schema/repository/API, editor core, XMind spatial UI and focused QA/QC all have traceable evidence.
- Preserve `DEV-060` as completed local entry/material-identity/revision integration; DEV-071 is a separate UX enhancement and does not reopen that delivery.
- Formal BOM remains a deterministic, auditable tree. XMind free positioning is only a Draft staging mental model, never canonical BOM order or relationship authority.
- Production/live migration、formal data、stage/commit/merge/PR/deploy/release remain separately gated.

Next step:

- Production adoption requires an explicit release gate: review migration plan, apply live PostgreSQL migration, deliberately activate `PDM_BOM_XMIND_EDITOR_V2_ENABLED`, deploy and run production smoke. None is authorized by this local completion.

Stop if:

- Floating Topics would share ambiguous `parent_line_id = null` semantics with root children, disappear after Draft save, bypass server submission/export gates, lose descendants silently, or cannot participate in deterministic Undo/Redo.
- Implementation would overwrite dirty `db/schema.sql`、`package.json`、`src/app/globals.css`、`src/lib/bom-create-context.ts` or untracked migration 033/034 instead of preserving them hunk-by-hunk.
...
- Current 圖料工作台 flat list repeats root/drawing/part rows and does not show the relationship.
...
### DEV-PDM-WORKBENCH-PREVIEW-GALLERY-001

Status: `Phase 1 Drawing + Phase 2 Part Local RD Implemented / SQLite + Browser QA Passed / PostgreSQL Shadow Blocked / Capability Default Off / Production Release Gated`.

Read:

1. `.ai-doc/dev_task.md` (`DEV-065`).
2. `.ai-doc/specs/SPEC-PDM-WORKBENCH-PREVIEW-GALLERY-001-drawing-part-3d-preview-mode.md` §0 — Phase 1 Drawing product／engineering／file-level RD authority；§0.16 — Phase 2 Part implementation authority與§0.16.18 local result；§0.16H與Historical marker後只保留provenance。
3. `.ai-doc/specs/SPEC-PDM-STATUS-DATA-REBUILD-001-canonical-workbench-state-and-branching.md` — canonical workbench、opaque row key 與 current list/detail authority。
4. `.ai-doc/qa/qa-dev-065-workbench-preview-gallery-validation-plan-2026-08-11.md` §0 — Phase 1 `CPG-001`～`CPG-024` QA authority與§0.6 PASS evidence；§0.7 — Phase 2 `PPC-001..018` execution contract與§0.7.5 local result；Historical marker後的`PG-*`不得當current PASS。
5. `.ai-doc/specs/SPEC-PDM-UNIFIED-DRAWING-AGGREGATE-001-single-data-layer.md` — canonical Drawing/Revision/File identity。
6. `.ai-doc/specs/SPEC-PDM-FILE-OWNERSHIP-001-contextual-drawing-part-files-and-3d-reuse.md` — controlled 3D ownership/reuse。
7. `.ai-doc/specs/SPEC-PDM-SW-NATIVE-PREVIEW-WORKER-001-windows-solidworks-preview-derivatives.md` — PNG derivative/hash/real-worker authority。
8. `.ai-doc/decisions/ADR-PDM-PART-PREVIEW-AUTHORITY-001-part-setting-and-shared-projection.md` — Part preview source、persistent setting、active delete與shared projection decision。
9. `.ai-doc/qc/qc-dev-065-part-preview-local-execution-2026-08-24.md` — current versions/hashes、SQLite/browser/regression evidence、PostgreSQL blocker與release boundary。

Phase 1 Drawing implementation contract：

- Phase 1只處理 canonical Drawing workbench；Relation不納入。
- 保留清單模式並新增預覽圖模式；首訪預設清單，有效 URL `layout` 優先於 Drawing local preference。
- 一個 canonical row 對應一張卡；production 與多筆 RD revision 不得因圖號相同而合併。
- Drawing list新增top-level`preview3dByRowKey`；preview必須使用row exact `revision_id`，並在existing list snapshot以兩個bounded statements批次取得；Drawing list總statements仍`<=12`。
- preview bytes 僅使用 `/api/pdm/file-assets/{fileAssetId}`；缺圖、等待、延遲、失敗或不可用仍保留卡片並可開同一明細抽屜。
- URL/storage key=`pdm-canonical-drawing-layout-v1`、switch replaceState、不refetch、latest-request-wins、append map merge；卡片保留最小資訊及完整keyboard/a11y/RWD。
- 不新增schema/migration/API route/feature flag；舊helper/route/flag/lane/token/status current caller必須為0。

Phase 1 implementation result（2026-08-23）：Phase 1A～1D 已完成；`qc:dev-065:contract` 24/24、`qc:dev-065:browser` 35/35，DEV-087 contract/repository/commands/file-read 分別 31/29/39/193，canonical workbench browser 118/118，typecheck、受影響檔 ESLint、isolated build（125/125）與 diff check 均 PASS。舊 `qc:pdm-entity-detail-drawer` 因期待已退役 `src/lib/pdm-entity-detail.ts` 無法執行，已記錄為 legacy runner disposition，不恢復舊 source；current drawer 已由 current canonical browser evidence 覆蓋。無 schema/migration/route/flag/data/release 變更。

Phase 2 Part RD implementation/result（2026-08-24）：

- Preview語意是Part本身。無custom override時只取direct unique`primary_manufacturing` Drawing；production ready優先，否則取latest open active RD ready。禁止root-min、最近附件、reference Drawing、historical／terminal RD、2D或任意附件fallback。
- 有`numbering.attachments.manage`且same-company owner scope者可在Part drawer上傳PNG／JPEG作custom override；custom標`自訂圖片`、auto依來源標`量產預覽／研發預覽`並帶Drawing/revision。Custom不是CAD、DrawingRevisionFile或approval authority。
- 有效custom優先；custom失效顯示`自訂圖片無法顯示`且不silent fallback。Reset必須是明確、idempotent、可readback command；upload／replace／reset失敗保留原來源。
- Part使用獨立`pdm-canonical-part-layout-v1`，list/gallery共享Part identity、排序、分頁、selection與drawer；formal/work/history同一Part共用entity-level override。
- 元件end-state採「共用穩定mechanics、分離domain policy」：safe `CanonicalPreviewProjection`、protected media/state renderer、single/tabs/grid panel與entity-neutral gallery共用；Drawing保留thin compatibility adapter，Part只新增source-control mutation。
- Part gallery與drawer使用同一PartPreviewResolver/projection；drawer只保留一個主要`料號預覽`、source label與`查看主要製造圖`入口，不常駐重複3D／2D board。真實任務證明需內嵌2D時，才另以`主要製造圖`區塊補回。
- 拒絕複製Part gallery/media、custom-as-Drawing-slot、entityType巨型component、boolean-prop soup與list/detail各自選圖SQL。
- 唯一資料權威為persistent `part_preview_settings`：無row=初始auto；set／replace／reset後保留row與monotonic row version，避免reset ABA。Custom是same-Part active `file_assets`、category=`part_preview_image`；setting才是active authority。
- Set／replace與reset routes、expected row version、Idempotency-Key、canonical receipt、append-only audit、permission/review lock、圖片PNG/JPEG <=10 MiB／64..8192、storage compensation均已固定。
- Active custom遭generic delete固定409 `PART_PREVIEW_ACTIVE_ASSET`；先reset／replace。舊asset不被隱式刪除，restore不重新指定。
- Phase 2原子將Drawing-only `preview3dByRowKey`收斂為neutral `previewByRowKey`；Part list/detail只用同一resolver/projection，query固定增量<=4、list/detail總上限<=14，2C exit由SQLite／PostgreSQL runner量測。
- Exact data/image：SQLite同marker＋PostgreSQL `046_part_preview_settings.sql`，DB trigger守company/custom pointer/active delete；direct pinned `sharp@0.35.3`正規化single PNG/JPEG。`part_preview_image`是dedicated route保留write category，generic附件POST不得寫。
- Capability固定default-off `PDM_PART_PREVIEW_V1`且依賴existing gallery/unified Part flags；rollback保留settings／assets／audit／receipts，不做destructive down migration。

Implementation facts：Human、engineering與local implementation blocker皆為0；`ADR-PDM-PART-PREVIEW-AUTHORITY-001`已Accepted。Schema 046／SQLite marker、image normalization、setting owner、best-available resolver、idempotent mutations、delete guard、neutral DTO、shared components與三支runner均已實作。Local evidence為contract 28/28、SQLite 30/30、Chromium 112 checks；A0005-P01四viewport顯示`研發預覽 · A0005-M01 · 0.1`；DEV-087/088 focused regressions與isolated build 126/126先前PASS；Part list 0/1/20/50 statements=2/7/7/7、detail=13。

Execution boundary：Phase 1／2本機RD已完成；PostgreSQL provider runner因缺safe disposable shadow為BLOCKED，不能宣稱full multi-provider QA或activation ready。Capability維持default off；production migration、deploy、release、stage/commit/merge/PR仍需獨立明確授權。

### DEV-PDM-SW-NATIVE-PREVIEW-WORKER-001

Status: `RD Implementation Complete / DEV-056 Phase 1E Local E2E Verified / P0 / Local Only / Production Release Gated`。舊auto-orchestration與Windows Shell `.SLDPRT` evidence只保留為partial baseline；A0002 `.SLDDRW`已完成UI-only credential hot apply、PNG kind一致、獨立renderer heartbeat與unified detail recovery，receipt與三viewport browser evidence已入庫。
...
- Prior Phase 1 PDM pipeline evidence verifies a fake local PNG worker plus a real Windows Shell worker for `.SLDPRT`; it remains partial baseline context.
- Current Phase 1E receipt proves the persistent Document Manager worker resolves the UI-activated DPAPI/GSM exact version, claims `native_thumbnail_png`, produces a current-hash A0002 `.SLDDRW` PNG and updates the unified workspace automatically.
...
- Local controlled Document Manager worker execution is authorized for Phase 1E evidence；Phase 2 `.SLDDRW -> PDF`, Phase 3 interactive 3D, production worker deployment, production migration/cutover and historical backfill remain unauthorized.
...
DEV-056 Phase 1E local completion receipt (2026-08-19):

- `npm.cmd run qc:dev-056:2d-preview-e2e`: `18/18`; `npm.cmd run qc:dev-056:2d-preview-browser`: desktop/tablet/phone `PASS`; `npm.cmd run qc:dev-035:completion-gate`: `PASS`.
- A0002 `.SLDDRW` source bytes/hash preserved; dedicated worker claim/completion, secure version 3 acknowledgement, ready 640×480 current-hash PNG and no-stuck-processing browser state are recorded under `output/qa/dev-056-2d-preview/20260819132108/` and `output/qa/dev-056-2d-preview/20260819133305-browser/`.
- Temporary worker process was stopped after evidence capture; existing 3000 runtime was preserved. Production deploy/release/migration remains separately gated.
...
5. Implemented surfaces: `src/lib/status-display.ts`, `src/components/status-help-popover.tsx`, `src/app/numbering/tasks/page.tsx`, `src/app/settings/page.tsx`, `src/app/numbering/reports/page.tsx`, `src/app/numbering/approvals/page.tsx`, `src/app/bom/workbench/page.tsx`, `src/components/number-state-workspace.tsx`, `src/app/parts/page.tsx`, `src/app/numbering/drawings/page.tsx`, `src/app/numbering/search/page.tsx`, `scripts/qc-pdm-status-ui-vocabulary.mjs`.
...
- 送審入口保留在圖號 / 圖料工作台；工作台可獨立成 `/drawings/[drawingNumber]/submission-workbench`.
...
- 圖號工作台 remains drawing-focused.
- 圖料工作台 is the root/drawing/part aggregation and submission-preparation workbench.
- Inline editing is allowed in 圖料工作台 but writes must go through owner domain APIs and audit.
...
## 8. DEV-090 supersession boundary

DEV-090 is the current Relation contract. It supersedes DEV-067／DEV-072／DEV-083／DEV-089 clauses that describe a current Relation list, drawer owner, full-page workspace, review route or `relation_change_works`. Those entries remain historical evidence only; current runtime owner surfaces are Drawing／Part drawers plus minimal `/numbering/search`, and the only formal relation writer is `RelationFormalAuthorityRepository`.
DEV-109 source-boundary latest checkpoint（2026-09-01 05:34Z）：DEV-032 commit plan 雙快照為 `stable=false`、included source=`294`、unknown risk=`0`，`safeToStageIncludedSource=false`／`safeToBuildForProduction=false`；commit-plan QC=`12/12`。本段為 point-in-time evidence，任何後續 source 變更均須重新產生 plan；不得以此作 exact release commit 或 production deployment 證據。

DEV-109 source-boundary recheck（2026-09-01 06:16Z）：plan 產生當下雙快照 `stable=true`、included source=`298`、unknown risk=`0`、`safeToStageIncludedSource=true`，但 `safeToBuildForProduction=false`、`exactReleaseCommitExists=false`；與 §31.10 planning ledger 僅 `52/54` 交集，另有 `246` 個非 CAPA paths，缺少 `src/lib/bom-submission-diff.ts` 與 `src/lib/repositories/numbering-repository.ts`。machine receipts=`output/qa/dev-109-capa-production/2026-09-01T0315Z/source-scope-overlap-2026-09-01T0616Z.json`、`release-owner-decision-packet-2026-09-01T0616Z.json`；本行更新後仍須於 scope/hunk 選定時重新產生 exact source boundary，未完成前不得 stage/build/deploy或關閉CAPA。

DEV-109 source-boundary latest recheck（2026-09-01 06:21Z）：雙快照期間 dirty entries `79,727 → 79,729`，故 `stable=false`、`safeToStageIncludedSource=false`、`safeToBuildForProduction=false`；commit-plan QC=`12/12`。並行 CUA/runtime 造成來源未凍結；須待 runtime 停止、scope/hunk 選定後重新產生 exact source boundary，未完成前不得 stage/build/deploy或關閉CAPA。

DEV-109 source-plan root-cause correction（2026-09-01 continuation）：stability gate 改以 release source/classification hashes、included source count 與 unknown-risk count 判定；generated evidence-only dirty count churn 不再被誤判為 source boundary change，仍保留 telemetry。修正後須重新產生 plan、選定 CAPA scope/hunk 並建立 exact commit；未完成前不得 stage/build/deploy或關閉CAPA。

DEV-109 corrected source-plan recheck（2026-09-01 06:34Z）：dirty evidence churn 下 plan `stable=true`、included source=`299`、unknown risk=`0`、`safeToStageIncludedSource=true`，但仍 `safeToBuildForProduction=false`、`exactReleaseCommitExists=false`；§31.10 planning ledger 交集 `52/54`、非 CAPA paths=`247`。scope/hunk selection與exact commit未完成前，不得進入production build/deploy或關閉CAPA；receipts=`output/qa/dev-109-capa-production/2026-09-01T0315Z/source-scope-overlap-2026-09-01T0634Z.json`、`release-owner-decision-packet-2026-09-01T0634Z.json`。
DEV-109 current local regression checkpoint（2026-09-01 05:44Z）：fresh unified aggregate `54/54 PASS`、`missing=[]`，task-owned provider/browser，`productionConnection=false`／`productionWrites=false`；evidence=`output/qa/dev-109-unified/2026-09-01T05-44-34-610Z/aggregate.json`。此證據仍不代表 production CAPA-P01～P04 完成。
