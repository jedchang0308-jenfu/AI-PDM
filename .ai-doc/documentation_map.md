...
| DEV-109 BOM 單一 domain、建立入口與候選料號 | `.ai-doc/dev_task.md` (`DEV-109 / DEV-PDM-BOM-CANONICAL-CREATE-PAGE-001`)；current contract=`.ai-doc/specs/SPEC-PDM-BOM-CREATE-PAGE-001-canonical-entry-and-candidates.md` §29～§34；current ADR=`.ai-doc/decisions/ADR-PDM-BOM-DOMAIN-002-unified-composition-and-deferred-execution-policy.md`；current QA=`.ai-doc/qa/qa-dev-109-unified-bom-domain-validation-plan-2026-09-01.md` §12；CAPA receipt=`.ai-doc/qc/qc-dev-109-capa-2026-09-01.md`；production read-only receipt=`output/qa/dev-109-capa-production/2026-09-01T0315Z/p01-readonly.json`；historical purpose ADR=`.ai-doc/decisions/ADR-PDM-BOM-PURPOSE-001-manufacturing-and-sales-kit-separation.md`；structure evidence=`.ai-doc/decisions/ADR-PDM-PART-STRUCTURE-CLASSIFICATION-001-deferred-exact-part-authority.md` 2026-08-31 amendment；historical QC=`.ai-doc/qc/qc-dev-109-local-completion-2026-08-31.md`。Status: `CAPA Local Corrected / CAPA-P01 Read-only Observed / Production Effectiveness Blocked / Production Release Gated / P1 / High / Lane 3`。正式 readback顯示 migration `053/054`缺少、A0044-P01為`Draft/single_part`且正式庫無active primary exact `.SLDASM`，reconcile exact target=`0`；禁止seed，P02僅NO_OP eligible，P03/P04待clean immutable artifact與activation。A0044的active primary SLDASM早於promotion功能，primary existing-data reconcile未執行；另有drawing-level relation helper／runner／candidate file projection錯加data-layer／lifecycle gate。Upload work helper本身沒有此缺陷。Current contract保留Part structure單一candidate authority，file只提供中性`assembly_file／有組合檔`reason；writer與read projection共享語意不變量，但不強迫共用同一SQL。CAPA-L01～L08已於task-owned SQLite／disposable PostgreSQL／Chromium通過，歷史54/54只作regression baseline。正式環境沿用唯一DEV-032入口與generic provenance／activation／rollback gate；live 054 apply、Part回補與activation須各自明確核准，完成exact dry-run／apply readback、canonical feature smoke及一次Release Capsule排程的read-only effectiveness audit前不得關閉CAPA。DEV-032 source boundary follow-up 已完成 `unknownRisk=0`、source manifest QC 14/14、commit plan QC 12/12；2026-09-01 05:04Z 雙快照產生當下 `stable=true`、`included=269`，但仍未建立 exact release commit，`safeToBuildForProduction=false`；依 SPEC §31.10 現況重算僅 `14/54` planning ledger 相符、`40/54` drift，原 ledger 不得直接建立 release commit，任何後續 source 變更均須重新產生雙快照並由 DEV-032 release owner 重審 scope。 |
...
| DEV-085 三工作台 Excel 式複選篩選器 | `.ai-doc/dev_task.md` (`DEV-085`, RD Implemented / QA-QC Pending)；`.ai-doc/specs/SPEC-PDM-WORKBENCH-MULTISELECT-FILTER-001-excel-style-filter-contract.md`；`.ai-doc/qa/qa-dev-085-workbench-multiselect-filter-validation-plan-2026-08-20.md`；parent `DEV-066`；related `DEV-062`、`DEV-078`、`SPEC-UX-PDM-WORKBENCH-TOPBAR-001`、`SPEC-PDM-WORKBENCH-CORE-001`、`ADR-PDM-WORKBENCH-CORE-001`。Status: `RD Implemented / Contract QC 6/6 / Selection QC 9/9 / Query QC 11/11 / Browser 12/12 / Focused Regressions PASS / QA-QC Pending / Local Only / Production Release Gated`。圖料、圖號、料號12個頂端下拉採 Excel 式草稿／確定複選；explicit `all / none / some` 避免「空值等於全部」，URL 以省略 key／`__none__`／repeated keys 正規化並相容 legacy scalar。共用 selection helper／popover mechanics，domain adapter 保留選項與 SQL；同欄 OR、跨欄 AND、server-side filter-before-limit、canonical cursor hash已實作，並有 selection、disposable query/cursor、四 viewport browser 與 focused regression evidence；剩餘 MSF 個案與 independent QC 仍待完成。無 schema、permission、mutation 或 lifecycle 變更；production release 仍受 gate 控制。 |
...
| DEV-066 三工作台頂部欄一致化與肌肉記憶 | `.ai-doc/dev_task.md` (`DEV-066`); authority `.ai-doc/specs/SPEC-UX-PDM-WORKBENCH-TOPBAR-001-unified-toolbar-muscle-memory.md`; validation `.ai-doc/qa/qa-dev-066-workbench-topbar-muscle-memory-validation-plan-2026-08-11.md`. Status: `RD Implemented / Focused Contract QC 13/13 / Browser Smoke Blocked by auth / Production Release Gated`。三模組固定 filter row、history checkbox/helper 已移除、可用 mode switch trailing、pagination bottom；圖料保留`關係樹／矩陣`，圖號／料號保留`清單／預覽圖`；`history=include` deep link 與 history-only detail 保持相容；純UI shell/DOM/CSS/ARIA 收斂，不改 API、schema、permission、preview authority或資料排序。 |
...
| DEV-079 圖號唯讀抽屜與全頁編輯工作區分流 | `.ai-doc/dev_task.md` (`DEV-079`, CAPA-corrected); primary UI／implementation authority `.ai-doc/specs/SPEC-PDM-ENTITY-DETAIL-DRAWER-001-unified-object-detail-contract.md`；recognition authority `.ai-doc/specs/SPEC-PDM-DRAWING-RECOGNITION-001-candidate-review-and-formalization.md` §32；cross-DEV CAPA `.ai-doc/qc/qc-dev-079-dev-101-recognition-owner-review-parity-capa-2026-08-27.md`；QA `.ai-doc/qa/qa-dev-079-drawing-readonly-drawer-fullpage-workspace-validation-plan-2026-08-19.md`。Status: `Owner-Review Parity CAPA Local Correction Verified / Focused Provider + Browser QA-QC PASS / Independent Full-Matrix QC Pending / Production Release Gated`。Canonical routes、Drawing drawer zero-write與visual-first workspace保留；all-adapter resolver、accepted-state invariant、controlled reconciliation及GET zero-write已在isolated SQLite／PostgreSQL PASS，並與DEV-101 immutable package/hash銜接；primary SQLite guard＋21筆repair已完成。QA-079-01～42完整actor／failure matrix與production effectiveness仍未宣告完成。 |
| DEV-080 全系統第一層狀態可見性與例外分層 | `.ai-doc/dev_task.md` (`DEV-080`, Local Implementation Complete); authority `.ai-doc/specs/SPEC-PDM-STATUS-UX-005-first-layer-status-visibility-policy.md`; amended parent `.ai-doc/specs/SPEC-PDM-STATUS-UX-004-human-status-projection.md` §20; amended ADR `.ai-doc/decisions/ADR-PDM-STATUS-UX-004-task-driven-human-status-projection.md`; QA `.ai-doc/qa/qa-dev-080-system-status-visibility-validation-plan-2026-08-19.md`; parents `DEV-055`、`DEV-078`; related `DEV-049`、`DEV-060`、`DEV-062`、`DEV-068`、`DEV-071`、`DEV-079`。Status: `RD Implemented Locally / Human Confirmed / DEV-080 Focused QC Passed / Existing Baseline Findings Recorded / Production Release Gated`。決策為每個item第一層一個primary＋最多一個最高嚴重度exception；正常／成功／重複訊號降到可及popover／drawer，critical/security/blocking/failed/missing-required不得hover-only。QA矯正後authority為42-route disposition、25 display contexts、13 axes、22 scopes與`58 direct files = 30 source + 27 test/QC + package.json`，另43 validation-only source與1 conditional CSS；數量差異以manifest治理，不改schema、API、permission、assignment、lifecycle或write flow。Focused gates為projection 15/15、contract 26/26、browser 240/240、DEV-071 browser 56/56、typecheck與124-page isolated build PASS；DEV-060/068 fixture與既有UX hierarchy retired API findings留在QA §15，未宣告full cross-DEV aggregate或production/release complete。 |
| DEV-081 主管與系統管理員跨負責人編輯 | `.ai-doc/dev_task.md` (`DEV-081`); authority `.ai-doc/specs/SPEC-PDM-SUPERVISOR-EDIT-SCOPE-001-manager-admin-nonowner-edit.md`; QA `.ai-doc/qa/qa-dev-081-supervisor-nonowner-edit-scope-validation-plan-2026-08-19.md`; parents `DEV-052`、`DEV-060`、`DEV-062`、`DEV-067`、`DEV-079`; related `DEV-068`。Status: `Local RD Implemented / Human Confirmed / Focused QA Passed / Disposable Mutation QC Pending / Production Release Gated`。`R&D Manager`與`Admin`可跨負責人編輯同公司Drawing／Part／Relation／BOM，並依action permission取消、撤回未決案、審核與發行；保留company、lifecycle／已決案鎖定、owner identity與audit actor，migration `040`補齊`rd_manager`發行權。Focused contract、typecheck、lint、isolated build、affected regressions與Manager／Admin／Engineer只讀browser matrix已PASS；正向mutation與audit證據待disposable runtime。 |
...
| DEV-069 AI-PDM 預上線 GCP 降本與低成本 Staging | `.ai-doc/dev_task.md` (`DEV-069`); implementation authority `.ai-doc/specs/SPEC-PDM-GCP-COST-OPTIMIZATION-001-prelaunch-runtime-topology.md`; decision `.ai-doc/decisions/ADR-PDM-GCP-COST-OPTIMIZATION-001-prelaunch-firebase-hosting-zonal-micro.md`; related platform authority `.ai-doc/decisions/ADR-PDM-ERP-PLATFORM-002-google-taiwan-cloud-sql-production.md`, `.ai-doc/specs/SPEC-PDM-ERP-GOOGLE-CLOUDSQL-002-five-year-platform-ontology-roadmap.md`。Status: `RD/QC Local Passed / Exact Commit 1065d4a7 / Blocked: Google OAuth + ADC Refresh / Live GCP Release Gated`。Production 改 `db-f1-micro`／`ZONAL`，Staging 改 Micro／Zonal／按需啟停且保留完整發布驗證鏈；兩環境以 Firebase Hosting `web.app` 為 canonical edge 並移除未使用 ALB；已完成 reconciliation 的 Restore target 在備份與證據確認後刪除。Production live 仍由 DEV-032 作唯一 release 入口。 |
