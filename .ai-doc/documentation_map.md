...
> **2026-09-22 DEV-118 Production current checkpoint（現行）**：118-C owner run `35592944590`已把source `68d019d93d267cf284ea8aba637f305558d9048e`發布為`ai-pdm-prod-face545d349c`，image `sha256:6b803aca8da983b909a05b2a4348f3c14ca47148197c44d71a91762ad81f2ec5`承接100% traffic；Platform／OrgMaster migration、DWD與admission均完成。Firebase `google.com` provider readback=`enabled=true`。Platform correction發布後，受控Workspace帳號由Platform normal entry建立session，Portal→AI-PDM未要求第二次登入，reload後session有效，`/api/admin/accounts`=200且`accounts.lifecycle.manage`由`orgmaster_authority:6 / role-system-admin`允許。這只構成C03-like target partial evidence；LOGIN六案、C01／C02四格、Free、deny-path與global logout仍未完成，因此不宣稱`LIVE_VERIFIED`。下方舊的production NOT_RUN／release pending文字均保留為歷史。

> **2026-09-22 DEV-118 local logout correction（現行、尚未發布）**：Production browser檢查發現已登入側欄帳號入口未結束AI-PDM本地session，無法為C01／C02建立fresh start。Current source已把該入口改為呼叫既有`POST /api/auth/logout`，成功才導向`/login?reason=local-logout`，失敗保留session並顯示錯誤；contract=`14/14 PASS`、real-browser=`33/33 PASS`、`typecheck:app=PASS`，證據run=`DEV118-browser-2026-09-21T22-24-37-833Z`。此source尚未進Production owner release；完整登入分母仍是`0/6 / NOT_RUN`。
...
| DEV-118 平台登入入口與 Google／工號契約 | [任務板](dev_task.md) → [PDM SPEC](specs/SPEC-PDM-PRODUCTION-GOOGLE-SIGN-IN-READINESS-001-provider-capability-and-entrypoint-gate.md)；[118-A QC](qc/qc-dev-118-local-implementation-2026-09-17.md)；[118-B 現行交接審查與歷史](qc/qc-dev-118-platform-handoff-readonly-2026-09-17.md)；[DEV-014 Production runbook](../../Jenfu-Platform/ai-doc/runbooks/DEV-014-production-protected-release.md)。118-A本地補修完成：contract 10/10；2026-09-21 exact snapshot browser 30/30、typecheck、affected lint 0 error及isolated build PASS。118-B owner-native=Platform DEV-014／014-LOGIN＋OrgMaster DEV-049，Local Development Complete／Architecture Finalized LOGIN-R1。118-C owner run `35592944590`已完成exact source／artifact-bound release與conformance receipt，active revision=`ai-pdm-prod-face545d349c`、100% traffic；Firebase Google provider亦已啟用。完成仍以production canonical LOGIN六案與PDM C01／C02為準。 |
| DEV-117 AI_PDM 獨立正式部署 adapter | `.ai-doc/dev_task.md`；SPEC §26；QA §15；registry `.ai-doc/qa/dev-117-current-case-registry.json`；V3 profile `config/release/dev117-ai-pdm-independent-production-v3.json`；workflow `.github/workflows/deploy-ai-pdm-independent-production.yml`；Platform對稱authority `../../Jenfu-Platform/ai-doc/specs/DEV-011-independent-platform-production-deployment-contract.md`。Status: `Architecture Finalized / RD Tech Lead PASS / V3 Owner Implementation Complete / S1B-20 PASS / DEV-012 S1C 8 of 8 PASS / S2 Paused for Operator Re-auth / Production Migration Replay PASS / Candidate、Entrypoint、Traffic NOT_RUN / Production Release Gated`。正式release需fresh clean main source lock、Billing／quota／capacity、Identity／entry baseline、Secret／notification、foundation／APP_INFRA、neutral candidate、DEV-116 R02及`run.app` canonical evidence。 |
