...
| Need | Read |
|---|---|
| DEV-104 BOM 工作台 V2 受控產品結構與精簡編輯重構 | `.ai-doc/dev_task.md` (`DEV-104 / DEV-PDM-BOM-WORKBENCH-V2-001`)。Status: `Brief Ready / Human Direction Captured / Need Human Decisions / RD Not Requested`。本Brief將BOM工作台定位為受控EBOM結構從Draft到Released Snapshot的工作區；保留DEV-095／096 exact Part入口、stable Definition／Revision、shared applicability、exact mapping、immutable evidence與Released Projection。初步建議階層表／Outliner為主、variant inspector情境顯示、diff-first review與desktop主編輯；`HD-104-01..03`尚待人類關閉，因此不取代`SPEC-BOM-WORKBENCH-001`、`SPEC-BOM-VISUAL-EDITOR-001`或`SPEC-PDM-ASSEMBLY-BOM-REBUILD-001`，不可直接派RD。 |
| DEV-103 工作臺抽屜預覽區關聯編輯入口與冗餘提示收斂 | `.ai-doc/dev_task.md` (`DEV-103`)；`src/components/canonical-pdm-workbench.tsx`、`src/components/canonical-preview-panel.tsx`、`src/components/drawing-detail-preview.tsx`；`scripts/qc-dev-090-contract.mjs`。Status: `Local RD Implemented / Focused Contract QC 25/25 / Typecheck + Isolated Build + Browser PASS / Production Release Gated`。只改 presentation/action placement：移除關聯矩陣常駐 helper，將既有「編輯關聯」置於圖號／料號預覽區；矩陣 editing、儲存／取消、API、schema、資料與權限不變。截圖所示「有新版本可用」獨立 modal 不存在於現行 source，沒有建立替代元件。 |
