...
| Need | Read |
|---|---|
| DEV-119／DEV-013 `013-S4-L3-AIPDM-ENV` shared-staging owner release | [任務板](dev_task.md) → [DEV-013 AI-PDM owner capsule](specs/DEV-013-ai-pdm-sso-consumer.md)；[local readiness receipt](qc/qc-dev-119-dev013-staging-release-readiness-2026-09-17.md)；profile `config/release/dev013-ai-pdm-managed-staging.json`；owner-native adapter／CLI／focused tests `scripts/lib/dev013-ai-pdm-managed-staging.mjs`、`scripts/dev013-ai-pdm-managed-staging.mjs`、`scripts/dev013-ai-pdm-managed-staging.test.mjs`；app-owned Infra A `infra/google-cloud/dev-013-l3-ai-pdm`。Status=`READY_FOR_NONPROD_APPLY`；manifest=`jenfu.dev013.l3-managed-staging.v2`／SHA `8d913f22…`，target固定`jenfu-platform-nonprod / asia-east1 / ai-pdm-stg / jenfu_stg`。Infra A只建立repository／evidence bucket／exact IAM並有8-address exact-after plan gate；existing service不納Terraform。Platform current exact read-only preflight確認service／identity存在，但service labels仍不符DEV-013、三組Secret refs仍為`latest`、repository／bucket尚未建立；owner-native numeric pinning candidate與traffic-only activation完成後才可產生bootstrap receipt v2。final owner receipt v2另要求source／digest／etag／SA uniqueId／origin／callback／三組numeric refs／mode=on／100% traffic與rollback-floor exact join。所有雲端mutation／DB migration／L3 browser=`NOT_RUN`。 |
| DEV-118 平台登入入口與 Google／工號契約 | [任務板](dev_task.md) → [唯一 SPEC](specs/SPEC-PDM-PRODUCTION-GOOGLE-SIGN-IN-READINESS-001-provider-capability-and-entrypoint-gate.md)；[118-A QC receipt](qc/qc-dev-118-local-implementation-2026-09-17.md)；[118-B Platform 唯讀交接查核](qc/qc-dev-118-platform-handoff-readonly-2026-09-17.md)。2026-09-17 取代直接 Google 主入口修復，收斂為開發點。118-A PDM 入口已 `Local RD／QA-QC Complete`（contract 10/10、browser 12/12）；118-B 平台雙入口與 non-Google provider-managed 管理員開通流程待 owner 承接，118-C 依既有 SSO／release gate。工號是別名，不是另一套憑證；不新增 PDM provider probe、PDM 自有 password 或 Workflow 契約。 |
