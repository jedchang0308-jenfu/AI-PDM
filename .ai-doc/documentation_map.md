...
- `DEV-PDM-NUMBERING-002`: Implemented / verification passed locally for Phase 1-3 on 2026-07-07 after user RD authorization. New normal records use compact v2 identities `00001`, `00001-P01`, `00001-M01`, `00001-R01`; root remains a reusable design-object root; normal drawing purpose is `M/R`; v1 `MA/OT` and old `D-/P-` rows remain readable/searchable through semantic manufacturing/reference helpers; migration dry-run reporting, import/export samples, downstream manufacturing checks and focused QC are updated. Verified with `tsc`, lint, v2 compact QC, migration dry-run QC, numbering core/API/data/concurrency/draft lifecycle/UI regressions, master attachments, master workbench and Supabase runtime migration QC. Build was blocked by the intentional local-dev guard because an existing Next server was listening on port 3000; no bypass was used. Production cutover, applying v1-to-v2 rewrite, direct DB repair/deletion, project/order/equipment numbering and extra visible category codes remain unapproved.
...
Status: Implemented / verification passed locally for Phase 1-3. Phase 4 production cutover and applying any v1-to-v2 rewrite remain approval-gated.
...
- Local Phase 1-3 implementation is complete.
- Stop if work needs production migration/cutover, applying v1-to-v2 rewrite, direct data repair/deletion, project/order/equipment numbering or more visible category codes.
