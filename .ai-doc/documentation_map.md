...
| Need | Read |
|---|---|
| DEV-119／DEV-013 `013-S4-L3-AIPDM-ENV` shared-staging owner release | [任務板](dev_task.md) → [DEV-013 AI-PDM owner capsule](specs/DEV-013-ai-pdm-sso-consumer.md)；[local readiness receipt](qc/qc-dev-119-dev013-staging-release-readiness-2026-09-17.md)；profile `config/release/dev013-ai-pdm-managed-staging.json`；adapter `scripts/lib/dev013-ai-pdm-managed-staging.mjs`；CLI／focused tests `scripts/dev013-ai-pdm-managed-staging.mjs`、`scripts/dev013-ai-pdm-managed-staging.test.mjs`。Status=`READY_FOR_NONPROD_APPLY`；target固定`jenfu-platform-nonprod / asia-east1 / ai-pdm-stg / jenfu_stg`。Platform current exact read-only preflight確認service／identity存在，但service仍為`dev_id=dev-010`且缺DEV-013 `owner／slice` labels，bootstrap receipt fail closed；owner-native correction後才可產生`TARGET_BOOTSTRAP_READY`。final receipt另要求source／digest／etag／SA uniqueId／origin／callback／mode=on／100% traffic與rollback-floor exact join。Cloud revision／traffic mutation、Terraform／DB migration／L3 browser=`NOT_RUN`。 |
