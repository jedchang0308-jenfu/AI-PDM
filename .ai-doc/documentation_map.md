...
> **2026-09-04 Platform DEV-010 physical-topology successor（現行）**：AI-PDM `DEV-032`第一版production release維持已完成，但三系統共用資料庫的physical project／instance／database、schema placement與cross-app role契約改由Platform [DEV-010 direct spec](../../Jenfu-Management-system/ai-doc/specs/DEV-010-three-system-database-consolidation-contract.md) 接管。Platform `010-N1A`、三repo `010-N2`與managed non-production `010-N1B`均已完成；AI-PDM N2D已對齊`6a8e4617128bee649d9be7433586608b50223498`的64-input baseline，migration 054明確退役，042只作fresh derived-baseline overlay，並保留`062_dev010_neutral_schema_boundary.sql`、fresh／existing-history runner、pool max 8、fixture hashes與48／48 aggregate gate。Current production target=`db-custom-1-3840 / ZONAL_DEDICATED / USD 100`；Platform capacity v2／preflight v3／release-adapter v2已前向更新，unit `5/5 + 15/15 + 11/11`及三項focused QC PASS。舊`jenfu-ai-pdm-prod / ai-pdm-prod-postgres / ai_pdm`及`DEV-032`證據是第一版provenance，不代表neutral topology終態；current R1仍有9項human／target／capacity blocker及15案provider execution。Production migration、deploy、DNS、authority／traffic switch仍只能由獨立`010-R1` release gate啟動；AI-PDM write smoke另要求Platform R1-07與DEV-116 R02共用exact company-smoke receipt。
...
| Need | Read |
|---|---|
| DEV-116 Production Level 4 smoke tenant隔離與可稽核證據關卡 | `.ai-doc/dev_task.md` (`DEV-116 / DEV-PDM-PRODUCTION-SMOKE-TENANT-ISOLATION-001`)；focused SPEC `.ai-doc/specs/SPEC-PDM-PRODUCTION-SMOKE-TENANT-001-level4-isolation-and-evidence.md`；fixed QA `.ai-doc/qa/qa-dev-116-production-smoke-tenant-isolation-validation-plan-2026-09-04.md`；machine registry `.ai-doc/qa/dev-116-current-case-registry.json`；R01 report `.ai-doc/reports/pm/pm-dev-116-r01-production-release-preflight-2026-09-04.md`；release capsule=`output/production-release/REL-116-20260904/capsule.json`。Status: `Local 31/31 PASS / R02 Receipt Contract 5/5 PASS / DEV-010 N2 clean 48/48 PASS / 116-R R01 Started but BLOCKED / Production Read-only Preflight Complete / R02-R03 NOT_RUN`。Production仍為legacy `public` ledger 53／056，DEV-010 roles/contracts/062不存在；legacy migration package為54支且硬排除DEV-010-owned 062。Legacy `db-f1-micro／USD 300`只描述DEV-032單系統現況，不是DEV-010 neutral authority，禁止原地改tier／budget冒充整併或形成無限期雙重成本。Current ZONAL machine contract focused QC已完成；neutral identity、RTO／RPO、owners／maintenance、R1-11 numeric capacity及production 15案provider execution仍是blocker。R1-07的AI-PDM子流程與R02必須共用exact company-smoke receipt；本機verifier已能fail closed驗章並輸出Platform projection，但authenticated candidate execution仍未執行。Workflow固定`prepare→candidate→promote`；R04 smoke增量成本upper bound=`USD 0.020812/run`、10 runs=`USD 0.208120/month`、fixed SKU=0。Production writes/deploy/traffic/principal provisioning均為0。 |
