...
| Need | Read |
|---|---|
| DEV-087 三工作臺狀態資料重建、Drawing最多三個研發分支與人類語意極簡整頓 | `.ai-doc/dev_task.md` (`DEV-087`)；SPEC `.ai-doc/specs/SPEC-PDM-STATUS-DATA-REBUILD-001-canonical-workbench-state-and-branching.md`；ADR `.ai-doc/decisions/ADR-PDM-STATUS-DATA-REBUILD-001-single-current-state-authority.md`；QA `.ai-doc/qa/qa-dev-087-status-data-rebuild-validation-plan-2026-08-21.md`；cross-spec amendments `SPEC-PDM-APPROVAL-PLATFORM-001`、`SPEC-PDM-ENTITY-DETAIL-DRAWER-001`與`SPEC-PDM-LIFECYCLE-ACTIONS-001`。Status: `RD Implementation Ready (RD Supervisor Reviewed) / Human Confirmed / RD Not Started / Local Phase 1A-1D Not Requested This Turn / High-Risk Data Migration Not Authorized`。Target authority重建唯一canonical current state：Drawing為0/1列production＋最多3個open hidden branch各一列latest RD（同組最多四列、不拆頁）；Part／Relation為formal 0/1＋專用work 0/1。Cloud SQL migration固定`042_status_data_rebuild.sql`（041屬DEV-084），SQLite使用`ensureDev087CanonicalWorkbenchSchema`；exact schema/DTO/routes/modules/query budget/converter已封口。current work固定使用`drawing_revision_works／part_change_works／relation_change_works`，legacy mixed workspace只作conversion source。DEV-087 review使用transient `pdm_work_review_requests` adapter，不寫會永久保存reviewer/decision/comment的`approval_platform_decisions`；永久只留minimal trace。revision以server tuple計算、global claim防重，stale-base branch只可續minor；latest approved idle RD可`申請作廢`，核准後關閉branch並釋放cap。Manufacturing只讀；延續same-company non-owner edit scope；Part附件維持DEV-084 live例外。Cutover採freeze/drain/fence/RPO=0 relational rollback/same-window retirement；physical bytes通過零引用、approved-artifact與canonical-only gate後永久刪除且無restore。與過去衝突以DEV-087新決策為主；direct active authorities已加supersession boundary，安全可拆的舊current-state/filter/projection/command/fallback在同一DEV拆除，舊QA只作activation前歷史證據。固定retirement hash-chain證據不可缺；QA=`QA-087-001..120`。DEV-086在activation前仍是runtime baseline；文件達implementation readiness不等於已實作、QA PASS或授權migration/delete/DROP/deploy/release。 |
| DEV-086 三工作台量產／研發最新版雙列投影 | `.ai-doc/dev_task.md` (`DEV-086`, CAPA Corrective Implementation Verified / Local QA-QC PASS)；`.ai-doc/specs/SPEC-PDM-WORKBENCH-PRODUCTION-RD-LANES-001-dual-latest-projection.md`；`.ai-doc/decisions/ADR-PDM-WORKBENCH-PRODUCTION-RD-LANES-001-dual-lane-authority.md`；`.ai-doc/qa/qa-dev-086-production-rd-lanes-validation-plan-2026-08-20.md`；CAPA `.ai-doc/qc/qc-dev-086-dual-lane-completion-capa-2026-08-21.md`。Status: `Current Runtime Baseline / CAPA Corrective Implementation Verified / Local QA-QC PASS / Production Release Gated / DEV-087 Supersession Pending`。目前runtime仍以每group最多production／RD各一列解決V1被active V1.1遮蔽；2026-08-21 evidence與manifest保留。DEV-087 activation後，Drawing的「最多雙列／單一RD aggregate／RD只代表active work」會被多open-branch latest contract有意取代；Part／Relation最多formal/work兩列仍保留。不得把DEV-087文件完成誤報為runtime已切換。 |
| DEV-084 替代料號附件人工沿用與安靜選擇 | `.ai-doc/dev_task.md` (`DEV-084`, RD Implementation Ready)；`.ai-doc/specs/SPEC-PDM-PART-ATTACHMENT-REUSE-001-replacement-snapshot-and-part-lock.md`；`.ai-doc/decisions/ADR-PDM-PART-ATTACHMENT-REUSE-001-snapshot-reference-and-whole-part-lock.md`；`.ai-doc/qa/qa-dev-084-part-attachment-reuse-and-lock-validation-plan-2026-08-20.md`；parent `DEV-061`；related change-control與material-identity authority。Status: `RD Implementation Ready / Human Confirmed / RD Not Started / Local First / Production Release Gated`。來源active direct part attachments預設全選、人工取消／新增、完整source token重驗，Drawing/Revision為0且UI無件數摘要／source badge／風險卡。Exact contract固定5-table content-binding-version-origin-lease模型、legacy ID backfill、Cloud SQL `041`＋SQLite ensure、same-company deterministic content ingestion、JSON/multipart route wire、5m/60s/15m lease＋fencing、interactive/controlled writer ledger、draft→formal atomic promotion、PartAttachmentPanel、feature flag/rollback、Phase 1A～1E及QA-084-01～40。料號附件在既有auth/company/visibility內不設專屬permission／confirmation／approval，所有生命週期可維護；Drawing authority、Part identity、release approval、`item_locks`與Supabase退役邊界不變。RD可由本機Phase 1A開始，production migration/enable/deploy/release未授權。 |
...
| DEV-065 圖號／料號 3D 預覽圖模式 | `.ai-doc/dev_task.md` (`DEV-065`); authority `.ai-doc/specs/SPEC-PDM-WORKBENCH-PREVIEW-GALLERY-001-drawing-part-3d-preview-mode.md`; validation `.ai-doc/qa/qa-dev-065-workbench-preview-gallery-validation-plan-2026-08-11.md`. Status: `RD Implemented / Focused Contract QC 10/10 / Browser Smoke Blocked by managed auth / Production Migration & Release Gated`。Human decisions `1A/2A/3A`：Part只取同canonical root最小drawing sequence之代表圖最新版3D；來源缺圖時保留卡片且不使用下一張/舊版/2D；首訪list、每模組各自記憶、URL layout優先。Part card必須顯示`代表圖`；Drawing/Part共用segmented switch/gallery機制但source resolver維持domain adapter。ADR not required；本機產品實作與031 artifact已完成，real Chromium需補 managed bootstrap auth 後再關閉 `PG-001`～`PG-014`。 |
...
| DEV-074 料號／圖號全生命週期 AI UI 真實操作驗證 | `.ai-doc/dev_task.md` (`DEV-074`); QA authority `.ai-doc/qa/qa-dev-074-pdm-complete-lifecycle-ui-real-operation-validation-plan-2026-08-15.md`; planned evidence `output/qa/dev-074-pdm-complete-lifecycle-ui/`; parent authorities `DEV-052/053/060/061/062/064/067/068/070/071/072/073`。Status: `QA Plan Ready / Workflow-only Scope Frozen / UI Execution Not Started / Local Isolated Only`。依使用者決策排除舊保留號，盤點 7 個家族、58 條 in-scope UI journey；`B09`、`D15`、`E02`、`F08` 與工程內容差異列為本輪 Out of Scope。所有 business mutation、SW 上傳、審核、發布、取消、作廢與 cleanup 只能由真實 rendered UI 操作，direct API／DB write、fixture injection、status repair 與測試 helper 禁止；本輪不執行 `apply_failed / ReleaseFailed` recovery。完整 PASS 必須 58/58、Blocked=0、P0/P1=0；目前尚未執行。 |
...
- `DEV-067 / Phase 1C-D`: Drawing/Part/Relation converge on one `UnifiedPdmEntityDetailDrawer` composer; domain projections remain owner-controlled, Drawing/Part normal surfaces are reduced, Relation is full, and assigned active review is a server-scoped full aggregate over the same locked owner data.
- Review context and decision descriptors enter the same projection order and single action bar; snapshot is evidence only, while approval request and decision authority remain in the platform core.
- Safe same-origin `returnTo` preserves the originating filters and selected request. The same DEV is RD Implementation Ready; local Phase 1A～1D is eligible, while production/schema/release remains unauthorized.
...
Status: `RD Implementation Ready / Human Confirmed / RD Not Started / Production Migration & Release Gated`.

Read:

1. `.ai-doc/dev_task.md` (`DEV-065`).
2. `.ai-doc/specs/SPEC-PDM-WORKBENCH-PREVIEW-GALLERY-001-drawing-part-3d-preview-mode.md` — 唯一產品／工程 authority。
3. `.ai-doc/qa/qa-dev-065-workbench-preview-gallery-validation-plan-2026-08-11.md` — `PG-001`～`PG-014` validation authority。
4. `.ai-doc/specs/SPEC-PDM-WORKBENCH-CORE-001-shared-read-and-controller-contract.md` — shared read/controller boundary。
...
- Part = same canonical root → minimum numeric drawing sequence → that drawing's latest valid revision primary 3D.
- Missing representative/latest 3D remains missing; never use next drawing, old revision or 2D.
- First visit list; Drawing/Part remember separately; valid URL layout wins.
- Part card permanently labels the source as `代表圖`.

Executable boundary: local Phase 1A～1D, additive SQLite index and PostgreSQL 031 artifact, focused/isolated QA-QC. Stop for live migration/backfill, preview regeneration, new authority/cache table, interactive 3D, Relation scope, production/deploy/release or per-row query.
