...
| Need | Read |
|---|---|
| DEV-087 三工作臺狀態資料重建、Drawing最多三個研發分支與人類語意極簡整頓 | `.ai-doc/dev_task.md` (`DEV-087`)；SPEC `.ai-doc/specs/SPEC-PDM-STATUS-DATA-REBUILD-001-canonical-workbench-state-and-branching.md`；ADR `.ai-doc/decisions/ADR-PDM-STATUS-DATA-REBUILD-001-single-current-state-authority.md`；主QA `.ai-doc/qa/qa-dev-087-status-data-rebuild-validation-plan-2026-08-21.md`；UI-only執行子契約 `.ai-doc/qa/qa-dev-087-ui-only-lifecycle-operation-validation-plan-2026-08-22.md`；QC `.ai-doc/qc/qc-dev-087-local-implementation-2026-08-22.md`、`.ai-doc/qc/qc-dev-087-local-preservation-migration-2026-08-22.md`。Status: `Local RD Complete / Focused QA-QC PASS / Local Preservation Migration Validated / Retirement Pending / UI-only Full Lifecycle Plan Ready / Production Migration & Release Gated`。14-table canonical schema、042/SQLite ensure、converter、三domain repository/commands/routes/UI、Drawing exact revision file/recognition、transient review與hard retirement已完成本機實作。使用者選擇A後，`output/qa/dev-087-local-preserve-apply/manifest.json`證明56筆legacy source graph保留、`retainedLegacy=56`、`unresolved=0`、authority canonical-only、integrity/foreign-key/API smoke PASS；source不進canonical list。完整UI操作契約仍要求AI只從rendered UI產生business mutation，API／DB只做唯讀取證；67條journey＋11個triad gates、完整`QA-087-001..165`、PostgreSQL rehearsal、SCALE-10K／soak／RTO、retirement與production gate仍受獨立執行約束；紅隊防作弊範圍依使用者決策延後。 |
