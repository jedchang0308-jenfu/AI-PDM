...
9. Since 2026-08-29, named-user Wave 0 testing and waiver references are retired from the active production release contract. Keep exact artifact, zero-traffic candidate, authenticated Level 4, zero open P0/P1, rollback readiness, Product Owner GO, explicit traffic promotion and canonical post-promotion smoke. Production identity allowlisting remains a separate security control.
...
- Local product-slice implementation and the production machine-verifiable baseline are complete, but each production release still needs exact candidate-bound closure. Production authority is Firebase Hosting → Cloud Run → Cloud SQL PostgreSQL; GCS is the file authority when enabled, and Supabase is retired/fail-closed. `DEV-032 Gate D` requires authenticated Level 4 evidence that exactly matches the current release. Since 2026-08-29, Gate E no longer requests 3–5 named Wave 0 users or waiver evidence; it requires zero open P0/P1, rollback readiness, Product Owner `go`, exact promotion approval and canonical post-promotion smoke. The only launch-moving boundary remains `DEV-032 Gate A-E`; `DEV-030/031` are its database/data-continuity sub-gates, while DEV-046 Phase 3B+ remains future scope.
