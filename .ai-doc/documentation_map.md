...
| Need | Read |
|---|---|
| DEV-087 三工作臺狀態資料重建、Drawing最多三個研發分支與人類語意極簡整頓 | `.ai-doc/dev_task.md` (`DEV-087`)；SPEC `.ai-doc/specs/SPEC-PDM-STATUS-DATA-REBUILD-001-canonical-workbench-state-and-branching.md`；ADR `.ai-doc/decisions/ADR-PDM-STATUS-DATA-REBUILD-001-single-current-state-authority.md`；QA `.ai-doc/qa/qa-dev-087-status-data-rebuild-validation-plan-2026-08-21.md`；QC `.ai-doc/qc/qc-dev-087-local-implementation-2026-08-22.md`。Status: `Local RD Complete / Focused QA-QC PASS / Source Conversion Quarantined (44) / Production Migration & Release Gated`。14-table canonical schema、042/SQLite ensure、converter、三domain repository/commands/routes/UI、Drawing exact revision file/recognition、transient review與hard retirement已完成本機實作。`npm run qc:dev-087` 8/8 gate PASS（170項focused checks＋typecheck＋isolated build）；latest browser manifest為`output/qa/dev-087/DEV087-2026-08-21T18-55-53-404Z/manifest.json`。來源DB唯讀dry-run deterministic repair A0005並辨識9筆part-only與3筆cancelled可清理，但44筆active legacy bundle無唯一lineage而QUARANTINE；未apply、未切authority、未DROP或release。完整`QA-087-001..165`、PostgreSQL rehearsal、SCALE-10K／soak／RTO與production migration仍受獨立gate約束；紅隊防作弊範圍依使用者決策延後。 |
...
| DEV-088 替代料號附件人工沿用與安靜選擇 | `.ai-doc/dev_task.md` (`DEV-088`, Next After DEV-087)；歷史來源`DEV-084`及其SPEC `.ai-doc/specs/SPEC-PDM-PART-ATTACHMENT-REUSE-001-replacement-snapshot-and-part-lock.md`、ADR `.ai-doc/decisions/ADR-PDM-PART-ATTACHMENT-REUSE-001-snapshot-reference-and-whole-part-lock.md`、QA `.ai-doc/qa/qa-dev-084-part-attachment-reuse-and-lock-validation-plan-2026-08-20.md`。Status: `待排 / Follows DEV-087 / Rescope Required / P1 / RD Not Eligible`。核心產品意圖是來源有效料號附件預設全選、可取消／新增、圖號檔案不進清單；獨立於DEV-087計入交付。只有DEV-087本機產品實作、獨立QA／QC完成且現行attachment authority穩定後，才重新盤點最小scope並至少達到`RD Contract Ready`。原DEV-084五表content-binding-version-origin-lease、041、權限改寫、所有生命週期自由維護、whole-part lease、Phase 1A～1E與QA-084-01～40只供歷史參考，不自動繼承；041編號保留由DEV-088持有但尚未授權建立。現行料號附件authority與`numbering.attachments.manage`繼續有效。 |
