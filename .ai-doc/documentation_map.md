...
| Need | Read |
|---|---|
| DEV-087 三工作臺狀態資料重建、Drawing最多三個研發分支與人類語意極簡整頓 | `.ai-doc/dev_task.md` (`DEV-087`)；SPEC `.ai-doc/specs/SPEC-PDM-STATUS-DATA-REBUILD-001-canonical-workbench-state-and-branching.md`；ADR `.ai-doc/decisions/ADR-PDM-STATUS-DATA-REBUILD-001-single-current-state-authority.md`；主QA `.ai-doc/qa/qa-dev-087-status-data-rebuild-validation-plan-2026-08-21.md`；UI-only子契約 `.ai-doc/qa/qa-dev-087-ui-only-lifecycle-operation-validation-plan-2026-08-22.md`；QC `.ai-doc/qc/qc-dev-087-new-architecture-zero-loss-local-2026-08-23.md`；CAPA `DEV-092`、`DEV-094`。Status: `Local QA-QC Restored by DEV-094 / Fresh Aggregate 16 of 16 PASS / CAPA Effective / Production Release Gated`。既有歷史evidence保留；current closure固定使用DEV-094主檔復原、startup/build isolation、pre-seed invariant與fresh aggregate證據。 |
...
| DEV-096 組立件情境式共用 BOM 重建 | `.ai-doc/dev_task.md` (`DEV-096`)；SPEC `.ai-doc/specs/SPEC-PDM-ASSEMBLY-BOM-REBUILD-001-contextual-shared-structure.md`；入口ADR `.ai-doc/decisions/ADR-PDM-ASSEMBLY-MASTER-ENTRY-001-canonical-workbenches-only.md`；共用結構ADR `.ai-doc/decisions/ADR-PDM-BOM-STRUCTURE-SHARING-001-variant-part-applicability.md`；QA `.ai-doc/qa/qa-dev-096-assembly-shared-bom-rd-contract-validation-plan-2026-08-24.md`。Status: `RD Implementation Ready / Human Confirmed / Local Implementation Eligible / RD Not Started / Production Release Gated`。不新增入口；Current Phase以Part Number structure type與Part drawer唯一action承接manufactured assembly＋primary M。repo-specific authority固定Definition、stable logical line、Definition/Draft Parent bindings、component candidates/exact selections、schema-v2 immutable review/release parent/resolved evidence與migration issues；SQLite initializer=`ensureDev096SharedAssemblyBomSchema`、PostgreSQL=`048_shared_assembly_bom.sql`、flag=`PDM_ASSEMBLY_SHARED_BOM_V1` default-off。初版／下一版共用single writer，latest base clone保留Parent superset；one-open Revision、archive／restore／whole obsolete、manual active退役、Definition權限、canonical approval source、audit/no-outbox、list/diff/query bounds及完整UI state已固定。096-A～E與QA-096-001..088七runner/evidence gate可直接派工；`.SLDASM` parser、purchased assembly及Released Parent移除／detach／fork仍為Future Phase。產品／schema／API／資料尚未實作或遷移。 |
...
| DEV-093 Canonical 統一建立編號 | `.ai-doc/dev_task.md` (`DEV-093`)；SPEC `.ai-doc/specs/SPEC-PDM-CANONICAL-NUMBER-CREATION-001-unified-contextual-create.md`；QA `.ai-doc/qa/qa-dev-093-canonical-number-creation-validation-plan-2026-08-24.md`。Status: `Local RD Implemented / Corrective QA-QC Passed / Production Release Gated`。單一`/numbering/create`保留；新圖料不再顯示`建立內容`，依圖製作件固定建立M圖＋料號，外購標準件預設料號且可選參考圖R；existing-root仍保留料號／圖號／兩者，追加料號時四項根號既有料件profile均唯讀沿用，只顯示`料件設定（沿用根號）`。主要名詞、含系列代號的建議品名、確定品名、相似候選、條件先行與查重鄰近呈現均保留。Phase 093-H將`自訂規格`與命名用`特性／規格型號`收斂為單一可見來源，Phase 093-I將共用件改為只勾選、不需原因，Phase 093-K將existing-root四項料件設定收斂為server profile authority，同一規格值進入建議品名、request與DB。QA-093-001..109兩輪fresh UI session完成115項check全數PASS，證據為`output/qa/dev-093/DEV093-2026-08-24T11-51-41-869Z/`、response 577；六條合法業務路徑、三種非法組合、DB/API/UI一致、legacy caller=0。人類料件類型仍只有`依圖製作件`／`外購標準件`，共用件獨立且無原因欄位；不新增schema，不恢復draft workspace、candidate reservation、legacy publication、fallback或dual-write。正式migration／deploy／release均未執行。 |
...
| DEV-079 圖號唯讀抽屜與全頁編輯工作區分流 | `.ai-doc/dev_task.md` (`DEV-079`, RD Implementation Contract); primary UI／implementation authority `.ai-doc/specs/SPEC-PDM-ENTITY-DETAIL-DRAWER-001-unified-object-detail-contract.md` (`2026-08-19 DEV-079 Amendment`、`HD-079-04 Visual-first amendment`與`RD Implementation Package`); workbench compatibility `.ai-doc/specs/SPEC-PDM-UNIFIED-DRAWING-WORKBENCH-001-single-page-lifecycle-workbench.md`; QA `.ai-doc/qa/qa-dev-079-drawing-readonly-drawer-fullpage-workspace-validation-plan-2026-08-19.md`; data/file authorities `SPEC-PDM-UNIFIED-DRAWING-AGGREGATE-001`、`SPEC-PDM-FILE-OWNERSHIP-001`; parents `DEV-053`、`DEV-057`、`DEV-067`; relations `DEV-061`、`DEV-064`、`DEV-070`、`DEV-072`。Status: `RD Implemented Locally / Contract + Layout + Recognition Browser PASS / Independent QC Pending / Production Release Gated`。Canonical owner route=`/numbering/drawings/[drawingId]/workspace`，Drawing reviewer route=`/approvals/[requestId]`；Drawing drawer zero-write不變。現行owner workspace為左側2D／3D大型主視覺、右側`版次與檔案／智慧辨識`task panel與底部單一生命週期action bar；OCR沿用candidate revision／recognition authority且不進submit gate。A0002 evidence resolver已改為locatable PDF優先、同一 PDF.js page surface direct crop、完整材質文字與單黃框；isolated generic layout與recognition-layout各3 viewport PASS。無schema／migration／新domain authority，未執行production release。 |
...
| DEV-035 SolidWorks 原生屬性讀取與辨識診斷可視化 | `.ai-doc/dev_task.md` (`DEV-035 / DEV-CAD-001`); implementation authority `.ai-doc/specs/SPEC-PDM-SOLIDWORKS-METADATA-READER-001-native-property-extraction.md`（§0 Reopen Amendment為現行authority）; QA `.ai-doc/qa/qa-dev-035-solidworks-native-metadata-reader-validation-plan-2026-08-19.md`; fixture expectation `.ai-doc/qa/fixtures/dev-035-a0002-property-expectations.md`; evidence `output/qa/dev-035-solidworks-native-reader/20260819T120907Z/a0002-real-reader.json`; parent `.ai-doc/specs/SPEC-PDM-DRAWING-RECOGNITION-001-candidate-review-and-formalization.md`; worker/credential authorities `ADR-PDM-SW-NATIVE-PREVIEW-WORKER-001`、`SPEC-PDM-GCP-SECRET-MANAGER-001`; relations `DEV-056`、`DEV-058`、`DEV-079`。Status: `Local RD Implemented / Human Confirmed / Real A0002 QA-QC Passed / Production Release Gated`。原035-A～D與22-file focused evidence只算partial baseline；Current Phase完成Windows DPAPI／GSM同UI lifecycle、test-double deny、real native probe、recognition worker exact-version hot apply／heartbeat與active＋probe＋online＋ack AND-gated readiness。A0002.SLDPRT兩個獨立real-reader session各14 observations，八欄value／owner／scope一致且repeatable；完整`qc:dev-035`與completion gate PASS。日常key操作不需PowerShell、`.env.local`或人工restart；本次只因舊常駐worker早於程式修正而做一次受控程式版本重啟。未儲存狀態、Add-in、CAD回寫、OCR、2D preview與production release仍排除。 |
