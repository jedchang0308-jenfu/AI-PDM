...
| DEV-076 還原資料候選關聯自動投影與移轉對帳 | `.ai-doc/dev_task.md` (`DEV-076`); authority amendment `.ai-doc/specs/SPEC-PDM-DRAWING-PART-RELATION-VIEW-001-root-drawing-part-relation-list.md` § DEV-076; validation amendment `.ai-doc/qa/qa-dev-062-unified-part-relation-workbench-validation-plan-2026-08-10.md` § DEV-076；parents `DEV-062/064`。Status: `RD Implementation Ready / Repair In Progress / Local + Staging Read-only / Production Release Gated`。修正 source-less candidate 硬編碼空 tree/matrix、逐料號 primary manufacturing completeness 與 migration reconciliation blind spot；禁止 production write與使用者手動轉換。 |
