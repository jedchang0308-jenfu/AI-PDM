...
- `HD-048-01 / 2026-07-17 replacement`: retain visible and operable local `料號草稿 / 領號申請 / 上傳送審 / 製造交接` entrypoints plus owner-surface CTAs. Every entrypoint delegates to the same production BFF/domain commands, permissions, transactions, audit/outbox and Cloud SQL schema; no second provider, data authority or official-number authority is allowed. This supersedes only the former `1C` navigation-removal decision.
