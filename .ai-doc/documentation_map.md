...
| DEV-064 圖號單一資料層與共用明細入口 | `.ai-doc/dev_task.md` (`DEV-064`); authority `.ai-doc/specs/SPEC-PDM-UNIFIED-DRAWING-AGGREGATE-001-single-data-layer.md`; decision `.ai-doc/decisions/ADR-PDM-UNIFIED-DRAWING-AGGREGATE-001-canonical-drawing-and-revision.md`; validation `.ai-doc/qa/qa-dev-064-unified-drawing-aggregate-validation-plan-2026-08-11.md`; QC `.ai-doc/qc/qc-dev-064-unified-drawing-aggregate-report-2026-08-11.md`. Status: `Local RD/QA/QC Passed / Human Directed / Production Migration & Release Gated`。此 DEV intentional-replace DEV-052/053 的 candidate→formal 雙物件權威：canonical Drawing/Revision/File 保持 stable identity，workspace 與既有 candidate/formal tables 僅作流程容器／相容投影；workbench只由`drawings`產生stable row，待處理與研發可用共用圖號明細入口；server/DB共同驗證狀態與受控不可變性。Isolated Chromium 28/28，focused aggregate 7/7；未執行production migration/deploy/release。 |
...
| DEV-068 圖面／CAD 全項辨識與人工確認入庫 | `.ai-doc/dev_task.md` (`DEV-068`); authority `.ai-doc/specs/SPEC-PDM-DRAWING-RECOGNITION-001-candidate-review-and-formalization.md`; concept prototype `output/dev-068-attribute-recognition-ui-preview.html`。Status: `RD Contract Ready / Human Confirmed / RD Estimation Eligible / Implementation Not Authorized`。3D custom property、圖框／註記、檔名與 OCR 都只產生可追溯候選，不要求 3D 屬性完整，也不直接覆寫正式 PDM。OCR 是來源而非獨立類別；六個辨識區段在同一審核分頁連續呈現。材料、規格、製程與外觀合併為料號屬性候選；單圖多料號以共用基準加逐料號差異審核，但正式權威仍是各料號完整有效值。`非限制欄位` 採 company-scoped stable field key，不使用動態 DB column。辨識 session 綁定 canonical asset fingerprint；重跑建立 successor；寫入影響計算 zero-write，正式化要求獨立權限、target concurrency、idempotency 與原子 transaction。A0005 是第一個 isolated pilot。RD 下一步只做 repository-specific Implementation Readiness Assessment；provider/license、physical schema、permission defaults、fixture 與使用者授權尚未通過，不得 coding 或處理 production。 |
| DEV-069 AI-PDM 預上線 GCP 降本與低成本 Staging | `.ai-doc/dev_task.md` (`DEV-069`); implementation authority `.ai-doc/specs/SPEC-PDM-GCP-COST-OPTIMIZATION-001-prelaunch-runtime-topology.md`; decision `.ai-doc/decisions/ADR-PDM-GCP-COST-OPTIMIZATION-001-prelaunch-firebase-hosting-zonal-micro.md`; related platform authority `.ai-doc/decisions/ADR-PDM-ERP-PLATFORM-002-google-taiwan-cloud-sql-production.md`, `.ai-doc/specs/SPEC-PDM-ERP-GOOGLE-CLOUDSQL-002-five-year-platform-ontology-roadmap.md`。Status: `RD Implementation Ready / Human Confirmed / Local IaC Preparation Ready / Live GCP Release Gated`。Production 改 `db-f1-micro`／`ZONAL`，Staging 改 Micro／Zonal／按需啟停且保留完整發布驗證鏈；兩環境以 Firebase Hosting `web.app` 為 canonical edge 並移除未使用 ALB；已完成 reconciliation 的 Restore target 在備份與證據確認後刪除。Production live 仍由 DEV-032 作唯一 release 入口。 |
| DEV-063 編號、圖號、料號與主根號使用者詞彙統一 | `.ai-doc/dev_task.md` (`DEV-063`); current SPEC amendment `.ai-doc/specs/SPEC-PDM-STATUS-UX-003-state-axis-vocabulary-and-header-help.md`; related authority `.ai-doc/specs/SPEC-PDM-UNIFIED-DRAWING-WORKBENCH-001-single-page-lifecycle-workbench.md`, `.ai-doc/specs/SPEC-PDM-NUMBER-STATE-FLOW-001-unified-numbering-draft-and-transfer-functional-spec.md`; ADR `.ai-doc/decisions/ADR-PDM-NUMBERING-IDENTITY-VOCABULARY-001-stored-label-rewrite.md`; QA `.ai-doc/qa/qa-dev-063-numbering-vocabulary-rewrite-validation-plan-2026-08-11.md`. Status: `RD Implementation Ready / Human Confirmed / RD Not Started / Production Release Gated`。使用者已確認 `1C / 2C / 3B / 4A / 5C`；本 phase 不要求稽核證據保存，5C 直接納入人類可讀文字改寫與不可變資料顯示投影，raw audit／snapshot 不覆寫，不改 machine code、ID、hash、enum、權限、API shape 或 authority。底層 `reserved`／`candidate`／`official`、舊 query 與資料 authority 保留。 |
