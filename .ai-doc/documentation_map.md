...
| BOM 模組硬刪除與未來重建邊界 | `.ai-doc/dev_task.md` (`DEV-095`); `.ai-doc/decisions/ADR-PDM-BOM-RETIREMENT-001-hard-delete-and-rebuild-boundary.md`; `.ai-doc/qa/qa-dev-095-bom-retirement-validation-plan-2026-08-24.md`; `.ai-doc/qc/qc-dev-095-bom-retirement-2026-08-24.md`; PostgreSQL `db/postgres/047_remove_bom_module.sql`; SQLite runner `scripts/migrate-dev-095-bom-retirement.mjs`。現行產品不提供 BOM、Where-used 或舊 BOM 審核契約；舊 BOM spec/QA/QC/report 只保留歷史，不得作現行 acceptance authority。Production execution已由使用者授權，但仍須backup/PITR、candidate、Level 4、Wave 0、product-owner與post-release gate。 |
...
| DEV-066 三工作台頂部欄一致化與肌肉記憶 | `.ai-doc/dev_task.md` (`DEV-066`); authority `.ai-doc/specs/SPEC-UX-PDM-WORKBENCH-TOPBAR-001-unified-toolbar-muscle-memory.md`; validation `.ai-doc/qa/qa-dev-066-workbench-topbar-muscle-memory-validation-plan-2026-08-11.md`. Status: `RD Implemented / Focused Contract QC 12/12 / Browser Smoke Blocked by auth / Production Release Gated`。三模組固定 filter row、history leading、mode switch trailing、pagination bottom；圖料保留`關係樹／矩陣`，圖號／料號保留`清單／預覽圖`；純UI shell/DOM/CSS/ARIA 收斂，不改 API、schema、permission、preview authority或資料排序。 |
...
| DEV-074 料號／圖號全生命週期 AI UI 真實操作驗證 | `.ai-doc/dev_task.md` (`DEV-074`); historical QA `.ai-doc/qa/qa-dev-074-pdm-complete-lifecycle-ui-real-operation-validation-plan-2026-08-15.md`。Status: `Superseded in current form by DEV-095 / Replan required`。原58條路徑含已退役BOM流程，不能再按原分母執行或宣稱PASS；未來若重啟須排除BOM並建立新QA基線。 |
...
| DEV-068 圖面／CAD 全項辨識與人工確認入庫 | `.ai-doc/dev_task.md` (`DEV-068`); authority `.ai-doc/specs/SPEC-PDM-DRAWING-RECOGNITION-001-candidate-review-and-formalization.md`; QA `.ai-doc/qa/qa-dev-068-drawing-recognition-validation-plan-2026-08-12.md`; QC `.ai-doc/qc/qc-dev-068-drawing-recognition-2026-08-12.md`; fixture `.ai-doc/qa/fixtures/dev-068-a0005-fixture-manifest.md`; evidence `output/qa/dev-068-drawing-recognition/`; concept prototype `output/dev-068-attribute-recognition-ui-preview.html`。Status: `Local RD Implemented / Focused QA-QC Passed / Production Release Gated`。3D custom property、圖框／註記、檔名與 OCR 都只產生可追溯候選，不要求 3D 屬性完整，也不直接覆寫正式 PDM；OCR 是來源而非獨立類別，六區在同一審核頁。14 張 additive tables、SQLite＋PostgreSQL 033、user/worker API、三權限、default-off flag、versioned external adapter、impact gate與atomic formalization已落地。A0005 21 candidates、P01-P03基準/變體、open field、missing/N/A、idempotency/stale/rollback及responsive browser均PASS；真實 provider/license/accuracy、full migration chain 004 drift remediation、production migration/deploy/release仍 gated。 |
...
Status: Phase 1A shared shell/core sections implemented locally; `DEV-067` cross-domain projection amendment is `Local RD Implemented / Local QA-QC Passed`; production release not authorized. The three task entry pages remain separate, but the target architecture is one `UnifiedPdmEntityDetailDrawer` composer with domain-owned projections. Object identity, data truth and permission never vary by source; visible projection depth intentionally varies by server policy: Drawing/Part owner surfaces are task-focused reductions, Relation is the full aggregate, and assigned active review is full only inside exact request/company scope.
