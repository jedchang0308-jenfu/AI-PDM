...
日期：2026-07-13
狀態：Accepted / Phase 1A Implemented / Phase 1B-1D RD Contract Ready / Sequentially Gated
Owner：Dev PM
...
本ADR的publication boundary、candidate lifecycle與official non-reuse決策維持Accepted；Phase 1A已實作且待獨立QC。2026-07-13使用者以`1C / 2C / 3C`關閉SPEC登錄的`HD-048-01..03`：Phase 1B立即移除四個可見舊側欄項目但保留舊URL導引；drawing/required-file publication必須有finalized evidence，純root/part-only可由server rule回`not_required`；同一自然人可完成submit/approve/publish，但不得合併權限、command或audit。Phase 1B-1D因此為`RD Contract Ready / Sequentially Gated`，仍須逐phase完成RD與獨立QC。
