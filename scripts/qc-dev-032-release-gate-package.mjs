...
record("DEV032-GATE-003 Wave 0 allowlist is named-user and fail-closed", seed.canaryAllowlist?.mode === "named-pdm-user-ids-only" && seed.canaryAllowlist?.failClosed === true && seed.canaryAllowlist?.wave0GoogleWorkspaceOnly === true && seed.canaryAllowlist?.nonCanaryAccessExpected === "denied");
record("DEV032-GATE-004 Phase 3A keeps GCS and non-Google Wave 0 closed", seed.configuration.some((item) => item.key === "gcs_file_authority_enabled" && item.value === "false") && seed.configuration.some((item) => item.key === "non_google_wave0_enabled" && item.value === "false") && seed.wavePolicy?.wave0?.controlledNonGoogleAllowed === false);
...
record("DEV032-GATE-006 HD-8-4 restore gate requires PITR and isolated target", seed.preCanaryRestoreReconciliation?.decision === "HD-8-4 / 1A" && seed.preCanaryRestoreReconciliation?.cloudSqlAutomatedBackupRequired === true && seed.preCanaryRestoreReconciliation?.pitrRequired === true && seed.preCanaryRestoreReconciliation?.separateIsolatedTargetRequired === true && seed.preCanaryRestoreReconciliation?.sourceOverwriteAllowed === false);
record("DEV032-GATE-007 reconciliation covers numbering and operational integrity", ["schema_and_migration_history", "principal_and_account_mapping", "audit_receipt_outbox_orphan_check", "official_number_ledger_consistency", "numbering_sequence_regression_check", "non_reuse_reservation_coverage", "clean_seed_no_business_draft_demo_test_source_rows"].every((check) => seed.preCanaryRestoreReconciliation?.checks?.includes(check)));
record("DEV032-GATE-008 rollback evidence remains required before canary", seed.rollbackReadiness?.requiredBeforeCanary === true && seed.rollbackReadiness?.status === "missing_evidence" && seed.rollbackReadiness?.cloudRunPreviousRevisionRequired === true && seed.rollbackReadiness?.databaseRestorePointRequired === true);
...
record("DEV032-GATE-014 activation readiness generator is local-only", readinessGenerator.includes("blocked_activation_readiness") && readinessGenerator.includes("productionActionPerformed: false") && !readinessGenerator.includes("execFileSync"));
