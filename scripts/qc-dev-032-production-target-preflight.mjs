...
record("DEV032-TARGET-007 production env source is proven without exposing values", !blockers.has("PRODUCTION_ENV_SOURCE_MISSING") && report?.envReadback?.ready === true && report?.envReadback?.valuesPersisted === false && (report.envReadback.providerReady === true || report.envReadback.localFilePresent === true));
...
record("DEV032-TARGET-009 release source exact commit matches current HEAD", !blockers.has("RELEASE_SOURCE_NOT_SELECTED_OR_COMMITTED") && /^[a-f0-9]{40}$/u.test(report?.releaseSource?.currentHead ?? "") && report?.releaseSource?.releaseCommitSha === report.releaseSource.currentHead && report?.releaseSource?.manifestHead === report.releaseSource.currentHead && report?.releaseSource?.headMatches === true && report?.releaseSource?.exactReleaseCommitExists === true && report?.releaseSource?.includedProductionSourceEntries === 0 && report?.releaseSource?.unknownRiskEntries === 0 && report?.releaseSource?.safeToBuildForProduction === false);
...
record("DEV032-TARGET-012 commands are read-only discovery commands", commands.length >= 6 && commands.every((item) => item.readOnly === true) && commandStrings.every((command) => /^gcloud (config get-value|projects describe|run services (?:list|describe)|sql instances list|secrets list)/u.test(command)));
