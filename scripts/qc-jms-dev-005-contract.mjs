...
function main() {
  ...
  assert.deepEqual(map.denominator, { uniqueFiles: 62, uniqueMethods: 76, policyEntries: 84 })
  assert.equal(new Set(map.entries.map((entry) => entry.path)).size, 62)
  assert.equal(new Set(map.entries.map((entry) => `${entry.path}\0${entry.method}`)).size, 76)
  assert.equal(map.entries.length, 84)
