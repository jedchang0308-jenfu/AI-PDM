...
assert.deepEqual(map.denominator, { uniqueFiles: 62, uniqueMethods: 77, policyEntries: 86 })
  assert.equal(new Set(map.entries.map((entry) => entry.path)).size, 62)
assert.equal(new Set(map.entries.map((entry) => `${entry.path}\0${entry.method}`)).size, 77)
assert.equal(map.entries.length, 86)
