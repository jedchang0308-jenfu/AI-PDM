...
async function main() {
  const [profileBytes, v1, n1c] = await Promise.all([fs.readFile(path.join(root, 'config/release/dev117-ai-pdm-independent-production-v3.json')), ...['config/release/dev117-ai-pdm-independent-production.json', 'config/platform/dev-010-n1c-ai-pdm.json'].map((file) => fs.readFile(path.join(root, file), 'utf8').then(JSON.parse))])
  ...
  const result = await executeOwnerStage({
    ...args, profile, profileSha256: createHash('sha256').update(profileBytes).digest('hex'), transport, validateIntent: assertDev117ReleaseIntent,
