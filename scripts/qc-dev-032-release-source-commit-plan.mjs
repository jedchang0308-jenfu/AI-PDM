...
record("DEV032-COMMIT-PLAN-008 included pathspec covers app docs scripts schema and build config", ["src/", ".ai-doc/", "scripts/", "db/", "package.json"].every((prefix) => included.some((item) => item === prefix || item.startsWith(prefix))));
...
record("DEV032-COMMIT-PLAN-010 safe-to-stage is true but safe-to-build remains false", plan?.releaseDecision?.safeToStageIncludedSource === true && plan?.releaseDecision?.safeToBuildForProduction === false);
