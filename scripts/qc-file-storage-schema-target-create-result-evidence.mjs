...
try {
  tempRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "ai-pdm-target-create-result-qc-"));
  ...
  const generatorSource = readProjectFile(root, "scripts/generate-file-storage-schema-target-create-result-evidence.mjs");
  ...
  record(
    "STORAGE-SCHEMA-TARGET-CREATE-RESULT-013 PM evidence references target create result lane",
    planSource.includes("Phase 5F") &&
      planSource.includes("storage:schema-target-create-result-evidence") &&
      planSource.includes("qc:file-storage-schema-target-create-result-evidence") &&
