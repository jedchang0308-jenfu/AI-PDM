...
const retirement = plan.schemaMigrations.at(-1);
const pool = new pg.Pool({ connectionString: postgresUrl, max: 1 });
const client = await pool.connect();

try {
  record("DEV095-PG-001 production package contains 38 ordered migrations", plan.schemaMigrationCount === 38, String(plan.schemaMigrationCount));
  record("DEV095-PG-002 retirement migration is the final package step", retirement?.version === "047", retirement?.output ?? "missing");
