...
const postgresInvariant = read("db/postgres/051_drawing_recognition_part_owner_invariant.sql");
...
expect("recognition review fields use a compact vertical rhythm while preserving mobile dock clearance", styles.includes(".dev079-recognition-panel { gap: 8px; }") && styles.includes(".dev079-recognition-sections { gap: 7px; }") && styles.includes(".dev079-recognition-candidate { gap: 4px; padding: 6px 0; }") && styles.includes(".dev079-recognition-candidate label input { padding: 6px 8px; }") && styles.includes(".dev079-recognition-save-status"));
expect("recognition removes the redundant heading and keeps candidate rows visually flat", !owner.includes("辨識與人工核對") && styles.includes(".dev079-recognition-candidate { border: 0; background: transparent; }") && styles.includes(".dev079-recognition-candidate.is-conflict,\n.dev079-recognition-candidate.is-blocked { border: 0; background: transparent; }"));
