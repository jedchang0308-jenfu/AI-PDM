...
record("DEV032-ACTIVATION-009 activation sequence covers source, target, env, plan, apply, seed, restore, smoke and go/no-go", ["A0-release-source", "A1-production-target-readback", "A2-provider-and-env-readback", "A3-credentialled-terraform-plan-review", "A4-production-resource-apply", "A5-clean-seed-and-principal-bootstrap", "A6-hd84-restore-reconciliation", "A7-level3-production-like-smoke", "A8-production-deploy-and-level4-smoke", "A9-production-go-no-go"].every(hasSequence));
record("DEV032-ACTIVATION-009B retired Wave 0 validation and waiver are absent from the active checklist", !/wave\s*0|wave0|waiver|3-5 user/iu.test(JSON.stringify(checklist)));
...
record("DEV032-ACTIVATION-012 checklist restricts Hosting to production, blocks direct-origin sessions, GCS Phase 3A and DEV-047 scope creep", shortcutText.includes("staging Firebase Hosting web.app") && shortcutText.includes("direct run.app") && shortcutText.includes("GCS file authority") && shortcutText.includes("DEV-047"));
