...
record("DEV032-ACT-READY-006 first incomplete gate is Wave 0 human go/no-go", report?.status === "pending_human_activation_readiness" && report?.gateSummary?.firstBlockedGate === "A9-wave0-go-no-go" && report?.gateSummary?.blocked === 0 && report?.gateSummary?.missingEvidence === 0 && report?.gateSummary?.pendingHuman === 1 && report?.gateSummary?.passed === 9);
...
record("DEV032-ACT-READY-007 Level 3 evidence is exact and current", level3.passed >= 14 && level3.failed === 0 && level3.revision === "ai-pdm-prod-00012-kaf" && level3.manifestDigest === "sha256:742fbcab536c0cbccb3794907fec3795f345181124e54164cbd69966e0e5c42d" && level3.runtimeDigest === "sha256:742fbcab536c0cbccb3794907fec3795f345181124e54164cbd69966e0e5c42d");
record("DEV032-ACT-READY-008 authenticated Level 4 has concrete UI evidence", level4.status === "passed" && level4.evidence?.productionDeploymentObserved === true && level4.evidence?.unauthenticatedProductionChecksPassed === true && level4.evidence?.authenticatedLevel4Status === "passed" && level4.evidence?.level4Failed === 0 && level4.evidence?.uiAcceptanceResult?.partNumber === "A0001-P01" && level4.evidence?.uiAcceptanceResult?.seriesCode === "DEV032-HF" && (level4.blockers ?? []).length === 0);
...
record("DEV032-ACT-READY-009B Gate E machine evidence is linked while human closure remains pending", gateE?.summary?.machineChecksPassed === true && gateE?.summary?.failed === 0 && gateEHuman?.requiredHumanInputs?.some((item) => item.id === "wave0_named_users") && wave0.evidence?.gateEMachineChecksPassed === true && wave0.evidence?.gateEAutomationEvidencePath === "output/dev-032-gate-e-automation/gate-e-automation-readback.json" && wave0.evidence?.humanWorkPackagePath === "output/dev-032-gate-e-automation/human-work-package.md");
record("DEV032-ACT-READY-010 next action is Wave 0 closure only", report?.nextRequiredAction?.includes("human work package") && report.nextRequiredAction.includes("3-5 explicitly named Wave 0 users") && report.nextRequiredAction.includes("product-owner go/no-go") && !/terraform\s+apply|Google account chooser/iu.test(report.nextRequiredAction));
