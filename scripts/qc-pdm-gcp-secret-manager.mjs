...
try {
  ...
  record("GSM-011 production defaults fail closed", includesAll(lifecycle, ["NODE_ENV === \"production\"", "GCP_SECRET_MANAGER_CONFIG_REQUIRED", "expected: isWindowsDpapiAvailable() ? \"windows_dpapi\" : \"google_secret_manager\""]));
  ...
  record("GSM-013 test result is provider-specific and redacted", includesAll(lifecycle, ["plaintextPersisted: false", "resolveSettingsSecretProbeCredential", "SECRET_VALUE_NOT_AVAILABLE"]));
  record("GSM-014 production blocks worker-local fallback without break-glass evidence", includesAll(lifecycle, ["PDM_ALLOW_WORKER_ENV_SECRET_FALLBACK", "PDM_BREAK_GLASS_CHANGE_ID", "workerEnvironmentFallbackAllowed"]));
  ...
  record("GSM-030 worker-facing failure explains next action", includesAll(worker, ["設定中心已啟用版本", "2D worker readiness", "DOCUMENT_MANAGER_OPEN_FAILED:swDmDocumentOpenErrorNoLicense"]));
