...
export function assertTargetInspectionReceipt(value, configInput, expected = {}) {
  ...
  if (value.schemaVersion !== 'jenfu.dev012.ai-pdm-target-inspection-receipt.v1' || value.ownerApplicationId !== 'ai-pdm' || value.projectId !== config.target.projectId || value.database !== config.target.database || value.schema !== config.target.schema || !H64.test(value.identityReceiptSha256 ?? '') || !/^dev012-ai-data-import-[a-f0-9]{12}-[a-z0-9]{5}$/u.test(value.executionName ?? '') || ![0, 1].includes(value.activeUidMappingCount) || value.rawRowsLogged !== false || value.status !== 'PASS' || !Number.isFinite(Date.parse(value.completedAt)) || (expected.releaseId && value.releaseId !== expected.releaseId) || (expected.sourceRevision && value.sourceRevision !== expected.sourceRevision)) fail('DATA_CUTOVER_TARGET_INSPECTION_RECEIPT_INVALID')
