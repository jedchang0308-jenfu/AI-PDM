...
record("DEV046-HOST-010 production pilot exception preserves the future ALB baseline", contract.productionEdgeBaseline?.type === "external-application-load-balancer" && contract.productionEdgeBaseline?.status === "future-cutover-baseline-after-internal-pilot" && contract.productionInternalPilotGateway?.decision === "firebase-hosting-default-domain-no-dns" && contract.productionInternalPilotGateway?.canonicalOrigin === "https://jenfu-ai-pdm-prod.web.app" && contract.productionInternalPilotGateway?.directRunAppOriginSessionExchange === "denied-when-origin-is-run-app");
record("DEV046-HOST-011 Hosting exceptions keep Cloud SQL GCS and portable BFF authority unchanged", contract.stagingInternalPilotGateway?.decision === "firebase-hosting-default-domain" && contract.stagingInternalPilotGateway?.firestoreAllowed === false && contract.stagingInternalPilotGateway?.firebaseStorageAllowed === false && contract.stagingInternalPilotGateway?.firebaseFunctionsAllowed === false && contract.productionInternalPilotGateway?.firestoreAllowed === false && contract.productionInternalPilotGateway?.firebaseStorageAllowed === false && contract.productionInternalPilotGateway?.firebaseFunctionsAllowed === false && contract.productionInternalPilotGateway?.gcsFileWorkflowEnabled === false);
