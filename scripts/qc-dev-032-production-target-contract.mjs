...
record("DEV032-PROD-TARGET-004 production custom domain and ALB baseline are required", contract.target?.publicBaseUrl === "https://pdm.jenfu.com.tw" && contract.edge?.type === "external-application-load-balancer" && contract.edge?.customDomainRequired === true && contract.edge?.firebaseHostingGatewayAllowed === false);
record("DEV032-PROD-TARGET-005 production rejects staging gateway defaults", contract.edge?.cloudRunIngress === "internal-and-cloud-load-balancing" && contract.edge?.cloudRunDefaultUrlDisabled === true && forbiddenEnvValues.some((value) => value.includes("stg")) && forbiddenEnvValues.some((value) => value.includes("web.app")));
...
record("DEV032-PROD-TARGET-012 stop conditions forbid staging/provider shortcuts", contract.stopConditions?.some((item) => item.includes("Firebase Hosting default web.app")) && contract.stopConditions?.some((item) => item.includes("staging Firebase project")) && contract.stopConditions?.some((item) => item.includes("Do not create")));
