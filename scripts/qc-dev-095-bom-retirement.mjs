...
check("DEV095-008 migration relies on runner transaction", !/\bBEGIN\b|\bCOMMIT\b/iu.test(postgresMigration));
check(
  "DEV095-009 production candidate verifies exact migration set",
  workflow.includes("schemaMigrationCount !== 38") && workflow.includes("047_remove_bom_module.cloudsql.sql")
);
check(
