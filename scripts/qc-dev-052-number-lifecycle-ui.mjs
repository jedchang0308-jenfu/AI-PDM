...
record(
  "DEV052-UI-001 existing reserved route and tab are retained without a V2 page",
  drawingsPage.includes("tab=reserved") || drawingsPage.includes('tab === "reserved"') || drawingsPage.includes('searchParams?.tab') &&
    workspace.includes("保留號") && !fs.existsSync(path.join(root, "src/app/numbering/v2")),
  "/numbering/drawings?tab=reserved + 保留號"
);
record(
  "DEV052-UI-002 V2 title and purpose sentence are flag-gated",
  has(workspace, ['feature?.lifecycleV2?.enabled === true', '"保留號／首版準備"', '"完成候選首版並送審，核准後由系統正式化。"']),
...
record(
  "DEV052-UI-014 legacy pending review keeps a visible continuation link",
  workspace.includes("workspace.latestApproval?.requestId ?? null"),
  "V2 review navigation falls back to the retained legacy approval request"
);
record(
