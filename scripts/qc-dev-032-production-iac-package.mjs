...
const guardedResources = resources.filter((resource) =>
  ...
  /count\s*=\s*local\.create_edge_resources\s*\?\s*1\s*:\s*0/u.test(resource.body) ||
...
record("DEV032-PROD-IAC-005 every Google resource is gated by create_resources or the narrower edge gate", resources.length >= 20 && guardedResources.length === resources.length, resourceNames.filter((_, index) => !guardedResources.includes(resources[index])).join(", "));
...
record("DEV032-PROD-IAC-007 Cloud SQL is prelaunch micro/zonal, private, IAM-auth and recovery-ready", variables.includes('default     = "db-f1-micro"') && variables.includes('default     = "ZONAL"') && database.includes("availability_type           = var.database_availability_type") && database.includes("point_in_time_recovery_enabled = true") && database.includes('name  = "cloudsql.iam_authentication"') && database.includes("ipv4_enabled                                  = false") && database.includes("deletion_protection = true"));
...
record("DEV032-PROD-IAC-009 deferred external ALB is fully gated off during Firebase Hosting prelaunch", variables.includes('variable "enable_external_load_balancer"') && locals.includes("create_edge_resources") && edge.includes("local.create_edge_resources") && edge.includes("EXTERNAL_MANAGED") && edge.includes('enable_cdn            = true') && edge.includes("google_compute_managed_ssl_certificate"));
...
record("DEV032-PROD-IAC-022 production runtime enforces the exact server-only numbering/draft slice", runtime.includes('name  = "PDM_PRODUCTION_SLICE_MODE"') && runtime.includes('value = "official-numbering-draft"') && !combinedTf.includes("NEXT_PUBLIC_PDM_PRODUCTION_SLICE_MODE") && readme.includes("PDM_PRODUCTION_SLICE_MODE"));
record("DEV032-PROD-IAC-023 production package keeps file and GCS authority absent", !/(?:google_storage_bucket|firebase_storage|FIREBASE_STORAGE|PDM_(?:GCS|FILE)_)/u.test(combinedTf));
