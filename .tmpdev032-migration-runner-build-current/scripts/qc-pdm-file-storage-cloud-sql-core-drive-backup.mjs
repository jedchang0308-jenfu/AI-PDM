...
async function main() {
  ...
  const archivedSupabaseSchema = readProjectFile(root, ".ai-doc/archived/legacy-supabase-migration-mirror/migrations/20260608000100_initial_ai_pdm_schema.sql");
  ...
  record("PDM-FILE-STORAGE-002 package registers Cloud SQL core/Drive backup QC", packageJson.scripts?.["qc:pdm-file-storage-cloud-sql-core-drive-backup"] === "node scripts/qc-pdm-file-storage-cloud-sql-core-drive-backup.mjs");
  ...
  record("PDM-FILE-STORAGE-006 historical Supabase pointer operations fail closed", includesAll(storage, ["RetiredSupabaseStorageAdapter", "SUPABASE_STORAGE_RETIRED_USE_GCS"]));
  ...
  record("PDM-FILE-STORAGE-009 archived Supabase migration preserves storage pointer history", includesAll(archivedSupabaseSchema, ["storage_provider TEXT NOT NULL DEFAULT 'local_repository'", "storage_bucket TEXT", "storage_key TEXT"]));
  ...
  record(
    "PDM-FILE-STORAGE-030 restore index excludes preview derivatives and preserves historical Supabase storage keys",
  ...
  record(
    "PDM-FILE-STORAGE-031 manifest template excludes preview derivatives and records Drive/historical-pointer evidence",
  ...
  record(
    "PDM-FILE-STORAGE-036 backup plan preserves historical Supabase bucket/key metadata without live access",
